...
description: "协调 plush-toy-erp 共享 Local checkout 的唯一 writer、提交批次、独立 push、遗留修改盘点与 Git 收口队列轮换。Use when 多个 Codex 顶层任务共享 dirty worktree、热点文件需要继续排队、App 重启后要恢复队列，或用户要求新建一模一样的收口队列；不用于单一 writer 的常规本地自动提交。"
---
...
把共享 Local 的并行协作收口为被动、事件驱动的协议。任务消息负责唤醒和交接，仓库只保存协议；不要创建 daemon、定时扫描、automation 或仓库内队列状态文件。
...
- `BATCH_READY`、`COMMIT_READY` 和用户授权互不替代：登记不等于执行，commit 授权不包含 push。
...
按需追加 `paths`、`owned_hunks`、`last_write_at`、`validation`、`commit_authorized`、`push_authorized`、目标分支和 exact commit range。`event_id` 在幂等重发时保持不变；内容实质变化才生成新 revision 和新 event_id。
...
队列收到 release / cancelled 后，应在同一轮从已登记申请中按顺序处理下一项，不要求各 worker 定时轮询或反复询问。授予前一次性重读 HEAD、index、lock / 活动 Git 进程和请求路径；热点仍在写、请求已漂移或 owner 不清楚时返回 `WAIT_WRITER` / `WAIT_HOT_FILE` 并保留队列项。

## 批次与本地提交

完成一个可验证切片后：

- 未获本地提交授权时发送 `BATCH_READY`。
- 已获明确本地提交授权时发送 `COMMIT_READY`，同时报告 exact paths / hunks、最后写入、验证和建议 Conventional Commit。
- 热点文件随后再次被写入时，把相关批次保留为 `WAIT_HOT_FILE`；不要丢弃、抢提或要求 worker 重做验证。热点释放后重新核对 revision 和归属。

`BATCH_READY` / `COMMIT_READY` 只登记。只有能追溯到用户明确提交授权的 `CLOSEOUT_REQUEST` 才执行收口；来源任务可以在授权范围内代用户发送，不需要用户重复确认。队列此时：

1. 暂停新 writer grant，确认当前无 writer、commit 或 push owner。
2. 重新读取 HEAD、status、index、lock / Git 进程和目标 diff；混合文件必须按 owned hunk 精确 stage。
3. 使用 `$git-closeout-coordination` 完成一次复杂现场收口；hook 改写、index 污染、路径重叠或 ownership 不完整时停止。
4. 提交成功后报告 exact commit、实际文件、验证和 index 状态；确认 index 复空后释放 Git owner，再处理下一项。
5. 未明确授权 push 时保持本地，不顺带推送。
...
4. 不 stage、commit 或 push。可证明批次仍需用户授权或既有授权支持的 `CLOSEOUT_REQUEST`；无法证明的文件原样保留。
...
- 队列默认休眠。任务消息是唤醒信号，协议是项目自定义协作层，不是 Git 或 Codex 自动提交机制。

## 精简输出

常规响应只保留两行：`ACK <event_id>` 与一个状态（如 `GRANT_WRITER`、`WAIT_HOT_FILE`、`COMMIT_READY`）。只有 reconcile、closeout、push 失败或 queue rotation 才展开必要证据，避免用持续状态播报消耗 token。
