...
- Active editable focus 复用共享工作区样式，按当前用户要求和正式编辑行为文档核对视觉合同；不要在模板里重复维护焦点边框。row / cell selection 可以独立高亮，但不能像第二套编辑边框。
...
- Edit-focus styling 要在浏览器里检查 computed styles、文字与 caret bounds：焦点框与文字、光标须有间距，不能遮挡相邻标签；空值、长值和缩放后仍可编辑。焦点切换不改变纸面几何，print media 不带出焦点样式；selection 与 editing focus 要保持视觉区分。
