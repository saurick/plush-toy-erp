...
- 后续与边界：须提交本次人工节点并发修复形成新 exact SHA；随后只对新候选重跑失效的 clean `prepare-push.sh --full` 和独立 Full Acceptance，再完成全部登记 Style L1 / 页面 / PDF / 真实 PostgreSQL / 角色回归、Core / Scenario 两次幂等读回和 release-grade strict，才可普通 push、核对远端 SHA、生成不可变制品并重新执行 133 的备份恢复、promotion、V8 配置、Scenario V6 首次 / resume、浏览器 / PDF 和运行读回。生产、真实客户数据导入和客户人工 UAT 均不在自动化完成声明内；最终发布事实只以绑定 exact SHA、数据库、配置和当前运行态的最新回执为准。
