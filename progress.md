...
- 门禁证据：回执夹具已补齐 `scripts/qa/lib/repository-identity.mjs` 真实依赖，并把仓库身份变化场景收紧为同时断言外层 `full_gate_failed`、内层 receipt 为 `failed` 且含 identity-change `notProven`，定向回归 `15 / 15` 通过。绑定 `9e54545ee1fa10a16f269ef0d6e1b2ae721a4c5c` 的后续 `prepare-push` 已将 scripts Node full 跑至 `1592 / 1592`，随后暴露 `customer-config-boundaries.mjs` 仍断言升级前的单一审批 capability 调用与旧调用次数；当前 Go revision visibility、审批 role / board 和监督读取五组回归通过，静态合同已改为校验当前 capability registry 遍历、权限过滤及逐 capability revision scope。完整 `prepare-push` 尚须在本修复提交后的新 clean SHA 从头重跑，没有修改运行门禁、删除测试或放宽失败条件。
