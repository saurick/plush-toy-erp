...
- 首次 GitLab release `173` 在发布 `2026.09.17-1` 时失败：CI 回执复用、版本目录、源码检查及五类真实业务 PDF 的 A4、嵌入字体和中文校验已通过，但 Trivy 默认 Google 镜像站连接超时，未登记正式制品。Runner 上已确认官方 GitHub 镜像站可达；扫描入口改为固定官方 `ghcr.io/aquasecurity/trivy-db:2`，继续要求完整漏洞扫描，新候选需重走同 SHA CI 和 release。
