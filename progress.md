...
- 当前状态：代码与正式文档已完成写入，目标仍是 31 个桌面页 / 52 个验收目标，以及 4 条返工回厂收货及补发链、4 条收付款和 3 条红冲固定矩阵；完整实现与原验证计划见本轮完整 progress 归档。
...
- 完成：返工回厂收货和生产订单补齐“导出筛选结果 / 列顺序”；主数据、销售订单、采购订单、采购入库、质量检验、库存台账、物料清单、出货单、委外订单和业务记录等既有入口统一改为按当前筛选读取全部严格分页结果后导出，不再把当前 20 条页面数据误称为筛选结果。导出沿用当前可见列顺序、业务可读状态 / 日期 / 引用值，并提供单飞、取消、空结果和失败反馈。
...
- 浏览器：复用当前外部 Style L1 服务执行生产订单刷新与返工回厂收货手机布局场景 `2 / 2`；另以真实 Chromium 打开两个列顺序弹窗并实际下载 CSV，分别读回返工回厂与生产订单业务内容，1440px 下页面 `scrollWidth = clientWidth = 1440`。
...
## 归档索引
- `docs/archive/progress-2026-08-09-before-rework-intake-closeout.md`：2026-08-03 至 2026-08-05 的非当前页面、移动任务、附件与协作治理过程摘要；已退出的旧业务分支不作为历史兼容保留。
...
- 后续状态：当时缺少返工回厂 Ent 生成物与文档链接同步的阻断已经解除；当前树已生成 `reworkintake` / `reworkintakeitem`，Atlas 迁移和 Schema 文档一致，隔离 strict 回执 `f6d6b0db-3c7a-4e75-8e5f-9aa211c34742` 为 `passed`。本节未连接或 apply 目标数据库、未部署、未完成客户岗位 UAT，也未 stage、commit 或 push。
...
- 验证 / 边界：AGENTS 体积门禁（`16278 bytes`）、Phase 标签边界、角色手册合同和精确 `git diff --check` 通过；当时的跨批次文档链接阻断已由返工回厂领域收口解除。本批只修改治理文档，不改 runtime、schema、migration、API、RBAC、数据库、部署或客户 UAT，也未 stage、commit 或 push。
