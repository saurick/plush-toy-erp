...
第十一次运行已真实证明成品质检、财务放行、仅 `id` 出货读回、出货和应收链连续通过，随后发现验收数据仍直接结清应付 / 应收。当前改由正式 `finance_payment_approval` 完成财务创建、老板审批、财务执行、显式过账核销和结清读回，并保留一个已审批收款样本供浏览器异常流；只有对账事实继续调用 `settle_finance_fact`。相关页面、帮助、DEV 状态目录与客户验收口径已同步，十一轮失败均清理为零残留。
