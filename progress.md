...
- 门禁证据：回执夹具依赖与仓库身份变化失败断言已收紧，定向回归 `15 / 15` 通过；客户配置静态合同也已同步当前 capability registry 遍历、权限过滤及逐 capability revision scope，相关 Go 五组与 standalone boundary 通过。绑定 `084b001f1d433799be467cbc7288cdd92bb6f138` 的后续 `prepare-push` 已越过上述门禁，完成 `1592 / 1592` scripts Node full，并在隔离库成功执行全部 `106` 个 migration（包括已授权的 trigger / function 移除），读回 function / procedure / 非内部 trigger 均为 `0`；随后 critical PostgreSQL suite 暴露返工测试仍用 SQLite 专用 helper 无条件重复插入四个唯一工序。改用现有的“复用并校验兼容工序，否则创建”helper、再为本 fixture 的唯一 BOM 明细绑定并读回 `fabric_processing` 后，critical PostgreSQL 已通过 `190 / 190`、`skip=0` 且完成临时库删除，业务并发断言未变。绑定其提交 `08f1220444b9a775894d6543ae2ef0bed72b1cd0` 的后续 server-all 又暴露 Go schema-doc 测试仍锁定旧的 `74 / 1143 / 139 / 329 / 29 / 247`；canonical `go run ./cmd/schema-doc --check` 已以当前 `74 / 1145 / 141 / 332 / 30 / 248` 读回八份输出零漂移，现仅同步该 Go 计数守卫。完整 `prepare-push` 尚须在本修复提交后的新 clean SHA 从头重跑，没有修改生成器、运行门禁、生成文档或业务代码，也没有删除测试或放宽失败条件。
