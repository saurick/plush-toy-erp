...
- 随后的 CI `174` 再现浏览器首场景 `ERR_NETWORK_CHANGED`，汇总继续按零重试合同阻断。只读 Docker events 与作业日志显示，浏览器开始加载时同机附件集成测试正在销毁网络；DAG 改为等待 PostgreSQL 升级及关键数据库检查结束后再启动浏览器，不忽略网络错误、不放宽重试要求。修正后需使用新 SHA 的 CI 与 release 终态作为交付证据。
