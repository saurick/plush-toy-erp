...
| 生产 bootstrap 管理员初始化 | `runtime_markers` / `runtime_audit_events` | 记录服务启动安全审计和一次性 marker；通过应用层守卫拒绝普通编辑删除，不替代领域业务审计 |
| 系统控制面变更 | `runtime_audit_events` | 记录管理员创建、角色绑定、账号启停、重置密码和角色权限变更的 actor、target、before / after 非敏感摘要；不记录密码、token 或密码 hash |
...
- 权限管理页已记录角色绑定和角色权限变更前后摘要；后续如支持批量授权，应沿用同一系统控制面审计口径。
