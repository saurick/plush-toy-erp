...
- Workflow task 成功而节点对账失败时保留可恢复证据。服务端对账器使用两个有界游标：一条结算终态 linked task 与 active 人工 / 审批节点的缺口；另一条补齐缺失 linked task、已持久化领域结果的节点结算、completed 节点路由和 end / 实例完成缺口。只依据持久证据幂等恢复，单条失败不会永久占满首批。日志使用 `component=process_runtime_workflow_reconciler` 和 task / process / node ID，不保存 payload 或审批意见。
...
- 服务端日志保留 request / trace 等公共字段，JSON-RPC 入口仍有文本日志；错误响应保留 `X-Request-Id`，用户界面展示岗位可理解的提示，不暴露 stack、SQL 或原始异常。
