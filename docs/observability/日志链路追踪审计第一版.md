...
| Workflow 任务创建与状态动作 | `workflow_tasks` 当前状态 | `workflow_task_events` 保存 created、状态、actor role、reason、task version 和 receipt | `confirmed` | 只覆盖单任务；分页有界，不能冒充跨节点完整审批链 |
...
| 附件上传 / 下载 | `business_attachments` 的 owner type / ID、类型、slot、文件元数据、SHA-256、上传人和内容 | 当前没有覆盖全部上传 / 下载动作的统一 audit event | `partial` | 当前存储在产品数据库，不存在对象存储运维链；下载完整性靠长度 / hash 与测试，模块级审计待补 |
| 附件删除 / 作废 | 普通证据附件没有通用 delete；产品图片槽有窄版 replace / clear | 对应边界按所属对象权限处理 | `partial` | 不能把“无删除 API”写成已有作废生命周期；Workflow 附件授权当前另有未提交 WIP，本轮不计入 HEAD 能力 |
...
`list_task_events` 只查询当前任务的处理记录。ProcessRuntime 业务轨迹另由 `get_task_process_context` 根据实例 / 节点锚点查询；两条链必须保持独立错误状态。
