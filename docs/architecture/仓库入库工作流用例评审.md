...
| 应付登记 | 已由已过账采购入库上的独立来源动作和 payable usecase 承担；仍不随 `warehouse_inbound done` 自动生成。 |
...
| `web/src/erp/mobile/pages/MobileRoleTasksPage.jsx` | 仓库岗位任务端采购 `warehouse_inbound done / blocked / rejected` 后走 `complete_task_action / block_task_action / reject_task_action` 并刷新任务列表，不再本地 upsert `inbound_done/blocked`，也不再创建采购应付登记任务。 | 当前前端没有写库存；应付登记只允许从已过账采购入库的独立来源动作进入 payable usecase。 |
...
- `inbound_done` 当前业务状态文案明确为“入库协同已完成”，并提示实际库存以入库单过账结果为准。
...
这条 `warehouse_inbound` 规则始终只属于协同层。当前 Process Runtime 与 Inventory usecase 已分别落地，但库存过账仍必须从正式采购入库单显式执行，不能把任务 `done` 解释成已写库存；幂等、批次、单位、仓库、冲正和事务边界以 Inventory 当前代码与测试为准。
