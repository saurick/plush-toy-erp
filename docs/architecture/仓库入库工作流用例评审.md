...
| `web/src/erp/mobile/pages/MobileRoleTasksPage.jsx` | 仓库岗位任务端采购 `warehouse_inbound done / blocked / rejected` 后走 `complete_task_action / block_task_action / reject_task_action` 并刷新任务列表，不再本地 upsert `inbound_done/blocked`，也不再创建采购应付登记任务。 | 当前前端没有写库存；应付登记后续必须按 payable usecase 单独评审。 |
