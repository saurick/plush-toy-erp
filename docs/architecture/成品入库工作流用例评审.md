...
> 当前出货补充：`shipment_release` 后续已经接入真实来源，但生产者不是 `finished_goods_inbound done`。它只在业务人员从 `DRAFT` 出货单显式提交放行时生成；放行任务完成只表示 `shipping_released`，确认出货仍由 Shipment / Inventory 领域事务独立校验并过账。因此本评审“不由成品入库任务派生出货”的结论保持不变。
...
| `web/src/erp/utils/finishedGoodsFlow.mjs` | 旧 `buildShipmentReleaseTask` 已删除；只保留对后端来源任务 `task_group=shipment_release + source_type=shipments` 的读取识别和业务状态展示。 | 前端不再具备构造保留任务的 helper，真实任务只能来自出货单提交放行。 |
| `web/src/erp/utils/finishedGoodsFlow.test.mjs` | 已测试移动端不再调用旧 follow-up，且 canonical 出货放行来源任务仍可被读取；旧 `shipping-release` 来源不会作为 fallback 识别。 | 前端测试同时保护“不能本地生成”和“能读取后端真源”两条边界。 |
...
## 5. 为什么不由成品入库任务派生 `shipment_release`
...
当前正式触发点已经确定为出货单上的显式“提交放行”，而不是 `finished_goods_inbound done`。提交动作锁定 `DRAFT` 出货单，在同一事务校验并冻结当时的出货前成品检验集合，再按 `workflow.source-task/v1` 生成确定性 `shipment_release` 任务；没有发起成品检验时仍按当前可选检验策略提交，一旦发起则必须达到 `PASSED + PASS / CONCESSION`。

放行任务 `done` 只把协同投影推进到 `shipping_released`，不写库存 `OUT`、不把出货单改为 `SHIPPED`、不生成应收或发票。后续确认出货会在同一 Shipment / Inventory 事务重新核对可信的确定性放行任务确为 `done`，并继续校验质检、来源数量、库存预留和可用量；全部通过后才写真实出货与库存流水。

草稿取消属于出货源单生命周期：没有放行任务时直接取消且不造任务、不碰库存；存在进行中的放行任务时拒绝取消；放行任务已经完成或退回时允许取消源单，并保留原任务结论。只有已真实出货后的取消才写库存冲正。真实出货、草稿取消或出货冲正都会把来源投影推进到 `shipped / cancelled`，但不会改写既有任务处理结论。
...
2. `web/src/erp/utils/finishedGoodsFlow.mjs`
   ...
   - `isShipmentReleaseTask` 只识别后端 canonical `source_type=shipments + task_group=shipment_release`，不保留旧 `shipping-release` 来源 fallback。
...
debug seed、mock、demo 和测试工具均不得生成 `production_scheduling / production_exception / shipment_release` 或占用其确定性编号前缀；需要演示岗位处理时使用明确的 `trial_*` 非正式任务组。三类保留任务只能分别由生产订单下达、返工事实过账和出货单显式提交放行生成。
...
本专项本身没有修改 Ent schema，没有生成 migration，没有写库存流水、库存余额或库存批次，也没有把出货和财务链路并入成品入库任务。后续独立落地的出货放行链已经明确由 `DRAFT` 出货单显式提交产生，不能再把它描述成 `finished_goods_inbound done` 的派生任务。
