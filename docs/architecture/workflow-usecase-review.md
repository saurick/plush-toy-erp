...
当前主干业务闭环已经跑通，其中前七条最小规则进入后端 usecase：第一条“订单提交 -> 老板审批 -> 工程资料任务”的老板审批完成 / 阻塞 / 退回派生规则，第二条“采购到货 -> IQC -> 入库”的 IQC 合格 / 阻塞 / 退回派生规则，第三条“采购仓库确认入库”的 `warehouse_inbound done / blocked / rejected` 协同状态推进规则，第四条“委外回货检验”的 `outsource_return_qc done / blocked / rejected` 委外入库 / 返工补做派生规则，第五条“成品抽检”的 `finished_goods_qc done / blocked / rejected` 成品入库 / 成品返工派生规则，第六条“成品入库”的 `finished_goods_inbound done / blocked / rejected` 协同状态推进规则，第七条“出货放行”的 `shipment_release done / blocked / rejected` 协同状态推进规则。整体主路径现在是“老板审批、IQC、委外回货检验和成品抽检派生后端化 + 采购仓库入库、成品入库和出货放行状态推进后端化 + 出货执行 / 财务等剩余闭环继续前端 v1 编排或手动入口 + 后端保存任务、事件和业务状态”。
...
- 识别边界只允许 `business_status_key` 为空、`warehouse_inbound_pending` 或 `blocked` 进入第六条规则；`blocked` 保持可进入，是为了让重复失败走同一事务内业务状态 upsert 保护。`inbound_done`、`shipment_pending`、`shipment_release_pending`、`shipped` 等已完成或已进入出货语义的状态不再触发第六条特殊规则。
...
- `finished_goods_inbound blocked` 后强制要求原因，写 `blocked` 业务状态，状态归属 `warehouse`，payload 保留 `inbound_task_id`、`finished_goods=true`、`critical_path=true`、`decision=blocked`、`transition_status=blocked` 和 `blocked_reason`；切换自 rejected 时会清理 `rejected_reason` 残留。
- `finished_goods_inbound rejected` 后强制要求原因，写 `blocked` 业务状态，状态归属 `warehouse`，payload 保留 `inbound_task_id`、`finished_goods=true`、`critical_path=true`、`decision=rejected`、`transition_status=rejected` 和 `rejected_reason`；切换自 blocked 时会清理 `blocked_reason` 残留。
...
- 单次后端事务内完成当前成品入库任务状态、状态事件和业务状态 upsert。

后端当前已经额外保证第七条出货放行规则：

- `shipment_release` 任务识别不靠 `task_name` 文案，按 `task_group=shipment_release + owner_role_key=warehouse + source_type in (shipping-release, production-progress, inbound) + payload.shipment_release=true 或 payload.finished_goods=true` 判断。
- 识别边界只允许 `business_status_key` 为空、`shipment_release_pending`、`shipment_pending` 或 `blocked` 进入第七条规则；`blocked` 保持可进入，用于重复失败幂等保护。`shipping_released`、`shipped`、`receivable_pending`、`invoice_pending` 等已完成或后续状态不再触发。
- `shipment_release done` 后写 `shipping_released` 业务状态，状态归属 `warehouse`，payload 保留 `shipment_release_task_id`、`shipment_release_result=done`、`shipment_release_deferred_inventory=true`、`shipment_execution_required=true`、`inventory_out_deferred=true`、`receivable_deferred=true`、`invoice_deferred=true`、`critical_path=true`、`decision=done` 和 `transition_status=done`。
- `shipment_release blocked` 后强制要求原因，写 `blocked` 业务状态，状态归属 `warehouse`，payload 保留 `shipment_release_task_id`、`critical_path=true`、`decision=blocked`、`transition_status=blocked` 和 `blocked_reason`；切换自 rejected 时会清理 `rejected_reason` 残留。
- `shipment_release rejected` 后强制要求原因，写 `blocked` 业务状态，状态归属 `warehouse`，payload 保留 `shipment_release_task_id`、`critical_path=true`、`decision=rejected`、`transition_status=rejected` 和 `rejected_reason`；切换自 blocked 时会清理 `blocked_reason` 残留。
- 第七条规则不创建任何下游任务，不派回 warehouse / sales / pmc / finance，不派生出货执行、`receivable_registration`、`invoice_registration` 或任何财务任务。
- 第七条规则不写 `inventory_txns`，不更新 `inventory_balances`，不创建 `inventory_lots`，不做库存预留 / 冻结或库存扣减；真实出货执行必须单独评审 `ShipmentUsecase / InventoryUsecase`。
- 单次后端事务内完成当前出货放行任务状态、状态事件和业务状态 upsert。
...
- 后续外部系统或批处理如果直接调后端 API，目前只能复用老板审批、IQC、采购仓库入库、委外回货检验、成品抽检、成品入库和出货放行规则，不能复用其余前端编排规则。
...
| 优先级 | 候选 | 评审结论 |
| --- | --- | --- |
| P0 | `update_task_status(done)` 后生成下游任务或推进关键业务状态 | 老板审批、IQC、采购仓库入库、委外回货检验、成品抽检、成品入库和出货放行七条最小规则已落地；其余闭环继续按优先级渐进迁入 |
| P0 | `update_task_status(blocked/rejected)` 后写阻塞状态和异常任务 | 老板审批、IQC、采购仓库入库、委外回货检验、成品抽检、成品入库和出货放行的 blocked/rejected 分支已落地并强制 reason；其余闭环待迁 |
...
| P1 | 出货放行完成 -> `shipping_released / blocked` | 已作为第七条最小规则落地：只迁 `shipment_release done -> shipping_released` 和 `blocked/rejected -> blocked`，不写库存流水 / 余额 / 批次，不推进 `shipped`，不派生应收 / 开票 |
...
这些能力会显著增加系统复杂度，并且当前主干闭环还没有行业专表和后端统一 usecase 的稳定基线，不适合现在引入。
...
8. 已落地的第七条规则继续由后端统一处理 `shipment_release done / blocked / rejected`，只推进 `shipping_released / blocked` 协同业务状态。
9. 前端保留展示、按钮、toast、刷新和字段口径工具，但不再本地创建同一条老板审批、IQC、采购 `warehouse_inbound`、委外 `outsource_return_qc`、成品 `finished_goods_qc`、成品 `finished_goods_inbound` 或 `shipment_release` 后端规则覆盖的下游 / 业务状态。
10. `finishedGoodsFlow.mjs` 的 `buildShipmentReleaseTask` 只保留 seed / test / demo / 未来出货专项辅助口径，不由真实 `finished_goods_inbound done` 动作调用。
11. `shipmentFinanceFlow.mjs` 可以继续保留 seed / test / demo / 手动财务入口辅助，但真实 `shipment_release done` 不再调用它派生 `receivable_registration` 或 `invoice_registration`。
12. 稳定后再迁真实出货执行到应收、应付到对账链路。

其余闭环继续前端 v1 编排的边界是：

- 只用于当前仍未迁入后端的 v1 主干闭环。
- 不接外部系统直接调用。
- 不承诺强一致任务派生。
- 不做并发高频创建。
...
| 项目 | 要求 |
| --- | --- |
| 幂等 | 老板审批、IQC、委外回货检验和成品抽检的下游任务派生已按 `source_type + source_id + task_group + owner_role_key + 非终态状态` 做应用层事务内查询后创建；采购 `warehouse_inbound`、成品 `finished_goods_inbound` 和 `shipment_release` 规则不创建下游任务，只走 business state upsert；暂未加 DB unique constraint，极端并发下后续仍需 DB unique constraint 或 advisory lock 加固 |
| 事务 | 老板审批、IQC、委外回货检验和成品抽检规则已在单次状态更新内完成任务状态、事件、业务状态和下游任务创建；采购 `warehouse_inbound`、成品 `finished_goods_inbound` 和 `shipment_release` 规则已在单次状态更新内完成任务状态、事件和业务状态 upsert；其余闭环仍待迁 |
