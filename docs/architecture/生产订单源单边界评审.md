...
3. `production fact linkage`：已完成本地独立切片。`FINISHED_GOODS_RECEIPT` 从生产订单行及确切包装验收 WIP 批次生成，`MATERIAL_ISSUE` 从发布时冻结的物料需求行生成，`REWORK` 从已过账完工事实生成；REWORK 过账原子创建 HANDWORK 根 WIP、事件和生产异常任务，后代继承来源，CLOSED 只放行该来源链继续办理和补完工。创建和过账复核各自来源、主体、单位、批次与累计数量，取消事实按 REVERSAL / CANCELLED 真源处理，并对已开始补制的 REWORK 失败关闭。订单取消与事实过账共享订单锁域。
