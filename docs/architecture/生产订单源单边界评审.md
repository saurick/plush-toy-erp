...
- 状态：领域边界、schema / migration、repo / usecase、成品入库 fact linkage、API/RBAC runtime、分页 readable option API、销售来源 eligibility 同事务复核，以及 Product Core UI / 通用菜单已完成本地实现；yoyoosun 最小生产订单菜单和生产岗位能力已纳入当前预览配置，seed、active revision、部署与岗位验收仍未接或未证明
...
`production_orders / production_order_items` 已补上销售需求 / 人工批准计划到生产事实之间的正式 Source Document。页面、Workflow payload 或模拟脚本仍不得另带一套生产计划真源；当前剩余缺口是客户 seed、active revision、目标部署 / 岗位验收，以及自动 MRP、工艺路线、WIP、工时和成本等高级生产能力。
...
5. `CLOSED` 表示不再接受新的订单直接领料或完工事实。所有行的有效成品入库量等于计划量时可正常关闭；存在未完成数量时必须填写短关闭原因。关闭只记录当时的计划停止决定，不把既有事实变成不可纠错：后续仍可取消既有事实并写库存 `REVERSAL`，也可从关闭前已过账完工显式发起来源返工；订单保持 CLOSED，不自动补写关闭原因或回退状态。首版不保存重复的 `IN_PROGRESS / COMPLETED` 状态。
...
- `FINISHED_GOODS_RECEIPT` 使用 `PRODUCTION_ORDER + production_orders.id + production_order_items.id`；产品、SKU 和单位从订单行派生，调用方只选择允许的仓库 / 批次并填写数量。
...
- `REWORK` 使用 `PRODUCTION_FACT + 已过账完工事实 ID + 对应生产订单行 ID`；产品、SKU、仓库、单位和批次全部从完工事实派生，调用方只填写返工数量和原因。
- 订单直接领料 / 完工只允许 `RELEASED`，创建与过账都复核来源状态；既有 POSTED 事实的取消 / 冲正以及从既有完工发起的来源返工属于纠错链，不要求订单回退状态，但必须继续复核完整来源结构。
...
3. `production fact linkage`：已完成本地独立切片。`FINISHED_GOODS_RECEIPT` 从生产订单行生成，`MATERIAL_ISSUE` 从发布时冻结的物料需求行生成，`REWORK` 从已过账完工事实生成；创建和过账复核各自来源、主体、单位、批次与累计数量，取消事实按现有 REVERSAL 与 CANCELLED 真源释放有效量。订单取消与事实过账共享订单锁域；不改 Workflow。
