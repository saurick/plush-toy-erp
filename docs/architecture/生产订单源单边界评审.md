...
- 状态：领域边界、schema / migration、repo / usecase、来源领料、精确 WIP 批次完工与成品返工补制闭环、API/RBAC runtime、分页 readable option API、销售来源 eligibility 同事务复核，以及 Product Core UI / 通用菜单已完成本地实现；yoyoosun 最小生产订单菜单和生产岗位能力已纳入当前预览配置，seed、active revision、迁移应用、部署与岗位验收仍未接或未证明
- 作用域：生产订单的 Product Core 真源、生命周期、与销售/BOM/生产事实的边界，以及进入实现的门禁
- 不代表：yoyoosun active revision、客户环境部署和客户签收已经完成，也不代表已具备自动 MRP、完整 MES、成本或工时报工
...
现有 `production_facts` 不能复用为生产订单：它是不可变事实，过账会写 `inventory_txns`，取消使用 `REVERSAL`；生产订单则是可编辑草稿和发布后的计划承诺。Workflow 的排产确认 / 异常任务也不能充当订单真源。

当前已按本评审落下 Ent schema、Atlas migration、repo/usecase、成品入库事实联动、API/RBAC runtime，以及 Product Core UI / 通用菜单：支持 DRAFT 聚合新建 / 整单编辑、发布、按完成量正常关闭或填写原因短关闭、DRAFT 或无有效已过账生产事实时取消 RELEASED 订单、expected_version CAS、V1 receipt 精确重放、同 key 改 intent 冲突和引用真源复核。`FINISHED_GOODS_RECEIPT` 创建与过账会在事实事务内锁定订单，复核订单行的产品、SKU、单位和计划量；有效已过账量按未取消事实聚合。CLOSED 停止新增和过账事实，但不封死既有事实的取消 / 冲正。正式服务已注册独立 `production_order` JSON-RPC 域和 PMC 权限 / production 模块门禁，五类分页 readable option API 与销售来源 eligibility 同事务复核也已完成本地实现；Product Core 页面已通过真实后端浏览器回归。yoyoosun 最小生产订单菜单和生产岗位能力已纳入当前预览配置，seed、active revision 和客户环境部署仍未接或未证明；本轮也不顺带实现完整 MES、工艺路线、工时、成本、报工、排程优化或自动 MRP。

成品返工补制已在本地代码形成独立闭环：路线完工事实以不可变 `production_wip_batch_id` 锚定确切包装验收批次；`REWORK` 过账在同一事务创建带 `origin_rework_fact_id` 的 HANDWORK 根批次、创建事件并投递生产异常任务，后续拆分、转序和返工子批继承该来源。生产订单关闭后只允许这一来源链继续办理工序和按确切包装验收批次登记补完工，普通 WIP 保持只读。来源异常任务达到终态且根批次仍是未执行、无子批 / 质检 / 分配 / 完工依赖的 `PLANNED` 初始状态时，才允许取消已过账 REWORK；已开始补制则必须保留审计链。迁移 `20260730161955_migrate.sql` 已生成并通过本地结构守卫，但尚未应用到目标数据库，也未发布或完成客户岗位 UAT。
...
`production_orders / production_order_items` 已补上销售需求 / 人工批准计划到生产事实之间的正式 Source Document。页面、Workflow payload 或模拟脚本仍不得另带一套生产计划真源；当前剩余缺口是客户 seed、active revision、目标迁移应用 / 部署 / 岗位验收，以及自动 MRP、动态路线编排、工时、成本和完整报工等高级生产能力。
...
5. `CLOSED` 表示不再接受新的订单直接领料或普通完工事实。所有行的有效成品入库量等于计划量时可正常关闭；存在未完成数量时必须填写短关闭原因。关闭只记录当时的计划停止决定，不把既有事实变成不可纠错：后续仍可取消既有事实并写库存 `REVERSAL`，也可从关闭前已过账完工显式发起来源返工；订单保持 CLOSED，不自动补写关闭原因或回退状态。关闭后的唯一补制例外是带 `origin_rework_fact_id` 的成品返工 WIP 来源链，且补完工必须绑定该来源链已验收的确切包装批次。首版不保存重复的 `IN_PROGRESS / COMPLETED` 状态。
6. 订单和行项目不提供物理删除或通用回收站；未发布误建草稿的删除是否需要，留到 usecase 评审，默认先用取消。

## 5. 与生产事实的合同

生产事实仍由现有 `OperationalFactUsecase` 拥有。当前来源合同按事实类型区分：

- `FINISHED_GOODS_RECEIPT` 使用 `PRODUCTION_ORDER + production_orders.id + production_order_items.id`；显式路线行还必须提交确切 `production_wip_batch_id`，并由服务端复核它是该订单行已通过质量关口的包装验收批次。产品、SKU 和单位从订单行派生，调用方只选择允许的仓库 / 入库批次并填写数量；无路线历史行不得伪造 WIP 批次。
- `MATERIAL_ISSUE` 使用 `PRODUCTION_ORDER + production_orders.id + production_order_material_requirements.id`；材料和单位从发布时冻结的需求行派生，调用方只选择仓库 / 既有批次并填写不超过需求剩余量的数量。
- `REWORK` 使用 `PRODUCTION_FACT + 已过账完工事实 ID + 对应生产订单行 ID`；产品、SKU、仓库、单位、入库批次和原完成 WIP 批次全部从完工事实派生，调用方只填写返工数量和原因。过账后由服务端原子创建 HANDWORK 根 WIP、事件和生产异常任务，根批次与全部后代通过 `origin_rework_fact_id` 回到不可变 REWORK 事实。
- 订单直接领料 / 普通完工只允许 `RELEASED`，创建与过账都复核来源状态；既有 POSTED 事实的取消 / 冲正以及从既有完工发起的来源返工属于纠错链，不要求订单回退状态。`CLOSED` 只允许成品返工来源链继续工序和补完工，前端不得根据订单状态或 Workflow payload 补造来源。
- 同一订单行未冲正的 `FINISHED_GOODS_RECEIPT` 净完成量不得超过计划量；同一需求行有效领料量不得超过计划需求；同一完工事实有效返工量不得超过原完工量。并发写入必须使用同一来源锁域和原子校验。
- Workflow payload 只能用于导航和展示，不能推导或覆盖上述来源字段。
- 已过账事实使用现有 `REVERSAL` 取消，不回写或删除订单；订单页面的完成量、领料量和剩余量只能从有效事实聚合。

首版不自动把订单发布变成领料，不把 Workflow task done 变成完工入库，也不把返工任务终态变成自动补制或自动入库；关闭订单不会自动生成财务或成本事实。
...
生产订单使用稳定 page key `production-orders`、路由 `/erp/production/orders`，作为“生产管理”的第一项。页面主任务是：PMC 或生产岗位维护生产计划单，确认产品、规格、销售来源、BOM、计划数量和计划日期后发布；发布后只处理排产确认、关闭或取消，不在本页登记生产事实。

现有两个页面不能复用或改名承接生产订单：

| 页面 | 当前真源 | 保持的职责 |
| --- | --- | --- |
| `production-scheduling` / 排产确认 | `workflow_tasks` | 排产协同、责任岗位、到期跟进、完成 / 阻塞 / 催办；从生产订单动作或任务链接进入 |
| `production-progress` / 生产记录 | `production_facts + inventory_txns` | 生产发料、成品入库、返工及取消 / 冲正事实 |
...
CLOSED 只表示停止新增 / 过账生产事实，不禁止既有事实纠错。订单页不提供“冲正事实”或本地回退订单状态；事实取消 / 库存冲正继续在生产记录的正式 Fact 主路径办理。若后续增加关联入口，只能跳转到按可读生产单号定位的事实页，不能在订单页伪造事实按钮。

生产管理采用轻量侧栏：只保留“生产订单”和“生产记录”。已发布或已关闭订单在具备对应路由权限时显示“排产确认”，并以精确 `source_type + source_id` 打开已有 PMC 待办；异常申请与审批从生产记录内的“异常处理 / 待审批”进入。两个内页继续使用既有 Workflow、Fact、RBAC 和审计真源，不新增排程模型、重复事实或前端双写。
...
2. 页面级 mock 浏览器回归（Style L1）：独立 `production-orders` 场景覆盖列表、筛选、分页、选择、聚合 Modal、新建 / 编辑 / 发布 / 关闭 / 取消、只读终态、空 / 错误 / 无权限、长文本 / 大数字 / 多明细、浅色 / 暗色和横向溢出。
3. 真实后端浏览器回归（E2E）：真实浏览器连接本地 API，证明创建草稿、保存、发布、正常关闭 / 短关闭、取消、stale version、刷新恢复和失败不丢草稿；不使用 mock 证明领域写入。
...
现有公开 API 不能直接作为生产订单表单的完整选项源。生产订单域已完成独立 canonical option API 的本地实现：只增加最小只读投影和查询，并在 create / save / release 事务内复核销售父单 active、来源行 open 及产品 / SKU / 单位一致性；未新增表、字段、migration 或权限体系。PostgreSQL 锁序测试已纳入并通过 critical gate；Product Core UI runtime 与通用菜单已接入，yoyoosun 最小生产订单菜单和生产岗位能力已纳入当前预览配置，seed、active revision 和部署仍未接或未证明。
...
- 不要求调用者额外拥有完整 `sales_order_item.read / bom.read`，因为该 endpoint 只暴露生产计划所需的最小只读投影；这不授予销售订单页或物料清单（BOM）页面访问权。
...
- 现有 production order API/RBAC、race、critical PostgreSQL、full / strict 在实现完成后重跑。页面级 mock 浏览器回归与真实后端浏览器回归留到后续 UI 切片，不能由 option API 单测代替。
...
3. `production fact linkage`：已完成本地独立切片。`FINISHED_GOODS_RECEIPT` 从生产订单行及确切包装验收 WIP 批次生成：生产岗位只创建 `DRAFT` 完工报告，仓库核对实收后确认 `POSTED` 才写成品库存；`MATERIAL_ISSUE` 从发布时冻结的物料需求行生成，`REWORK` 从已过账完工事实生成。REWORK 过账原子创建 HANDWORK 根 WIP、事件和生产异常任务，后代继承来源，CLOSED 只放行该来源链继续办理和补完工。创建和过账复核各自来源、主体、单位、批次与累计数量，取消事实按 REVERSAL / CANCELLED 真源处理，并对已开始补制的 REWORK 失败关闭。订单取消与事实过账共享订单锁域。
4. `API / RBAC`：runtime 已完成独立切片，使用 canonical JSON-RPC、现有 PMC 权限、production module gate 和统一错误码；分页 readable option API 已实现。
5. `UI / browser`：Product Core 独立 Source Document 页面和通用菜单注册已完成本地实现，列表、聚合表单、发布 / 关闭 / 取消、stale version 草稿保留与刷新恢复已通过真实后端浏览器 E2E；浏览器用本机隔离 customer key 绑定通用构建，不注入菜单、权限、数据或 API 响应。yoyoosun 最小生产订单菜单和生产岗位能力已纳入当前预览配置，seed、active revision、部署和目标环境证据仍未接或未证明。

此后若需要动态路线编排、工时、成本、自动 MRP 或客户专属字段，仍必须停止并另行评审，不能借成品返工补制扩大本切片。
