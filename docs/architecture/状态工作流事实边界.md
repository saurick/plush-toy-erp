...
- 生产领料引用发布时冻结的 BOM 物料需求行，生产完工引用生产订单行，返工引用已过账完工事实；来源存疑时 fail closed，不由前端补造。
...
- 完工入库和委外回货必须在源单上选择既有批次或输入新批次，两者互斥；生产领料和委外发料只允许选择既有可用批次。新批次创建、草稿事实和幂等 receipt 在同一事务内落库。
