...
- Workflow 任务已 `done / rejected` 而关联人工 / 审批节点仍 `active` 时，后台只使用任务已持久化的原处理人重放同一幂等结算。该周期对账不扫描领域命令、不补造 Fact，也不代替补偿恢复或逾期调度。
...
- 正式 Atlas migration `20260714055504_migrate.sql` 已承载 Workflow 四状态 CHECK，当前来源驱动事实变更也进入后续 versioned migration；migration 文件和本地 fresh / upgrade 不能证明任一目标数据库已迁移，必须以具体数据库 status、apply 与读回为准。
