...
- 当前应收和开票事实门禁：`FinanceFactCreate` 中 `RECEIVABLE / INVOICE` 必须引用 `source_type = SHIPMENT` 且对应 `shipments.status = SHIPPED`；不能引用 `shipping-release`、草稿出货单或缺失来源。
...
flowchart LR
  ...
  financeGate["Finance source gate<br/>source_type = SHIPMENT<br/>shipment.status = SHIPPED"]
  ...
  shipped --> financeGate
  financeGate --> financeUsecase
  financeUsecase --> financeFact

  task -. 禁止直接写 .-> shipmentFact
  task -. 禁止直接写 .-> inventoryFact
  task -. 禁止直接生成 .-> financeFact
  released -. 不能绕过 .-> financeGate
  released -. 不能等同 .-> wrongLabel
```

上图只把既有边界可视化：`shipment_release done` 当前只能推进协同状态到 `shipping_released`。真实出货、库存扣减、应收、开票和收付款必须分别由对应事实 usecase 承接，不能由 Workflow 任务完成或旧 payload 伪造。当前 `RECEIVABLE / INVOICE` 的最小门禁是引用真实 `SHIPMENT` 来源，且对应 shipment 必须已经 `SHIPPED`。
...
```mermaid
stateDiagram-v2
  state "待开始 / pending" as pending
  state "可执行 / ready" as ready
  state "处理中 / processing" as processing
  state "阻塞 / blocked" as blocked
  state "已退回 / rejected" as rejected
  state "已完成 / done" as done
  state "已取消 / cancelled" as cancelled
  state "已关闭 / closed" as closed

  [*] --> pending: create_task
  pending --> ready: 排队到可执行
  pending --> processing: 开始处理
  ready --> processing: 开始处理
  processing --> blocked: blocked + reason
  processing --> rejected: rejected + reason
  processing --> done: done
  blocked --> processing: 解除阻塞后继续
  blocked --> done: 重新处理后完成
  rejected --> pending: 补资料 / 重开
  rejected --> processing: 重新处理
  ...
  note right of done
    done / cancelled / closed 后不能继续处理或催办。
    特殊 workflow rule 也不会重复触发。
  end note

  note right of blocked
    blocked / rejected 必须提供非空 reason。
    切换分支时清理旧 blocked_reason / rejected_reason。
  end note
```
