...
| `web/src/erp/mobile/pages/MobileRoleTasksPage.jsx` | `runFinishedGoodsFollowUp` 对仓库 `finished_goods_inbound` 已直接 return；真实岗位任务端动作只调用 `update_task_status` 后刷新任务列表，不再本地 upsert `inbound_done`，也不再调用 `buildShipmentReleaseTask + createWorkflowTask`。 | 真实运行时不保留第六条旧前端 follow-up 双写。 |
