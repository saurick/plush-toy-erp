...
| 已落事实类型 | 创建预留、释放预留、消耗预留、列表；正式出库管理页当前不提供页面级无来源新建入口 |
| 状态边界 | `ACTIVE` / `RELEASED` / `CONSUMED`；消耗只能从 active 进入 |
| 库存影响 | 创建预留检查 `inventory_balances - active reservations`，不写 `inventory_txns` |
| 冲正 | 释放和消耗只变更预留状态；已消耗后的库存冲正仍跟随出货事实冲正处理 |
| RBAC | 复用 `sales_order.update`、`warehouse.inventory.read` 和 `warehouse.outbound.confirm` |
| API / UI | `operational_fact` JSON-RPC 暴露 create / release / consume / list；UI 不把菜单显隐当权限边界，正式出库管理页只暴露释放、消耗和查询 |
...
4. 前端 UI：旧内部直达页面 `/erp/operations/facts` 已退出正式 ERP 运行时入口并重定向到业务看板；五条事实链的登记、过账 / 发货 / 释放 / 消耗 / 结清 / 取消动作由生产进度、出货单 / 出库、财务等收窄 V1 页面承接；不再提供客户可直达的内部事实汇总新建页。
...
业务事实扩展已按目标环境内部模拟事实闭环关闭，执行手册为 `客户目标发布验收手册`，本次发布和模拟闭环 evidence 为 `客户目标发布 evidence`。自动派生、报表、打印、核销、物流退货或并发锁升级仍是后续增强，不作为 业务事实扩展 本地最小实现的补尾；若启动这些增强，仍必须按 `docs/product/模块实施治理.md` 明确允许路径、禁止路径、验收命令和停止条件。
