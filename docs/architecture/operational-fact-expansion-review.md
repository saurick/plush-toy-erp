...
- 状态：业务事实扩展 统一 review、本地最小实现闭环、当前目标环境发布、登录态只读 API smoke 和内部模拟事实写入闭环已完成
...
本文最初作为 docs-only review，随后同一 业务事实扩展 内已落本地最小实现闭环：schema / migration、repo / usecase、API / RBAC、最小 UI 和测试。2026-06-08 已发布到当前目标环境并执行 migration 到 `20260608134530`，健康检查、前端业务事实处理路由、未登录鉴权 smoke、目标试用账号 RBAC 核对、登录态只读 operational_fact API smoke 和内部受控模拟事实写入闭环均已通过。客户使用确认属于交付后的业务确认，不再作为 业务事实扩展 完成阻塞；该结论仍不代表真实客户数据导入、完整打印、完整报表、发票和收付款核销已经交付。
...
| API / UI | `operational_fact` JSON-RPC 暴露 create / post / cancel / list；`/erp/operations/facts` 只提交 usecase 动作 |
...
| API / UI | `operational_fact` JSON-RPC 暴露 create / release / consume / list；UI 不把菜单显隐当权限边界 |
...
4. 前端 UI：保留内部直达页面 `/erp/operations/facts`，统一处理五条事实链的创建、过账 / 发货 / 释放 / 消耗 / 结清 / 取消动作；该页面不进入客户侧栏、前端默认菜单或后端内置菜单。
...
业务事实扩展已按目标环境内部模拟事实闭环关闭，执行手册为 `客户目标发布验收手册`，本次发布和模拟闭环 evidence 为 `客户目标发布 evidence`。自动派生、报表、打印、核销、物流退货或并发锁升级仍是后续增强，不作为 业务事实扩展 本地最小实现的补尾；若启动这些增强，仍必须按 `docs/product/implementation-governance.md` 明确允许路径、禁止路径、验收命令和停止条件。
