...
“流”描述系统应该怎样运转；“链”是把一次真实运行中已经持久化的对象和事件，按明确维度排序、串联或投影后的查询结果。流程设计不是运行历史，当前状态不是状态历史，页面聚合也不是新的业务真源。
...
两者必须分层展示。单任务记录不能冒充跨节点审批链，业务轨迹也不能证明库存、质检、出货、生产、核销或财务 Fact 已过账。完整业务链、完整审批处理链、通用状态历史链、通用运行时数据血缘和独立通知链当前都没有统一查询合同；页面遇到证据缺口必须失败关闭或明确标为局部投影。

继续阅读：

1. 流的设计边界见 [各类流程建模边界评审](各类流程建模边界评审.md)。
2. 七条正式 ProcessRuntime 及领域副作用见 [业务与协同流程地图](../workflow/业务与协同流程地图.md)。
...
流至少分为业务流、状态流、工作流、审批流、任务流、异常流、通知流和自动流转。审批流只是工作流中处理审核、同意、拒绝、退回等决定的部分；审批决定不等于业务事实已经发生。

## 链的类型与适用场景

| 链类型 | 对象范围与适用场景 | 当前能力 | 不得推断 |
| --- | --- | --- | --- |
| 业务链 | 跨 Source Document、Workflow / ProcessRuntime 与 Fact 的业务关联；用于回答一笔业务从承诺到事实走到哪里 | **局部投影**：各领域已有来源 ID、ProcessRuntime 锚点和部分 Fact 关联；没有通用跨域业务链 API | 不从任务标题、payload、业务单号文本或页面拼接猜完整链 |
...
| 单任务处理记录 | 单个 WorkflowTask 的创建、状态、转交、阻塞、恢复、完成、退回、催办等事件 | **可直接查询**：`list_task_events`，当前有界返回且最新在前 | 不跨任务拼成完整审批链；不表示来源对象已批准或已过账 |
...
## 七条正式 ProcessRuntime 的轨迹边界

Product Core 当前登记七个 process key。节点图和领域副作用以代码、schema、migration 和测试为准：

| ProcessRuntime | 业务轨迹终点 | 轨迹之外的领域事实 |
| --- | --- | --- |
| `sales_order_acceptance` | 销售订单提交、审批、激活及所选工程 / 复核任务后到 `end` | 不写库存、生产、出货或财务 Fact |
| `material_supply` | 采购订单提交、审批、批准后到 `end` | 不创建收货、IQC、入库或应付 |
| `finished_goods_delivery` | Shipment 财务审批、版本化财务放行门禁后到 `end` | 真实出货、库存 `OUT`、应收和发票走独立领域命令 |
| `sales_return_acceptance` | 审批决定后，批准分支经仓库任务和显式收货到 `end`；拒绝分支到 `rejected_end` | 审批只改 Source；显式收货才写退回库存与 RMA 质检 |
...
`list_task_events` 的对象范围始终是一条任务。它在独立 RBAC 和任务可见性检查后返回任务事件，按稳定事件标识和创建时间最新在前，当前查询有界。记录可以表达任务版本、动作 / 事件类型、前后状态、操作者、角色、原因和责任流转，但只证明这条任务发生过什么。
...
3. 事件分页被截断，却没有明确 `has_more` / cursor 或局部范围说明。
...
| 单任务处理记录 | Product Core 可查询；桌面 / 移动已消费 | 大结果分页 / cursor 合同若扩展须保持稳定排序 |
