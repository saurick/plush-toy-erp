...
“流”描述系统应该怎样运转；“链”是把一次真实运行中已经持久化的对象和事件，按明确维度排序、串联或投影后的查询结果。流程设计不是运行历史，当前状态不是状态历史，页面聚合也不是新的业务真源。流可以在 Product Core 允许范围内选择；链是不可编辑的历史查询结果，不能被客户、页面或管理员重新编排。
...
本地开发“业务链与运行观察台”默认展示完整的 **Product Core 业务链设计总图**：总图只放 12 个链级节点，按主链、供给支撑、异常返工和纠正冲正分区，并使用目录登记的明确关系表达链间衔接；点击任一节点后，才进入该条业务链的来源单据、Workflow、ProcessRuntime、Fact 和状态规则明细。12 条主链、支撑链、异常链、返工链和冲正链覆盖 28 个业务状态对象、6 个 process key / 7 个 variant 和 20 个 Fact / Ledger 定义；另外 6 个横切控制面对象保留在状态目录中，不硬塞进线性业务链。总图不是伪造的第 13 条业务链，也不会把所有链的内部节点挤成一张巨型图。页面可用真实 `task_id` 最多高亮其所属的一条业务链；进入单链后才定位对应的 ProcessRuntime 链段。这张地图回答“正式设计允许怎样衔接”，不是跨域运行历史 API；高亮一个任务也不证明前后 Source Document 或 Fact 已经发生。
...
2. 六条正式 ProcessRuntime 及领域副作用见 [业务与协同流程地图](../workflow/业务与协同流程地图.md)；返工回厂与补发使用独立受控领域链。
...
返工回厂不使用 ProcessRuntime：`rework_intakes` 保存原出货明细和回厂接收状态，接收事务创建 `HOLD` 批次与库存 `IN`，随后由既有生产 `REWORK`、WIP、工序质检 / 完工和 `REWORK_RESHIPMENT` 形成事实链。
...
| --- | --- | --- |
| Product Core 业务链设计地图 | DEV-only 观察台登记 12 条链，覆盖 28 个业务状态对象、7 个 ProcessRuntime variant 和 20 个 Fact / Ledger 定义；目录新增未覆盖对象、未知引用、重复 key 或不可达节点时 fail closed | 只证明当前代码合同的设计覆盖，不证明任一真实业务实例已走完，不替代目标环境 smoke 或客户 UAT |
| Fact / Ledger 定义与凭证 | 20 个定义可查；当前没有通用跨领域运行凭证查询 | 目录、代码引用和本地测试不等于真实凭证；凭证仍须由各领域权威真源和目标环境读回证明 |
