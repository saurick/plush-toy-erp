...
| 生产 | `production_facts` | 生产提交完工只生成 `FINISHED_GOODS_RECEIPT / DRAFT`；仓库核对后确认 `POSTED` 才写成品入库和库存，发料与返工仍按各自正式动作过账 |
...
- 领域事实只有一个写入 owner；Workflow 可触发已经评审的领域动作，但同样经过服务端权限、来源、状态、幂等与事务检查，任务完成不能代替事实过账。看板、余额和统计只能读取事实投影，不能反向推进业务状态。
- 库存、采购、质检、生产、委外、出货和财务的事实能力变更还应覆盖来源不匹配、数量/单位/仓库/批次/SKU 组合、重复提交、事务失败、取消/冲正和余额一致性。
...
对象状态细节见[状态字典与生命周期索引](状态字典与生命周期索引.md)、[状态、Workflow 与 Fact 边界](状态工作流事实边界.md)和当前代码测试。
