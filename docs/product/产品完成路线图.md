...
本文是产品路线图，不替代 `docs/当前真源与交接顺序.md`、代码、Ent schema、Atlas migration、测试或本轮具体任务说明。如果涉及“当前已经实现了什么”，必须回到当前代码、测试、`docs/当前真源与交接顺序.md` 和 `docs/product/产品能力进度台账.md` 交叉确认。
...
| `docs/product/产品能力进度台账.md` | 全局产品能力成熟度与下一步 | roadmap、客户矩阵和当前实现真源 |
...
| 9 | 岗位任务端与岗位协同 | 将高频任务投影到岗位任务端 | mobile task entry、workflow action UI、task attachment entry、role task smoke、simulated workflow closure | 不做空壳岗位任务端；任务附件不改变事实过账，旧文本线索不冒充附件 |
...
| Print Template Candidate | 单客户打印 / 导出样式差异 | 通用模板已存在；多客户重复后再评审行业模板变体 |
| Demo Seed | 演示样本 | 否 |
| QA Debug | 验收或调试输入 | 否 |
| Forbidden | 明确不能做 | 否 |
...
* 不创建 schema 字段。
* 不自动生成出货、库存或财务事实。
* 不把单客户合同格式写进通用模板的数据真源和事实规则。

## 4. 能力切线与领域模型（当前基线）

目标：记录当前 Product Core 已有能力和仍需单独评审的边界，避免早期规划状态继续误导实现。

当前能力切线：

| 分类 | 对象 |
| --- | --- |
| Product Core 已有 | customers、suppliers、contacts、products、product_skus、materials、units、warehouses、processes、BOM、sales_orders、purchase_orders、outsourcing_orders、purchase receipts / quality / inventory / reservation / shipment / production / outsourcing / finance facts、Workflow / ProcessRuntime |
| 客户试用仍需验收 | 永绅角色投影、三类加工合同、主料仓 / 成品仓 / 其他仓、采购与加工打印、岗位任务端、客户配置 revision |
| 已有 schema / migration / repo / usecase / finished-goods fact linkage / Close 领域闭环 / API/RBAC runtime；五类分页可读 option API 与销售来源 eligibility 同事务复核已完成本地实现，真实 PostgreSQL、full、strict 尚待补证；UI runtime / 菜单注册 / seed / 部署未接 | production orders Source Document；CLOSED 阻止新增 / 过账事实但保留既有事实取消与库存冲正；补齐本切片门禁后再实现独立 `production-orders` 页面与通用菜单，不复用 Workflow 排程或 Fact 进度页（见 `docs/architecture/生产订单源单边界评审.md`） |
| 设计候选 | purchase demands、客户地址扩展、结算条款、缺陷明细、完整核销 / 对账明细、扫码和外部集成 |
| 当前不做 | SaaS 多租户、`tenant_id`、完整 MES、总账 / 税控、动态插件平台、低代码表单 / BPMN 平台 |
...
| products / product_skus / materials / units / warehouses | 基础主数据 | BOM SKU 粒度、旧库存重分类、库位和单位换算后续评审 |
...
* `canExecuteRealImport` 必须保持 `false`；真实数据到位并获授权后，通过独立的通用导入批次 usecase 评审进入，不给 dry-run 增加隐藏执行开关。
...
生产、委外、出货、库存预留和财务事实的最小主路径已经实现；后续扩展仍必须按对应 usecase / schema / migration / API / UI / 测试边界评审，模拟试用不能替代客户验收。

## 10. 业务事实扩展：生产 / 委外 / 出货 / 财务事实扩展

目标：补齐 ERP 主干闭环。`docs/architecture/业务事实扩展总评审.md` 覆盖生产、委外、出货、库存预留和财务五条事实链；当前代码已有 schema / migration、repo / usecase、API / RBAC、最小 UI 和测试。`docs/archive/customer-evidence/yoyoosun/operational-fact-target-release-evidence-2026-06-08.md` 只证明当时目标环境的发布与内部模拟结果，不代表当前运行时或客户已签收；每次交付仍需重新执行 preflight、migration、smoke 和验收。后续不先扩大到完整报表、核销、物流退货或自动派生，除非按增强任务重新评审。
...
| 库存预留 / Stock Reservation | 可用量、预留、释放、随出货消费顺序 | 已落 stock_reservations；预留检查余额减 active reservation，消费只随真实出货事务发生 | 不从销售订单直接扣库存，不开放独立消费 |
...
* 岗位任务端保留 v1 当前列表基线，并以 v2 当前选中任务流程承接全屏详情、处理和可信结果回执。
* 详情页统一提供真实任务附件入口；有 `workflow.task.update` 且可办理时可查看与补充附件，跨岗位催办等场景只读查看。
* 动作页只提交完成反馈或阻塞、退回、解除阻塞、催办原因，不再提供自由文本证据框或重复附件上传；新动作不生成 `mobile_action_evidence_refs`。
* 旧任务已有文本证据引用时，只在详情和回执中以“历史处理线索”只读展示；异常报告继续按动作合同展示。
* `/m/<role>/guide` 入口绝对跳转到 `/m/<role>/tasks`，避免 wildcard 形成 `tasks/tasks` 循环。
...
* `smoke:mobile-auth-login-route` 只验证当前生产单端口 `/m/<role>/tasks` 主路径，旧多实例岗位端已退出。
...
* 不把历史 workflow payload 中的 evidence 文本 / refs 冒充真实附件；拍照直传和扫码仍需单独评审。
...
2026-06-09 已新增 `config/private-deployment-template/templateConfig.mjs` 和私有化客户包模板 QA / 模拟闭环脚本，把新增客户包所需的客户资料、客户配置、部署资料、导入 dry-run、客户矩阵、备份恢复、发布 runbook 和验收清单沉淀为私有化复制模板候选。当前模板状态是 `template_candidate`，`runtimeEnabled=false`；`SIM-PRIVATE-DEPLOYMENT` 只作为本地模拟 customer key，不创建正式客户目录。
