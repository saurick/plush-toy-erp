...
Phase 8 已开启，但不拆任何字母子阶段。生产事实、委外事实、出货事实、库存预留和财务事实只作为同一 Phase 内的事实链范围项；每条事实链进入实现前仍必须先完成 docs-only review，确认真源、状态机、冲正、幂等、RBAC、API、UI、测试和停止条件；不得一次性把 schema、runtime、权限、页面和财务 / 库存事实混在同一轮实现里。
...
- 不把 Phase 8 开启、总评审或事实链范围确认写成 runtime、schema、migration、API、UI 或客户交付已经完成。
