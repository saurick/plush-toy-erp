...
本文直接定义本项目的业务域划分，但不是 schema 真源。任何新增表、字段、状态机、API 或页面都必须回到对应架构评审、`docs/当前真源与交接顺序.md`、`docs/product/产品完成路线图.md` 和本轮具体任务说明。

当前仍可用于边界核对的评审文档：

- `docs/architecture/主数据源单据事实边界评审.md`
- `docs/architecture/客户供应商主数据评审.md`
- `docs/architecture/产品款号物料清单边界评审.md`
- `docs/architecture/订单采购边界评审.md`
...
| 业务域 | 正式职责 | 不负责 |
| --- | --- | --- |
| MasterData | 客户、供应商、联系人、物料、产品、SKU、单位、仓库等长期稳定资料 | 不承接临时订单字段或客户样本残值 |
...
| Shipment | 出货计划、财务放行门禁、预留、拣货、实际出库和出货事实 | 不把财务 approval / release 或历史 `shipment_release done` 当 `SHIPPED` |
| Finance | 应收、应付、发票、收付款和对账事实 | 不从财务放行门禁、历史 `shipping_released` 或采购入库任务 done 直接生成财务事实 |
