...
- 工作台只保留少量高频入口，例如销售订单、入库通知/检验/入库、库存和单据模板；这些入口必须标注为常用入口 / 快捷入口，具体客户菜单仍由客户配置和 RBAC 决定。
...
| 常用业务对象 | 侧栏正式业务入口和客户菜单配置 | 是否进一步压缩客户首版菜单数量。 | 原型只展示快捷入口；入口显隐不替代 RBAC 和后端 usecase。 |
