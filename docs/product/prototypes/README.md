...
| `admin-command-center-v1/index.html` | To Implement | Core | 后台工作台标准样板，把工作台收敛为登录后的今日处理台：今日焦点、可点击队列筛选、当前任务上下文、当前任务关联记录入口和交接边界；运行时另有权限完整管理员专属的只读权限关系图，不属于通用业务快捷入口。 |
...
`core-menu-coverage-v1/index.html` 例外地逐菜单列出 51 个项目内候选细项并映射到下列页面类型样板，方便后续确认菜单内容是否遗漏；它是自包含的 To Implement 设计清单，进入实现前仍按当前运行时菜单、正式产品文档、schema、API、RBAC 和测试真源复核。`formal-menu-candidate-v1/index.html` 在覆盖矩阵之上再做一层正式菜单候选，把左侧导航压缩为约 12 个高频主入口；当前正式菜单继续以运行时配置为准。候选菜单进入运行时前，先按 `../菜单映射评审表.md` 对齐当前运行时菜单、正式候选主入口和 51 个内部覆盖细项，再单独评审文案、分组、显隐、路由、权限、seed、测试和正式文档影响。

| 页面类型 | 标准样板 | 参照范围 | 正式吸收边界 |
| --- | --- | --- | --- |
| 工作台 / 总控页 | `admin-command-center-v1/index.html` | 后台首页 / 工作台、任务看板、业务看板和模板打印中心等总控入口可参照；运行态工作台承接待我处理、权限门控的待我审批与阻塞 / 逾期风险队列，只保留当前任务关联记录入口，不再铺通用快捷入口列表。具备员工、岗位、功能权限和客户设置完整只读能力的管理员可从标题栏打开权限关系图。 | 正式菜单仍按菜单评审和客户配置决定；关联记录跳转只作为当前任务上下文入口。权限关系图只汇聚权限相关真源并回到权限配置页维护，不复制任务、单据或业务事实。 |
...
| 产品核心菜单覆盖矩阵 | `core-menu-coverage-v1/index.html` | 首页、我的待办、主数据、销售、采购、质检、库存、生产、外协、出货、财务、数据导入、报表、系统和历史归档等项目内候选细项可用它核对内容覆盖。 | 正式口径回到当前 seedData、客户菜单配置、后端 RBAC、路由、正式菜单承诺和字段真源。 |
...
同类菜单默认参照上述样板，不单独设计。可参照标准列表页的菜单包括客户档案、供应商档案、产品、销售订单、采购订单、加工合同/委外下单、入库通知/检验/入库、库存、待出货/出货放行、出库、生产排单、生产进度、延期/返工/异常、品质检验、对账/结算、待付款/应付提醒、应收/开票登记和发票登记。审计日志已因系统控制面追溯、风险事件和详情 payload 结构独立补为 `audit-log-page-v1/index.html`。项目内更细的 SKU 管理、物料管理、单位管理、仓库库位、采购订单、采购收货、采购退货、库存批次、库存流水、库存预留、库存调整、库存调拨、生产任务、生产上报、生产领料、外协任务、外协发料、外协回货、出货单、装箱/发货明细、应收事实、应付事实、收付款记录、导入任务、导入预检、未匹配队列、订单看板、库存看板、交付看板、用户管理、角色权限、客户配置、系统配置、备份恢复和 Legacy Archive 候选先通过 `core-menu-coverage-v1/index.html` 核对页面内容和样板归属；只有当某个菜单存在独特业务流程、独特信息结构或关键交互差异时，再单独做原型评审。
...
指标卡交互语义样板用于统一只读统计、动作入口和筛选指标的视觉边界。产品核心菜单覆盖样板把 51 个项目内候选细项压成可筛选内容矩阵，帮助后续逐项核对字段、动作、事实源和样板归属；正式菜单候选原型再把这些内部覆盖项压缩为约 12 个高频主入口，用于评审左侧导航候选；审计日志页原型单独覆盖系统控制面追溯、风险事件和事件详情。`../菜单映射评审表.md` 进一步把当前运行时菜单、12 个候选主入口和 51 个内部覆盖细项放到同一张评审表里，用于进入真实菜单实现前拆分文案、分组、显隐、路由、权限、seed、测试和正式文档影响。它们都不改变当前运行时菜单、正式菜单承诺、客户菜单配置、后端 API、RBAC、schema、migration 或 Fact 语义。
