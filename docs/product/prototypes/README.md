...
| `business-form-page-standard-v1/index.html` | To Implement | Core | 销售订单整页编辑样板，对齐订单与客户、联系人与负责人、结算条件、税费与报价运费、交付与收货、附件，以及订货明细、已有规格选择、带出摘要、单位、复制行和明细排序；保留保存 / 返回列表、来源清值和只读评审状态，不提供通用整表重置。 |
...
| 新建 / 编辑业务表单 | `business-form-page-standard-v1/index.html` | 当前以销售订单运行态结构为代表：主字段按业务语义分组，订货明细区包含需求录入、已有规格选择、数量、价格、交期、复制行与明细排序，样板展示已选规格的填写示例；其他菜单主列表的新增 / 编辑复用整页骨架及适用模式，快捷新建、来源选择和专项动作继续使用弹窗，不机械复制销售字段。 | schema、API、RBAC、保存 usecase、默认值及残值 / 缺值防护回到对应主路径；原型评审状态不是运行态字段，来源选择器细节与危险确认回到局部动作弹窗。 |
