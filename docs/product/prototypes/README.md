...
Tab、视图切换、筛选与操作按钮以开发工作台 **产品原型 → 交互控件规范**（`/__dev/prototypes?controls=1`）的真实共享组件样例为准；使用方式与验证合同见[交互控件规范](../../engineering/研发效能工作台与CI-CD设计.md#interaction-controls)。历史原型的局部控件皮肤不作为新实现依据。
...
4. 实现和验证
   ...
   - 至少做页面级浏览器回归（Style L1）；触达共享组件、响应式断点或全局样式时扩大到共享区域或全局布局回归。
...
| `business-detail-page-standard-v1/index.html` | To Implement | Core | 业务只读详情 Modal 样板，对齐共享详情组件的标题、字段描述表、分组明细卡片、固定底部分页和加载 / 空 / 失败状态；不再保留独立详情页外壳或业务动作区。 |
| `business-form-page-standard-v1/index.html` | To Implement | Core | 销售订单整页编辑样板，对齐订单与客户、联系人与负责人、结算条件、税费与报价运费、交付与收货、附件，以及订货明细、行内规格关联、带出摘要、单位、复制行和明细排序；保留保存 / 返回列表、来源清值和只读评审状态，不提供通用整表重置。 |
...
| `mobile-role-tasks-v1/implemented-reference.html` | Current | Core | 岗位任务端列表样式基线（不含旧导航），包含紧凑顶部、我的显示设置、标签、筛选、分批展开、局部刷新反馈和任务选择；详情、处理与结果回执由 v2 唯一定义。 |
...
`core-menu-coverage-v1/index.html` 例外地逐菜单列出 51 个项目内候选细项并映射到下列页面类型样板，方便后续确认菜单内容是否遗漏；它是自包含的 To Implement 设计清单，进入实现前仍按当前运行时菜单、正式产品文档、schema、API、RBAC 和测试真源复核。`formal-menu-candidate-v1/index.html` 在覆盖矩阵之上再做一层正式菜单候选，把左侧导航压缩为约 12 个高频主入口；当前正式菜单继续以运行时配置为准。候选菜单进入运行时前，按 `../菜单与正式入口合同.md` 从结构化运行时真源核对菜单、路由和权限，再单独评审文案、分组、显隐、seed、测试和正式文档影响。
...
| 任务中心 / 职责处理台 | `task-command-center-v1/index.html` | 我的任务、任务看板和工作台风险队列可参照；入口按职责和任务状态组织。任务看板运行时采用全宽分类与单击卡片打开详情，原型中的常驻选中详情仅作设计备选。 | 跨记录、跨模块待办由任务中心承接；三步处理、原因、提交与回执只由 `workflow-task-action-flow-v1` 定义，业务对象入口回到正式菜单。 |
...
| 业务只读详情 | `business-detail-page-standard-v1/index.html` | 销售订单、客户 / 供应商、产品、采购入库、库存批次、质检、出货和财务等只读查看可参照共享详情 Modal。 | 字段来自页面列配置，完整明细来自正式加载函数；底部固定总条数、分页和关闭，编辑、状态动作和附件维护回各自入口。 |
| 新建 / 编辑业务表单 | `business-form-page-standard-v1/index.html` | 当前以销售订单运行态结构为代表：主字段按业务语义分组，订货明细区包含需求录入、行内规格关联、数量、价格、交期、复制行与明细排序，样板展示已选规格的填写示例；其他菜单主列表的新增 / 编辑复用整页骨架及适用模式，快捷新建、来源选择和专项动作继续使用弹窗，不机械复制销售字段。 | schema、API、RBAC、保存 usecase、默认值及残值 / 缺值防护回到对应主路径；原型评审状态不是运行态字段，来源选择器细节与危险确认回到局部动作弹窗。 |
...
同类菜单默认参照上述样板，不单独设计。可参照标准列表页的菜单包括客户档案、供应商与加工厂、产品、销售订单、采购订单、加工合同/委外下单、入库通知/检验/入库、库存、待出货/出货放行、出库、生产排单、生产进度、延期/返工/异常、品质检验、对账/结算、待付款/应付提醒、应收/开票登记和发票登记。审计日志已因系统控制面追溯、风险事件和详情 payload 结构独立补为 `audit-log-page-v1/index.html`。项目内更细的 SKU 管理、物料管理、单位管理、仓库库位、采购订单、采购收货、采购退货、库存批次、库存流水、库存预留、库存调整、库存调拨、生产任务、生产上报、生产领料、外协任务、外协发料、外协回货、出货单、装箱/发货明细、应收事实、应付事实、收付款记录、导入任务、导入预检、未匹配队列、订单看板、库存看板、交付看板、用户管理、角色权限、客户配置、系统配置和备份恢复候选先通过 `core-menu-coverage-v1/index.html` 核对页面内容和样板归属；只有当某个菜单存在独特业务流程、独特信息结构或关键交互差异时，再单独做原型评审。
...
本地工作树运行态已承接本轮看板中心低密度重构：工作台和任务看板减少重复动作，改为“处理任务 / 查看详情 + 查看相关单据”的单一主路径，并增加由当前任务状态、账号可用操作和已授权关联入口派生的只读“处理提示”；“查看相关单据”只有在服务端确认真实来源关联、当前账号可读取来源、入口路由可解析且菜单入口可见时才显示。任务可见性仍独立表达责任、阻塞和到期，不因来源菜单不可见而吞掉任务；来源权限失配时仍可查看任务上下文并保留原有催办能力，但不能完成、阻塞、退回或解除阻塞。有独立转交权限的管理人仍可把任务转给具备来源读取能力的同岗位人员。工作台使用全宽任务队列、单击进入共享详情抽屉，并为具备任一有效审批能力的账号显示“待我审批”；完整计数、筛选和互斥 Workflow 分类由任务看板承接；任务看板使用全宽分类区域，单击卡片直接打开共享详情抽屉，关闭后保留筛选、滚动与焦点，“查看全部”进入分页宽表格。业务看板在全宽业务记录表之前展示关注事项、任务原因和责任，删除重复汇总卡，统计说明按需展开，明确 0 与不可用的差异；跨模块风险任务统一由工作台的阻塞 / 逾期队列和任务看板承接，不再提供重复的通用异常总控页。共享任务处理抽屉已把核对任务、选择处理、确认结果改为可点击步骤，催办只保留为一种处理动作，确认页继续受动作和原因校验约束。移动岗位任务端采用 v1 列表 + v2 选中任务流程：运行时按权限显示任务 / 风险 / 我的三项主导航，有进度读取权限时加入进度；待办 / 已办在任务页内切换，保留筛选和分页；具备任一有效审批能力时在待办内显示“待我审批”，不增加第五个底部导航。岗位状态数量来自服务端首屏同一读快照，满足 `todo=ready+blocked`、`history=done+rejected+withdrawn`、`total=todo+history`；审批、风险、超时是可重叠关注面，监督权限下明确显示“跨岗风险”。浏览器已加载数组只负责当前页渲染，不再冒充岗位总量；独立详情、处理和可信回执结束后使数量快照失效并恢复原列表状态。桌面抽屉、移动详情和已确认回执复用 ProcessRuntime 执行轨迹，只显示已执行 / 当前 / 受阻节点、本任务锚点和重试次数，不把可能属于未选分支的 `waiting` 节点画成确定未来步骤。服务端 `todo / approval / risk / history` 四视图、客户 guard、action explain、真实附件和 Workflow / Fact 边界继续保留。处理提示不是人工填写字段，也不承诺唯一业务结论；人工只填写当前动作合同要求的反馈或原因，任务附件统一在详情页按权限查看或管理，旧证据引用仅作为历史处理线索只读显示。

指标卡交互语义样板用于统一只读统计、动作入口和筛选指标的视觉边界。产品核心菜单覆盖样板把 51 个项目内候选细项压成可筛选内容矩阵，帮助后续逐项核对字段、动作、事实源和样板归属；正式菜单候选原型再把这些内部覆盖项压缩为约 12 个高频主入口，用于评审左侧导航候选；审计日志页原型单独覆盖系统控制面追溯、风险事件和事件详情。[菜单与正式入口合同](../菜单与正式入口合同.md)说明如何从当前菜单、客户投影、路由和 RBAC 真源吸收候选。原型本身不改变运行时菜单、客户配置、后端 API、RBAC、schema、migration 或 Fact 语义。
