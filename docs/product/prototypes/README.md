...
直接用浏览器打开本目录的 `index.html`，或在本地前端开发环境访问 `http://localhost:5175/__dev/prototypes`。PNG 资产通过 `index.html`、`/__dev/prototypes` 或所在 `images/` 目录访问。
...
维护 `index.html` 时保持手工登记和相对链接，不新增依赖或构建工具；新增资产后同步补充名称、类型、状态、所属目录、用途和打开入口。
