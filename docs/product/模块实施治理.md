...
### 2.2 写入重试与结果未知治理 / Write Retry And Unknown-result Governance

`trace_id` / `request_id` 只用于跨层关联日志、事件和调用链，不是业务幂等键，也不能阻止重复执行。新增或修改写入口时，必须按业务影响说明超时、断连、5xx 或响应不可解析后的结果未知语义，并选择一种可验证策略：

| 策略 | 适用边界 | 必须证明 |
| --- | --- | --- |
| durable receipt 精确回放 | 高影响 Source Document、Fact、Workflow，或存在自动重试、外部调用方的写入 | 同一业务意图和 key 返回首次结果且不产生第二次副作用；同 key 改意图会拒绝；事务回滚不留下伪 receipt |
| CAS / 版本校验 + 结果回查 | 有稳定版本真源的状态或配置变更 | 冲突后能区分首次请求已成功还是未执行，迟到重试不会覆盖更新后的状态 |
| 唯一业务意图、天然幂等或终态保护 | 能由唯一约束、同一最终状态或不可逆终态阻止二次生效的写入 | 重复提交不产生第二笔事实或审计副作用，首次结果或当前结果可被可靠查询 |
| 明确不自动重试并登记延期 | 低影响、仅内部使用且没有正式自动重试或外部生产者的入口 | 写明暂缓原因、调用方行为和重新评审触发条件，不把“暂不处理”描述为已具备重放安全 |

不要求所有写操作一律补 receipt，也不新增依赖源码关键词扫描的“写入口语义 hook”。高影响入口仍由对应领域测试覆盖重复提交、并发、事务回滚和结果未知；现有 affected、full、strict 等门禁负责执行已登记的真实测试，不能用静态命中数量代替业务语义验证。

暂缓入口只有在出现自动重试、对外接口或多个正式生产者、真实重复数据 / 结果未知事故、或多人协作持续漏检时，才重新评审局部 receipt 或显式 mutation registry。registry 即使引入也只检查登记完整性，不能替代领域行为测试。

### 2.3 业务列表筛选治理 / Business List Filter Governance
...
PostgreSQL 本地验收脚本、测试环境变量、测试数据库名和 Make target 已改用 `inventory-*`、`bom_lot-*`、`purchase_receipt-*` 和 `purchase_return-*` 业务域命名。历史阶段编号只保留在归档和历史 evidence 中作检索线索，活跃脚本、测试和文档入口不再提供阶段编号兼容命令。
...
- 如果现有文件已经过大或职责混杂，先按业务责任和变更边界确认拆分目标：前端拆页面壳、表格、表单、详情 / 协同入口、状态展示和数据 / action hook；后端按业务域拆 handler、usecase、repo、validator 和 types。拆分必须降低真实耦合，并保持 API、状态语义、测试入口和文档真源稳定。触达页面或样式时，还要同步确认页面主职责、可见模块语义、重复入口、未完成能力误导、空 / 长文本 / 无权限 / 错误态、焦点和键盘可访问性，并把这些状态纳入验收命令或最终盲区说明。
