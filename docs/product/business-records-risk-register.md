...
| 旧页面或旧方法名恢复 | 用户继续误认为旧通用记录是正式入口 | 旧页面只显示退出提示；旧 `business` 方法 unknown method；L1 和单测守卫 | 是 |
...
| P1 | 前端恢复旧通用表单或旧业务 API | 前端只保留退出页和 `dashboard_stats`；L1 / test 继续守卫 |
