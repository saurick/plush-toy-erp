...
| current 客户字段污染 Product Core | 客户专属字段变成通用必填字段，后续产品化失败 | `partners`、`products`、current 样本含付款、税号、地址、SKU 等字段 | 字段先分类为 Product Core Candidate / Customer Material / Print Template / Import Adapter | Productization | 是 |
