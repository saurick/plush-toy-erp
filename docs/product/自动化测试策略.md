...
| `bash scripts/qa/prepare-push.sh`          | clean HEAD 推送前按真实范围复算并执行可闭合的 affected 计划，生成短期回执 | required follow-up、发版前 strict 和目标环境证据 |
| `bash scripts/qa/prepare-push.sh --full`   | 经明确授权后处理 high-risk 计划或发布候选，运行完整本地门禁并生成短期回执 | 发版前 strict 和目标环境证据                     |
...
开发阶段只运行 affected、同名测试、受影响模块和单链浏览器。clean exact SHA 冻结后，`prepare-push` 会按真实 push range 重新生成计划；`localGate=focused` 且没有 required follow-up 时复跑 affected 并签发回执。命中 full local gate、仍有 required follow-up 或作为发布候选时，默认调用只报告原因并停止，必须显式使用 `--full`。同一候选只运行一轮匹配门禁；失败即停止，不自动扩圈或重跑。fixture、mock、选择器、测试文案、开发工作台或证据展示问题若不使生产结论失效，记录为后续事项。
...
网页 GPT 先按 `review/gpt` 相对 `main` 请求 findings、TODO 或实施规划；连接器若不能稳定取得非默认 ref，或需要 GitHub/Codex 的标准行级评论，就为同一 ref 按需建立 Draft PR，评审完成后关闭而不合并。评审意见只是外部输入，Codex 实施前仍回到当前仓库核对；最终代码进入 `main` 前，对最终 HEAD 重新走普通 `prepare-push`，高风险或发布候选仍需显式确认 `--full`。
