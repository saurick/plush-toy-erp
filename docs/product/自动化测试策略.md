...
| T7 业务事实 / E2E | 库存、采购、质检、未来出货、财务、生产、委外真实事实 | 当前已有事实层按 T3 + 领域 PG 防呆测试；`node scripts/qa/mvp-closure.mjs` 可生成 MVP 闭环验收计划和 no-write evidence；完整真实 E2E 后续再设计 | MVP 闭环入口不替代领域测试、浏览器回归或部署 smoke；不存在稳定 runner 时，不得把手工点按或未来命令写成已自动化 |
