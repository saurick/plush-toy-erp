...
| `bash scripts/qa/prepare-push.sh --review --remote github` | 为网页 GPT / GitHub 审查更新固定 review ref，只跑提交格式与严格 secrets   | affected/full、正式 main 推送、发布和 UAT        |
...
| 用户表述 | 目标与顺序 |
| --- | --- |
| “推送代码”，未指定分支或审查意图 | 先更新 GitHub `review/gpt`；GPT 审查与 Codex 核对无有效阻断后，再把同一最终 HEAD 推到 GitLab `main` |
| 明确指定 `review/gpt` 或“让 GPT 分析” | 只更新 GitHub `review/gpt` |
...
项目负责人明确说“提交推送代码让 GPT 分析”时，这句话本身同时授权暂存并提交本轮可证明归属的相关改动，以及把形成的 clean `main` 推到 GitHub 固定 `review/gpt`，无需再分别确认；它不授权吸收无关脏路径，也不表示把代码正式推到 GitLab `main`、发布或部署。所有 writer 收口后不创建本地审查分支，而是依次执行：

```bash
git fetch origin
git merge --ff-only origin/main
git fetch github
bash scripts/qa/prepare-push.sh --review --remote github
git push github refs/heads/main:refs/heads/review/gpt
```

`git merge --ff-only origin/main` 只在工作区干净且本地没有分叉时同步 GitLab 最新提交，不生成 merge commit；已经包含远端提交时是空操作。发生分叉、范围或归属不清、仍有活动 writer、检查失败时停止，不自动生成 merge commit，不 rebase、force 或扩大授权。`--review --remote github` 只允许固定的 `main -> review/gpt`，并要求 GitHub `main` 与已有 `review/gpt` 都是当前 HEAD 的祖先，因此只能 fast-forward。首次建立 `review/gpt` 时范围是 `github/main..HEAD`；以后是上次 review SHA 到当前 HEAD。它运行 `git log --check` 与逐范围严格 secrets，另把 affected 计划的正式推送建议写入独立短期回执，但不执行 affected/full。该回执只能验证这一条 GitHub review ref，不能用于 GitLab `main`、tag、其他 ref、CI、发布或部署。
