...
`affected` 不自动执行会改写生成物的命令，也不能猜测普通业务页面的唯一浏览器场景；它会把这些步骤列为 follow-up。普通前端模块同时核对自身测试和调用方测试，不以存在同名测试作为影响范围完整的证明。DEV 工作台是桌面内部工具：`web/src/dev-workbench/**`、`web/dev-server/**` 和 DEV Style L1 优先走同名或边界测试，页面变更只提示受影响桌面 smoke；涉及 migration、交付命令、凭据或安全桥接时仍追加更强的专项复核。未知路径、全局入口、发布执行逻辑或无独立测试的 QA 脚本可以保守升级，不能静默判为无需测试。`fast` 是跨模块基础检查集合，不是每次小改的默认命令。命令编排、分组、回执、锁和 CI 细节只在脚本及 [`scripts/qa/README.md`](../../scripts/qa/README.md) 维护，不在本文复制。
