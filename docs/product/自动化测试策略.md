...
前置报告不登录、不调用 JSON-RPC、不启动浏览器，也不创建任务或写数据库；显式前端地址不是 yoyoosun 入口时以 `external-base-url-not-yoyoosun-entry` 阻断。真实报告覆盖老板阻塞 / 完成 / 退回、品质岗位完成、仓库入库完成和跨角色催办。它只写本地脱敏报告，包含模拟任务计划 coverage 摘要和未证明项，不保存密码、token、Authorization header、raw customer package 或 action 列表，也不进入 release evidence；因此不证明目标环境发布或客户验收。
