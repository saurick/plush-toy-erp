...
| CAP-008       | business_records 旧入口退出                    | Productization         | Compatibility           |    L5 | 已完成引用审计、cutover plan、data map draft、risk register；`partners / project-orders` 旧路径已退出正式菜单和权限选项，访问旧路径只重定向到正式 V1 入口 | 未做迁移、删除或客户可见归档承诺；其他旧模块仍按领域后续评审 | `docs/product/business-records-*.md`、`businessModules.mjs`、`router.jsx`、`rbac.go` | business_records data migration / archive decision | 旧书签和历史数据仍需培训和迁移口径 | Limited | No |
...
| CAP-010       | V1 前端页面                                   | Product Core           | UI                      |    L7 | customers / suppliers / contacts / sales_orders 页面、路由、桌面正式菜单、dashboard 入口、前后端菜单权限选项和旧路径重定向已完成 | 产品内 docs registry 已下线；未实现客户菜单 runtime config loader | `V1MasterDataPage.jsx`、`V1SalesOrdersPage.jsx`、`businessModules.mjs`、`router.jsx`、`rbac.go` | customer menu config review / import loader design | 旧书签和正式入口切换仍需培训说明 | Yes, with note | No |
...
| CAP-031       | 业务帮助 / 开发验收 / 客户交付说明分离                    | Help / Delivery        | Help / QA               | L1-L2 | 前端产品内文档中心、帮助中心、高级文档和开发与验收页已移除；仓库正式文档和测试策略保留                                           | 普通业务用户版帮助若要恢复，需单独设计产品内入口、菜单、权限和测试 | `docs/current-source-of-truth.md`、`web/README.md`、`docs/product/test-strategy.md` | business-help-entry-redesign-if-needed | 开发术语暴露给业务用户或误导实现状态              | No             | No    |
...
| yoyoosun     | V1 前端页面              | CAP-010             | Trial Ready    | 桌面正式菜单 / route            | 页面可操作，且客户、供应商、销售订单已进入正式菜单；旧 `partners / project-orders` 路径重定向到正式 V1                 | 客户菜单 runtime config loader 未做                 | customer menu config review | 甲方试用入口             | 旧书签仍需培训说明                    |
| yoyoosun     | business_records 旧入口 | CAP-008             | Deprecated     | 不进入正式菜单 / 旧路径重定向 | `partners / project-orders` 仅可作为迁移来源或审计线索，不作为交付写入能力         | 不承诺客户可见归档页；其他旧模块仍按各自领域后续评审            | legacy removal direction / data migration decision | 如需迁移历史数据再确认范围 | 旧数据迁移和培训口径风险                     |
...
| yoyoosun     | 正式菜单入口               | CAP-010 / CAP-011   | Trial Ready    | 桌面正式菜单                 | `客户档案`、`供应商档案`、`销售订单` 已接入桌面菜单、dashboard 和前后端菜单权限选项；旧重叠路径重定向到正式 V1 | 未实现客户菜单 runtime config loader | customer menu config | 确认旧入口不进入正式产品菜单 | 旧书签和培训口径风险 |
...
| DELTA-YOYOOSUN-023 | yoyoosun | 菜单入口                                 | V1 页面可试用需要   | Customer Config / UI                       | 正式菜单已接入核心 V1 入口，旧重叠路径重定向到正式 V1         | 否                 | 保持正式菜单指向 V1；后续再做客户菜单配置和旧数据迁移 / 归档决策 | business_records cutover    | 旧书签和培训口径风险                  | customer menu config / migration decision |
