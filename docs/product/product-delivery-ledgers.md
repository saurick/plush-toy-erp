...
| yoyoosun     | V1 前端页面             | CAP-010             | Trial Ready    | 桌面正式菜单 / route                                                      | 页面可操作，且客户、供应商、销售订单已进入正式菜单；旧 `partners / project-orders` 路径不再保留产品内路由、重定向或权限别名；桌面菜单可由 yoyoosun 菜单配置生成；试用培训说明、账号角色菜单核对清单和目标试用环境执行手册已补                                                                                                                                                       | 当前只能使用模拟数据试用；真实数据导入不可执行                                             | simulated data trial rehearsal / trial feedback / trial environment runbook                       | 甲方试用入口 / 试用账号角色菜单核对                                    | 旧书签和账号权限仍需培训确认                            |
...
| yoyoosun     | yoyoosun 数据导入       | CAP-009             | Internal Ready | 本地 CLI evidence / Markdown 报告 / execution report                      | 已有来源清单、字段分类、unresolved queue、只读 dry-run tooling、source freeze evidence 和受控 JSON-RPC execution loader；当前没有客户真实数据，只作为模拟数据试用前门禁和报告模式工具                                                                                                                                                                                             | 当前不可执行真实导入；模拟数据不代表客户真实数据                                           | simulated data trial rehearsal                                                                  | 字段含义、冲突处理和未来数据治理条件                                   | 样本语义不清，execution loader 或模拟数据可能被误读成已完成导入 |
