...
| CAP-010       | V1 前端页面                                   | Product Core           | UI                      |    L7 | customers / suppliers / contacts / sales_orders 页面、路由、桌面正式菜单、dashboard 入口和前后端菜单权限选项已完成；旧重叠路径不再保留产品内路由、重定向或权限别名；前端桌面菜单已接入客户菜单 config loader；yoyoosun 试用培训说明和账号角色菜单核对清单已补 | 产品内 docs registry 已下线；客户菜单 loader 只控制前端菜单，不替代后端 RBAC；培训说明和核对清单仍需真实试用反馈复核 | `V1MasterDataPage.jsx`、`V1SalesOrdersPage.jsx`、`businessModules.mjs`、`router.jsx`、`rbac.go`、`customerMenuConfig.mjs`、`trial-training-note.md`、`trial-account-role-menu-checklist.md` | import loader design / trial feedback | 旧书签、正式入口和账号权限仍需试用确认 | Yes, with note | No |
| CAP-011       | V1 API/RBAC                               | Product Core           | API / RBAC              |    L7 | JSON-RPC API + 动作权限已完成；试用账号角色菜单核对清单已明确普通试用账号不使用 super admin、不分配 debug_operator，岗位任务端只认 `mobile.<role>.access` | 未接客户化角色模板 runtime；未创建真实试用账号 | `jsonrpc_masterdata_order.go`、`rbac.go`、`trial-account-role-menu-checklist.md` | API smoke / menu permission review / trial account setup | JSON-RPC handler 位于 data 层历史架构；权限模板还需真实账号核对 | Yes, with note | No    |
...
| CAP-029       | Customer Config 配置形态                         | Customer Config        | Productization          | L2-L3 | 已有公司名、logo、主题色、菜单、字段、编号、角色、权限、初始化数据和打印样本引用等配置项口径；yoyoosun 配置包已新增桌面菜单配置并接入前端 loader | 不新增 `tenant_id`，不含通用打印模板引擎；当前 loader 只影响前端菜单，不改变后端 usecase / RBAC / schema | `docs/product/product-delivery-ledgers.md`、`config/customers/yoyoosun/README.md`、`config/customers/yoyoosun/menuConfig.mjs`、`customerMenuConfig.mjs` | customer field / numbering config review | 被误读为 Runtime Tenant、SaaS 多租户或模板系统 | Limited | No |
...
| CAP-031       | 业务帮助 / 开发验收 / 客户交付说明分离                    | Help / Delivery        | Help / QA               | L2 | 前端产品内文档中心、帮助中心、高级文档和开发与验收页已移除；仓库正式文档和测试策略保留；yoyoosun 已新增试用培训说明和账号角色菜单核对清单，明确正式入口、旧入口退出、菜单配置、销售订单边界、普通试用账号和岗位任务端边界 | 普通业务用户版产品内帮助若要恢复，需单独设计产品内入口、菜单、权限和测试；当前清单不创建真实账号或记录密码 | `docs/current-source-of-truth.md`、`web/README.md`、`docs/product/test-strategy.md`、`docs/customers/yoyoosun/trial-training-note.md`、`docs/customers/yoyoosun/trial-account-role-menu-checklist.md` | business-help-entry-redesign-if-needed / trial feedback | 开发术语暴露给业务用户、账号权限误配或敏感信息被写进文档 | Limited | No    |
...
| yoyoosun     | V1 API/RBAC          | CAP-011             | Internal Ready | 后端 JSON-RPC / 试用账号角色菜单核对清单 | 有权限码和后端校验；已明确普通试用账号不使用 super admin、不分配 debug_operator，岗位任务端只认 `mobile.<role>.access` | 未创建真实试用账号；未接客户化角色模板 runtime | 菜单入口 / 用户角色配置 / `trial-account-role-menu-checklist.md` | 角色权限模板 / 试用账号清单 | 权限模板还未客户化，账号误配会影响试用 |
| yoyoosun     | V1 前端页面              | CAP-010             | Trial Ready    | 桌面正式菜单 / route            | 页面可操作，且客户、供应商、销售订单已进入正式菜单；旧 `partners / project-orders` 路径不再保留产品内路由、重定向或权限别名；桌面菜单可由 yoyoosun 菜单配置生成；试用培训说明和账号角色菜单核对清单已补 | 真实数据导入未做                 | import loader design / trial feedback | 甲方试用入口 / 试用账号角色菜单核对 | 旧书签和账号权限仍需培训确认                    |
...
| yoyoosun     | 正式菜单入口               | CAP-010 / CAP-011   | Trial Ready    | 桌面正式菜单 / yoyoosun menu config | `客户档案`、`供应商档案`、`销售订单` 已接入桌面菜单、dashboard 和前后端菜单权限选项；旧重叠路径不再保留产品内路由、重定向或权限别名；`config/customers/yoyoosun/menuConfig.mjs` 已可控制桌面菜单分组、排序、显隐和文案；`trial-training-note.md` 与 `trial-account-role-menu-checklist.md` 已说明旧入口退出、账号角色和岗位任务端边界 | 需用真实试用反馈复核；不创建真实账号或记录密码 | trial feedback / trial account setup | 确认旧入口不进入正式产品菜单，确认试用账号角色 | 旧书签、培训口径和账号权限误配风险 |
...
| yoyoosun     | 客户配置包                | CAP-029             | Config Draft   | config/customers/yoyoosun | 已有目录骨架和配置项口径，桌面菜单配置已接入前端 loader | 不是 `tenant_id`，不含通用打印模板引擎，字段 / 编号 / 角色模板 runtime 尚未做 | customer field / numbering config review   | 公司信息、主题、编号、打印样本记录 | 被误读为 SaaS tenant 或模板系统    |
| yoyoosun     | 客户扩展边界               | CAP-030             | Not Planned    | 无                     | 当前没有需要落地的专属 extension          | 不创建 extension runtime         | 真实出现专属逻辑后再评审              | 暂无                 | 为假想定制过早造层                |
| yoyoosun     | 业务帮助 / 交付说明          | CAP-031             | Draft Ready        | 试用培训说明 / 账号角色菜单核对清单 / 后续业务帮助 | 已新增 `trial-training-note.md` 和 `trial-account-role-menu-checklist.md`，用于说明正式入口、旧入口退出、菜单配置边界、销售订单边界、普通试用账号和岗位任务端边界 | 仍不是产品内帮助中心；不创建真实账号或记录密码；需按试用反馈继续业务化 | trial feedback / business help split review / trial account setup | 客户培训材料 / 试用账号角色菜单确认 | 开发术语误导业务用户或账号权限误配 |
