...
| CAP-019       | stock_reservations                                         | Product Core           | Inventory / Shipment    |         L6 | Phase 8 本地最小实现已完成并已发布到当前目标环境：`stock_reservations` schema / migration、usecase、JSON-RPC、最小 UI 和测试已落；预留检查 `inventory_balances - active reservations`，不写库存流水                                                                                                                        | 目标环境登录态只读 smoke 已通过；客户业务验收未完成；未做并发锁升级、可用量 read model、自动预留或出货自动消耗                     | `stock_reservation.go`、`phase8.go`、`phase8_repo.go`、`jsonrpc_phase8.go`、`Phase8FactsPage.jsx`、`phase8_repo_test.go`、`phase8-target-release-evidence-2026-06-08.md`                                                                                                                                                                                       | customer acceptance / reservation read model              | 预留和出库易混                                                          | Limited        | No         |
| CAP-020       | shipments / shipment_items                                 | Product Core           | Shipment                |         L6 | Phase 8 本地最小实现已完成并已发布到当前目标环境：`shipments` / `shipment_items` schema / migration、usecase、JSON-RPC、最小 UI 和测试已落；`SHIPPED` 才代表真实出货                                                                                                                                                     | 目标环境登录态只读 smoke 已通过；客户业务验收未完成；未做拣货、装箱、物流、退货、打印或自动应收                                     | `shipment.go`、`shipment_item.go`、`phase8.go`、`phase8_repo.go`、`jsonrpc_phase8.go`、`Phase8FactsPage.jsx`、`phase8_repo_test.go`、`phase8-target-release-evidence-2026-06-08.md`                                                                                                                                                                             | customer acceptance / shipment trial / print review       | `shipping_released != shipped`                                          | Limited        | No         |
| CAP-021       | shipment outbound inventory fact                           | Product Core           | Shipment / Inventory    |         L6 | Phase 8 本地最小实现已完成并已发布到当前目标环境：发货按出货行写 `inventory_txns.OUT`，取消已发货出货单写 `inventory_txns.REVERSAL`，测试已覆盖出库和冲正                                                                                                                                                                  | 目标环境登录态只读 smoke 已通过；客户业务验收未完成；未做并发扣减压力测试、退货链路或出货预留自动消耗                               | `phase8_repo.go`、`phase8_repo_test.go`、`inventory_txns`、`phase8-target-release-evidence-2026-06-08.md`                                                                                                                                                                                                                                                     | customer acceptance / inventory regression                | 高风险，必须继续按实现门禁控制                                          | Limited        | No         |
| CAP-022       | AR/AP/invoice/payment/reconciliation                       | Product Core           | Finance                 |         L6 | Phase 8 本地最小实现已完成并已发布到当前目标环境：`finance_facts` schema / migration、usecase、JSON-RPC 和最小 UI 已落，支持 draft / posted / settled / cancelled                                                                                                                                                          | 目标环境登录态只读 smoke 已通过；客户业务验收未完成；未做发票明细、收付款核销、对账单、总账、红冲或自动派生                         | `finance_fact.go`、`phase8.go`、`phase8_repo.go`、`jsonrpc_phase8.go`、`Phase8FactsPage.jsx`、`phase8-target-release-evidence-2026-06-08.md`                                                                                                                                                                                                                   | customer acceptance / finance detail / settlement review  | 不能从放行直接生成财务事实                                              | Limited        | No         |
| CAP-023       | production facts                                           | Product Core           | Production              |         L6 | Phase 8 本地最小实现已完成并已发布到当前目标环境：`production_facts` schema / migration、usecase、JSON-RPC、最小 UI 和测试已落；生产领料 / 返工写 OUT，成品入库写 IN，取消写 REVERSAL                                                                                                                                    | 目标环境登录态只读 smoke 已通过；客户业务验收未完成；未接生产订单专表、移动端岗位任务、成本归集或完整报工                           | `production_fact.go`、`phase8.go`、`phase8_repo.go`、`jsonrpc_phase8.go`、`Phase8FactsPage.jsx`、`phase8_repo_test.go`、`phase8-target-release-evidence-2026-06-08.md`                                                                                                                                                                                        | customer acceptance / production order review             | 不能只靠任务状态                                                        | Limited        | No         |
| CAP-024       | outsourcing facts                                          | Product Core           | Outsourcing             |         L6 | Phase 8 本地最小实现已完成并已发布到当前目标环境：`outsourcing_facts` schema / migration、usecase、JSON-RPC 和最小 UI 已落；委外发料写 OUT，委外回料写 IN，取消写 REVERSAL                                                                                                                                               | 目标环境登录态只读 smoke 已通过；客户业务验收未完成；委外结算不写入委外库存事实，需进入 `finance_facts`；未做委外订单专表、质检对接或应付自动生成 | `outsourcing_fact.go`、`phase8.go`、`phase8_repo.go`、`jsonrpc_phase8.go`、`Phase8FactsPage.jsx`、`phase8-target-release-evidence-2026-06-08.md`                                                                                                                                                                                                                | customer acceptance / outsourcing order review            | 委外发料/回货/结算要分开                                                | Limited        | No         |
...
| CAP-026       | 永绅 yoyoosun 私有化部署包                                 | Delivery               | Deployment              |      L3-L4 | deployments/yoyoosun README 已有；Phase 8 已执行目标环境发布、migration、健康检查、镜像加载、试用账号 RBAC、登录态只读 API smoke 和 evidence 记录                                                                                                                                                                                 | 缺 pre-migration 备份 evidence；仅补 post-deploy 逻辑备份；客户正式验收和受控写入验收未完成                                       | `deployments/yoyoosun/README.md`、`docs/customers/yoyoosun/phase8-target-release-acceptance.md`、`docs/customers/yoyoosun/phase8-target-release-evidence-2026-06-08.md`                                                                                                                                                                                        | customer acceptance                                      | 后续发布必须先补 pre-migration 备份 evidence；客户验收仍需补齐           | Limited        | No         |
| CAP-027       | 客户验收体系                                               | Delivery               | Acceptance              |      L2-L3 | 已有导入验收、阶段验收口径；Phase 8 目标环境发布 smoke evidence 已形成，页面路由、未登录鉴权、试用账号 RBAC 和登录态只读 API smoke 通过                                                                                                                                                                                           | 未执行客户业务验收或受控写入验收；不可把发布 smoke 写成客户签收；当前不可执行真实导入                                               | `docs/customers/yoyoosun/phase7-simulated-trial-acceptance.md`、`docs/customers/yoyoosun/phase8-target-release-acceptance.md`、`docs/customers/yoyoosun/phase8-target-release-evidence-2026-06-08.md`                                                                                                                                                          | customer acceptance                                      | 验收口径要业务化，且不能扩大到打印、报表、核销和真实导入                | No             | No         |
...
| yoyoosun     | 出货事实                | CAP-020             | Target Deployed | Phase 8 目标环境发布 smoke                                                | 目标环境已落 `shipments` / `shipment_items`、发货库存 OUT、取消冲正和统一事实页面；`phase8` 路由和登录态只读 API smoke 已通过                                                                                                                                                                                                                                          | 客户业务验收未完成；不等于出货放行；未做打印、物流、退货或自动应收                          | `phase8-target-release-evidence-2026-06-08.md`                                                   | 出货流程 / 客户验收                                                    | 高风险事实层                                            |
| yoyoosun     | 库存预留                | CAP-019             | Target Deployed | Phase 8 目标环境发布 smoke                                                | 目标环境已落 `stock_reservations`、可用量检查和统一事实页面；预留不写库存流水；`phase8` 路由和登录态只读 API smoke 已通过                                                                                                                                                                                                                                             | 客户业务验收未完成；不从销售订单直接扣库存；未做自动预留或出货自动消耗                      | `phase8-target-release-evidence-2026-06-08.md`                                                   | 是否需要预留 / 客户验收                                                | 容易和出库混                                            |
| yoyoosun     | 财务应收 / 应付         | CAP-022             | Target Deployed | Phase 8 目标环境发布 smoke                                                | 目标环境已落 `finance_facts`、AR/AP/invoice/payment/reconciliation 状态事实和统一事实页面；`phase8` 路由和登录态只读 API smoke 已通过                                                                                                                                                                                                                                | 客户业务验收未完成；不能从放行生成财务事实；未做发票明细、收付款核销或对账单                | `phase8-target-release-evidence-2026-06-08.md`                                                   | 对账/开票/收付款流程 / 客户验收                                        | 不能从放行生成                                          |
| yoyoosun     | 生产事实                | CAP-023             | Target Deployed | Phase 8 目标环境发布 smoke                                                | 目标环境已落 `production_facts`、生产领料 / 返工 OUT、成品入库 IN、取消冲正和统一事实页面；`phase8` 路由和登录态只读 API smoke 已通过                                                                                                                                                                                                                                | 客户业务验收未完成；未接生产订单专表、移动端岗位任务、成本归集或完整报工                    | `phase8-target-release-evidence-2026-06-08.md`                                                   | 排产/领料流程 / 客户验收                                                | 不要只做状态                                            |
| yoyoosun     | 委外事实                | CAP-024             | Target Deployed | Phase 8 目标环境发布 smoke                                                | 目标环境已落 `outsourcing_facts`、委外发料 OUT、委外回料 IN、取消冲正和统一事实页面；`phase8` 路由和登录态只读 API smoke 已通过                                                                                                                                                                                                                                       | 客户业务验收未完成；委外结算进入财务事实；未做委外订单专表、质检对接或应付自动生成           | `phase8-target-release-evidence-2026-06-08.md`                                                   | 加工合同/外发流程 / 客户验收                                            | 委外合同样本需人工确认                                  |
| yoyoosun     | 岗位任务                | CAP-025             | Planned        | 骨架/方向                                                                 | 任务驱动方向明确                                                                                                                                                                                                                                                                                                                                                                    | 真实岗位任务未做                                                                           | mobile task review                                                                                | 哪些岗位用手机                                                         | 不做空壳入口                                            |
| yoyoosun     | 私有化部署包            | CAP-026             | Target Released | deployments/yoyoosun / Phase 8 发布验收手册 / 发布 evidence               | 已执行目标环境镜像加载、migration、健康检查、Compose 重建、试用账号 RBAC、登录态只读 API smoke 和 post-deploy 备份                                                                                                                                                                                                                                                                    | 缺 pre-migration 备份 evidence；客户正式验收和受控写入验收未完成                            | `phase8-target-release-evidence-2026-06-08.md`                                                   | 客户验收                                                               | 运维风险                                                |
| yoyoosun     | 客户验收                | CAP-027             | Acceptance Pending | checklist 草案 / 模拟数据试用目标 / Phase 8 发布验收手册 / 发布 evidence  | Phase 7 本地模拟试用已关闭；Phase 8 目标环境发布 smoke、试用账号 RBAC 和登录态只读 API smoke 通过                                                                                                                                                                                                                                                                                   | 客户业务验收和受控写入验收未做；当前不可执行真实导入；模拟数据不生成真实出货、库存或财务事实 | `phase7-simulated-trial-acceptance.md` / `phase8-target-release-acceptance.md` / `phase8-target-release-evidence-2026-06-08.md` | 验收范围、模拟数据范围、真实导入不可执行边界                           | 范围过大或把发布 smoke 误读为客户签收                   |
