...
| CAP-019       | stock_reservations                                         | Product Core           | Inventory / Shipment    |         L6 | Phase 8 本地最小实现已完成：`stock_reservations` schema / migration、usecase、JSON-RPC、最小 UI 和测试已落；预留检查 `inventory_balances - active reservations`，不写库存流水                                                                                                                                               | 目标环境未 migration / 未上线；未做并发锁升级、可用量 read model、自动预留或出货自动消耗                                             | `stock_reservation.go`、`phase8.go`、`phase8_repo.go`、`jsonrpc_phase8.go`、`Phase8FactsPage.jsx`、`phase8_repo_test.go`                                                                                                                                                                                                                                     | target migration / browser trial / reservation read model | 预留和出库易混                                                          | Limited        | No         |
| CAP-020       | shipments / shipment_items                                 | Product Core           | Shipment                |         L6 | Phase 8 本地最小实现已完成：`shipments` / `shipment_items` schema / migration、usecase、JSON-RPC、最小 UI 和测试已落；`SHIPPED` 才代表真实出货                                                                                                                                                                          | 目标环境未 migration / 未上线；未做拣货、装箱、物流、退货、打印或自动应收                                                           | `shipment.go`、`shipment_item.go`、`phase8.go`、`phase8_repo.go`、`jsonrpc_phase8.go`、`Phase8FactsPage.jsx`、`phase8_repo_test.go`                                                                                                                                                                                                                           | target migration / shipment trial / print review          | `shipping_released != shipped`                                          | Limited        | No         |
| CAP-021       | shipment outbound inventory fact                           | Product Core           | Shipment / Inventory    |         L6 | Phase 8 本地最小实现已完成：发货按出货行写 `inventory_txns.OUT`，取消已发货出货单写 `inventory_txns.REVERSAL`，测试已覆盖出库和冲正                                                                                                                                                                                         | 目标环境未 migration / 未上线；未做并发扣减压力测试、退货链路或出货预留自动消耗                                                     | `phase8_repo.go`、`phase8_repo_test.go`、`inventory_txns`                                                                                                                                                                                                                                                                                                    | target migration / inventory regression                   | 高风险，必须继续按实现门禁控制                                          | Limited        | No         |
| CAP-022       | AR/AP/invoice/payment/reconciliation                       | Product Core           | Finance                 |         L6 | Phase 8 本地最小实现已完成：`finance_facts` schema / migration、usecase、JSON-RPC 和最小 UI 已落，支持 draft / posted / settled / cancelled                                                                                                                                                                               | 目标环境未 migration / 未上线；未做发票明细、收付款核销、对账单、总账、红冲或自动派生                                               | `finance_fact.go`、`phase8.go`、`phase8_repo.go`、`jsonrpc_phase8.go`、`Phase8FactsPage.jsx`                                                                                                                                                                                                                                                                  | finance detail / settlement and reporting review          | 不能从放行直接生成财务事实                                              | Limited        | No         |
| CAP-023       | production facts                                           | Product Core           | Production              |         L6 | Phase 8 本地最小实现已完成：`production_facts` schema / migration、usecase、JSON-RPC、最小 UI 和测试已落；生产领料 / 返工写 OUT，成品入库写 IN，取消写 REVERSAL                                                                                                                                                         | 目标环境未 migration / 未上线；未接生产订单专表、移动端岗位任务、成本归集或完整报工                                                 | `production_fact.go`、`phase8.go`、`phase8_repo.go`、`jsonrpc_phase8.go`、`Phase8FactsPage.jsx`、`phase8_repo_test.go`                                                                                                                                                                                                                                      | production order / mobile task review                     | 不能只靠任务状态                                                        | Limited        | No         |
| CAP-024       | outsourcing facts                                          | Product Core           | Outsourcing             |         L6 | Phase 8 本地最小实现已完成：`outsourcing_facts` schema / migration、usecase、JSON-RPC 和最小 UI 已落；委外发料写 OUT，委外回料写 IN，取消写 REVERSAL                                                                                                                                                                    | 目标环境未 migration / 未上线；委外结算不写入委外库存事实，需进入 `finance_facts`；未做委外订单专表、质检对接或应付自动生成           | `outsourcing_fact.go`、`phase8.go`、`phase8_repo.go`、`jsonrpc_phase8.go`、`Phase8FactsPage.jsx`                                                                                                                                                                                                                                                             | outsourcing order / quality / payable review              | 委外发料/回货/结算要分开                                                | Limited        | No         |
...
| CAP-031       | 业务帮助 / 开发验收 / 客户交付说明分离                     | Help / Delivery        | Help / QA               |         L2 | 前端产品内文档中心、帮助中心、高级文档和开发与验收页已移除；仓库正式文档和测试策略保留；yoyoosun 已新增试用培训说明、账号角色菜单核对清单和目标试用环境执行手册，明确正式入口、旧入口退出、菜单配置、销售订单边界、普通试用账号、岗位任务端和目标环境执行边界                                                                    | 普通业务用户版产品内帮助若要恢复，需单独设计产品内入口、菜单、权限和测试；当前清单和执行手册不创建真实账号或记录密码                | `docs/current-source-of-truth.md`、`web/README.md`、`docs/product/test-strategy.md`、`docs/customers/yoyoosun/trial-training-note.md`、`docs/customers/yoyoosun/trial-account-role-menu-checklist.md`、`docs/customers/yoyoosun/trial-environment-runbook.md`                                                                                                 | business-help-entry-redesign-if-needed / trial feedback  | 开发术语暴露给业务用户、账号权限误配或敏感信息被写进文档                | Limited        | No         |
...
| yoyoosun     | 出货事实                | CAP-020             | Internal Ready | Phase 8 本地最小实现                                                      | 本地已落 `shipments` / `shipment_items`、发货库存 OUT、取消冲正和统一事实页面                                                                                                                                                                                                                                                                                                 | 目标环境未 migration / 未上线；不等于出货放行；未做打印、物流、退货或自动应收                | `phase8-fact-expansion-review.md`、`Phase8FactsPage.jsx`                                          | 出货流程                                                               | 高风险事实层                                            |
| yoyoosun     | 库存预留                | CAP-019             | Internal Ready | Phase 8 本地最小实现                                                      | 本地已落 `stock_reservations`、可用量检查和统一事实页面；预留不写库存流水                                                                                                                                                                                                                                                                                                    | 目标环境未 migration / 未上线；不从销售订单直接扣库存；未做自动预留或出货自动消耗            | `phase8-fact-expansion-review.md`、`Phase8FactsPage.jsx`                                          | 是否需要预留                                                           | 容易和出库混                                            |
| yoyoosun     | 财务应收 / 应付         | CAP-022             | Internal Ready | Phase 8 本地最小实现                                                      | 本地已落 `finance_facts`、AR/AP/invoice/payment/reconciliation 状态事实和统一事实页面                                                                                                                                                                                                                                                                                    | 目标环境未 migration / 未上线；不能从放行生成财务事实；未做发票明细、收付款核销或对账单      | `phase8-fact-expansion-review.md`、`Phase8FactsPage.jsx`                                          | 对账/开票/收付款流程                                                   | 不能从放行生成                                          |
| yoyoosun     | 生产事实                | CAP-023             | Internal Ready | Phase 8 本地最小实现                                                      | 本地已落 `production_facts`、生产领料 / 返工 OUT、成品入库 IN、取消冲正和统一事实页面                                                                                                                                                                                                                                                                                  | 目标环境未 migration / 未上线；未接生产订单专表、移动端岗位任务、成本归集或完整报工          | `phase8-fact-expansion-review.md`、`Phase8FactsPage.jsx`                                          | 排产/领料流程                                                          | 不要只做状态                                            |
| yoyoosun     | 委外事实                | CAP-024             | Internal Ready | Phase 8 本地最小实现                                                      | 本地已落 `outsourcing_facts`、委外发料 OUT、委外回料 IN、取消冲正和统一事实页面                                                                                                                                                                                                                                                                                         | 目标环境未 migration / 未上线；委外结算进入财务事实；未做委外订单专表、质检对接或应付自动生成 | `phase8-fact-expansion-review.md`、`Phase8FactsPage.jsx`                                          | 加工合同/外发流程                                                      | 委外合同样本需人工确认                                  |
...
| yoyoosun     | 客户验收                | CAP-027             | Planned        | checklist 草案 / 模拟数据试用目标                                         | 当前无客户真实数据，Phase 7 不拆字母子阶段，先一次性用 `scripts/seed-phase7-sim-masterdata.sh`、`scripts/qa/phase7-simulated-trial-data.mjs`、seed、fixture 或手工构造的模拟数据验证账号、RBAC、菜单、V1 页面、岗位任务端入口和培训口径                                                                                                                                                  | 完整试点验收未做；当前不可执行真实导入；模拟数据不生成出货、库存或财务事实                 | trial acceptance / simulated data trial rehearsal                                                 | 验收范围、模拟数据范围、真实导入不可执行边界                           | 范围过大或把模拟数据误读为真实导入                      |
...
| yoyoosun     | 业务帮助 / 交付说明     | CAP-031             | Draft Ready    | 试用培训说明 / 账号角色菜单核对清单 / 目标试用环境执行手册 / 后续业务帮助 | 已新增 `trial-training-note.md`、`trial-account-role-menu-checklist.md` 和 `trial-environment-runbook.md`，用于说明正式入口、旧入口退出、菜单配置边界、销售订单边界、普通试用账号、岗位任务端和目标环境执行边界                                                                                                                                                                     | 仍不是产品内帮助中心；不创建真实账号或记录密码；需按试用反馈继续业务化                     | trial feedback / business help split review / trial account setup / trial environment runbook     | 客户培训材料 / 试用账号角色菜单确认 / 目标环境执行记录                 | 开发术语误导业务用户、账号权限误配或敏感信息写入文档    |
