...
| Shipment finance approval / release != shipped | 财务审批和版本化门禁只表示财务已放行，不等于已出库、已发货、已扣库存 |
| 历史 `shipment_release done -> shipping_released` | 只保留既有协同投影，不再是新 Shipment 的 producer 或真实出货门禁 |
...
flowchart TD
  ...
  subgraph customer["客户投影 / Customer Projection"]
    customerDocs["docs/customers/&lt;customer-key&gt;<br/>客户资料 / 客户矩阵"]
