...
当前已新增 `config/private-deployment-template/templateConfig.mjs` 作为模板候选。它只供 QA、文档评审和未来客户包准备使用，`runtimeEnabled=false`，不接运行时。

`deployments/yoyoosun/` 已按当前 yoyoosun 私有化部署资料包落地 env / compose 样例、runbook、checklist、evidence 模板和薄脚本；该目录只保存可交付、可复现、已脱敏的部署资料，不保存真实 `.env`、真实 secret、生产备份或客户 raw files。
...
- 不把 `SIM-PRIVATE-DEPLOYMENT` 创建为正式客户目录。
- 不把单一客户样本直接升为行业默认。
