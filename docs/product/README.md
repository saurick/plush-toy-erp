...
- 产品完成路线图。
- 产品原则、模块边界、配置权限策略。
- 全局产品能力进度台账和产品台账索引；客户交付矩阵和客户差异台账按客户放在 `docs/customers/<customer-key>/`。
- `business_records` 删除记录、正式菜单、导入准备、发布门禁等产品化治理文档。
- 可长期追溯的产品原型资产，用于评审信息架构、交互入口和视觉方向。
