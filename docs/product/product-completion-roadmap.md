...
* 后续扩展打印、报表、核销、客户配置和目标环境发布时仍必须按 `docs/product/implementation-governance.md` 明确允许路径、禁止路径、验收命令和停止条件。
...
只有私有化多客户成熟后，再评审 SaaS。当前已新增 `docs/product/saas-entry-review.md` 作为 SaaS 进入门禁 docs-only 评审入口；评审结论是当前不进入 SaaS 实现，仍优先验证私有化客户包。
