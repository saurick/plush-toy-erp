...
Phase 7 关闭后，Phase 8 可以开始事实层实现，但不得跳过生产、委外、出货、库存预留和财务事实的 usecase / schema / migration / API / UI / 测试评审。

## 10. Phase 8：生产 / 委外 / 出货 / 财务事实扩展

目标：补齐 ERP 主干闭环。Phase 8 不拆任何字母子阶段；`docs/architecture/phase8-fact-expansion-review.md` 已作为统一 review 一次覆盖生产、委外、出货、库存预留和财务五条事实链。当前本地实现已落 schema / migration、repo / usecase、API / RBAC、最小 UI 和测试；2026-06-08 已按 `docs/customers/yoyoosun/phase8-target-release-evidence-2026-06-08.md` 发布到当前目标环境，并通过健康检查、前端路由、未登录鉴权 smoke、目标试用账号 RBAC 核对、登录态 Phase 8 只读 API smoke 和 `SIM-YOYOOSUN-PHASE8` 内部模拟事实写入闭环。Phase 8 按内部模拟事实闭环关闭；客户使用确认属于交付后的业务确认，不作为 Phase 8 完成阻塞。后续不先扩大到打印、报表、核销、物流退货或自动派生，除非按增强任务重新评审。
...
* 不把 Phase 7 本地模拟试用通过写成目标客户环境交付完成。
* 不把 Phase 8 内部模拟闭环写成客户已签收、真实客户数据已导入或完整业务交付。
* 不把打印、报表、核销、物流退货或自动派生当成 Phase 8 必补收尾。
* 后续扩展打印、报表、核销、客户配置和目标环境发布时仍必须按 `docs/product/implementation-governance.md` 明确允许路径、禁止路径、验收命令和停止条件。

## 11. Phase 9：岗位任务端与岗位协同

目标：把高频岗位任务投影到移动端，减少现场回填和等待。Phase 9 不拆任何字母子阶段；2026-06-09 已按 `docs/customers/yoyoosun/phase9-target-release-evidence-2026-06-09.md` 发布到当前目标环境，并通过健康检查、仓库岗位任务端目标路由 smoke、目标试用账号 RBAC 核对和 `SIM-YOYOOSUN-PHASE9` 内部模拟 workflow 闭环。Phase 9 按内部模拟岗位任务闭环关闭；客户使用确认属于交付后的业务确认，不作为 Phase 9 完成阻塞。
...
* `scripts/qa/phase9-simulated-mobile-closure.mjs` 可在报告模式或显式确认后创建 / 更新 `SIM-YOYOOSUN-PHASE9` 模拟 workflow 任务，覆盖老板审批、成品抽检、仓库入库确认和出货放行异常。
...
Phase 10 不拆任何字母子阶段；2026-06-09 已新增 `config/industry-templates/plush/templateConfig.mjs` 和 Phase 10 QA / 模拟闭环脚本，把默认角色、桌面菜单、岗位任务模式、字段显示、编号规则、导入模板、培训验收清单和 deferred 项沉淀为行业模板候选。当前模板状态是 `candidate`，`runtimeEnabled=false`；yoyoosun 只作为候选输入来源，不代表单客户样本已经成为行业默认。
...
* 不写 `business_records`。
* 不写库存、出货、预留、财务或其他事实表。
* 不把 `Phase 8` 或 `事实闭环` 这类内部工程入口放进客户侧栏或行业默认菜单。
...
-> Phase 9 Mobile task workflow simulation and target release
