...
* 不把本地 Phase 8 实现写成目标客户环境已上线、真实客户数据已导入或 Delivery Ready。
* 不在目标环境发布和客户验收前，把打印、报表、核销、物流退货或自动派生当成 Phase 8 必补收尾。
