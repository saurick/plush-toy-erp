...
本文用于描述 `plush-toy-erp` 从项目起步、服务永绅 yoyoosun 甲方、沉淀产品内核，到未来可复制交付多个客户的完整过程和结果。
...
## 0.2 yoyoosun MVP 基线关系

后续如果新增 `docs/product/yoyoosun-mvp-requirements-baseline.md`，它只用于约束 yoyoosun 甲方试用和近期交付 Goal 的执行范围，不能覆盖 `docs/current-source-of-truth.md`、代码、migration、测试，也不能悄悄覆盖本文的长期产品路线。

如果 yoyoosun MVP 基线与本文或当前真源冲突，必须先显式修正文档并写清原因，再拆具体 `docs/codex-goals/*.md` 执行；不能因为永绅 yoyoosun 客户急需某个入口，就把单客户样本直接写成 Product Core、schema 必填项或长期产品路线。
...
| 14 | yoyoosun 数据导入 | yoyoosun 甲方数据 dry-run、人工确认、受控导入 | import dry-run、unresolved queue、loader、acceptance report | 不自动生成出货/库存/财务事实 |
| 15 | 客户试点上线 | yoyoosun 甲方可真实试用主链路 | 私有化部署、迁移、备份、培训、试运行 | 不无备份上线 |
...
| 层级                 | 含义          | 应包含                            | 不应包含           |
| ------------------ | ----------- | ------------------------------ | -------------- |
| Product Core       | 所有客户共用的产品内核 | 主数据、事实模型、核心 usecase、API、权限、审计  | 永绅 yoyoosun 客户专属字段 |
...
| 客户交付矩阵 | yoyoosun 或其他客户是否真的能看到、试用、验收该能力？ | 防止 roadmap 写“可试用”但客户入口、数据、权限、培训或回滚缺失 |
| 客户差异台账 | 客户输入属于 Product Core、Industry Template、Customer Config、Extension、Import、Print Template Candidate、Reporting、Deferred 还是 Forbidden？ | 防止 永绅 yoyoosun 样本字段、打印格式或旧快照字段直接污染 Product Core |
...
| --- | --- | --- | --- | --- |
| Product Core 通用产品内核 | 所有客户共享的 schema、usecase、事实、权限、API / UI 和正式帮助口径 | 产品能力进度台账 + 当前真源 | 继续按主数据、源单据、事实闭环、API / UI、测试和文档逐步演进 | 不写死 yoyoosun 甲方名称、logo、特殊字段或特殊流程 |
| Industry Template 行业模板 | 毛绒玩具行业默认角色、菜单、流程模式、字段样本、编号规则和初始化模板 | 产品能力台账 + 客户差异台账 | 先做模板清单，区分行业共性、永绅 yoyoosun 样本和 deferred 输入 | 不把第一个客户样本或打印格式等同于行业标准 |
...
## 16. 第 14 阶段：永绅 yoyoosun 客户数据导入全过程
...
永绅 yoyoosun 客户数据可受控导入，但不会污染 Product Core，也不会凭旧资料伪造出货、库存、财务事实。
...
| 初始配置            | 公司信息、角色、权限、仓库、单位      | yoyoosun 可使用    | 配置不进 Product Core |
| 数据导入            | dry-run -> 人工确认 -> 导入 | yoyoosun 初始数据就绪 | 不生成事实             |
...
yoyoosun MVP 具体能力清单、客户可见方式、前置条件和风险不放在 roadmap 主文中，统一由 `docs/product/product-delivery-ledgers.md` 的客户交付矩阵维护。更细的近期试用需求可后续拆到 `docs/product/yoyoosun-mvp-requirements-baseline.md`。

本路线图只保留试点目标：

* yoyoosun 甲方可以使用正式主数据和销售订单源单据入口。
* 旧 `business_records` 写入口不再作为正式业务入口。
* yoyoosun 数据经过 freeze、dry-run、人工确认、备份和回滚方案后受控导入。
* 菜单、权限、部署、备份、培训和验收能支撑小范围真实试运行。
* 已有后端事实能力不自动等于本期必须开放完整 UI / API；是否进入试用由客户交付矩阵和单独 Goal 决定。

最终结果：

yoyoosun 甲方能真实试用系统，并且反馈能分类进入产品、配置、模板、扩展或数据导入改进。
...
| 来源         | 沉淀过程      | 最终产物                      |
| ---------- | --------- | ------------------------- |
| yoyoosun 甲方 | 只作为第一客户样本 | yoyoosun config package |
...
014 yoyoosun-customer-import-loader-design
015 yoyoosun-customer-import-loader-implementation
016 customer-import-execution-and-audit
...
如果目标是尽快给 yoyoosun 甲方试用：

```text
workspace checkpoint
-> roadmap / product-delivery-ledgers impact check
-> V1 formal menu and legacy entry exit
-> yoyoosun import loader design
-> yoyoosun import loader implementation
-> yoyoosun import execution and audit
