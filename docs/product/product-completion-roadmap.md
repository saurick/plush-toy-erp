...
| 13 | 旧写入口退役与只读归档       | business_records 退出正式业务写路径，保留 source snapshot / 审计 / 只读归档价值        | transition audit、cutover plan、legacy write retirement plan | 不无证据删除历史数据，也不保留旧写入口            |
...
| readonly archive boundary  | 旧重叠入口退为只读归档或隐藏             | 避免双真源            | 不双写                                 |
...
| Legacy archive entry | 旧入口归档 | 只读、管理员可见或默认隐藏，不作为正式业务入口 |
...
* 旧 `business_records` 重叠入口不再作为正式业务菜单，只能进入只读归档或隐藏。

### 14.2 当前菜单拆分建议

截图中的菜单不应继续作为一组固定入口整体交付。后续应拆成产品能力，再由默认行业菜单和客户菜单配置组合出可见菜单。

| 当前入口 | 能力归类 | 产品默认 | 客户菜单配置 | 业务约束 |
| --- | --- | --- | --- | --- |
| 任务看板 | Work Queue / Workflow | 必须有可达入口 | 可按角色换位置或改名，不应全局关闭 | 如果有任务流转，必须至少有任务入口或移动端任务入口 |
| 业务看板 | Dashboard / Reporting | 默认可见 | 可隐藏 | 不能作为业务事实或状态机真源 |
| 客户/供应商 | MasterData | current MVP 必须 | 可对非相关角色隐藏 | 订单、采购、导入依赖时不能禁用对应后端能力 |
| 产品 | MasterData | current MVP 必须 | 可对非相关角色隐藏 | 订单、BOM、库存依赖时不能禁用对应后端能力 |
| 订单/款式立项 | Source Document | current MVP 必须 | 无销售链路客户可隐藏 | 只是源单据，不自动生成出货、库存、财务事实 |
| 材料 BOM | Product Structure | 行业默认 | 可在未启用 BOM 客户隐藏 | 若启用生产、采购需求或成本核算，需先保证 BOM 入口或导入路径 |
| 辅材/包材采购 | Purchase | 行业默认 | 可按客户是否使用采购模块隐藏 | 隐藏后不得创建只能通过该入口处理的采购任务 |
| 加工合同/委外下单 | Outsourcing | 行业默认可选 | 无委外客户可隐藏 | 隐藏后不得把任务派给不可见且无权限的委外入口 |
| 入库通知/检验/入库 | Purchase / Quality / Warehouse | 行业默认 | 可拆成收货、IQC、入库或按客户隐藏部分入口 | 上游启用采购或质检时，必须保留任务入口或移动端处理入口 |
| 库存 | Inventory | 行业默认 | 可对非仓库角色隐藏 | 库存事实仍由后端 `inventory_txns / inventory_balances` 保证，不能靠菜单显隐控制 |
| 待出货/出货放行 | Shipment Release | 行业默认可选 | 未启用出货链路客户可隐藏 | `shipping_released != shipped`，隐藏入口时不得自动把放行当出库 |
| 出库 | Shipment / Inventory Outbound | 后续评审 | 可在出库事实未启用时隐藏 | 启用后必须走正式 Shipment / Inventory usecase，不从 workflow 直接扣库存 |
| 生产排单 | Production Planning | 后续评审 | 无生产排程客户可隐藏 | 隐藏后不得创建只能由生产排单入口处理的任务 |
| 生产进度 | Production Execution | 后续评审 | 无生产进度客户可隐藏 | 启用后生产事实与 workflow 协同状态必须分开 |
...
| 入口只读归档 | 旧入口保留查看 | 禁止写入正式业务事实 | 只能查历史和审计，不参与新流程推进 |
...
## 15. 第 13 阶段：business_records 旧写入口退役全过程

| 阶段                      | 过程                         | 最终结果                         | 禁止事项          |
| ----------------------- | -------------------------- | ---------------------------- | ------------- |
| 引用审计                    | 查后端/前端/seed/docs/tests     | 知道哪里在用                       | 不改代码          |
| 重叠矩阵                    | 对比 V1 正式模型                 | 知道重叠风险                       | 不自动替换         |
| source snapshot         | 冻结旧数据作为迁移和审计参考             | 有可追溯证据                       | 不写新数据         |
| data map draft          | 设计映射                       | 知道可迁移字段                      | 不真实迁移         |
| dry-run                 | 扫描旧数据                      | 输出可迁移/冲突/待确认                 | 不写正式数据        |
| import decision         | 人工确认导入范围                   | 有批准或阻断依据                     | 不自动批准         |
| hard cutover            | V1 成为唯一正式写入口               | 旧写入口关闭                       | 不双写           |
| readonly archive        | 历史数据只读归档                   | 审计可查                         | 不作为业务入口       |
| remove / hide legacy menu | 隐藏或移出客户正式菜单                | 用户默认走 V1                     | 不保留旧正式写入口     |

最终结果：

`business_records` 从 runtime 正式业务写入口退役，转为迁移来源、source snapshot、审计证据、回滚参考和必要的 admin-only readonly archive。历史数据不无证据删除，但不做长期可写兼容、不双写，也不让旧入口继续作为正式业务菜单。
...
| 旧 `business_records` 写入口退役 | 必须防止双真源；历史数据可作为 source snapshot / readonly archive |
| 新菜单正式入口                  | 用户默认走 V1 正式入口，不再把旧入口作为正式业务菜单              |
