...
本路线图只回答“产品从现在到成熟应该按什么阶段推进、哪些能力先后做、哪些候选 Goal 需要重排”。它不直接回答某项能力当前是否可交付给客户。
...
* 本文调整阶段顺序或候选 Goal 时，应检查三类台账是否需要同步更新。
...
| 12 | 建立正式产品入口          | 桌面端 + 手机端按产品能力、权限职责和客户菜单配置投影入口                                     | V1 pages、menu projection review、customer menu overlay、mobile roles、task entry | 不为每个客户 fork 菜单，也不把菜单隐藏当权限边界 |
...
| menu entry review          | 评审 V1 如何进入正式菜单             | 菜单切换方案           | 不改 seedData 前先评审                    |
| capability projection review | 评审产品能力到菜单 / 权限 / docs / 打印 / mobile 的投影方式 | 后续菜单重构有统一口径      | `Product Capability Registry` 只是 review 目标 |
| customer menu overlay      | 评审每客户如何配置菜单启用、隐藏、排序、文案和默认入口 | 每客户看到自己的菜单       | 不为每个客户 fork 一套菜单；隐藏菜单不是权限边界        |
...
前端从旧通用入口切到正式 V1 产品入口。菜单、移动端、权限、打印和 docs 后续应围绕产品能力投影评审；每个客户的菜单通过 customer menu overlay 配置隐藏、启用、排序和显示文案，而不是复制一套菜单代码。`Product Capability Registry` 不应在本阶段被误读为必须立即实现的新架构层。

### 14.1 菜单目标形态

当前菜单需要从“固定页面列表 / 旧 `business_records` 入口混排”调整为“产品能力基线 + 客户菜单配置”的投影结构。
...
| Default industry menu | 行业默认菜单 | 从产品能力投影出的毛绒玩具行业默认菜单 |
| Customer menu overlay | 客户菜单配置 | 控制每客户启用 / 隐藏 / 排序 / 文案 / 默认入口 |
| RBAC action permission | 后端动作权限 | 决定用户能否执行动作，不能被菜单隐藏替代 |
| Mobile task projection | 手机端岗位入口 | 从任务和职责投影，不直接复制桌面菜单 |
| Docs / print projection | 帮助文档和打印模板入口 | 跟随产品能力和客户启用状态展示 |
| Legacy removal marker | 旧入口退出标记 | 只用于说明旧入口已退出正式产品入口；不承诺客户可见归档页 |
...
* 旧 `business_records` 重叠入口不再作为正式业务菜单，产品方向上默认删除或隐藏；如需保留历史数据，只作为迁移、审计或回滚参考，不作为客户交付入口。

菜单命名和入口颗粒度必须稳定，不应把多个流程节点用 “/” 绑成一个长期菜单名。这里不要求把左侧菜单改成可展开的多级树；当前这种“分组标题 + 功能入口”的形态可以继续存在，但功能入口本身必须表达稳定业务对象或管理页。流程步骤应放在页面内 Tab、按钮、待办、移动端任务或工作流节点里。

颗粒度规则：

| 层级 | 应表达什么 | 不应表达什么 |
| --- | --- | --- |
| 菜单分组 | 稳定业务域，如看板中心、基础资料、销售链路、采购仓储、生产环节；后续可按行业默认调整成销售、采购、仓储、质检、生产、委外、出货、财务、系统等分组 | 多个流程节点拼接名 |
...
| 业务域 | 功能入口推荐 | 流程步骤放置位置 |
| --- | --- | --- |
| 入库 | 入库管理 | 入库通知、质检、入库确认、入库单、入库记录 |
| 出货 | 出货管理 | 出货计划、出货放行、出货单、出货异常 |
| 出库 | 出库管理 | 拣货、出库确认、库存出向流水 |
| 委外 | 委外加工 | 委外单、委外发料、委外回货、委外结算 |
| 销售 | 销售订单 | 审核、履约状态和异常处理 |
| 产品工程 | 款式立项 | 款式资料、工程资料、打样和评审状态 |

### 14.1.1 默认行业核心菜单基线

下面是长期产品方向上的默认行业菜单基线，不代表当前 UI 已全部实现，也不代表每个客户都必须启用。客户菜单 overlay 只能在这套能力基线之上启用、隐藏、排序或改文案，不应重新发明一套业务对象。
...
| 打印 | 打印模板 | Print Template | 行业默认 / 客户配置 | 模板展示可配置，底层数据结构不分叉 |
...
如果客户没有质检、委外或某个移动端入口，应通过模块开关、权限模板和客户菜单 overlay 控制可见入口与待办派生；不能因此改变库存、出货、财务、审计等核心事实规则。
...
### 17.1 Current MVP 试用范围口径

current MVP 只描述 current 甲方试用必须能用到的入口、交付条件和硬边界，不把“后端事实已存在”自动等同为“本期必须暴露完整 UI / API”。更细的当前需求基线后续可拆到 `docs/product/current-mvp-requirements-baseline.md`。

| 能力                       | current MVP 口径                              |
| ------------------------ | ------------------------------------------- |
| 客户 / 供应商 / 联系人           | 必须有正式主数据入口，不能继续依赖旧 `business_records` 写入口    |
| 产品 / 材料 / 单位 / 仓库        | 至少有受控初始化、导入或维护路径；完整 UI 范围按试用链路确认         |
| 销售订单 / 销售订单明细            | 必须有 Source Document 正式入口                    |
| current 数据受控导入           | 必须经过 source freeze、dry-run evidence、人工决策和回滚方案 |
| 旧 `business_records` 写入口退役 | 必须防止双真源；旧入口默认删除或隐藏，历史数据只按迁移 / 审计需要处理 |
| 新菜单正式入口                  | 用户默认走正式产品入口，不再把旧入口作为正式业务菜单              |
| 客户菜单配置                    | 每客户通过配置隐藏、启用、排序和调整菜单文案，不为客户 fork 菜单代码     |
| 基础角色权限                   | 菜单 + 后端动作权限最低可用，菜单隐藏不是安全边界                |
| 关键打印模板                   | 只包含 current 试用实际需要的合同、采购或出货模板               |
| 部署 / 备份 / 回滚             | 真实试用必需                                      |
| 培训 / 验收清单                | 甲方试点必需                                      |

### 17.2 后端事实已存在但暴露范围需另评审

这些能力不能因为已有 schema / repo / usecase / 测试就自动进入 current 试用 UI。是否接 API、菜单、打印、导入和培训，需要按 current 试用链路单独拆 Goal。

| 能力                | current MVP 口径                         |
| ----------------- | -------------------------------------- |
| BOM 基础维护          | 若 current 试用需要维护 BOM，再拆正式入口和验收范围       |
| 采购入库 / 退货 / 调整   | 不能自动等同为本期完整采购 UI；按试用链路拆 API / UI / 权限 |
| 库存流水 / 库存余额 / 批次 | 本期至少不能伪造库存；是否提供用户可见台账另拆 Goal         |
| 来料质检             | 若 current 试用要跑 IQC，再拆状态、入口、权限和打印       |
...
其中 `roadmap / product-delivery-ledgers impact check` 不单独占用编号，除非未来明确需要一次专门的台账收口 Goal。它只要求在拆新 Goal 前回答：
...
```text
013 v1-menu-entry-capability-projection-and-customer-menu-overlay-review
014 current-customer-import-loader-design
015 business-records-legacy-write-retirement-review
...
workspace checkpoint
-> roadmap / product-delivery-ledgers impact check
-> V1 menu entry, capability projection and customer menu overlay review
