...
目标：补齐 ERP 主干闭环，但每条事实链都必须单独评审。Phase 8 已开启，当前入口是 `docs/architecture/phase8-fact-expansion-review.md`；该文档只表示进入总评审和事实链范围确认，不代表 runtime、schema、migration、API、UI 或测试已经完成。Phase 8 不拆任何字母子阶段。

范围项：

| 范围项 | 目标 | 首轮产物 | 不做 |
| --- | --- | --- | --- |
| 生产事实 / Production Facts | 生产订单、生产领料、成品入库、返工事实边界 | production fact review | 不把任务 done 当成入库或成本事实 |
| 委外事实 / Outsourcing Facts | 委外订单、委外发料、委外回货、委外结算边界 | outsourcing fact review | 不把加工合同样式当委外核心规则 |
| 出货事实 / Shipment Facts | 出货计划、放行、拣货、实际出库、冲正边界 | shipment fact review | 不把 `shipping_released` 当成 `shipped` |
| 库存预留 / Stock Reservation | 可用量、预留、释放、并发扣减和出库扣减顺序 | stock reservation review | 不从销售订单直接扣库存 |
| 财务事实 / Finance Facts | AR/AP、invoice、payment、reconciliation 和冲正边界 | finance fact review | 不从放行或任务状态直接生成财务事实 |
...
* 不直接新增 schema / migration。
* 不直接新增 production、outsourcing、shipment、reservation 或 finance runtime API。
* 不让 WorkflowUsecase 写库存、出货、应收、应付、发票、收付款或对账事实。
* 不把 Phase 7 本地模拟试用通过写成目标客户环境交付完成。
* 每条事实链进入实现前必须按 `docs/product/implementation-governance.md` 完成专项评审和门禁确认。
