...
目标：补齐 ERP 主干闭环。Phase 8 不拆任何字母子阶段；`docs/architecture/phase8-fact-expansion-review.md` 已作为统一 review 一次覆盖生产、委外、出货、库存预留和财务五条事实链。当前本地实现已落 schema / migration、repo / usecase、API / RBAC、最小 UI 和测试，但仍不代表目标客户环境已 migration / 已上线 / 已完成客户验收。
