...
| CAP-008       | business_records 旧入口退出                                | Productization         | Compatibility           |         L6 | 已完成引用审计、cutover plan、data map draft、risk register；`partners / project-orders` 旧入口定义、旧路径重定向和权限别名已删除，正式菜单和权限选项只保留领域入口，普通 `business` API 已全量冻结 `business_records` create / update / delete / restore；业务看板 `dashboard_stats` 已切为领域 usecase 投影；旧 `BusinessModulePage` 运行时已禁用新建、保存、状态流转、删除、恢复、手工创建协同任务和从 archive 记录继续派生旧链路动作；移动岗位任务端不再回写 `business_records` 状态；当前 dev DB 中 `partners=0`，`project-orders` 4 条 debug seed 已软归档，相关 debug workflow task / state 已清理为 0 | 未做客户库 / 生产库迁移、物理删除或客户可见归档承诺；旧页面仍可能作为 legacy/archive 查询、字段参考或打印带值候选                                                         | `docs/product/business-records-*.md`、`businessModules.mjs`、`router.jsx`、`rbac.go`、`business_record.go`、`jsonrpc_business.go`、`BusinessModulePage.jsx`、`MobileRoleTasksPage.jsx`                                                                                                                                                                                                                                                    | business_records data migration / physical archive decision       | 旧书签和历史数据仍需培训和迁移口径；未评审旧数据迁移、物理归档或客户可见归档页                                      | Limited        | No         |
