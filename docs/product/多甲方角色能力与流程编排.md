...
状态顶层设计、中文 / English 双树、`Current / Planned / Deferred` 演进标记，以及各对象的 canonical key、流转规则、数据库约束、中文显示映射和测试入口，统一见 [`../architecture/状态字典与生命周期索引.md`](../architecture/状态字典与生命周期索引.md)；Workflow / Fact 的事务与推进边界见 [`../architecture/状态工作流事实边界.md`](../architecture/状态工作流事实边界.md)。本文只保留角色、菜单、字段和流程投影口径，不再复制易漂移的状态枚举。
