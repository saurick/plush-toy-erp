...
| `boss` | 老板/管理审批 | 全局看板、任务、采购审批 | 审批/退回协同任务，审批采购订单 | 审批任务 `ready/processing/done/rejected` | 审批不绕过领域 usecase；不直接改明细 |
...
状态按所有者分层，不建立一个万能 `business_status` 覆盖所有领域：
...
└─ BOM: DRAFT → ACTIVE → ARCHIVED / DISABLED
...
流程节点
└─ waiting / active / completed / skipped / failed / blocked

协同任务
└─ pending / ready / processing / blocked / done / rejected / cancelled / closed

处理结果 outcome
└─ approved / rejected / returned / waived / completed / 领域命令返回结果
```

`rejected` 是任务终态和节点 outcome，不是领域单据通用状态。退回后如需重做，ProcessRuntime 创建新的节点 attempt 或回到明确节点；不能把同一已结算任务重新打开。
