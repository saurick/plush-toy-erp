...
- Product Core 的 RBAC、权限中心审批责任、客户配置 revision/effective session、ProcessRuntime、Workflow tasks 和主要领域 usecase 已存在，但 `config/customers/yoyoosun/customerPackage.mjs` 中多条 workflow 仍标记 `preview_only`，不能写成已在生产自动运行。正式配置只承接已登记的销售订单、采购订单和 Shipment 财务放行；付款、库存调整、PMC / 工程自由审批不得由 preview 配置伪造。
