...
这里的“实际动作”是通过基础授权层筛选后的**有效动作权限集合**，不是数值运算，也不表示该动作在当前业务对象上一定能够执行成功：

- `∩` 表示集合交集：后端 RBAC、模块状态和当前生效配置必须同时允许该动作。
- `-` 表示集合扣除：前述交集中的动作仍要移除当前角色明确 revoke 的部分。
- 单角色先独立计算自己的 entitlement 与 revoke；多角色账号再把各角色结果取并集，不能先合并所有角色后统一扣除 revoke。
...
```text
RBAC          = {shipment.read, shipment.create, shipment.ship}
enabled 模块   = {shipment.read, shipment.create, shipment.ship}
entitlement   = {shipment.read, shipment.create}
revoke        = {shipment.create}
有效动作       = {shipment.read}
```

这个结果只回答“基础授权层允许哪些动作”。真正的当前可执行动作还必须叠加来源读权限、owner / assignee / 责任池、数据范围、对象状态、版本 / CAS、幂等和审批、质检、库存等领域门禁；任一条件失败，服务端都必须拒绝执行，前端显示或隐藏不能代替后端校验。
...
本矩阵把“页面上能看到什么”和“服务端实际允许什么”分开。前端隐藏不是安全边界；super admin 可以管理配置或在受审计条件下执行明确的控制面动作，但不能凭管理身份冒充销售、采购、财务、仓库、品质或生产岗位。任一业务动作都必须重新执行：

```text
实际动作 = 后端 RBAC ∩ enabled 模块 ∩ active revision entitlement - 当前角色 revoke
```

并叠加来源读权限、owner role / responsibility pool / assignee、数据范围、状态、版本 / CAS 和幂等合同。

| 业务对象 | 页面 / 移动端入口 | 用户可见动作 | 后端命令或 JSON-RPC | 前置状态 | RBAC 权限码 | owner role / 责任池 / assignee | active revision entitlement | 版本 / CAS | 幂等键 | Source Document 变化 | Workflow / ProcessRuntime 变化 | Fact / Ledger 副作用 | 取消、冲正或恢复 | 审计证据 | 当前实现层级 / 目标环境证据 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 销售订单 | 销售订单页；统一审批箱；桌面 / 移动任务详情 | 提交审批、审批、工程资料、复核 | `start_sales_order_acceptance_process`、`execute_sales_order_acceptance_submit`；approval 结算后内部激活命令 | Source `DRAFT`；对应节点 `active` | `sales_order.submit + sales_order.read`；审批任务另需 `workflow.task.approve` | 提交人为 sales；approval 使用 `approval.sales_order` 主办 / 备用 / 升级和可选 assignee；后续工程 / PMC 责任池 | 必须命中当前动作 capability 和 customer scope | Source 聚合保存 CAS；节点 / 任务 `expected_version` | start / domain command / task action 各自稳定 key | `DRAFT → SUBMITTED → ACTIVE` | 创建实例、approval task，批准后推进所选工程 / 复核节点到 `end` | 无库存、出货、生产或财务 Fact | Source 取消按订单门禁；任务退回使流程受阻，恢复按同一实例证据处理 | Source 生命周期、任务事件、节点 durable result、request_id | Product Core L7；目标 active revision、岗位 smoke、发布和 UAT 仍分别取证 |
| 采购订单 | 采购订单页；统一审批箱；桌面 / 移动任务详情 | 提交审批、采购审批 | `start_material_supply_purchase_order_process`、`execute_material_supply_purchase_order_submit`；approval 后内部批准命令 | Source `DRAFT`；approval 节点 `active` | `purchase.order.update + purchase.order.read`；审批任务另需 `workflow.task.approve` | purchase 提交；`approval.purchase_order` 主办 / 备用 / 升级和可选 assignee | 必须命中 `purchase.order.update / workflow.task.approve` 对应 entitlement | Source / node / task version CAS | start、submit、approval 使用各自稳定 key | `DRAFT → SUBMITTED → APPROVED` | `submit_purchase_order → purchase_order_approval → approve_purchase_order → end` | 不创建收货、IQC、库存或应付 | 关闭 / 取消走采购 Source usecase；任务退回不伪造采购拒绝状态 | Source 状态、任务事件、节点 durable result、request_id | Product Core L7；采购需求读模型 blocked，目标发布 / 岗位 / UAT 未证明 |
| Shipment 财务放行 | 出货单页；统一审批箱；桌面 / 移动任务详情 | 启动财务审批、审批放行 | `start_finished_goods_delivery_process`；approval 结算后内部 `shipment.finance_release` | Shipment `DRAFT`；已有质检必须通过 / 让步；门禁 `PENDING` | `shipment.create + shipment.read`；审批任务另需 `workflow.task.approve` | `approval.shipment_finance` 主办 / 备用 / 升级和可选 assignee | 必须命中 Shipment 页面、启动和 approval entitlement | Process node / task version；门禁 `finance_release_version` CAS | start、approval 与自动命令各自稳定 key | Shipment 业务状态仍为 `DRAFT`；只把版本化门禁写为 `APPROVED` | `shipment_finance_approval → shipment_finance_release → end` | 无 `SHIPPED`、库存 `OUT`、应收或发票 | 草稿取消受 active 流程 / 质检 / 财务依赖门禁；历史 `shipment_release` 不参与新单恢复 | approval task、ProcessRuntime 节点、门禁 actor / time / anchors、request_id | Product Core L7；目标 migration、active revision、财务岗位 smoke 和 UAT 未证明 |
| Shipment 真实出货 | 出货单页；仓库岗位入口 | 确认出货、撤销已出货 | `ship_shipment`、`cancel_shipment` | `DRAFT + finance_release_status=APPROVED`；质检、来源、预留和库存均满足 | `shipment.ship`；取消另需 `shipment.cancel` | warehouse 业务岗位和仓库数据范围；不由 approval assignee 继承 | 页面 / 动作 entitlement 必须有效 | Shipment / 来源 / 库存事务锁与领域 CAS | 领域稳定幂等键 | `DRAFT → SHIPPED`；取消后 `CANCELLED` | 不推进或改写已完成财务 approval task | 原子写 Shipment Fact 与 `inventory_txns.OUT`；已出货取消写 `REVERSAL` | 草稿取消零库存；已出货取消保留原事实并追加反向流水 | Shipment / Inventory 事实、actor、时间、原因、request_id | Product Core L7；目标数据库 apply、仓库 smoke 和 UAT 未证明 |
| 客户退货 | 客户退货页；老板审批任务；仓库任务 | 提交审批、批准 / 退回、确认收货、取消 / 冲正 | `start_sales_return_acceptance_process`、`execute_sales_return_receive` 及领域 cancel / reverse | 退货 `DRAFT`；receive 前必须 `APPROVED` 且仓库任务已结算 | `sales_return.create / approve / receive / cancel / reverse` 加来源读权限 | approval 固定 boss；执行固定 warehouse；仍校验精确 assignee | 固定流程 capability 仍必须命中 active revision entitlement | Source / node / task version CAS | start、task action、receive / reverse 各自稳定 key | approval 只写 `APPROVED / REJECTED`；receive 写 `RECEIVED` | 批准后创建仓库任务；拒绝到 `rejected_end` | receive 才写退回库存和 RMA `HOLD` / 质检；reverse 追加反向效果 | cancel / reverse 由下游质检和库存依赖门禁 | Source 决定、任务事件、节点 result、库存 / RMA 事实、request_id | Product Core L7；退款 / 换货 / 补发 blocked，目标发布和 UAT 未证明 |
| FinancePayment | 收付款核销页；老板审批任务；财务执行任务 | 登记并审批、确认过账核销、冲正 | `start_finance_payment_approval_process`、`execute_finance_payment_post`、领域 reverse | Payment `DRAFT`；post 前 `APPROVED` 且财务任务已结算 | `finance.payment.create / approve / post / reverse` 加来源读权限 | approval 固定 boss；执行固定 finance；精确 assignee | 固定流程 capability、finance 页面与动作 entitlement 必须有效 | Source / node / task version CAS；分配行锁 | start、task action、post / reverse 各自稳定 key | approval 只写批准 / 拒绝；post 写 `POSTED`；reverse 写 `REVERSED` | 批准后创建财务执行任务；拒绝到 `rejected_end` | post 才写多来源分配和结清读模型；reverse 写反向结果 | reverse 独立，不重开原 approval task | Source 决定、任务事件、分配 / 结清 / 反向事实、request_id | Product Core L7；银行直连 / 总账 / 税控范围外，客户财务 UAT 未证明 |
| 人工库存调整 | 库存台账调整入口；老板审批任务；仓库执行任务 | 提交、审批、过账 | `start_inventory_adjustment_approval_process`、`execute_inventory_adjustment_submit`、`execute_inventory_adjustment_post` | `MANUAL_ADJUSTMENT + DRAFT`；post 前 `APPROVED` | `warehouse.adjustment.create / approve`；任务另需 Workflow 权限 | submit / execute 为 warehouse；approval 固定 boss；精确 pool / assignee | 固定流程 capability 与仓库动作 entitlement 必须有效 | Source / node / task version；库存 grain 事务锁 | start、submit、task action、post 各自稳定 key | `DRAFT → SUBMITTED → APPROVED / REJECTED → POSTED` | 批准后创建仓库执行任务；拒绝到 `rejected_end` | 只有 post 写 `inventory_txns` 和余额 | 草稿取消零库存；已过账错误走对应调整 / 冲正合同，不改原流水 | Source maker-checker 字段、任务 / 节点、库存交易、request_id | Product Core L7，正式审批已接；目标 migration、仓库 smoke 和 UAT 未证明 |
| 生产异常决定 | 生产异常处置页；老板审批任务；生产执行任务 | 提交报废 / WIP 让步 / 超领申请、审批、显式执行 | `start_production_exception_approval_process`、`execute_production_exception_process` | Decision `SUBMITTED`；SCRAP / 让步执行前 `APPROVED` | `production.exception.submit / approve`；执行还需生产事实动作权限 | approval 固定 boss；SCRAP / 让步执行固定 production；精确 assignee | 固定流程 capability 与生产动作 entitlement 必须有效 | Decision / node / task version；WIP / 需求事务锁 | start、task action、execute / reverse 各自稳定 key | approval 只写决定和批准数量 | SCRAP / 让步进入生产任务；超领到 `over_issue_end`；拒绝到 `rejected_end` | 显式 execute 才写 SCRAP / WIP 效果；超领额度由正常领料 Fact 消费 | 报废 / 让步按下游门禁冲正；超领审批不直接造库存 | Decision、任务 / 节点、WIP / Fact、额度消费、request_id | Product Core L7；目标发布、生产岗位 smoke 和 UAT 未证明 |
| Workflow 单任务 | 任务看板、业务 Drawer、`/m/<role>/tasks` | 完成、阻塞、退回、恢复、催办、桌面转交 | `complete_task_action / block_task_action / reject_task_action / resume_task_action / urge_task / reassign_task` | `ready` 或 `blocked` 的精确合法转换；终态 settled | `workflow.task.read / update / complete / approve / reject / assign` 按动作精确要求 | owner role、责任池、assignee 和任务冻结 revision 同时匹配 | 必须命中该 task capability 与 customer scope | `expected_version` CAS | 每次业务 intent 使用稳定 key，receipt replay | 不直接改变任意 Source，除非 linked ProcessRuntime 结算后调用登记领域命令 | 写任务状态、事件、receipt、归属或节点结算；业务轨迹与本任务记录分开 | task action 本身无 Fact；领域副作用只由随后白名单命令写入 | `blocked → ready` 清旧原因；终态不重开；转交不改状态 / Fact | `workflow_task_events`、运行审计、节点 result、request_id | Product Core runtime 已接；目标岗位真实账号 smoke 和客户 UAT 另验 |

### 客户流程编排与链查询边界

集中治理链和流的价值，是让每个甲方都在同一套 Product Core 合同内做差异化，而不是让客户配置变成第二套业务引擎。

| 维度 | 甲方可以配置 | Product Core 固定 / 当前禁止 |
| --- | --- | --- |
| 流程选择 | 在当前 active revision 中选择销售、采购、Shipment 财务放行、客户退货、收付款、库存人工调整和生产异常七条已登记 ProcessRuntime；销售只能选择已登记受控变体 | 不能增加、删除或重排节点，不能注入脚本、BPMN、任意状态机或领域命令 |
| 审批责任 | 销售、采购、Shipment 财务放行三项可维护启停、主办 / 备用 / 升级角色或具名成员 | 客户退货、收付款、库存人工调整和生产异常使用注册合同固定 approval / execution 责任池 |
| 页面与动作 | 模块状态、`rolePageProjections`、`access_entitlements` 和角色 revoke 在后端 RBAC 上限内收窄 | `businessFlows / stateMachines / processPolicies` 当前 preview-only；不能提升 RBAC 或改写 Source / Fact |
| 在途版本 | 新实例使用当时 active revision；任务和实例冻结 revision 后继续按 `active / superseded` 合同授权 | 新 revision 不改写旧实例，不把 published-only 或 preview revision 当运行真源 |
| 链查询 | 后端授权后可按客户岗位收窄对象可见性、筛选和展示字段 | 链是持久化历史的不可编辑投影；不能补写、删除、改序，也不能把局部任务记录改名为完整审批链 |

新增甲方时的安全顺序是：

1. 先确认真实岗位、模块、Source / Fact 和需要启用的七条注册流程，不从客户表格标题猜流程。
2. 再编译客户 manifest，校验模块、页面、entitlement、revoke、三类可配责任和四类固定责任。
3. 发布并激活 immutable revision 后读回 effective session；失败时不回退到前端全量菜单或管理员权限。
4. 用真实岗位账号验证可见页面、可执行动作、责任池、业务轨迹和本任务处理记录；业务链仍只显示当前已持久化证据。
5. 最后分别完成目标 migration / 结构读回、部署、岗位 smoke 和客户 UAT；这些交付证据不能由本地测试代替。
...
| `boss` | 老板/升级审批 | 全局看板、任务、重大异常与升级审批 | 承接备用 / 升级责任，不替代各领域日常主办 | 审批任务 `ready/blocked/done/rejected` | 审批不绕过领域 usecase；不直接改明细 |
...
固定审批事项当前只开放销售订单、采购订单和 Shipment 财务放行。管理员从“权限管理 → 审批责任”维护启停、主办、备用、升级、岗位成员或具名员工；草稿先预览有效候选和阻塞原因，再通过一次“保存并生效”按 active revision/hash CAS 原子写入 immutable revision、完整投影和 active 切换。该动作同时要求发布与激活权限，失败整体回滚；通用客户配置控制面仍保留独立 publish / activate 流水线。启用事项必须至少保留一个责任成员，停用事项可以清空成员；事务提交后，停用只阻断尚未创建的新流程，并在 ProcessInstance 与来源写入前失败关闭，不改写旧 revision 的在途实例。具名员工必须启用且持有所选岗位，多岗位员工可按其任一真实岗位承接；每个层级只允许一个成员，运行时只开放最低可用层级，主办失效后才依次交给备用和升级。普通管理员不能把自己或自己持有的任一业务角色加入新的有效责任，系统 / 调试角色不能作为审批成员。已具名分配的待办在高优先级成员恢复后仍由原处理人办理；员工停用、注销或被移除负责岗位才会在账号变更事务中把未结束待办退回原冻结责任池，不改写业务事实。
...
- Product Core 的 RBAC、权限中心审批责任、客户配置 revision/effective session、ProcessRuntime、Workflow tasks 和主要领域 usecase 已存在，但 `config/customers/yoyoosun/customerPackage.mjs` 中多条 workflow 仍标记 `preview_only`，不能写成已在生产自动运行。正式 runtime manifest 当前可选择销售订单、采购订单、Shipment 财务放行、客户退货、收付款、库存人工调整和生产异常七条注册流程；权限中心仍只允许维护前三项的主办 / 备用 / 升级责任，后四项使用注册流程合同的固定责任池。PMC / 工程自由审批不得由 preview 配置伪造。
