...
Status / 状态: Audit / 审计
Runtime Implemented / 运行时已实现: Superseded By Archive Guard / 已由 archive 守卫覆盖
...
本文件是 009 docs-only 审计结论。后续运行时已经将普通 `business` API 写入口全量冻结为 legacy/archive 只读；当前实现真源以 `docs/current-source-of-truth.md` 和 `docs/product/business-records-cutover-plan.md` 为准。
...
- source snapshot。
- 调研入口。
- 旧通用业务页面、打印和移动端任务跳转的历史承载层。
- 早期尚未拆专表前的通用业务记录保存层；当前普通写入口已冻结。
...
`business_records` 可以保留历史文本和快照，但不能用旧页面继续写正式业务或扩展已经有正式模型的核心能力。
...
4. 永绅 yoyoosun 客户产品样本字段必须先分类为 Product Core Candidate、Customer Material、Print Template Input / Candidate、Reporting 或 Import Adapter input。
...
当前普通 `business` API 写入已整体冻结，下面条件只保留为历史审计判断，不再作为逐模块继续开放写入的门槛。
...
当前实现已经不再只按 `partners`、`products` 和 `project-orders` 分批处理；全部普通 `business_records` create / update / delete / restore 都按 archive read-only 处理。
...
- 正式入口已替代对应写入路径。
- 旧页面只承担历史查看或 demo。
- seedData、Dashboard、菜单权限、移动端跳转、测试和产品内帮助入口均已完成切换；当前产品内 docs registry 已下线。
...
| 永绅 yoyoosun 客户专属字段 | 可能只是客户材料、打印样本输入或导入适配字段 |
...
- 旧订单直接生成出货、库存、财务事实。
- 永绅 yoyoosun 客户样本字段直接变成 Product Core 必填字段。
