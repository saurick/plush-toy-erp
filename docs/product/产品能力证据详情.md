...
- 所属层 / 业务域：Product Core / MasterData
- 当前成熟度：L5-L7
- 当前结果：已有既有 schema / runtime 能力；产品档案可选维护 `numeric(20,6)` 的产品单重（净重），按产品默认单位以 kg 保存，空值为未知、非空必须大于 0，JSON-RPC 使用十进制字符串 / NULL，表单、列表和 CSV 同步；材料档案已补最小 repo/usecase/API/RBAC/UI 和正式菜单，可维护 code、name、category、spec、color、default_unit、is_active；核心产品模拟基础资料 seed 已新增，可写入带 `SIM-PLUSH-CORE` 前缀的单位、材料、产品、仓库、工序和 BOM，并输出试用模拟 / 业务事实模拟可复用 ID
- 当前不包含：产品单重未做历史回填、SKU 覆盖、单位换算、出货行快照、总净重计算或打印带值；产品 / 单位 / 仓库仍未重新评审全部 UI/API；材料默认单位暂用 ID；core demo seed 不写客户、供应商、联系人、销售订单、`business_records`、库存流水、生产、出货或财务事实
- 证据：`product.go`、产品单重 Atlas migration、`masterdata.go`、`masterdata_repo.go`、`jsonrpc_masterdata_product.go`、`MasterDataForm.jsx`、`masterDataColumns.jsx`、`masterDataOrderView.mjs`、对应 Go / Node / PostgreSQL 合同测试、`server/internal/data/core_demo_seed.go`、`server/cmd/seed-core-demo-data`、`scripts/seed-core-demo-data.sh`
- 下一步：后续与导入、SKU 覆盖优先级、单位换算和 Shipment Fact 快照评审对齐
- 风险：产品默认单位变更时必须重新确认单重；不能把当前主档值即时推导成历史出货 / 打印事实；删除前旧样本或归档 evidence 不能被误读为仍可查询的 `business_records` 兼容层，模拟基础资料也不能被误读成真实客户数据
- 客户试用 / 交付承诺：有限 / 否
...
- 当前成熟度：L7
- 当前结果：`outsourcing_orders / outsourcing_order_items` 已接入 schema / migration / repo / usecase / JSON-RPC / RBAC 和 `/erp/purchase/processing-contracts` 页面；加工明细以 `subject_type=PRODUCT|MATERIAL` 明确产品或材料二选一，车缝 / 手工可选择产品，布料加工可选择材料，切换主体时清除另一侧 ID 与快照；页面和打印共用同一源单映射。只有 `draft` 可编辑，聚合更新要求正整数 `expected_version` 并先做单头 CAS；保存响应确认后先绑定真实 `id / version`，附件和列表 / Workflow 刷新失败只提示后置动作。数量与单价由后端计算金额，提交后只能走确认 / 关闭 / 取消等明确动作；当前操作区不承接质检、库存或应付跨模块处理入口。
- 当前不包含：不自动写委外发料、回货、质检、应付、发票、付款、库存流水或 Workflow 完成；不登记查货结果、合格 / 不合格 / 让步 / 返工判定；不含工序报价、工序质检联动、委外结算、合同审批流、电子签章或真实客户 Excel 导入
- 证据：`outsourcing_order.go`、`outsourcing_order_item.go`、`outsourcing_order_repo.go`、`jsonrpc_outsourcing_order_document.go`、`masterDataOrderApi.mjs`、`V1OutsourcingOrdersPage.jsx`、`OutsourcingOrderForm.jsx`、`sourceDocumentMutation.mjs`、`sourceDocumentMutation.test.mjs`、`sourceDocumentMutationIntegration.test.mjs`、`processingContractTemplate.mjs`、对应 usecase / repo / service tests
