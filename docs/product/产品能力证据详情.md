...
- 当前成熟度：L7
- 当前结果：已明确 `workflow done != fact posted`、`shipping_released != shipped`；协同页面只更新任务和业务进度投影。普通 `workflow_tasks` 已接严格 `version / expected_version` CAS 和稳定 `workflow.task-mutation-result/v1` receipt，正式完成、阻塞、退回和催办要求顶层 `idempotency_key`，服务端以 actor、命令和业务 intent 计算 SHA-256 hash；V1 writer / reader 严格校验 canonical key、command、任务状态及正式状态字典，损坏 receipt fail closed。精确重放返回首次任务快照，同 key 改 intent 返回 `40920`，新 key 不绕过终态。公开 `create_task` 只接受正式创建字段并拒绝 receipt / CAS、`customer_key` 和未知字段，不实现 create replay。状态、事件、催办计数、业务投影、派生任务和成功 break-glass 审计同事务提交。前端对 HTTP 408、网络 / 5xx 与结构不合法的 success response 保留同一冻结 attempt 和 key，并以 task 级同步 in-flight lease 阻止同一 task 跨动作双发；Go / JS 已共同消费共享 intent vectors，mixed evidence、raw whitespace key 和 relations 均由同一 golden 锁定，Node 24.14 定向 33/33、联合 62/62、受影响 ESLint 0 error。`20260711063237 / 20260711075355` 已作为不可变 revision 执行，`20260711104729` 增加 portable receipt bundle CHECK；本项目迁移前且无法证明版本的事件保留 `task_version=NULL`，不伪造回填。本地 fresh migration chain 为 54 个 revision、pending 0。ProcessRuntime domain command 已接只读 preflight、SHA-256 intent fingerprint、durable result / effect ref / compensation、同 intent 并发终态对账和 `linked_business_refs` CAS；protocol v1 的写 handler 在领域事务内同时落业务副作用与结果证据，两个无写 gate 在锁定短事务内记录 none，结果重放不再次执行 handler。销售取消、入库冲正、出货冲正和财务取消会标记 compensation
- 当前不包含：公开 `create_task` 不在普通任务网络 replay 合同内；Workflow 任务完成不自动写 shipment、库存或财务事实；本项目迁移前的 protocol 0 与迁移前无法精确证明的 result-missing 仍人工核对；本轮没有 final full/strict/L1 通过证据；完整应收、开票、收付款和核销闭环仍不能仅凭协同状态判定
...
- 下一步：将三条普通 Workflow revision 部署目标环境后补 authenticated replay / migration / release evidence；不扩大 Workflow payload 语义
- 风险：UI 的协同文案、`workflow_business_states` 或本项目迁移前 protocol 0 可能被误当成事实 / 自动恢复证据；本地 fresh 54 / pending 0 不能替代目标环境发布证据
- 客户试用 / 交付承诺：是 / 否
...
- 当前成熟度：L7
- 当前结果：客户侧 `出货单` V1 页面 `/erp/warehouse/shipments` 复用 `shipments / shipment_items`、`operational_fact` JSON-RPC、`shipment.*` RBAC 和库存 OUT / REVERSAL 主路径；`SHIPPED` 前后端在事务内锁定销售订单来源行，校验订单 active、客户、产品 / SKU、单位和累计出货量，并发确认不得超过订单行数量。出货草稿只通过严格幂等的 `create_shipment_with_items` 在一个事务内创建头和全部明细；公开头单创建 / 逐行追加拆分写入口已退出，既有明细只读，消除网络重试重复和多行中途失败留下半保存草稿的路径。关键并发 / 原子测试已进入 `full / strict` 的不可跳过 PostgreSQL 门禁；目标环境仍需绑定新 revision 重新发布和验证
...
- 当前成熟度：L6
- 当前结果：本地已完成 `production_orders / production_order_items / production_order_events` schema、Atlas migration、聚合 repo/usecase、`FINISHED_GOODS_RECEIPT` fact linkage 与 canonical API/RBAC runtime。DRAFT 可整单编辑，发布、关闭、取消使用 expected_version CAS 与 V1 receipt；RELEASED 订单只有在同一事务与订单锁域内确认无有效 POSTED production fact 时才能取消。成品入库事实创建和过账复核 RELEASED 订单、订单行、产品、SKU、单位，累计未冲正入库量不得超过计划量；Close 在同一订单锁域逐行聚合有效量，全部完成可无原因正常关闭，未完成必须填写短关闭原因。CLOSED 阻止新增 / 过账事实，但既有事实仍可取消并写 CANCELLED + 库存 REVERSAL，不回退订单或改写 Close receipt。`production_order` 域只开放七个 snake_case 方法，actor 来自认证管理员，复用 production 模块和 `pmc.plan.*`，详情使用一致快照，版本冲突统一映射为 40922。真实 PostgreSQL 已覆盖并发幂等、越量单赢家、取消 / 事实过账竞争和 Close / 过账 / 冲正两种锁序
- 当前不包含：五类分页 readable option API 与销售父单 active + 来源行 open 同事务复核已完成本地实现，但本切片真实 PostgreSQL、full、strict 尚待补证；尚无 UI runtime、菜单注册、客户投影、seed、部署或客户验收；`MATERIAL_ISSUE / REWORK` 与订单的 BOM / 材料来源合同尚未评审，也不包含 WIP、工时、成本、自动 MRP 或完整 MES
- 证据：`production_order.go`、`production_order_item.go`、`production_order_event.go`、`production_order_repo.go`、`operational_fact.go`、`operational_fact_repo.go`、`20260712071319_migrate.sql`、`production_order_schema_test.go`、`production_order_schema_postgres_test.go`、`production_order_repo_test.go`、`production_order_postgres_concurrency_test.go`、`production_order_fact_linkage_test.go`、`production_order_fact_postgres_concurrency_test.go`、`docs/architecture/生产订单源单边界评审.md`
- 下一步：先补齐本切片真实 PostgreSQL、full、strict，再独立实现 production-order UI / 通用菜单注册；不顺带接客户配置、seed 或部署
...
- 当前不包含：客户使用确认属于交付后业务确认；生产订单 Source Document 已完成本地 schema / migration / repo / usecase、`FINISHED_GOODS_RECEIPT` fact linkage、Close 领域闭环、API/RBAC runtime、五类分页 readable option API 与销售来源 eligibility 同事务复核，但尚无 UI runtime、菜单注册、客户投影、seed 或目标环境部署；`MATERIAL_ISSUE / REWORK` 尚未按 BOM / 材料来源接订单，未接成本归集或完整报工
- 证据：`production_fact.go`、`operational_fact.go`、`operational_fact_repo.go`、`jsonrpc_operational_fact.go`、`OperationalFactsPage.jsx`、`operational_fact_repo_test.go`、`operational-fact-simulated-closure.mjs`、`production_order.go`、`production_order_item.go`、`production_order_event.go`、`production_order_repo.go`、`20260712071319_migrate.sql`、`production_order_schema_test.go`、`production_order_schema_postgres_test.go`、`production_order_repo_test.go`、`production_order_postgres_concurrency_test.go`、`production_order_fact_linkage_test.go`、`production_order_fact_postgres_concurrency_test.go`、`docs/architecture/生产订单源单边界评审.md`、`docs/archive/customer-evidence/yoyoosun/operational-fact-target-release-evidence-2026-06-08.md`
- 下一步：实现 production-order UI / 通用菜单注册；production facts 继续收集 usage feedback
- 风险：不能只靠任务状态
- 客户试用 / 交付承诺：有限，需标边界 / 否
