...
- 当前不包含：工序主数据本身不是路线编辑器、进度或质检事实；当前只有固定 `PLUSH_SEW_HAND_V1` v1 在生产订单发布时解析并冻结启用的布料加工、车缝、手工和包装工序快照；不含通用路线设计器、工序报价、工时、成本或自动委外结算
