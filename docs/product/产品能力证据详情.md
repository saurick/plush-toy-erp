...
- 当前不包含：未做 `quality_inspection_items`、逐出货行来源锚点、逐件不良数量、缺陷字典、复杂抽检 / AQL、全局可配置关口编辑器、首次到货检验不合格后的退厂处置单、质检任务与 Workflow 自动闭环、委外不合格的自动退回加工厂事实或客户真实数据导入；生产分段质检只对显式 `PLUSH_SEW_HAND_V1` 行生效，不推断历史订单路线。相关 WIP migrations 尚未 apply 到共享开发库或目标环境，不能将本地源码实现写成已上线质检主链。
...
- 当前不包含：不含通用路线编辑器、路线动态变更、按材料名称猜首道归属、首道布料加工拆批、将 BOM 全部材料默认视为首道外发材料、WIP 作为可盘点库存、自动创建委外合同 / 发料 / 回货库存事实、生产关口让步审批、自动报废或完整 MES。完整 migration chain 已在一次性 PostgreSQL 18 隔离库 apply 并读回，含旧 WIP 关联的中间态夹具也已证明 preflight fail closed；登记的个人开发库已到 `20260718125909`、pending 0，但工序标准位置仍须显式绑定后才能使用固定路线。目标环境未 apply，尚无部署 health / smoke、真实岗位运行或客户 UAT 证据。
