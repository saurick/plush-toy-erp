...
- 当前成熟度：L7
- 当前结果：已明确 `workflow done != fact posted`、`shipping_released != shipped`。普通 `workflow_tasks` 的创建、完成、阻塞、退回和催办均要求顶层 `idempotency_key`，服务端以 actor、命令和规范化业务 intent 计算 SHA-256；exact replay 返回首次快照，同 key 改 intent 返回 `40920`。公开 `create_task` 仍拒绝内部 receipt / CAS、`customer_key`、未知字段和三个保留来源任务组。来源任务由正式业务事件原子生成。前端对断网、408 / 5xx、非法 success 和 stale / 并发保留冻结 attempt 并支持读回恢复。ProcessRuntime domain command 保持副作用与 durable result 同事务；补偿后仅当已激活下游尚无领域副作用时，管理员才可用唯一决策 `terminate_and_withdraw_downstream` 撤回节点和关联任务并阻塞流程，已完成或已有 effect ref 的下游一律转人工核对。
- 当前不包含：来源任务没有独立 `cancelled` 状态；Workflow 完成不自动写 shipment、库存或财务事实；迁移前 protocol 0 与无法精确证明的 result-missing 仍人工核对；收付款和核销必须走 Finance usecase，不能凭协同状态判定
- 证据：`docs/architecture/状态工作流事实边界.md`、`server/internal/biz/workflow.go`、`server/internal/biz/workflow_source_tasks.go`、`server/internal/biz/workflow_source_tasks_test.go`、`server/internal/biz/workflow_idempotency.go`、`server/internal/biz/workflow_idempotency_test.go`、`server/internal/data/workflow_source_task_repo.go`、`server/internal/data/shipment_release_source_task_integration_test.go`、`server/internal/data/workflow_repo.go`、`server/internal/data/workflow_repo_postgres_concurrency_test.go`、`server/internal/data/workflow_repo_idempotency_test.go`、`server/internal/data/model/migrate/20260711063237_migrate.sql`、`server/internal/data/model/migrate/20260711075355_migrate.sql`、`server/internal/data/model/migrate/20260711104729_migrate.sql`、`scripts/qa/workflow-task-mutation-intent-v1.vectors.json`、`web/src/erp/utils/workflowSourceTask.mjs`、`web/src/erp/utils/workflowTaskMutation.mjs`、`web/src/erp/api/workflowApi.test.mjs`、`server/internal/biz/process_runtime.go`、`server/internal/data/process_runtime_repo.go`、`server/internal/data/process_domain_command_atomic_postgres_test.go`、`server/internal/data/process_runtime_postgres_concurrency_test.go`、`server/internal/data/model/migrate/20260710150000_migrate.sql`、`server/internal/data/model/migrate/20260710150001_migrate.sql`、各领域 ProcessCommand tests
...
- 当前成熟度：L7
- 当前结果：customers / suppliers / contacts / materials / sales_orders 页面、路由、桌面正式菜单、dashboard 入口和前后端菜单权限选项已完成；旧重叠路径不再保留产品内路由、重定向或权限别名；正式客户构建从后端 active revision effective session 读取页面、动作、字段和打印投影，普通账号菜单按后端 RBAC 与客户页面投影取交集，字段策略只作用于已登记 surface；本地试用模拟浏览器入口 smoke 已通过。`生产排程`、`生产异常` 和 `出货放行` 已接 Workflow V1 协同任务读取 / 处理，并分别从生产订单发布、REWORK 过账和出货单“提交出货放行”生成；任务按严格 source contract 精确返回生产订单、生产进度或出货单。yoyoosun PMC 页面投影已补生产订单，试用通用任务使用 `trial_*`，不冒充正式 producer。
- 当前不包含：产品内 docs registry 已下线；前端隐藏仍不替代后端 RBAC / usecase；本地 smoke 不等于目标客户环境验收；三页不提供无来源创建协同任务入口，任务完成也不代替生产 / 出货事实动作。目标客户 active revision、真实岗位运行态和发布证据尚未因本轮本地接线得到证明。
- 证据：`V1MasterDataPage.jsx`、`V1SalesOrdersPage.jsx`、`V1ProductionOrdersPage.jsx`、`OperationalFactsPage.jsx`、`ShipmentsPage.jsx`、`WorkflowBusinessModulePage.jsx`、`businessPageLineage.mjs`、`workflowSourceTask.mjs`、`businessModules.mjs`、`router.jsx`、`rbac.go`、`customer_config.go`、`config/customers/yoyoosun/projectionMatrix.mjs`、对应 Node tests、`试用培训说明.md`、`试用账号角色菜单核对清单.md`、`试用环境执行手册.md`、`docs/archive/customer-evidence/yoyoosun/simulated-trial-acceptance.md`
...
- 所属层 / 业务域：Product Core / MasterData
- 当前成熟度：L5-L7
- 当前结果：已有既有 schema / runtime 能力；产品档案可选维护 `numeric(20,6)` 的产品单重（净重），按产品默认单位以 g 保存，空值为未知、非空必须大于 0；SKU 同样可选维护 `unit_net_weight_g`，但填写时必须同时有显式 SKU `default_unit_id`，SKU 未填单重时才允许下游回退产品单重。产品 / SKU JSON-RPC 均使用十进制字符串 / NULL，表单、列表和 CSV 同步；材料档案已补最小 repo/usecase/API/RBAC/UI 和正式菜单，可维护 code、name、category、spec、color、default_unit、is_active；核心产品模拟基础资料 seed 已新增，可写入带 `SIM-PLUSH-CORE` 前缀的单位、材料、产品、仓库、工序和 BOM。其业务事实参数当前只供旧 report-only 计划审查，不能用于已退役的通用 apply
...
- 当前成熟度：L7
- 当前结果：已新增 `processes` schema / migration / generated code，并接入 MasterData repo/usecase、`masterdata` JSON-RPC、`process.*` RBAC、后端内置菜单、桌面正式菜单、yoyoosun 客户菜单、毛绒行业模板和 `/erp/engineering/processes` V1 页面；页面可维护工序编号、名称、自由文本类别、唯一可选的标准路线位置 `production_route_operation_code`、可委外 / 可内制 / 需质检标记、排序、备注和启停状态；工序名称和类别输入支持行业默认候选并允许自由新增，固定路线位置必须显式选择且每个位置最多绑定一条工序；核心演示 seed 明确绑定机裁、车缝、手工、包装四条默认工序，并保留查货、制作刀模、裁片IQC、丝印、贴合等不参与固定路线的可扩展候选；`查货` 在这里仅表示工序候选，不登记合格 / 不合格 / 让步 / 返工结果
- 当前不包含：工序主数据本身不是路线编辑器、进度或质检事实；当前只有固定 `PLUSH_SEW_HAND_V1` v1 在生产订单发布时按 `production_route_operation_code` 解析并冻结启用的布料加工、车缝、手工和包装工序快照，不从名称、类别、普通工序编码或排序推断；不含通用路线设计器、工序报价、工时、成本或自动委外结算
- 证据：`process.go`、`20260617085305_migrate.sql`、`masterdata.go`、`masterdata_repo.go`、`jsonrpc_masterdata.go`、`rbac.go`、`V1MasterDataPage.jsx`、`businessModules.mjs`、`core_demo_seed.go`、`production_wip.go`、`production_wip_repo.go`
- 下一步：目标环境 migration / release evidence / 客户岗位验收；通用路线配置只在出现第二条已确认生产路线时再评审
...
- 当前成熟度：L7
- 当前结果：已落 Ent schema / Atlas migration / generated code；`product_skus` 表包含 `sku_code`、`barcode`、`customer_sku`、颜色、色号、尺寸、包装版本和可选默认单位；已接入 masterdata JSON-RPC、`product_sku.*` RBAC 和 `/erp/master/products` SKU 最小维护页面，可创建、编辑、列表、详情和启停 SKU。SKU 创建后所属产品不可更换，后端拒绝改挂，正式编辑表单同步锁定该字段并显示业务说明；销售订单行 UI/API 已接 SKU 选择带值和 `product_sku_id` 保存；显式 SKU 已贯通 `inventory_lots / inventory_txns / inventory_balances`、生产 / 委外事实、出货和预留，库存查询、锁、幂等、可用量、扣减与冲正都保持同一 SKU grain
- 当前不包含：BOM SKU 粒度、导入受控创建 SKU、订单颜色自动建 SKU 和旧未分规格库存人工重分类；导入工具仍不得自动创建 SKU
- 证据：`product_sku.go`、`inventory_lot.go`、`inventory_txn.go`、`inventory_balance.go`、`20260710150001_migrate.sql`、`inventory_sku_grain_test.go`、`inventory_sku_postgres_test.go`、`jsonrpc_sku_grain_test.go`、`sales_order_repo.go`、`operational_fact_repo.go`、`MasterDataForm.jsx`、`productSKUParentField.test.mjs`、`V1SalesOrdersPage.jsx`、`V1InventoryLedgerPage.jsx`、`docs/architecture/产品款号物料清单边界评审.md`
- 下一步：BOM SKU grain / import controlled SKU / old unallocated stock reclassification review
...
- 当前成熟度：L7
- 当前结果：`bom_headers.product_id` 是 BOM 必填产品关联，当前保持 product-level grain；已接入 `bom` JSON-RPC、`bom.*` RBAC 和 `/erp/purchase/material-bom` V1 页面。草稿只通过 `save_bom_with_items` 在一个后端事务中保存头和全部明细，更新以 `expected_version` 对 `id + DRAFT + updated_at` 做 CAS，省略的旧明细在同一事务删除，冲突或任一步失败都不会留下半保存；旧分拆写 RPC 已退出公开路由。复制、激活和设为历史版本仍是独立生命周期动作；激活新版本会把同产品旧 `ACTIVE` 设为历史版本（底层状态 `ARCHIVED`），已激活 BOM 不允许直接改头或明细。物料明细和 BOM 来源作业指导书在每次打开时读取关联产品当前主图 / 辅图并冻结进该打印窗口草稿
- 当前不包含：不含 `product_sku_id` BOM 粒度、采购需求生成、MRP、替代料、成本核算、订单 / 采购需求版本快照、BOM 激活时产品图片版本快照或目标客户真实数据验收
- 证据：`inventory.go`、`inventory_repo_bom_save.go`、`inventory_repo_bom_save_test.go`、`jsonrpc_bom.go`、`jsonrpc_bom_version.go`、`jsonrpc_bom_test.go`、`BOMVersionsPage.jsx`、`bomApi.mjs`、`productPrintImages.mjs`、`productPrintImageIntegration.test.mjs`、`docs/architecture/产品款号物料清单边界评审.md`
- 下一步：BOM SKU grain / purchase demand snapshot review
- 风险：草稿聚合保存的原子性不等于激活、库存、采购、生产或成本事实已发生；重新打开历史 BOM 会读取产品主档当时的当前图片，不能表述为 BOM 历史附件
- 客户试用 / 交付承诺：有限 / 否
...
- 当前成熟度：L7
- 当前结果：已有 schema / migration / repo / usecase / JSON-RPC / RBAC / 测试和 V1 页面闭环；`purchase` 域支持创建草稿、加行、过账、取消、读取和列表，过账写 `inventory_txns.IN`，取消写 `REVERSAL`。正式来源入口从已审批采购订单生成 `DRAFT` 入库草稿，并为每个入库行同步创建零余额 `HOLD` 批次和一张 `SUBMITTED` 来料检验；`post_purchase_receipt` 聚合检查全部行，存在待检或不合格时拒绝过账，只有全部行已有 `PASS / CONCESSION` 且批次为 `ACTIVE` 时才写入库流水并置为 `POSTED`。采购来源行在过账前锁定对应采购行，并按 `POSTED 入库 + 已过账数量增加 - 已过账数量减少` 计算有效已收量；取消的入库 / 调整退出统计，分次收货、超收拒绝和并发竞争由 PostgreSQL 事务守住。正式来源入口 `create_purchase_receipt_from_purchase_order` 与公开 `add_purchase_receipt_item` 使用稳定幂等 key 和服务端规范化 SHA-256 intent hash，Process Runtime 的 `purchase_receipt.create` 也持久化首次结果。当前本地工作树还把已过账入库的采购退货和入库调整正式聚合命令接入同一 `purchase` 域与采购入库页：调用方只选已过账入库行、数量和允许的更正方向，材料、单位、原仓库 / 批次和价格由后端派生；创建、过账、取消 / 冲正、列表、详情、幂等 replay 和并发防超量保持在正式领域事务中。`purchase_receipts.supplier_id` 只从唯一可确定的采购来源回填，是已过账入库生成应付的稳定供应商引用。
...
- 当前成熟度：L7
- 当前结果：已有 schema / migration / repo / usecase / JSON-RPC / RBAC、来源页动作、测试和 V1 页面闭环；`quality` 域支持创建草稿、提交质检、判定合格 / 让步接收、判定不合格、取消、读取和列表。`/erp/production/quality-inspections` 已收口为通用“质量检验”读模型，按采购到货待检、委外回货、生产工序和出货前成品展示业务来源、对象与批次。采购入库草稿创建时会逐行建立零余额 `HOLD` 批次和 `SUBMITTED` 来料检验，全部行判定为 `PASS / CONCESSION` 后才允许 `POSTED` 入库，任一初始检验 `REJECTED` 都会阻断该入库单过账；委外检验从已过账 `RETURN_RECEIPT` 派生产品 / 仓库 / 批次 / 数量，同一有效委外回货不能重复生成质检；显式 `PLUSH_SEW_HAND_V1` 生产行在工序完成或外发返回后，以 `PRODUCTION_WIP / PRODUCTION_STAGE / WIP` 来源绑定精确 WIP 批次，按顺序生成裁片、皮套、成品、针检、抽检及条件客户验货检验。生产阶段关口当前只有 `PASSED + PASS` 才生成下一关或将批次转为 `ACCEPTED`；关口级让步审批策略与审计尚未建立，因此通用 `CONCESSION` 在 WIP 关口 fail closed，`REJECTED` 转入明确返工。品质岗位另可从 `DRAFT` 出货单按产品 / SKU、仓库、批次送检粒度生成出货前成品检验；这条出货侧链仍与生产 WIP 分段质检分开。现有“退供应商”专用动作只接受已 `POSTED` 入库行的追加不合格检验，不适用于首次到货检验被拒绝的入库草稿。
- 当前粗略不良率能力：现有 `quality_inspections` 事实已增加成对校验、可回显的 `defect_rate_operator / defect_rate_percent`；页面提供 `5% / 10% / 20% / >50% / 100%` 等快捷档位并允许自定义百分比，列表、详情和 CSV 同时展示业务来源单号与粗略比例。`>50%` 保留“大于”语义，历史空值显示为未记录且不猜测回填；比例不自动决定合格、让步或不合格，也不推导退货或库存数量。Atlas migration 已生成，专用测试库已 apply 并通过 PostgreSQL 关键事务验证；共享开发库和目标环境未 apply，也未发布或完成客户岗位验收。
- 异常流 V1：首次来料初检拒绝可登记退厂 / 补换处置并取消未入账收货；委外回货不合格可返厂或创建返工 WIP；生产报废、超领和 WIP 让步使用显式审批决定；无已生效处置、财务或出货下游时可保留原判定并生成新的 `SUBMITTED` 更正单。客户退货质检因隔离库存可能已有下游，当前不开放结果更正。
- 当前不包含：未做 `quality_inspection_items`、逐件不良数量、缺陷字典、复杂抽检 / AQL、全局可配置关口编辑器、质检任务与 Workflow 自动闭环或客户真实数据导入；生产分段质检只对显式 `PLUSH_SEW_HAND_V1` 行生效。本地 migration 尚未 apply 到目标环境，不能将本地源码写成已上线能力。
- 证据：`quality_inspection.go`、`quality_inspection_process_command.go`、`quality_inspection` Ent schema、`20260716162104_migrate.sql`、`20260717035245_migrate.sql`、`quality_inspection_repo.go`、`production_wip.go`、`production_wip_repo.go`、`operational_fact_repo.go`、`inventory_postgres_quality_inspection_test.go`、`shipment_finished_goods_quality_gate_test.go`、`production_wip_quality_inspection_repo_test.go`、`jsonrpc_quality.go`、`jsonrpc_production_wip.go`、`qualityApi.mjs`、`productionWipApi.mjs`、`productionWipModel.mjs`、`V1QualityInspectionsPage.jsx`、`ProductionRouteExecutionModal.jsx`及对应 Go / Node 定向合同
- 下一步：隔离数据库 apply / PostgreSQL 事务回归 / 目标环境 release smoke / customer role acceptance / 缺陷字典与报告评审
- 风险：不能把 Workflow task done 当成 quality passed，不能把出货前成品检验侧链冒充生产 WIP 分段质检或逐出货行事实，不能把包材业务确认写成 Quality Fact，不能把 `>50%` 降格为精确 `50%`，也不能把不合格判定或粗略比例直接等同为退货和库存事实
- 客户试用 / 交付承诺：有限 / 否
...
- 当前成熟度：L7
- 当前结果：已有 schema / migration / repo / usecase / JSON-RPC / RBAC / 测试和 V1 页面闭环。除来源事实写入外，`inventory_operations / items` 提供盘点、仓间调拨和受控人工调整；盘点冻结 expected quantity 防 stale，人工调整强制审批引用，过账 / 取消使用 version CAS 并在同一事务写库存流水 / REVERSAL，所有出库方向继续防负数。页面支持网络未知结果按原幂等 key 查询恢复，不开放任意直接改余额。
- 当前不包含：不含真实库位、冻结量聚合 read model、预留明细专项页、单条预留部分消费、客户真实数据导入 / backfill 或完整 WMS。
...
flowchart TD
  ...
  purchaseOrder --> receiptDraft["采购入库草稿<br/>逐行建立零余额 HOLD 批次"]
  receiptDraft --> quality["逐行来料检验<br/>quality_inspections SUBMITTED"]
  quality -->|全部 PASS / CONCESSION| receipt["采购入库过账<br/>purchase_receipts POSTED"]
...
- 库存台账已提供受控盘点、仓间调拨和人工调整，但不提供任意改余额、批次状态维护、独立出库确认或预留明细维护。
- 客户真实数据导入、装箱 / 物流 / 签收和完整报表仍不在当前闭环承诺内；客户退货仅完成来源 RMA、隔离入库与质检 V1。
...
- 当前成熟度：L7
- 当前结果：客户侧 `出货单` V1 页面 `/erp/warehouse/shipments` 复用 `shipments / shipment_items`、`operational_fact` JSON-RPC、`shipment.*` RBAC 和库存 OUT / REVERSAL 主路径；确认 `SHIPPED` 时，后端在事务内锁定销售订单来源行，校验订单 active、客户、产品 / SKU、单位和累计出货量，并发确认不得超过订单行数量。同一确认事务会优先解析 SKU 单重与其显式默认单位，SKU 未填单重时再回退产品单重与产品默认单位，并冻结 `shipment_items.unit_net_weight_g_snapshot`；全部有效行可解析时由后端按 decimal 累加并写入 `shipments.total_net_weight_g`，任一行不可解析时不做部分合计，可保留可空的人工实际总净重且第一版不阻断出货。已出货记录不随主档修改、不历史回填，取消出货保留原快照和总净重。出货草稿只通过严格幂等的 `create_shipment_with_items` 在一个事务内创建头和全部明细；公开头单创建 / 逐行追加拆分写入口已退出，既有明细只读。关键并发 / 原子测试已进入 `full / strict` 的不可跳过 PostgreSQL 门禁；目标环境仍需绑定新 revision 重新发布和验证
- 当前不包含：草稿不占用销售订单剩余量，最终强阻断发生在确认出货时；净重第一版不做单位换算，不含毛重；客户使用确认、拣货、装箱、物流追踪、签收和打印仍未完成；RMA 已接但不等于物流签收或客户现场验收；应收 / 开票需要用户显式发起
...
- 当前成熟度：L6
- 当前结果：已落地 `business_attachments` schema / migration / repo / usecase、canonical `attachment` JSON-RPC 和共享前端面板，并接入销售订单、采购订单、委外订单、采购入库、来料质检、出货单、财务事实、生产 / 委外事实、SKU、BOM、Workflow 桌面页和岗位任务端详情；单个附件上限 5MB，HTTP 编码请求体和业务解码双门禁已接入。Workflow owner 读取 / 上传复用 active revision 行级责任范围，终态拒绝上传；普通已上传附件物理删除已退出。当前工作树新增的产品媒体是受控窄例外：只允许 `owner_type=product + attachment_type=product_image + primary|secondary`，只接受 PNG / JPEG / WEBP，并允许在产品保存后按固定槽替换或清空
- 当前不包含：不含对象存储、流式大文件、缩略图、Office / HEIC 在线转换、OCR、病毒扫描、电子签章、受控证据撤销或客户外链共享；产品图不含版本、审批或 SKU 覆盖；附件上传、下载、预览、替换或清空都不改变 Source Document、Fact、Workflow、库存、质检或财务状态
- 证据：`docs/architecture/业务附件证据边界评审.md`、`business_attachment.go`、`business_attachment_repo.go`、`jsonrpc_attachment.go`、`business_attachment.go` Ent schema、`20260716145704_migrate.sql`、`BusinessAttachmentPanel.jsx`、`ProductImageSlots.jsx`
- 下一步：customer feedback / object-storage-preview review
- 风险：把附件证据误读为业务事实、质检判定、财务凭证已入账或任务已完成
- 客户试用 / 交付承诺：有限 / 否

### sales_returns 客户退货 / RMA

- 所属层 / 业务域：Product Core / Sales / Inventory / Quality
- 当前成熟度：L7（本地源码与页面层）
- 当前结果：只能从已 `SHIPPED` 的出货行创建 RMA，后端冻结客户、产品 / SKU、仓库、单位和数量来源；累计有效退货不得超过已出货量。`DRAFT → APPROVED → RECEIVED → CANCELLED` 使用 version CAS，创建 intent 可 exact replay；收货在同一事务写库存 IN，并使用新的 `HOLD` 隔离批次和 `CUSTOMER_RETURN` 质检，不回写原出货批次。未完成质量处置时取消已收货 RMA 写 REVERSAL 并取消质检、停用隔离批次；质检已有结果后禁止取消越过下游事实。
- 当前不包含：不含物流取件、换货新发、退款自动生成、客户门户、真实退货导入或客户验收。
- 证据：`sales_return.go`、`sales_return_item.go`、`operational_fact_sales_return_repo.go`、`jsonrpc_operational_fact_sales_return.go`、`SalesReturnsPage.jsx` 及生命周期、累计数量、隔离质检和页面合同测试。
- 下一步：target release evidence / refund and replacement review / customer acceptance
- 风险：退货收货、质量判定、退款和换货是不同事实，不能由一个 RMA 状态互相替代。
- 客户试用 / 交付承诺：本地有限联调 / 否
...
- 当前不包含：客户使用确认、退货物流与现场验收、单条预留部分消费、BOM SKU 粒度或旧库存重分类；RMA 只处理系统内来源、隔离批次、质检和冲正
...
- 当前成熟度：L7
- 当前结果：`finance_facts` 继续承载来源应收、应付、发票和核对；`finance_payments / finance_allocations / finance_credit_notes` 承载真实收付款、同往来方同币种多单核销、冲正和应收 / 应付红冲 V1。过账在同一事务落核销并结清，冲正追加反向审计并重开余额；红冲保留不可变原单，错误红冲以反向红冲纠正，不物理删除或改写已过账事实。页面隐藏内部 ID，并按往来方与币种筛选候选。
- 当前不包含：不含银行直连、收付款申请 / 审批、银行流水导入、支付凭证、发票明细、税控、总账、会计凭证或客户财务验收；本地 migration 未发布到目标环境。
...
- 当前成熟度：L7
- 当前结果：本地已完成 `production_orders / production_order_items / production_order_events` schema、Atlas migration、聚合 repo/usecase、canonical API/RBAC、Product Core 独立页面和通用菜单。DRAFT 可整单编辑，发布、关闭、取消使用 expected_version CAS 与 V1 receipt；发布在同一事务原子生成 PMC `production_scheduling` 任务、created event 和 `production_ready` 状态。Close 在有效完工量判定和短关闭原因之外，还要求排程任务 `done`；RELEASED 关闭或取消均要求没有活动 WIP，取消还要求无有效 POSTED production fact 且排程任务已 `done / rejected`。生产订单发布会按每条订单行锁定的 active BOM 版本冻结 `production_order_material_requirements`，包括 BOM 行显式维护的可选 `production_operation_code`。当前该字段只允许 `FABRIC_PROCESSING`；显式选择 `route_code=PLUSH_SEW_HAND_V1` 的行至少要冻结一条这类材料需求，否则发布 fail closed，不按“面料”名称、类别或备注猜测。该行同时按工序主档唯一 `production_route_operation_code` 绑定冻结固定 v1 工序快照、客户验货条件和首个 WIP 批次；绑定缺失或停用时发布失败，不提供发布后补初始化。已发布历史订单只在 BOM 来源可确定时回填物料需求；历史行不猜测路线或首道材料归属，`route_code=NULL` 保留旧完工边界。页面已在订单行与物料需求行上提供来源完工、领料、工序办理和关联记录入口。
- 当前不包含：yoyoosun 当前预览配置包只同步来源办理所需的最小菜单和岗位能力投影，客户私有资料、流程 manifest、seed、active revision、目标环境部署和客户验收未因本轮闭环得到证明；物料需求快照不是自动 MRP，固定路线也不是通用路线设计器、工时、成本或完整 MES。完整迁移链已在一次性 PostgreSQL 18 隔离库验证，登记的个人开发库已到 `20260718125909`、pending 0；schema apply 不会自动产生工序标准位置绑定，固定路线使用前仍须显式绑定。目标客户数据库没有本轮 apply evidence。
- 证据：`bom_item.go`、`process.go`、`production_order.go`、`production_order_item.go`、`production_order_operation.go`、`production_wip_batch.go`、`production_wip_event.go`、`production_wip_outsourcing_allocation.go`、`production_packaging_confirmation.go`、`production_order_material_requirement.go`、`production_order_repo.go`、`production_wip_repo.go`、`workflow_source_tasks.go`、`workflow_source_task_repo.go`、`production_order_repo_test.go`、`production_wip_repo_test.go`、`operational_fact.go`、`operational_fact_repo.go`、`20260714122346_migrate.sql`、`20260714124000_backfill_production_material_requirements.sql`、`20260717035245_migrate.sql`、`20260717043625_migrate.sql`、`20260718110227_migrate.sql`、`populated-upgrade-20260714055504.sql`、`V1ProductionOrdersPage.jsx`、`ProductionRouteExecutionModal.jsx`、`ProductionMaterialIssueModal.jsx`、`ProductionCompletionModal.jsx`
...
- 所属层 / 业务域：Product Core / Production / Quality
- 当前成熟度：L7（本地源码与页面层）
- 当前结果：已落固定 `PLUSH_SEW_HAND_V1` v1，按 `FABRIC_PROCESSING → SEWING → HANDWORK → PACKAGING` 冻结订单工序快照，保证“先车缝、后手工”。布料加工正常流只允许整单数量外发且首道不拆批；必须绑定且仅绑定恰好覆盖冻结 `production_operation_code=FABRIC_PROCESSING` 材料需求的 MATERIAL 加工合同明细，开始前还要有足量已过账委外发料。裁片检验 `PASS` 并转入 `SEWING` 后，才允许按产品数量一次拆成至少两个 WIP 子批，子批总量必须精确等于父批；车缝与手工每道独立允许 `IN_HOUSE / OUTSOURCED`，包装只允许本厂。返工回到 FABRIC 后若再次外发，返工 WIP 使用新的等量 PRODUCT 合同行，不复用正常流 MATERIAL 行。`production_wip_batches / production_wip_events / production_wip_outsourcing_allocations` 保存拆量、执行方式、工序与委外来源绑定、开始 / 完工、外发回仓、车间 WIP 移交、返工、未开工批次带原因取消和稳定幂等 receipt；取消只允许 `PLANNED → CANCELLED`，不重新拆量，也不冲正既有事实。内制流转不写仓库入库，裁片 / 皮套 / 手工成品不冒充原料或成品库存。工序完成后通过 `quality_inspections` 的 WIP 来源锚点顺序办理裁片、皮套、成品、针检、抽检和条件客户验货；生产阶段关口只有 `PASS` 放行，未批准的 `CONCESSION` fail closed。包材版面 / 版本使用独立 `production_packaging_confirmations`，不冒充品质 IQC。包装批次未 `ACCEPTED` 的数量不能生成或过账完工入库。`production_wip` JSON-RPC、五个 WIP / 包材权限码、生产订单“工序办理”弹窗和质量检验生产工序读模型已接入本地源码；生产订单列表 / 详情允许 `pmc.plan.read` 或 `production.wip.read`，但计划维护仍只属于 PMC。
- 当前不包含：不含通用路线编辑器、路线动态变更、按材料名称猜首道归属、WIP 作为可盘点库存、自动创建委外合同 / 发料 / 回货、自动异常判定或完整 MES。报废、超领和 WIP 让步只通过显式审批决定；批准后仍由对应 Fact usecase 写库存或 WIP 状态。新 migration 仅在本地生成，目标环境未 apply，尚无发布或 UAT 证据。
- 证据：`docs/architecture/生产工艺路线与在制品边界评审.md`、`bom_item.go`、`process.go`、`production_order_material_requirement.go`、`production_wip.go`、`production_wip_repo.go`、`production_order_operation.go`、`production_wip_batch.go`、`production_wip_event.go`、`production_wip_outsourcing_allocation.go`、`production_packaging_confirmation.go`、`quality_inspection.go`、`operational_fact_repo.go`、`jsonrpc_production_wip.go`、`jsonrpc_quality.go`、`rbac.go`、`20260717035245_migrate.sql`、`20260717043625_migrate.sql`、`20260718110227_migrate.sql`、`populated-upgrade-20260714055504.sql`、`production_wip_test.go`、`production_wip_repo_test.go`、`production_wip_quality_inspection_repo_test.go`、`jsonrpc_production_wip_test.go`、`productionWipApi.mjs`、`productionWipModel.mjs`、`ProductionRouteExecutionModal.jsx`、`V1ProductionOrdersPage.jsx`、`V1QualityInspectionsPage.jsx`及对应 Node 定向合同
- 下一步：对归属明确的长期开发 / 目标库按正式 preflight、status、apply、status / 结构读回执行；再完成目标环境发布、按角色 smoke 与客户人工验收
- 风险：本地编译 / 定向测试或页面截图不等于 migration 已应用、目标环境已发布或甲方已验收
- 客户试用 / 交付承诺：否 / 否
...
- 当前成熟度：L7
- 当前结果：`production_facts` schema / migration、usecase、JSON-RPC、RBAC、V1 列表与源单专用动作已落；通用事实页不再提供需要业务用户手填来源、对象、幂等键或数字 ID 的无来源登记。`MATERIAL_ISSUE` 从发布时冻结的物料需求行派生材料 / 单位，只能选既有批次，累计已过账量不超计划需求；`FINISHED_GOODS_RECEIPT` 从 RELEASED 订单行派生产品 / SKU / 单位，可选既有批次或在事务内新建批次，累计已过账量不超计划量。对显式 `PLUSH_SEW_HAND_V1` 订单行，完工入库另以最终包装工序 `ACCEPTED` WIP 数量为上限；只完成包装动作、只确认包材或只完成 Workflow 都不能写成品库存。`REWORK` 只从已过账完工事实发起，复用原产品 / SKU / 仓库 / 单位 / 批次领出，累计已过账返工不超原完工量。REWORK 过账在同一库存 / 事实事务生成生产岗位 `production_exception` 任务，异常原因取返工备注快照；已过账 REWORK 冲销要求异常任务先进入 `done / rejected`。三类事实过账写 IN / OUT，取消写 REVERSAL；存在有效返工时不允许取消原完工。
- 当前不包含：返工完成仍应使用新的完工入库事实，当前 `REWORK` 只表达已过账完工批次的返工领出；未建设质量不合格自动返工、替代料 / 超领审批、工时、成本归集、完整报工或客户验收。
- 证据：`production_fact.go`、`operational_fact.go`、`operational_fact_repo.go`、`workflow_source_tasks.go`、`workflow_source_task_repo.go`、`production_order_material_issue_test.go`、`production_rework_source_test.go`、`production_rework_source_postgres_test.go`、`jsonrpc_operational_fact_production.go`、`jsonrpc_operational_fact_rework_source_test.go`、`V1ProductionOrdersPage.jsx`、`OperationalFactsPage.jsx`、`ProductionReworkModal.jsx`
...
- 当前不包含：委外订单行目前按自身材料 / 产品主体解释发料 / 回货，尚未建设委外材料需求快照或多材料发料；不合格回货的返厂 / 返工处置已接，但不自动做完整委外结算、成本归集、客户验收或目标发布。
...
- 当前成熟度：L2
- 当前结果：前端已恢复 `/erp/help-center` 单一岗位使用帮助入口，为 boss、sales、purchase、production、warehouse、finance、pmc、quality、engineering 和 admin 提供不同的当日重点、推荐顺序、交接、注意事项和常见问题；页面捷径仅从当前账号已开放菜单中取交集。旧 Markdown 文档中心、高级文档和开发与验收页仍不恢复；仓库正式文档、测试策略和 yoyoosun 试用执行手册继续保留在各自真源入口，客户侧栏不展示 `业务事实扩展` 或 `事实闭环` 这类内部工程入口
- 当前不包含：岗位帮助不是仓库正式 Markdown 文档镜像，不承接开发调试、发布验收、客户私密资料、真实账号或密码，也不代表目标环境已发布或验收
...
- 下一步：trial feedback / target-environment role readback
