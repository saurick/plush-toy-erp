Doc Type / 文档类型: Current Customer Import Strategy / current 客户导入策略
...
| shipments / stock_reservations | deferred fact domains；不得从 current 或旧快照自动生成。 |
...
- 不把 current 样本字段写进 Product Core。
