...
- 后台工作台、业务模块标准页和协同入口原型仍按 `docs/product/prototypes/README.md` 登记为 `待实现 / To Implement`，未经用户明确确认不得晋级 `Current`。当前运行时已吸收这些原型的主要页面骨架和交互路径：`/erp/dashboard` 是后台首页 / 工作台，`/erp/task-board` 是 Workflow 任务看板，`/erp/business-dashboard` 是业务摘要看板，`/erp/print-center` 是模板打印中心，`/erp/operations/exceptions` 是异常 / 阻塞闭环；`BusinessModulePage` 承接通用业务记录页的标题统计、筛选、表格工具、当前操作区、分页和底部协同入口；`客户档案`、`供应商档案` 和 `销售订单` 的正式 V1 页面已复用同一业务页骨架；`材料 BOM`、`入库通知/检验/入库`、`库存` 和 `出库` 在标准页外补充只读变体区说明 BOM、质检 / 入库、库存事实和出库事实边界。不吸收原型固定数字、静态任务、假客户、mock 脚本或 dev-only 外壳；这些运行时页面也不把 Workflow 任务完成等同于库存、出货、财务、发票或收付款事实过账。
...
- 开发环境可访问独立隐藏路径 `/__dev/prototypes` 查看产品原型资产；该入口只浏览 `docs/product/prototypes` 下的 HTML 原型、PNG 方案图和截图证据，可按全部 / 当前实现 / 待实现 / 参考资料四类筛选并在右侧预览，只服务本地开发和产品评审查阅，不进入 ERP 菜单、seedData、RBAC、后端业务、产品内 docs registry 或生产构建。
...
- 开发环境可访问独立隐藏路径 `/__dev/testing` 查看测试入口；该入口只读解析 `docs/product/test-strategy.md` 和 `docs/**/*.md` 中的测试、验收、QA、smoke、`style:l1` 等相关文档，展示测试分层、命令块和相关文档索引，并为 T0-T8 每层及常用预设提供一键复制命令 / 清单；复制按钮不在浏览器内执行 shell，只服务本地治理查阅，不进入 ERP 菜单、seedData、RBAC、后端业务、产品内 docs registry 或生产构建；`docs/product/test-strategy.md` 仍是测试选择真源，可视化不替代实际命令执行结果、业务事实、权限边界、部署状态或客户交付结论。
