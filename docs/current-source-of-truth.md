...
- 003 schema-only 已按 V1 cutline 新增 `server/internal/data/model/schema/customer.go`、`supplier.go`、`contact.go`、`sales_order.go` 和 `sales_order_item.go`；004 已基于这 5 个 schema 生成 Ent 代码和 Atlas migration；005 已新增客户 / 供应商 / 联系人 MasterData repo/usecase 和测试。当前只完成 `customers / suppliers / contacts` 后端 usecase，`sales_orders / sales_order_items` usecase、API、RBAC、UI、docs registry、seedData 和 `business_records` transition 仍未实现。
