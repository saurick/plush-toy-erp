...
- 后台工作台与看板原型已部分承接到运行时：`/erp/dashboard` 作为 Workflow 任务看板入口，`/erp/business-dashboard` 作为业务摘要看板，`/erp/print-center` 作为模板打印中心。`/erp/dashboard` 的任务筛选状态使用 URL query 作为可分享 / 可刷新恢复 / 可一键清空的页面状态，首屏按本页待办、阻塞异常、今日到期和已完成协同四泳道展示，不写后端用户偏好或业务事实。`docs/product/prototypes/admin-command-center-v1/` 仍只是参考原型，不替代当前代码、测试、schema、API 或正式边界文档；这些运行时页面也不把 Workflow 任务完成等同于库存、出货、财务、发票或收付款事实过账。
