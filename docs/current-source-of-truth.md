...
- 003 schema-only 已按 V1 cutline 新增 `server/internal/data/model/schema/customer.go`、`supplier.go`、`contact.go`、`sales_order.go` 和 `sales_order_item.go`；004 已基于这 5 个 schema 生成 Ent 代码和 Atlas migration。它们只表达客户 / 供应商 / 联系人 MasterData 与销售订单 Source Document 的 schema / migration 起点；尚未接 repo/usecase、API、RBAC、UI、docs registry、seedData 或 `business_records` transition。当前 runtime 业务逻辑真源仍是既有 usecase。
