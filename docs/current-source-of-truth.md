...
- 003 schema-only 已按 V1 cutline 新增 `server/internal/data/model/schema/customer.go`、`supplier.go`、`contact.go`、`sales_order.go` 和 `sales_order_item.go`；004 已基于这 5 个 schema 生成 Ent 代码和 Atlas migration；005 已新增客户 / 供应商 / 联系人 MasterData repo/usecase 和测试；006 已新增 `sales_orders / sales_order_items` 后端 repo/usecase 和测试；007 已为这些 V1 对象接入后端 JSON-RPC API 和 RBAC 动作权限；008 已新增 V1 客户 / 供应商 / 联系人 / 销售订单 / 销售订单行前端页面。Sales order 仍是 Source Document / Business Commitment，不写 shipment、inventory、stock reservation、finance、invoice、payment 或 Workflow facts；docs registry、seedData 和 `business_records` transition 仍未实现。
