...
- 003 schema-only 已按 V1 cutline 新增 `server/internal/data/model/schema/customer.go`、`supplier.go`、`contact.go`、`sales_order.go` 和 `sales_order_item.go`。这些文件只表达客户 / 供应商 / 联系人 MasterData 与销售订单 Source Document 的 Ent schema 起点；尚未生成 Ent 代码，尚未新增 Atlas migration，尚未接 repo/usecase、API、RBAC、UI、docs registry 或 seedData。当前 runtime 真源仍是既有生成代码、既有 migration 和既有 usecase。
