...
- 原始 Excel 必须先通过 `docs/customers/yoyoosun/source-manifest.json` 和 `scripts/import/customerSourceManifestCheck.mjs` 校验，再由 `scripts/import/customerSourceExtract.mjs` 按 manifest 中 `structuredExtract.enabled=true` 的来源生成本地 `output/customers/yoyoosun/source-extract/*` evidence；该输出不纳入 git，不是真实导入批准。
