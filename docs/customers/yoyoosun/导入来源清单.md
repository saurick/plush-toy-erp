...
| `docs/product/旧业务记录证据映射草案.md` | Source Snapshot / Data Import Source | Product / Data Import | partners, products, project-orders | 是，仅作为 dry-run 设计输入 | V1 MasterData / Sales Order 候选 | High | 是 | 009 输出的映射草案是本轮 data map 输入，不是迁移执行。 |
| `docs/product/旧业务记录引用审计.md` | Audit / Deletion Boundary | Product | retired business_records references | 否 | 无直接导入目标 | High | 是 | 用于确认旧表族已删除和哪些字符串只是边界守卫。 |
