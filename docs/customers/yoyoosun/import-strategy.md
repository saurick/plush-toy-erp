...
本策略从 Product 层约束 永绅 yoyoosun 客户数据导入。当前已新增 dry-run preview package tooling、source snapshot freeze checker、real dry-run evidence preparation 和受控 import execution loader。执行器只按审批清单走 JSON-RPC V1 API；没有客户确认、备份证据、显式 `--execute` 和确认短语时不会真实写入。
...
1. 备份计划明确，包含数据库和 source artifacts，且 `--backup-evidence` 指向实际存在的证据文件。
2. rollback / forward-fix 计划明确。
3. approval JSON 只批准 dry-run candidates 中的 supported target：`customers`、`suppliers`、`contacts`、`sales_orders`、`sales_order_items`。
4. approval 不得批准 unresolved block 或 forbidden auto-import 证据。
5. 真实执行必须显式传 `--execute`、`--backend-url` 或 `CUSTOMER_IMPORT_BACKEND_URL`，并设置 `CUSTOMER_IMPORT_CONFIRM=EXECUTE_YOYOOSUN_IMPORT`。
