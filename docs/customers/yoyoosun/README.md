...
| 要做什么 | 先读 | 再核对 |
| --- | --- | --- |
| 与甲方确认岗位、审批、交接、异常和范围 | [甲方角色职责与业务流转确认表](甲方角色职责与业务流转确认表.md) | [决策日志](决策日志.md)、[问题待办](问题待办.md)、[待验证假设](问题待办.md#待验证假设) |
...
| 记录和追溯甲方意见、既有来源摘记 | [甲方反馈记录](甲方反馈记录.md)（仅作备忘） | 正式决定查[决策日志](决策日志.md)，生产主线查 D-006 与角色 / 流程矩阵 |
| 判断 Excel 字段是否进入 Product Core | [Excel 字段产品核心映射评审](Excel字段产品核心映射评审.md) | [导入字段分类](导入字段分类.md)、当前 schema 和测试 |
| 做来源校验、freeze、dry-run 或 BOM Excel 辅助录入 | [导入策略](导入策略.md) | [导入准备工具](../../../scripts/import/README.md)、客户 Private 仓、BOM 页面与导入测试 |
...
| 分组 | 当前文档 | 权威边界 |
| --- | --- | --- |
| 客户输入与决策 | `甲方反馈记录.md`、`决策日志.md`、`问题待办.md`、`问题待办.md#待验证假设`、`README.md#客户输入与变更` | 反馈仅作备忘，正式决定、未决和推断分开；客户原件不在产品仓 |
...
| 导入准备与 BOM 辅助录入 | `导入策略.md`、`../../../scripts/import/README.md`、`导入字段分类.md`、`Excel字段产品核心映射评审.md` | 通用数据仅校验 / extract / freeze / dry-run；BOM `.xlsx` 可经页面复核后新建草稿 |
...
只有项目负责人明确标注甲方提出、要求、反馈或确认的输入才归入客户来源；裸“甲方”当前指 `yoyoosun`，明确其他客户名称或 key 时不写入本目录。未标注的输入属于项目负责人和 Codex 的产品/研发判断。

| 内容 | 记录入口 |
| --- | --- |
| 甲方意见、初始输入和样本线索 | [甲方反馈记录](甲方反馈记录.md)，仅作输入备忘；原件保留在客户 Private 仓 |
| 尚未决定的业务选择或推断 | [问题待办](问题待办.md)及其“待验证假设”分区 |
| 已确认且影响长期业务合同的选择 | [决策日志](决策日志.md)，保留确认来源、适用范围和替代关系 |
| 实现、配置、发布与 UAT 状态 | 对应正式合同和[客户交付矩阵](客户交付矩阵.md)，分别取证 |

尚未决定的澄清更新原问题或假设；已确认或已实施行为发生变化时追加决策，不能覆盖旧决定。只改变未来行为时注明新单/新版本范围；影响已生效订单、库存、生产、出货、财务或审计时，必须评审版本化、迁移、调整或冲正。未确认的高风险选择只阻断对应切片。

普通正确性、易用性和技术修复由代码、测试与必要专题记录，不逐项建立需求或决策 ID。具体实现过程见[模块实施治理](../../product/模块实施治理.md)，差异归属见[配置与权限策略](../../product/配置与权限策略.md)。会话中的甲方意见统一追加或补充到反馈记录，不新建每会话文件、Change 模块或第二套需求台账。
