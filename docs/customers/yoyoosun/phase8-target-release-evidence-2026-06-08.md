...
| Simulated MasterData | `SIM-YOYOOSUN-PHASE8-PCS` unit_id `1`；`SIM-YOYOOSUN-PHASE8-PRODUCT` product_id `1`；`SIM-YOYOOSUN-PHASE8-WH` warehouse_id `1` |
...
| 真实客户数据导入 | 当前不可执行；本次只使用 `SIM-YOYOOSUN-PHASE8` 前缀的模拟主数据和模拟事实 |
