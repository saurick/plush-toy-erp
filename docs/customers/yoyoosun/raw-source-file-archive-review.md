...
本文件评审 `/Users/simon/Downloads/永绅erp/原文件/` 下这批永绅 yoyoosun 客户原始文件的用途、边界和仓库归档落点。

## 结论 / Decision

- 原始 Excel / PDF / PNG 本轮纳入项目，归档到 `docs/customers/yoyoosun/raw-source-files/`，保留原始文件名，方便后续功能、字段和模板溯源。
- `/Users/simon/Downloads/永绅erp/原文件/` 只作为本次复制来源，不再作为唯一可追溯位置。
...
- 当前仓库未启用 Git LFS，本批约 24MB 原件直接纳入 Git；后续若继续批量增加原始二进制文件，应另评审 Git LFS、对象存储、脱敏样本或只提交结构化 fixture。
...
```text
source: /Users/simon/Downloads/永绅erp/原文件/
