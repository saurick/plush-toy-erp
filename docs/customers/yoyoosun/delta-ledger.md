...
- 本客户交付矩阵见 `delivery-matrix.md`。
- 总入口索引见 `../../product/product-delivery-ledgers.md`。
- 新客户必须新建 `docs/customers/<customer-key>/delta-ledger.md`，不要继续追加到本文。
...
| DELTA-YOYOOSUN-023 | yoyoosun | 菜单入口                                | V1 页面可试用需要               | Customer Config / UI                         | 正式菜单已接入核心 V1 入口，旧重叠路径不再保留产品内路由、重定向或权限别名，旧业务记录 API 已退出运行时       | 否                    | 保持正式菜单指向 V1；后续只在客户提出历史查看时评审只读归档模型                              | business_records deletion                     | 旧书签和培训口径风险           | customer menu config / archive decision     |
...
| DELTA-YOYOOSUN-027 | yoyoosun | 客户配置包 / 配置 loader                | 产品化交付需要                  | Customer Config                              | 品牌 / 桌面菜单展示配置 loader 已接入前端；字段、编号、导入配置、角色模板等 runtime loader 未做               | 否                    | 不新增 `tenant_id`；字段 / 编号 / 导入草案先保持 `runtimeEnabled=false`，继续按低风险配置项逐步接入 | 字段 / 编号 / 导入配置评审                    | 被误读为 Runtime Tenant / SaaS / 真实导入批准 | customer config review                      |
