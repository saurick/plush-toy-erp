...
| DELTA-YOYOOSUN-015 | yoyoosun | 手机端任务处理                          | 汇报资料/产品方向               | Industry Template / UI                       | Phase 9 已发布目标环境最小岗位任务端和模拟 workflow 闭环                                                     | 可能                  | 保持 RBAC / owner_role_key / task_status_key 守卫，不让任务端写事实                          | 任务/岗位权限                                 | 把模拟闭环误读为客户签收       | phase9 target evidence                     |
| DELTA-YOYOOSUN-016 | yoyoosun | 老板审批更快                            | 汇报资料                        | Industry Template                            | Phase 9 已覆盖老板审批模拟任务处理                                                                            | 可能                  | Workflow/mobile                                                                               | 审批节点设计                                  | 只做 UI 不做权限               | phase9 target evidence                     |
| DELTA-YOYOOSUN-017 | yoyoosun | 仓库 / 品质现场扫码拍照                 | 汇报资料                        | Industry Template / Product Core             | Phase 9 已覆盖现场留痕文本 / refs 和异常报告展示；拍照上传、附件服务和扫码仍未交付                            | 可能                  | 质检/库存/移动端评审                                                                          | 事实模型                                      | evidence 被误读为附件服务      | mobile + attachment review                 |
...
| DELTA-YOYOOSUN-021 | yoyoosun | yoyoosun 数据导入                       | 客户落地需要                    | Data Import Adapter / Customer Config        | Excel source extract、dry-run / freeze tooling、受控 execution loader 和 `importConfig.mjs` 配置草案已完成；当前没有可执行客户真实数据，Phase 7 只能模拟 | 否                    | source extract / freeze / dry-run evidence -> tracked config draft -> simulated data trial rehearsal | 人工确认、existing V1 snapshot review        | 误导入/误生成事实/配置草案被当真实导入 | existing V1 snapshot review                 |
...
| DELTA-YOYOOSUN-030 | yoyoosun | 多客户私有化复制                         | Phase 11 产品化方向             | Delivery / Customer Config                   | 已形成私有化客户包模板候选和模拟闭环；不代表真实第二客户或 SaaS runtime                                       | 否                    | 通过 docs/config/deployments 客户包复制，真实客户另评审 customer key、资料边界、部署、备份和导入批准 | 真实客户资料和部署目标                         | 客户包模板被误读为代码分叉或 tenant | private package review                       |
