...
- output 是 evidence，不是 import approval；fixture / sample 生成的 dry-run package 只能 report-only，不能进入 `--execute`。
...
报告模式会校验 approval、backup evidence、unresolved block、forbidden auto-import 和 supported target，不连接后端、不写 DB。真实 `--execute` 还会拒绝路径或内容含 fixture / sample / placeholder 的 dry-run report、approval、backup evidence 和 recovery plan，并要求 backup evidence 提供 `backupId`、`releaseVersion`、`databaseSnapshot`、ISO `backupTime`、正数 `databaseBackupSize`、合法 `databaseBackupHash` 和 `operator`，同时要求 recovery plan 明确 owner、rollback target、forward-fix path、failure triggers、post-recovery verification，且 `recoveryPlan.backupEvidence` 必须匹配 `backupEvidence.backupId`。
