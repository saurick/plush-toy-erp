...
| DELTA-YOYOOSUN-027 | yoyoosun | 客户配置包 / 配置 loader                | 产品化交付需要                  | Customer Config                              | 品牌 / 桌面菜单展示配置已改为部署侧 `customer-config.js` 注入；默认产品前端不静态打包 yoyoosun；字段、编号、导入配置、角色模板等 runtime loader 未做               | 否                    | 不新增 `tenant_id`；字段 / 编号 / 导入草案先保持 `runtimeEnabled=false`，继续按低风险配置项逐步接入 | 字段 / 编号 / 导入配置评审                    | 被误读为 Runtime Tenant / SaaS / 真实导入批准 | customer config review                      |
