...
| DELTA-YOYOOSUN-006 | yoyoosun | 颜色、尺寸、客户款号                    | Excel/订单字段线索              | Industry Template Candidate / Deferred       | SKU schema 已落，但不从 Excel / 订单颜色自动创建 SKU                                                          | 暂不进入运行时写入     | 作为 SKU API / UI / 导入专项输入                                                              | product_sku runtime review                    | 过早自动建 SKU                 | SKU API/UI/import review                    |
