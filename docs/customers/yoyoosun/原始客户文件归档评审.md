...
- 若后续需要进入导入 dry-run，应先登记或更新 `docs/customers/yoyoosun/source-manifest.json`，通过 manifest checker 校验原件 path、sha256、size 和用途，再从允许结构化提取的原件生成脱敏、结构化、可审查的 snapshot fixture，落到 `scripts/import/fixtures/customers/yoyoosun/*`。
...
- 当前仓库仍可在受控开发工作区读取这些已跟踪原件；这不等于允许把它们带入产品发布物。根 `.dockerignore` 会从 Docker build context 排除 `docs/`、`deployments/` 和客户私有 `assets/`，根 `.gitattributes` 会从标准 `git archive` 排除 `docs/customers/**` 与 `config/customers/*/assets/**`。客户公开构建只允许复制经审查的 `public-assets/`；直接 clone 或复制整个工作区仍会包含原件，必须继续依赖仓库访问控制，不能当成对外源码交付方式。
...
| 后续用途 | 允许落点 | 禁止事项 |
| --- | --- | --- |
| 原始资料用途登记和原件归档 | `docs/customers/yoyoosun/source-manifest.json`、`docs/customers/yoyoosun/raw-source-files/*`、`docs/customers/yoyoosun/原始客户文件归档评审.md` 和 `docs/customers/yoyoosun/来源材料.md` | 不接 runtime，不进 docs registry，不当 Product Core |
...
3. Excel 列是否已进入 `导入字段分类.md` 和 unresolved queue。
4. PDF / 图片是否只是打印样式或截图线索，还是需要形成正式模板 / 页面需求。
5. 是否会误把合同金额、生产数量、未出货数、采购数量或截图状态写成库存、出货、财务事实。
6. 是否需要更新 `docs/文档清单.md`、帮助中心入口、测试断言或 docs registry。
