...
该报告只记录后端健康检查、演示账号密码环境变量是否存在、预期账号数量、静态角色投影检查和阻塞项；静态投影会核对 Go seed、后端 RBAC mobile 权限、前端移动角色入口、浏览器 smoke 账号和文档入口是否仍覆盖同一组试用角色。不读密码、不登录、不调用 `admin_login / me`、不写数据库，不保存 access token 或 Authorization header，也不证明真实 RBAC 已通过。
...
该报告只记录后端健康检查、演示账号密码环境变量是否存在、是否需要脚本托管 Vite、yoyoosun customer config 脚本是否存在、静态菜单投影覆盖计划和阻塞项；静态计划会输出桌面账号菜单应见 / 禁见、客户隐藏菜单、旧入口清理、岗位任务端路径和 `demo_admin` 移动端拒绝态。不读密码、不登录、不调用 JSON-RPC、不启动浏览器、不启动 Vite、不读取客户配置脚本、不写数据库，也不证明真实角色菜单 / 岗位任务端已通过。
