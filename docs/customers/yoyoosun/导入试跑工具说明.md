...
| 永绅 GitHub Private 仓库 | 保存真实来源、私密 manifest、评审和验证入口；当前 inventory 与版本以私有仓 manifest / README 为准，不作为 Product Core submodule |
| 远端完整性 | hash / size、结构化提取、no-real-import 与远端回读结果以私有仓正式验证记录为准 |
| Product Core 当前树 | 当前提交树与索引已移除旧原件目录、真实 manifest 与客户工程指导图片 |
| Product Core 版本锁 | 私有仓 `product.lock.json` 是可变真源；Product Core `HEAD` 推进后必须重新锁定并通过 `FORMAL_PRODUCT_PIN=1` validate |
| Git 历史 | 既有原件仍在历史中，尚未执行历史重写 |

因此当前可以确认 Product Core 当前树已与客户原件隔离；客户原件存储、远端完整性和锁定 Product Core 版本的当前证据只从客户专属 Private 仓库读取。该证据不等于真实导入批准、全产品发布验收、Git 历史清理或客户签收。
...
产品仓与客户仓使用兄弟目录。正式验证只在两边已提交且工作树清洁、`product.lock.json` 指向当前 Product Core `HEAD` 时运行：
...
7. 确认私有仓 `product.lock.json` 指向本次 Product Core 提交，并在两边清洁时通过 `FORMAL_PRODUCT_PIN=1` validate；旧验证不能证明后续 `HEAD`。
