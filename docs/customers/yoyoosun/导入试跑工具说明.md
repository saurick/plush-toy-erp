...
Current Implementation Source of Truth / 当前实现真源: `docs/customers/yoyoosun/source-manifest.json`, `scripts/import/customerSourceManifestCheck.mjs`, `scripts/import/customerSourceExtract.mjs`, `scripts/import/customerImportDryRun.mjs`, `scripts/import/customerSourceSnapshotFreezeCheck.mjs`

# 永绅 yoyoosun 客户导入 dry-run 工具说明 / Yoyoosun Customer Import Dry-run Tooling

已新增 永绅 yoyoosun 客户 source manifest、manifest checker、来源 Excel 提取 CLI、导入 dry-run CLI、source snapshot freeze checker，并用 sanitized freeze fixtures 生成 freeze evidence 与 real dry-run evidence package。这组工具只生成本地导入前 evidence 和人工 review 材料；它们不执行真实导入。由 fixture / sample 生成的 dry-run package 只允许 report-only 自检，不能进入真实 `--execute`；真实 `--execute` 还必须提供结构化 backup evidence 和 reviewed recovery plan JSON，且两者绑定同一 `backupId`。

## Source Manifest 主路径 / Source Manifest Main Path
...
`docs/customers/yoyoosun/source-manifest.json` 是导入前来源清单主路径。它记录每个原始 Excel / PDF / 图片的稳定 `sourceId`、path、sha256、size、用途、domain、敏感复查标记和 `structuredExtract` 策略。manifest checker 会阻断 sha256 漂移、size 漂移、重复路径、未登记 raw source 文件，以及非 Excel 文件被标记为结构化提取。
...
提取器只处理 source manifest 中 `structuredExtract.enabled=true` 的本地 Excel，不直接 glob `raw-source-files/*.xlsx` 作为正式主路径。PDF / 图片仍保留为人工来源引用，不做 OCR，不从图片生成结构化事实。输出目录在 `output/` 下，不纳入 git，也不是 import approval。

`source-snapshot.extracted.json`、freeze metadata 和 dry-run validation summary 会保留 `sourceManifest.path` 与 `sourceManifest.sha256`，用于追溯本次 evidence 基于哪一版来源清单生成。`customer-import-config.candidate.json` 是本地生成的 evidence 候选，不是 tracked runtime 配置。已人工收口后的客户配置草案落在 `config/customers/yoyoosun/importConfig.mjs`：该文件只记录统计、字段映射分组、导入顺序、review queue、deferred runtime 项和 forbidden auto-import targets，不嵌入 raw rows，不接 loader，不执行真实导入。
...
这些 output 目录只作为本地 evidence，不纳入 git，也不是 import approval。若 dry-run report 的输入路径或内容仍包含 fixture / sample / placeholder，只能用于 report-only，不能作为真实执行输入。
...
`customerImportDryRun.mjs` 本身仍只读取 JSON snapshot，不直接解析 Excel / PDF / OCR。永绅原始 Excel 必须先通过 source manifest 校验，再由 `customerSourceExtract.mjs` 提取为 `source-snapshot.extracted.json`；PDF / 图片仍必须人工整理或后续单独工具处理。
