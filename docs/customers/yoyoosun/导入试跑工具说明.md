...
已新增 永绅 yoyoosun 客户 source manifest、manifest checker、来源 Excel 提取 CLI、导入 dry-run CLI、source snapshot freeze checker，并用 sanitized freeze fixtures 生成 freeze evidence 与 real dry-run evidence package。这组工具只生成本地导入前 evidence 和人工 review 材料；它们不执行真实导入。
