...
本文用于把 `字段编号确认清单.md` 的客户确认结果回写到正确位置。本文不是 runtime、schema、migration、API、RBAC、导入或真实账号真源；它只规定确认结果如何记录、分类和进入后续评审。
...
| 结果类型 | 写入位置 | 不写入位置 |
| --- | --- | --- |
| 已确认客户配置 | `客户配置草案.md`、`fieldNumberingConfig.mjs` | 不直接写 schema、migration、API |
| 仍未确认问题 | `问题待办.md` 或 `导入待确认队列.md` | 不塞进默认值或 fallback |
| 客户差异 | `客户交付矩阵.md` 的“客户差异与决策” | 不升级为 Product Core |
| 已确认正式决策 | `决策日志.md` | 不把猜测写成已决策 |
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | customers | 客户编码 |  | 客户配置草案.md / fieldNumberingConfig.mjs | No / Review Required |  |
| 2 | sales_orders | 销售订单编号 |  | 客户配置草案.md / fieldNumberingConfig.mjs | No / Review Required |  |
...
| 字段结论 | 回写动作 |
| --- | --- |
| 客户确认字段显示但不要求必填 | `fieldNumberingConfig.mjs` 中保留 `decision=review_required`，在 note 写明显示偏好；`客户配置草案.md` 更新状态 |
| 客户确认字段必须出现但当前模型无字段 | 进入 `问题待办.md` 或 Product Core 评审；不直接加 schema |
| 客户确认字段只是 yoyoosun 习惯 | 进入 `客户交付矩阵.md` 的“客户差异与决策”，分类为 Customer Config |
| 客户确认字段属于导入来源 | 进入 `导入待确认队列.md` 或 future import loader 设计 |
...
| 结论 | 原因 |
| --- | --- |
| 颜色 / 尺寸直接形成 SKU | `product_skus` 已有维护 API / UI，显式 SKU 已进入下游库存 grain；客户导入受控创建、订单颜色自动建 SKU、BOM SKU 粒度和旧库存重分类仍需专项评审后才能写入 |
| 采购订单号直接生成采购订单 | `purchase_orders` 已是正式 Source Document，但客户字段 / 编号映射和受控导入尚未批准，原始字段不能绕过聚合校验直接写入 |
| 交期 / 出货日期代表已出货 | `shipping_released != shipped`，销售订单不是出货事实 |
| 未出货数 / 生产数量写入订单明细事实 | 出货和生产事实已有正式来源动作；未出货量 / 完成量应由源单与有效事实派生，不能作为客户原始字段回写真源 |
| 单价 / 金额生成应收、发票或收款 | 应收 / 发票只从 `SHIPPED` 出货来源动作生成；收付款和核销也必须走独立 FinancePayment 生命周期。原始单价 / 金额、Excel 或 Workflow 不能直接生成任何财务事实；银行直连 / 流水导入仍不在当前范围 |
