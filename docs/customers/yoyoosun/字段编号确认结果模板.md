...
| 客户差异 | `差异登记.md` | 不升级为 Product Core |
...
| 客户确认字段只是 yoyoosun 习惯 | 进入 `差异登记.md`，分类为 Customer Config |
...
| 单价 / 金额生成应收、发票或收款 | 应收 / 发票只从 `SHIPPED` 出货来源动作生成；原始单价 / 金额不能直接生成财务事实，`PAYMENT`、银行收付和完整核销仍 deferred |
