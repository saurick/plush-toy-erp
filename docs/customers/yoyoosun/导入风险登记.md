...
| 产品与 `product_skus` 混淆 | 把颜色、尺寸、包装版本误写入 product core 或自动建 SKU | `product_skus` 已有基础维护 API / UI，但导入和页面运行时仍未开放自动创建 | SKU 类字段 deferred，不自动创建 | Product / Sales | 是 |
...
| 自动生成出货 / 库存 / 财务事实 | 造成 shipped、库存余额、应收应付或收付款错误 | `shipping_released != shipped`；应收 / 应付 / 发票、真实收付款和多来源核销虽然已有各自正式 usecase，但任何一类都不得由 Excel、旧快照或 Workflow 自动生成 | shipment / inventory / finance 全部 Forbidden Auto Import | Workflow / Fact | 是 |
...
|---:|---|---|
| 阻断（P0） | 从旧快照自动生成 shipment / inventory / finance facts | Forbidden Auto Import + block queue |
| 阻断（P0） | 永绅 yoyoosun 字段污染 Product Core | field classification + Product layer strategy |
| 阻断（P0） | 旧 `business_records` 被恢复成双真源 | 旧表族已删除；dry-run 不允许恢复 runtime |
| 阻断（P0） | dry-run 报告被误当真实导入批准 | `validation-summary.canExecuteRealImport=false` + `dry-run-report.md` No real import |
| 阻断（P0） | freeze evidence 被误当真实导入批准 | `freeze-metadata.canExecuteRealImport=false` + `来源快照人工复查清单.md` import-not-approved conclusion |
| 重要（P1） | SKU / purchase order 过早落地 | deferred domain policy |
| 重要（P1） | 未确认字段丢失或塞 note | unresolved queue + manual review |
