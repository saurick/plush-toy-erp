...
| 自动生成出货 / 库存 / 财务事实 | 造成 shipped、库存余额、应收应付错误 | `shipping_released != shipped`；应收 / 应付 / 发票只允许正式来源动作，`PAYMENT`、银行收付和多单核销仍 deferred | shipment/inventory/finance 全部 Forbidden Auto Import | Workflow / Fact | 是 |
