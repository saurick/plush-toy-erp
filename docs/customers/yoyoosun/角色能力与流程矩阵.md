...
| 永绅跟踪岗位投影 | 9 个业务岗位、321 条岗位权限分配、137 个去重 permission key；116 条菜单分配、29 个去重菜单 | 第 5 章逐项列全；全部应为对应 Product Core 默认角色的子集。 |
| 永绅本地有效权限 | 当前配置编译 revision 为 `yoyoosun-customer-package-v7.local-40a5a9924b269f4b.runtime-v1`，工程 / PMC 的 F02 来源读取和销售关闭订单投影调整后尚待重新激活 / 读回和全角色 smoke；旧 revision `yoyoosun-customer-package-v7.local-931c8fda75bc4b6a.runtime-v1` 的证据已失效 | 编译成功不是运行证据；不能外推为 133 / 生产或客户 UAT。 |
...
| --- | ---: | --- | --- | --- | --- | ---: |
| 销售 `sales` | 34 | sales | 客户订单、包装要求、客户退货来源 | 审批源单、包材确认、出货确认线索 | F01、F02、F09、F14、F16、F21 | 10 |
| 老板 `boss` | 44 | boss | 固定责任池升级、销售退货 / 收付款 / 生产和库存异常审批 | 通过、拒绝或受控调整未结束任务的个人归属 | F02、F05、F20、F21 | 21 |
| 工程 `engineering` | 28 | engineering | 已审订单 | 产品 / SKU / 材料 / 工序 / BOM 齐套资料 | F02–F05、F21 | 8 |
| PMC `pmc` | 26 | pmc | 订单与工程资料 | 排期、齐套、生产评审、风险 | F02–F05、F09–F14、F21 | 12 |
| 采购 `purchase` | 39 | purchase | 物料需求、审批意见 | 采购源单、合同、到货来源 | F05–F09、F21 | 13 |
...
| Workflow | `workflow.task.read`<br>`workflow.task.update`<br>`workflow.task.complete`<br>`workflow.task.approve` |
