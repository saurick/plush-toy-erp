...
| 权限中心最终有效解释 | 后端 `admin.effective_role_access` 统一解释 RBAC、active revision、模块与岗位页面投影；缺 active revision 时 fail-closed | “页面与导航”分开页面可进入、页面内操作和导航位置；可选择系统推荐或自定义 1–5 个常用入口并排序，草稿即时预览。布局设置随岗位版本保存并审计，但只消费最终可进入页面；未保存权限不冒充实际导航，也不能替代目标账号实登验收。 |
