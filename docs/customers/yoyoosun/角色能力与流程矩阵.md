...
| 永绅本地有效权限 | 当前配置编译 revision 为 `yoyoosun-customer-package-v7.local-f2f31bc4c170a126.runtime-v1`；销售 / 采购 / 工程审批候选的受控退回上限补齐后，旧 revision `yoyoosun-customer-package-v7.local-40a5a9924b269f4b.runtime-v1` 的本地证据已失效，尚待重新激活、读回和全角色 smoke | 编译成功不是运行证据；不能外推为 133 / 生产或客户 UAT。 |
...
| 仓库 `warehouse` | 37 | warehouse | 到货、完工品、放行请求、销退收货 | 入库、出货、库存和销退事实 | F06–F10、F14–F19、F21；F10 仅实物交接 / 只读 | 11 |
| 品质 `quality` | 29 | quality | 到货、WIP、成品、出货前待检 | 判定、生产异常申请、返工 / 放行结果 | F06–F13、F15、F17、F21 | 12 |
| 财务 `finance` | 42 | finance | 合格来源、出货放行申请、收付款来源 | 应收 / 应付 / 发票 / 对账、收付款和贷项事实 | F15、F17–F21；F05 仅组合账号 | 16 |
...
| 跟踪预览页面（非 runtime 证明） | `global-dashboard`、`materials`、`products`、`inventory`、`inbound`、`quality-inspections`、`shipping-release`、`outbound`、`shipments`、`sales-returns`、`task-board` |
...
| 跟踪预览页面（非 runtime 证明） | `global-dashboard`、`customers`、`suppliers`、`reconciliation`、`payables`、`receivables`、`invoices`、`finance-payments`、`processing-contracts`、`sales-orders`、`quality-inspections`、`inventory`、`inbound`、`shipments`、`task-board`、`print-center` |
