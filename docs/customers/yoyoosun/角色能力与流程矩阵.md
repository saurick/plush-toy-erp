...
永绅桌面导航采用 `role_guided` 展示：“看板中心”固定在最前，并按工作台、任务看板、业务看板的顺序仅显示当前已授权看板；“岗位使用帮助”固定进入可折叠的“更多功能”。“常用工作”默认由系统按岗位推荐，普通岗位约 3 个，财务默认显示应付、应收、发票、对账 4 个；管理员也可为业务岗位从当前最终可进入页面中自定义并排序 1–5 个常用入口。该设置只是岗位级视觉布局，不删除 `menuSurfaces`、不改变直接路由或页面内操作权限，也不把参考查询页升级为可办理动作。多岗位账号仍合并各岗位最终有效页面并按岗位轮流推荐；帮助页必须显式切换本次办理岗位。
...
| 跟踪预览页面（非 runtime 证明） | `global-dashboard`、`customers`、`suppliers`、`reconciliation`、`payables`、`receivables`、`invoices`、`finance-payments`、`processing-contracts`、`sales-orders`、`quality-inspections`、`inventory`、`inbound`、`shipping-release`、`shipments`、`task-board`、`print-center` |
