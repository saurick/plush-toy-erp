...
| 层级 | 当前数量 / 状态 | 本文展示方式 |
| --- | --- | --- |
| Product Core 权限注册表 | 149 个 permission key、11 个内置角色默认组合、29 个后端菜单 | 作为上限和代码默认，不冒充当前数据库值。 |
...
| `is_super_admin=true` | 账号标志，不是角色；后端可通过全部 149 个权限检查。 | `F22` break-glass；不自动取代 owner / assignee，必须显式、有期限、审计，仍不能绕过状态、版本、幂等和领域 usecase。 |
