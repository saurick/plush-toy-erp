...
当前审批岗位口径：九个业务角色的 `workflow.task.approve` 只是 Product Core RBAC 上限，不等于可跨池办理。管理员在“权限管理 → 审批责任”只维护销售订单、采购订单和 Shipment 财务放行三类正式审批；yoyoosun 新配置默认由销售、采购、财务分别主办，老板仅作升级。具名员工必须启用且持有所选岗位，多岗位员工按其真实岗位参与；运行时只开放最低可用层级，主办失效才进入备用 / 升级。最终资格取任务冻结 revision 的 entitlement、责任池岗位 / 具名员工、账号状态、owner / assignee、状态/version 与幂等交集。2026-07-24 本地开发后端已发布、激活并由管理员读回这三项推荐责任；133 / 生产目标环境尚未发布，真实岗位办理 smoke 与客户 UAT 未执行，不能把本地生效写成客户环境已生效。
...
| 层级 | 当前数量 / 状态 | 本文展示方式 |
| --- | --- | --- |
| Product Core 权限注册表 | 160 个 permission key、11 个内置角色默认组合、31 个后端菜单 | 作为上限和代码默认，不冒充当前数据库值；采购审批只使用统一 `workflow.task.approve`，已移除重复的 `purchase.order.approve`。 |
| 永绅跟踪岗位投影 | 9 个业务岗位、288 条岗位权限分配、119 个去重 permission key | 第 5 章逐项列全；全部应为对应 Product Core 默认角色的子集。 |
| 永绅本地有效权限 | 当前独立 Worktree 未取得 fresh 数据库 active revision、9 岗位真实 JSON-RPC 或 Chromium 登录读回 | 本轮代码与静态 / 自动化证据单列；不能复用旧本地报告冒充当前运行态。 |
...
| --- | ---: | --- | --- | --- | --- | ---: |
| 销售 `sales` | 31 | sales | 客户订单、包装要求 | 审批源单、包材确认、出货确认线索 | F01、F02、F09、F14、F16、F21 | 9 |
| 老板 `boss` | 38 | boss | 销售 / 采购审批；付款仅候选预览 | 通过、退回或受控调整未结束任务的个人归属 | F02、F05、F21；F20 仅 `preview_only` 付款审批需求 | 19 |
...
| 采购 `purchase` | 38 | purchase | 物料需求、审批意见 | 采购源单、合同、到货来源 | F05–F09、F21 | 12 |
| 仓库 `warehouse` | 32 | warehouse | 到货、完工品、放行请求 | 入库、出货、库存事实 | F06–F10、F14–F19、F21；F10 仅实物交接 / 只读 | 9 |
| 品质 `quality` | 28 | quality | 到货、WIP、成品、出货前待检 | 判定、异常、返工 / 放行结果 | F06–F13、F15、F17、F21 | 12 |
| 财务 `finance` | 36 | finance | 合格来源、出货放行申请 | 应收 / 应付 / 发票 / 对账与放行线索 | F15、F17–F21；F05 仅组合账号 | 15 |
| 生产 / 委外 `production` | 40 | production | 排程、物料、品质结果 | WIP 内外发、移交、返工、完工来源 | F04、F09–F15、F17、F21 | 13 |
...
| 上游 → 老板 | 销售、采购或财务主办池无法承接或需要升级的正式事项；付款放行仅是 `preview_only` 候选结构。 |
...
| `is_super_admin=true` | 账号标志，不是角色；后端可通过全部 160 个权限检查。 | `F22` break-glass；不自动取代 owner / assignee，必须显式、有期限、审计，仍不能绕过状态、版本、幂等和领域 usecase。 |
...
| 代码 | 证据入口 | 能证明 | 不能证明 |
| --- | --- | --- | --- |
| E-RBAC | 手册同步、客户配置、closure / readiness / acceptance Node 合同 | 9 岗位、288 分配、菜单 / 职责 / 预览对象与当前配置同步 | 目标有效权限或账号可登录 |
...
| --- | --- | --- |
| 角色 / 配置 / 文档 | 9 岗位、288 分配、菜单、责任池、预览边界、链接、隐私和 Mermaid | effective session、账号 allow / deny 与甲方职责签认另验 |
...
“反复 review”依次核对 Private 来源、9 岗位 / 288 分配、Source / Workflow / Fact / 目标 / UAT 分层、Mermaid 和受影响门禁。发现问题就从受影响层重跑；连续一轮无新缺陷且已承诺门禁为非零测试、零 fail、零 skip 时停止。
