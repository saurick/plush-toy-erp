...
| 财务放行 | 可选 / 部分 ProcessRuntime 的 EffectNone 候选节点；非直连强制闸口 | 不自动生成收付款、应收、发票或总账 |
...
| F20 | 付款、银行流水、多单核销、总账 / 税控 | finance | FinancePayment / Allocation / CreditNote | Product Core 本地部分；永绅未授权 | 真实收付款、同往来方同币种多单核销、冲正和红冲 V1 已实现；永绅无收付款页面 / 权限，133 当前版本未发布；付款审批、银行直连、总账和税控仍缺 |
...
| --- | --- | --- | --- |
| 订单与资料 | sales → boss → engineering → pmc | 审批任务可办理；SalesOrder 激活、甲方 PMC 前置和老板二审未闭环 | F02；流程矩阵 4.1 |
...
| 出货 | sales / warehouse → quality → finance 候选 → warehouse | 放行任务与正式出货分开；财务仍非强制门禁 | F16–F18；流程矩阵 4.3 |
| 财务 | shipment / purchase / outsourcing → finance；finance → boss 候选 | 来源财务和 Core 收付款 V1 已有；永绅未授权收付款，付款审批仍 preview | F19–F20；流程矩阵第 6、9 章 |
...
| P2 | Product Core 已有真实收付款、多来源核销、冲正和红冲 V1，但永绅 finance 未获收付款页面 / 权限，付款审批、银行直连、总账、税控仍未实现 | 不能把 Core 本地能力写成永绅已可用，也不能称完整财务系统 | 先确认永绅本期范围和审批职责，再同步 entitlement、目标发布和 UAT |
...
| --- | --- | --- |
| 角色 / 配置 / 文档 | 9 岗位、280 分配、菜单、责任池、预览边界、链接、隐私和 Mermaid | effective session、账号 allow / deny 与甲方职责签认另验 |
| Workflow / ProcessRuntime | 来源、节点、审批 / 退回 / 阻塞、revision、version、幂等和失败恢复 | 当前审批切片按固定 release 重发；客户流程 UAT 另验 |
| Quality / Inventory / WIP / Shipment | usecase、repository、PostgreSQL 事务、页面和 Workflow / Fact 分界 | 首次 IQC 处置、委外 / 生产异常仍需目标 apply、真实岗位 smoke 与 UAT；财务强制放行仍 open |
...
“反复 review”依次核对 Private 来源、9 岗位 / 280 分配、Source / Workflow / Fact / 目标 / UAT 分层、Mermaid 和受影响门禁。发现问题就从受影响层重跑；连续一轮无新缺陷且已承诺门禁为非零测试、零 fail、零 skip 时停止。
