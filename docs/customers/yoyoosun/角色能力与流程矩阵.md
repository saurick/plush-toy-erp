...
老板、PMC 额外具备 `workflow.task.supervise`，该权限只扩展 `list_tasks` 与任务看板的跨责任池只读范围，用于查看、跟进和识别卡点，不授予办理动作。老板另有 `workflow.task.assign`，可在桌面任务看板把 `ready / blocked` 任务转给同一负责岗位的合格 active 人员，或取消个人指派并退回原岗位池；super admin 也可通过全权限执行，但不会因此自动成为业务岗位候选人。PMC、普通 `admin` 和其他业务岗位当前没有该转交 entitlement。转交只改变个人归属并保留事件、幂等 receipt 与审计，不绕过 owner、状态、active revision 或任何领域事实 usecase。
