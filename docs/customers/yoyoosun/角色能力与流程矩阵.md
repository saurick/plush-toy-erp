...
| 永绅跟踪岗位投影 | 9 个业务岗位、320 条岗位权限分配、136 个去重 permission key；116 条菜单分配、29 个去重菜单 | 第 5 章逐项列全；全部应为对应 Product Core 默认角色的子集。 |
| 永绅本地有效权限 | 当前配置编译 revision 为 `yoyoosun-customer-package-v7.local-b8b14c39f0e3d7d6.runtime-v1`，工程与 PMC 的 F02 来源读取投影调整后尚待重新激活 / 读回和全角色 smoke；旧 revision `yoyoosun-customer-package-v7.local-931c8fda75bc4b6a.runtime-v1` 的证据已失效 | 编译成功不是运行证据；不能外推为 133 / 生产或客户 UAT。 |
...
| --- | ---: | --- | --- | --- | --- | ---: |
| 销售 `sales` | 33 | sales | 客户订单、包装要求、客户退货来源 | 审批源单、包材确认、出货确认线索 | F01、F02、F09、F14、F16、F21 | 10 |
...
| 销售订单 / 退货 | `sales_order.read`<br>`sales_order.create`<br>`sales_order.update`<br>`sales_order.submit`<br>`sales_order.cancel`<br>`sales_order_item.read`<br>`sales_return.read`<br>`sales_return.create` |
