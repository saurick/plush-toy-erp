...
当前审批岗位口径：Product Core 登记的通用与领域 approval capability 只是 RBAC 上限，不等于可跨池办理。yoyoosun 跟踪配置已选择销售订单、采购订单、Shipment 财务放行，以及客户退货、收付款、手工库存调整、生产异常七条正式 ProcessRuntime；前三条的责任可配置，后四条使用流程合同固定责任池。最终资格取任务冻结 revision 的 entitlement、责任池岗位 / 具名员工、账号状态、owner / assignee、状态/version 与幂等交集。登记的本地隔离开发库已完成 active revision 读回、10 个桌面账号、9 个手机业务岗位、管理员手机拒绝及四条异常流真实写入；当前切片仍未在 133 / 生产目标环境发布和读回，客户 UAT 未执行，不能把本地证据写成客户环境已生效。
...
| 把 Product Core 能力自动授予永绅岗位 | 跟踪配置虽已包含 `finance.payment.*`、`sales_return.*` 等新 entitlement，本地也已激活读回，但 133 / 生产目标仍未发布当前 revision；必须继续按目标 effective session 和真实账号核对。 |
...
| 层级 | 当前数量 / 状态 | 本文展示方式 |
| --- | --- | --- |
| Product Core 权限注册表 | 165 个 permission key、11 个内置角色默认组合、31 个后端菜单 | 作为上限和代码默认，不冒充当前数据库值；采购审批只使用统一 `workflow.task.approve`，已移除重复的 `purchase.order.approve`。 |
| 永绅跟踪岗位投影 | 9 个业务岗位、324 条岗位权限分配、137 个去重 permission key；116 条菜单分配、29 个去重菜单 | 第 5 章逐项列全；全部应为对应 Product Core 默认角色的子集。 |
| 永绅本地有效权限 | 当前配置编译 revision 为 `yoyoosun-customer-package-v7.local-8ab8deaa7b7e9c6f.runtime-v1`；出货放行页已从 warehouse 投影转入 finance 并绑定 `shipment_finance_approval`，旧 revision `yoyoosun-customer-package-v7.local-f2f31bc4c170a126.runtime-v1` 的本地证据已失效，尚待重新激活、读回和全角色 smoke | 编译成功不是运行证据；不能外推为 133 / 生产或客户 UAT。 |
...
| --- | ---: | --- | --- | --- | --- | ---: |
| 销售 `sales` | 35 | sales | 客户订单、包装要求、客户退货来源 | 审批源单、包材确认、出货确认线索 | F01、F02、F09、F14、F16、F21 | 10 |
| 老板 `boss` | 44 | boss | 固定责任池升级、销售退货 / 收付款 / 生产和库存异常审批 | 通过、拒绝或受控调整未结束任务的个人归属 | F02、F05、F20、F21 | 21 |
| 工程 `engineering` | 29 | engineering | 已审订单 | 产品 / SKU / 材料 / 工序 / BOM 齐套资料 | F02–F05、F21 | 8 |
| PMC `pmc` | 26 | pmc | 订单与工程资料 | 排期、齐套、生产评审、风险 | F02–F05、F09–F14、F21 | 12 |
| 采购 `purchase` | 40 | purchase | 物料需求、审批意见 | 采购源单、合同、到货来源 | F05–F09、F21 | 13 |
| 仓库 `warehouse` | 37 | warehouse | 到货、完工品、财务放行结果、销退收货 | 入库、出货、库存和销退事实 | F06–F10、F14–F19、F21；F10 仅实物交接 / 只读 | 10 |
| 品质 `quality` | 29 | quality | 到货、WIP、成品、出货前待检 | 判定、生产异常申请、返工 / 放行结果 | F06–F13、F15、F17、F21 | 12 |
| 财务 `finance` | 42 | finance | 合格来源、出货放行申请、收付款来源 | 出货财务审批、应收 / 应付 / 发票 / 对账、收付款和贷项事实 | F15、F17–F21；F05 仅组合账号 | 17 |
| 生产 / 委外 `production` | 42 | production | 排程、物料、品质结果 | WIP 内外发、移交、返工、异常申请、完工来源 | F04、F09–F15、F17、F21 | 13 |
...
| 跟踪预览页面（非 runtime 证明） | `global-dashboard`、`customers`、`sales-orders`、`sales-returns`、`products`、`production-orders`、`inventory`、`shipments`、`task-board`、`print-center` |
...
| 销售订单 / 退货 | `sales_order.read`<br>`sales_order.create`<br>`sales_order.update`<br>`sales_order.submit`<br>`sales_order.close`<br>`sales_order.cancel`<br>`sales_order_item.read`<br>`sales_return.read`<br>`sales_return.create` |
...
| 参与流程 | `F02`、`F05`、`F20`、`F21`；`F20` 的正式 `finance_payment_approval` 只批准 / 拒绝 Source，不会代替财务显式 post、核销或冲正。 |
...
| 异常审批 | `production.exception.approve`<br>`warehouse.adjustment.approve`<br>`sales_return.approve`<br>`sales_return.cancel`<br>`finance.payment.approve` |
...
| 跟踪预览页面（非 runtime 证明） | `global-dashboard`、`materials`、`products`、`inventory`、`inbound`、`quality-inspections`、`outbound`、`shipments`、`sales-returns`、`task-board` |
...
| 客户退货 | `sales_return.read`<br>`sales_return.receive`<br>`sales_return.reverse` |
...
| WIP / 生产 Fact | `production.fact.read`<br>`production.wip.read`<br>`production.wip.assign`<br>`production.wip.execute`<br>`production.wip.rework`<br>`production.completion.create`<br>`production.material_issue.create`<br>`production.rework.create`<br>`production.fact.post`<br>`production.fact.cancel`<br>`production.exception.submit` |
...
| `is_super_admin=true` | 账号标志，不是角色；后端可通过全部 165 个权限检查。 | `F22` break-glass；不自动取代 owner / assignee，必须显式、有期限、审计，仍不能绕过状态、版本、幂等和领域 usecase。 |
...
| F17 | 成品质检、财务放行；甲方财务确认要求 | quality → finance → warehouse | `finished_goods_delivery` / `shipment_finance_approval` + Shipment finance gate | 本地强制门禁已接：财务审批后写版本化审计锚点，普通 / 流程出货均不可绕过；目标环境与岗位 UAT 未执行 | E-RBAC + E-DOMAIN + E-PG；E-TARGET open |
...
| 代码 | 证据入口 | 能证明 | 不能证明 |
| --- | --- | --- | --- |
| E-RBAC | 手册同步、客户配置、closure / readiness / acceptance Node 合同 | 9 岗位、312 条权限分配、菜单 / 职责 / 预览对象与当前配置同步 | 目标有效权限或账号可登录 |
...
| `sales_return_acceptance` | `v1` | `approval_receipt` | `sales_return` | `runtime_enabled_local_target_evidence_required`；批准 / 拒绝只改客户退货 Source，仓库显式 receive 才写退货库存与 RMA 待检；取消、收货冲正仍走领域命令。 |
...
| `process_runtime` | `runtime_enabled_local_target_evidence_required` | `sales_order_acceptance`、`material_supply`、`finished_goods_delivery`、`sales_return_acceptance`、`finance_payment_approval`、`inventory_adjustment_approval`、`production_exception_approval` |
...
| `sales_return_acceptance` | `runtime_enabled_local_target_evidence_required` | `approval`、`human_task`、`domain_command`、`end` |
...
| 出货 | sales / warehouse → quality → finance → warehouse | 财务 ProcessRuntime 审批、Shipment 版本化门禁和仓库正式出货分层；本地强制门禁已接，目标证据待补 | F16–F18；流程矩阵 4.3 |
| 财务 | shipment / purchase / outsourcing → finance；finance → boss | 来源财务和 Core 收付款 V1 已有；`finance_payment_approval` 本地已接，目标当前版本未核验 | F19–F20；流程矩阵第 6、9 章 |
...
| P2 | Product Core 与永绅跟踪配置已接真实收付款、多来源核销、冲正、红冲和 `finance_payment_approval`，但当前版本尚未发布到 133 / 生产，银行直连、支付凭证、总账和税控仍未实现 | 不能把本地激活和模拟账号验证写成目标客户已可用，也不能称完整会计系统 | 绑定固定 release / migration / active revision 做目标发布、财务真实账号 smoke 和 UAT；外部银行 / 会计能力另行评审 |
