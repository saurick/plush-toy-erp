...
| current PDF 样本 | Customer Material / Print Template Input | Product / Delivery | print, outsourcing, purchase, finance | 否，仅抽取字段线索 | Print Template Input / Customer Material | Medium | 是 | 合同、报表和打印版式可作为样本输入；不直接决定 schema、事实或模板内核。 |
