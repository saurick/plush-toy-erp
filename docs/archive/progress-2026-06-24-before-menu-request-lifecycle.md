...
## 2026-06-24 业务筛选区清空筛选按钮

- 完成：`BusinessOperationPanel` 新增可选“清空筛选”按钮入口，统一放在筛选控件队列内；未传 `onClearFilters` 的页面不显示按钮，已有页面可通过 `clearFiltersDisabled` 在无活跃筛选时禁用。
- 完成：主数据、销售订单、采购订单、采购入库、质检、出货、库存台账、运营事实、委外订单、BOM、Workflow 业务模块和 Formal 业务模块接入清空筛选；只清空关键词、状态、对象、日期、可见路由上下文等筛选条件，并按需回到第一页，不清排序、选中行、当前视图、菜单、RBAC 或后端事实状态。
- 完成：共享样式补齐按钮型筛选控件的桌面宽度和移动端 100% 换行规则，避免新增按钮挤压日期区间、搜索框或相邻工具栏。
- 验证：追加前 `progress.md` 为 376 行、71358 字节，未达到归档阈值；已执行 `pnpm --dir web css`、`pnpm --dir web test`、`pnpm --dir web lint`、`STYLE_L1_SCENARIOS=purchase-order-date-filter-desktop,shipment-date-filter-desktop,shipment-date-filter-mobile,business-module-dark-customers-desktop pnpm --dir web style:l1`，均通过；前端单测 400 个通过，targeted L1 覆盖采购 / 出货筛选桌面与移动、主数据暗色列表的筛选区布局。
- 下一步：后续新增 `BusinessOperationPanel` 筛选条件时，同步提供 `hasActiveFilters / clearFilters`；若页面存在 URL tag 型筛选上下文，应在清空筛选里一并清掉可见 tag。
- 阻塞/风险：本轮不改 schema、migration、API、RBAC、菜单、WorkflowUsecase、Fact usecase、客户配置、原型状态或部署脚本；完整 `pnpm --dir web style:l1` 当前被工作区内生产事实入口并行改动阻塞，失败点是旧场景仍等待“登记生产事实”按钮，但当前 `V1OperationalFactPage.jsx` 已把该入口隐藏并改为“从生产任务生成事实”，该失败不属于本轮清空筛选按钮改动。

## 2026-06-24 业务前端工程字段隐藏治理

- 完成：出货弹窗和 Operational Fact 表单不再向用户展示“幂等键”，改为隐藏字段随提交携带；库存流水、运营事实列表不再展示内部主键、幂等键、内部引用等工程字段，生产进度页取消无来源手填生产事实入口。
- 完成：新增 `userVisibleTechnicalFields.test.mjs` 并接入 `pnpm test`，静态扫描正式业务前端页面、组件和移动端，禁止可见文案继续出现幂等键、内部引用、内部主键、内部流水等技术字段；同步更新 code review / 页面设计 skill 和产品文档规则。
- 验证：追加前 `progress.md` 为 385 行、73380 字节，未达到归档阈值；已执行 `node --test web/src/erp/utils/userVisibleTechnicalFields.test.mjs`、目标文件 `eslint`、`git diff --check`、`pnpm --dir web test`、`STYLE_L1_PORT=4249 STYLE_L1_SCENARIOS=business-formal-module-shells-desktop pnpm --dir web style:l1`，均通过；前端单测 400 个通过，targeted L1 覆盖正式业务模块壳层、出货、库存、生产进度和相关弹窗负向断言。
- 下一步：如果后续要重新开放生产事实创建，必须从生产任务 / 工单 / 领料 / 入库等真实来源生成，并使用业务可读选择器，不允许恢复手填内部 ID 或幂等键的通用登记弹窗。
- 阻塞/风险：本轮不改 schema、migration、后端 usecase、RBAC、菜单、客户配置、原型状态或部署脚本；隐藏字段仍保留提交语义以维持后端幂等保护，当前未实现 source-driven 的生产事实新建闭环。

## 2026-06-24 产品档案主视图 Tab 统一

- 完成：全局扫描正式业务页主视图切换后，确认产品档案是唯一仍用 `Segmented` 承载页面级主表视图的正式业务页；登录入口、主题、dev-only 页面和审计风险筛选语义不同，本轮不改。
- 完成：产品档案从独立胶囊分段切换为表格卡片顶部 `Tabs`，与库存台账、运营事实等页面级主视图切换一致；保留“产品基础信息 / 产品规格”语义、默认基础信息视图、切换后重置筛选 / 选中 / 分页的既有行为。
- 完成：`style:l1` 产品档案场景改为通过 `role=tab` 断言和切换“产品规格”，继续覆盖新建产品、新建产品规格、列排序、无删除 / 回收站和无横向溢出。
- 验证：追加前 `progress.md` 为 393 行、75008 字节，未达到归档阈值；已执行 `pnpm --dir web lint`、`pnpm --dir web css`、`pnpm --dir web test`、`git diff --check`、`STYLE_L1_SCENARIOS=business-formal-module-shells-desktop pnpm --dir web style:l1`，均通过；前端单测 400 个通过，targeted L1 覆盖产品档案 tab 默认态、切换态、表格相邻区域和正式业务模块壳层。
- 下一步：后续新增同一左侧菜单下的主表视图切换，默认使用表格卡片顶部 `Tabs`；`Segmented` 仅保留给轻量模式、筛选 lens、登录入口、主题或 dev-only 快捷切换。
- 阻塞/风险：本轮不改 schema、migration、API、RBAC、菜单、WorkflowUsecase、Fact usecase、客户配置、正式产品文档口径、原型状态或部署脚本；未跑完整 `style:l1`，已用覆盖产品档案的正式业务模块目标场景回归。

## 2026-06-24 产品档案 Tab 背景与动效回归修正

- 完成：修正产品档案主视图 tab 与库存台账不一致的问题，移除产品档案 `Tabs` 上的 `erp-business-view-tabs` 特殊 class，不再给 active tab 叠加浅绿背景和阴影。
- 完成：全局取消 `.erp-business-view-tabs .ant-tabs-tab-active` 的背景 / 阴影规则，只保留权限中心 `erp-permission-tabs` 的卡片化激活态；业务主视图 tab 回到库存台账同款 AntD 下划线样式。
- 完成：`style:l1` 产品档案场景新增 computed style 负向断言，要求“产品基础信息 / 产品规格”active tab 不出现激活背景色和阴影，防止后续再套回特殊背景动效。
- 验证：追加前 `progress.md` 为 402 行、76700 字节，未达到归档阈值；已执行 `pnpm --dir web css`、`git diff --check`，均通过；`STYLE_L1_SCENARIOS=business-formal-module-shells-desktop pnpm --dir web style:l1` 已跑过产品档案并通过新增 tab 背景 / 阴影断言，后续在 Workflow 页等待“发起排程协同”时失败；`pnpm --dir web test` 为 399/400，通过项包含本轮相关测试，失败项为 `businessModuleNavigation.test.mjs` 对 `WorkflowBusinessModulePage` 的并行改动断言；`pnpm --dir web lint` 被并行改动 `OperationalFactForms.jsx` 未使用 `CURRENCY_OPTIONS` 阻塞。
- 下一步：如果继续统一其他业务主视图 tab，按库存台账的无背景下划线样式执行；需要先隔离或收口当前 Workflow / Operational Fact 并行改动后再跑完整 lint、完整 test 和完整 style:l1。
- 阻塞/风险：本轮不改 schema、migration、API、RBAC、菜单、WorkflowUsecase、Fact usecase、客户配置、正式产品文档口径、原型状态或部署脚本；当前未获得完整 L1 绿灯，原因是工作区其他页面并行改动，不是产品档案 tab 背景修正本身。

## 2026-06-24 业务前端技术选择器与裸编号清理

- 完成：运营事实页面移除无来源创建 / 维护表单，Workflow V1 三页移除无来源“发起协同”入口，只保留任务读取、处理、催办和附件；出货、销售订单、采购订单、采购入库、质检、库存台账、审计日志和通用引用选项不再用 `#数字`、主键、内部引用、幂等键或“来源选择器”文案作为用户可见兜底。
- 完成：`userVisibleTechnicalFields.test.mjs` 扩展到通用引用选项，新增裸编号、ID 控件和“来源选择器”文案守卫；`businessModuleNavigation.test.mjs` 和 `style:l1` 同步改为断言 Workflow 无来源创建入口不存在；`plush-page-design-governance` 和产品文档补充“业务对象控件只显示名称 / 单号 / 行号 / 状态 / 已关联，内部 ID 只能隐藏携带”的规则。
- 验证：追加前 `progress.md` 为 411 行、78602 字节，未达到归档阈值；已执行运行态文案 `rg` 负向扫描、`node --test src/erp/utils/businessModuleNavigation.test.mjs src/erp/utils/userVisibleTechnicalFields.test.mjs`、`pnpm --dir web test`、`pnpm --dir web css`、`pnpm --dir web lint`、目标文件 ESLint、`STYLE_L1_PORT=4250 STYLE_L1_SCENARIOS=business-formal-module-shells-desktop pnpm --dir web style:l1`、`git diff --check`，均通过；前端单测 400 个通过，targeted L1 覆盖正式业务壳层、出货来源导入、生产事实入口隐藏和 Workflow 无来源创建入口隐藏。
- 下一步：后续若要恢复生产事实或 Workflow 创建能力，必须从生产任务、来源单据、事实行或后端规则生成，并在页面上展示业务可读来源、余额、状态和不可生成原因；不得恢复手填 `source_id`、`source_line_id`、幂等键、裸主键或 `#数字` 兜底。
- 阻塞/风险：本轮不改 schema、migration、后端 usecase、RBAC、菜单、客户配置、原型状态或部署脚本；原型和内部设计文档仍可使用“来源选择器”作为模式名称，但正式运行页面和业务用户可见文案已禁止该技术口吻。

## 2026-06-24 产品档案 Tab 切换刷新真源修复

- 完成：撤回产品档案页内 record 缓存、idle 预取和字典跳过请求方案；保留不影响业务数据新鲜度的优化：tab 激活与表格内容更新拆分、产品档案表格固定布局。
- 完成：主数据页面顶部“刷新当前页”改为重新请求当前列表、单位字典和 SKU 产品引用；全局菜单点击当前路径时不再静默返回，而是调用当前页刷新 handler，保证重复点击当前菜单也会重新读后端。
- 完成：`style:l1` 正式业务壳层场景新增请求断言，锁住产品档案切换“产品规格”必须请求 `list_product_skus / list_products`、顶部刷新必须重新请求当前 SKU 列表和产品引用、点击“材料档案”菜单必须请求 `list_materials / list_units`。
