...
Status / 状态: Phase 8 Internal Simulated Closure Passed / Phase 8 内部模拟事实闭环通过
...
Phase 8 已发布到当前目标环境，Atlas migration 已执行到最新版本，服务端、前端、只读 Phase 8 路由 smoke、目标环境试用账号 RBAC 核对、登录态 Phase 8 只读 JSON-RPC smoke 和内部受控模拟事实写入闭环已通过。

客户使用确认属于交付后的业务确认，不作为 Phase 8 完成阻塞。本次 evidence 不等于客户已签收，也不代表真实客户数据导入、完整打印、报表、发票明细、收付款核销、对账单、物流退货、并发锁升级或自动派生已经交付。
...
| Closure Server Image | `plush-toy-erp-server:20260608T2345-phase8-closure-amd64` |
...
| Simulated MasterData | `SIM-YOYOOSUN-OPFACT-PCS` unit_id `1`；`SIM-YOYOOSUN-OPFACT-PRODUCT` product_id `1`；`SIM-YOYOOSUN-OPFACT-WH` warehouse_id `1` |
...
11. 发现并修复无批次库存余额扣减路径中的 PostgreSQL placeholder 间隙问题，服务端更新到 `plush-toy-erp-server:20260608T2345-phase8-closure-amd64`。
12. 使用显式模拟主数据和 `scripts/qa/phase8-simulated-fact-closure.mjs` 执行 Phase 8 内部受控模拟事实写入闭环。
...
| closure app-server image | `plush-toy-erp-server:20260608T2345-phase8-closure-amd64` |
...
| Phase 8 simulated fact closure | OK，`TARGET-20260608-CLOSURE-V2` 覆盖生产 create/post/cancel、预留 release/consume、委外 create/post/cancel、出货 create/add/ship/cancel、财务 post/settle/cancel |
| simulated fact DB check | OK，生产事实 `id=1` 和 `id=2` 均 `CANCELLED`；委外 V2 `CANCELLED`；出货 V2 `CANCELLED`；财务 V2 `SETTLED` / `CANCELLED`；库存流水包含 production / outsourcing / shipment 正反流水 |
...
| 项目 | 原因 / 结论 |
| --- | --- |
| 客户使用确认 | 属于交付后的业务确认，不作为 Phase 8 完成阻塞 |
| 真实客户数据导入 | 当前不可执行；本次只使用 `SIM-YOYOOSUN-OPFACT` 前缀的模拟主数据和模拟事实 |
| pre-migration 备份 evidence | 本次执行前未找到并记录明确 pre-migration 备份 evidence；已补 post-deploy 逻辑备份作为当前状态恢复点 |
| 旧失败残留 DRAFT | 首次闭环失败前创建的 `outsourcing_facts.id=1` 仍为 `DRAFT`，无库存流水影响；旧已过账生产事实已用修复后服务冲正为 `CANCELLED` |
| 镜像清理 | 已执行 `docker builder prune -f`；为保留上一版回滚镜像且本次可回收空间很小，未执行 `docker image prune -a -f` |

## 6. 结论状态 / Status

| 对象 | 状态 |
| --- | --- |
| CAP-019 到 CAP-024 | Phase 8 Internal Simulated Closure Passed |
| CAP-026 私有化部署包 | Target Release Executed / Evidence Captured |
| CAP-027 客户使用确认 | Post-delivery Business Confirmation |

禁止把本 evidence 写成客户已签收、真实客户数据导入完成或完整业务交付。Phase 8 本身按内部模拟事实闭环关闭；后续客户使用确认、打印、报表、核销、物流退货、自动派生和并发锁升级都应作为后续交付确认或增强任务单独评审。
