...
- 完成：把全页面数据覆盖从“29 个桌面页 / 50 个验收目标”收口为当前正式菜单的 31 个桌面页 / 52 个验收目标，不再把“客户退货”和“收付款与核销”排除在长期业务场景数据之外。固定数据矩阵新增 4 条客户退货（草稿、已批准、已收货、已冲回）、4 条收付款（待过账、已过账收款、已过账付款、已冲销）和 3 条红冲记录（生效、原记录、反向红冲），继续通过正式 Source Document、ProcessRuntime、领域命令和权威查询生成及核对，不在前端补造业务事实。
...
- 阻塞 / 风险：BOM 与采购需求读模型仍缺 SKU grain、需求快照真源、在途 / 退货计算合同；库存预留已有权威事实和消费路径，但专项读模型及单条部分消费仍缺正式合同。跨 Source Document / Workflow / Fact 的完整业务链、跨审批节点完整审批链、通用状态历史 / lineage / 通知链、任务事件游标、对象存储、附件普通删除 / 作废、流程合同 `writes_fact` v2 迁移、旧责任池退役、前三条流程的补偿 / 恢复、RMA 退款换货补发和真实客户数据写入均保持 partial、blocked 或 deferred，不用页面聚合、fixture 或新规则冒充闭环。
