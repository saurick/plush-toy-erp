...
## 2026-06-26 输入控件焦点环完整显示收口

- 完成：将业务表单、业务筛选、弹窗、登录卡片和暗色主题里的输入控件 focus ring 从外扩 `box-shadow` 收口为控件内部 inset 绘制，避免被 `overflow:hidden`、圆角容器或滚动区域裁掉；补齐 `.erp-business-form`、`.erp-business-action-form`、`.erp-business-filter-control`、全局 AntD affix wrapper 和 Select `focus-within` 规则。
- 完成：补齐业务表单 `Form.Item`、control-content、AntD Input / Select / DatePicker / Number 控件的 `min-width:0 / width:100% / max-width:100% / box-sizing:border-box` 约束，解决“联系电话”等控件右边框被父容器裁掉或撑出可见区域的问题；提高业务弹窗 Select / AutoComplete focus/open/focus-within 规则特异性，避免 AntD 外扩 focus shadow 回流。
- 完成：撤回内层 input 固定 caret line-box 方案，不再给联系人 AutoComplete、邮箱 / 电话 allowClear 输入框强制 22px 高度或行高，避免破坏 AntD 原生垂直居中和控件内部布局。
- 完成：`style:l1` 新增可见输入控件焦点扫描，按真实 focus/focus-within 采样 input、textarea、数字框、日期框和 Select 搜索输入，断言焦点环在控件内部完整绘制、控件 box 完整落在 Form.Item 内容盒内，并按浅色 / 暗色主题分别校验绿色 / 蓝色主题边框；同时修复联系人列表和销售订单明细列表的横向滚动回归污染，采样后恢复默认 `scrollLeft`，避免截图停留在非默认滚动状态；补齐暗色登录卡片输入框 focus 覆盖，避免仍使用浅色登录卡片 focus 色。
- 验证：追加前 `progress.md` 为 357 行、68691 字节，补修前复查 `progress.md` 为 365 行、70334 字节，caret 补修前复查为 366 行、70960 字节，最终复查为 382 行、74493 字节，均未达到归档阈值；`corepack pnpm css`、`corepack pnpm test`、`corepack pnpm exec eslint --no-warn-ignored scripts/styleL1.mjs scripts/style-l1/lineItemUnitAssertions.mjs`、`STYLE_L1_SCENARIOS=business-formal-module-shells-desktop corepack pnpm style:l1`、`STYLE_L1_SCENARIOS=business-module-dark-customers-desktop corepack pnpm style:l1`、`STYLE_L1_SCENARIOS=textarea-show-count-layout-desktop corepack pnpm style:l1`、`corepack pnpm style:l1`、`git diff --check` 均通过；完整 L1 验证 66 个场景，前端单测 409 条通过。
- 下一步：后续若新增第三方输入组件或自定义可编辑控件，应复用当前焦点变量和 inset ring 规则，并让 L1 扫描覆盖真实 focus 路径；不要再通过固定 inner input line-height 修复 caret。
- 阻塞/风险：本轮不改 schema、migration、JSON-RPC、RBAC、菜单真源、WorkflowUsecase、Fact usecase、客户配置、原型状态或部署脚本；当前 shell 的默认 `pnpm` 为 11.7.0，与 `web/package.json` 要求的 10.13.x 不一致，验证命令统一用 `corepack pnpm` 运行。

## 2026-06-26 业务模块原型关联入口文案收口

- 完成：将 `business-module-page-standard-v1` 标准业务模块原型当前操作条里的“关联表格”改为“关联单据”，与 plush 已有业务详情、移动端任务详情和标准页面语义保持一致；仅调整原型可见文案，不改正式菜单、路由、RBAC、Workflow / Fact、schema、JSON-RPC 或客户配置。
- 验证：追加前 `progress.md` 为 367 行、71317 字节，未达到归档阈值；扫描活跃文件确认排除历史归档和过程流水后不再存在“关联表格”文案。
- 下一步：如后续把该原型同步到正式运行时页面，继续使用“关联单据”作为上下游业务对象入口文案。
- 阻塞/风险：本轮只改原型 HTML 文案；未执行浏览器 L1 或单测，剩余验证盲区仅限原型静态展示截图未重新生成。

## 2026-06-26 相关单据入口文案收口

- 完成：将销售订单、采购订单、采购入库、质检、库存流水和业务事实页选中记录后的跨模块跳转下拉统一从“关联 / 查看关联”改为“相关单据”，强调这是查看和跳转到已有业务单据，不暗示新建绑定关系或写入事实。
- 完成：同步 `business-module-page-standard-v1` 标准业务模块原型示例按钮为“相关单据”，并把加工合同页的 L1 反向断言更新为同时拦截“关联 / 相关单据 / 查看关联”类跨模块下拉，避免无事实闭环的页面误加入口。
- 验证：追加前 `progress.md` 为 374 行、72348 字节，未达到归档阈值；`corepack pnpm exec eslint --no-warn-ignored src/erp/pages/V1SalesOrdersPage.jsx src/erp/components/purchase-orders/PurchaseOrderOperationPanel.jsx src/erp/pages/V1PurchaseReceiptsPage.jsx src/erp/pages/V1QualityInspectionsPage.jsx src/erp/pages/OperationalFactsPage.jsx src/erp/pages/V1InventoryLedgerPage.jsx scripts/style-l1/businessFormalScenarios.mjs`、`corepack pnpm css`、`corepack pnpm test`、`STYLE_L1_PORT=4174 STYLE_L1_SCENARIOS=business-formal-module-shells-desktop corepack pnpm style:l1`、限定 `git diff --check` 均通过；前端单测 409 条通过，定向 L1 覆盖 1 个正式业务页面壳场景。首次 L1 使用默认 4173 端口时因端口短暂占用启动失败，改用 4174 后通过。
- 下一步：若后续新增同类按钮，默认使用“相关单据”；只有真实执行绑定写入、且后端 usecase / RBAC / 审计闭环完成时才使用“关联单据”。
- 阻塞/风险：本轮不改 schema、migration、JSON-RPC、RBAC、菜单真源、WorkflowUsecase、Fact usecase、客户配置、部署脚本或原型状态；未运行 `pnpm lint`，因为该脚本会 `--fix` 整个 `src/`，当前工作区已有非本轮改动，已用定向 no-fix ESLint 替代。

## 2026-06-26 导出当前筛选文案收口

- 完成：将正式业务列表、主数据、BOM、采购订单操作区、加工合同、质检、销售订单和共享 `BusinessListToolbarActions` 的导出按钮文案从“导出当前结果”统一改为“导出当前筛选”；同步 L1 断言、导航测试、业务模块能力说明和相关产品 / 原型文档口径。
- 验证：追加前 `progress.md` 为 382 行、74493 字节，未达到归档阈值；扫描活跃代码、脚本和产品文档确认不再存在旧“导出当前结果”文案；`corepack pnpm exec eslint --no-warn-ignored ...`、`corepack pnpm exec node --test src/erp/utils/businessModuleNavigation.test.mjs`、`STYLE_L1_PORT=4175 STYLE_L1_SCENARIOS=business-formal-module-shells-desktop corepack pnpm style:l1`、限定 `git diff --check` 均通过；定向单测 6 条通过，定向 L1 覆盖 1 个正式业务页面壳场景。
- 下一步：后续新增列表导出入口时默认使用“导出当前筛选”，避免把筛选后的导出误读为全量结果或选中行导出。
- 阻塞/风险：本轮不改 schema、migration、JSON-RPC、RBAC、菜单真源、WorkflowUsecase、Fact usecase、客户配置、部署脚本或归档历史；未运行完整 `pnpm lint`、`pnpm test` 或完整 `style:l1`，因为本轮为窄文案改动，已用定向 ESLint、单测和 L1 覆盖按钮可见文案与可访问名称。

## 2026-06-26 输入控件垂直节奏全局修复

- 完成：按运行时 computed style 定位输入框污染源：业务表单和弹窗里 AntD affix wrapper 实际 `align-items: normal`，Select placeholder 被 AntD 默认 `30px` line-height 覆盖，导致联系人 / 联系电话一类控件的 placeholder 和 caret 垂直节奏不稳定；修复为业务表单、业务弹窗和筛选控件的单行输入统一由控件内高接管，不再固定 inner input 高度。
- 完成：在 `control-radius.css` 里补齐 affix wrapper `align-items:center`、inner input `height:auto / padding-block:0 / line-height:1.5715`、Select item / placeholder / search input 使用 `control-height - 2px` line-height；保留业务弹窗明细行的宽度边界，不扩大 `min-width:0` 到会压缩订单行窄列的层级。
- 完成：`style:l1` 新增单行输入文本垂直节奏断言，覆盖业务表单、业务弹窗、业务筛选和登录卡片，检查 affix wrapper 不再 stretch、inner input 居中且 line-height 不超出自身高度、Select placeholder/search line-height 跟控件内高一致。
- 验证：追加前 `progress.md` 为 389 行、75915 字节，未达到归档阈值；用 Playwright 复现销售订单新建表单，确认联系人 Select placeholder / search input 为 34px line-height，联系电话 affix wrapper 为 `align-items:center`，inner input 为约 22px line-height 且居中；`corepack pnpm css`、`corepack pnpm test`、`corepack pnpm exec eslint --no-warn-ignored scripts/styleL1.mjs scripts/style-l1/lineItemUnitAssertions.mjs`、`STYLE_L1_SCENARIOS=business-formal-module-shells-desktop corepack pnpm style:l1`、`STYLE_L1_SCENARIOS=business-module-dark-customers-desktop corepack pnpm style:l1`、`corepack pnpm style:l1`、`git diff --check` 均通过；完整 L1 覆盖 66 个场景，前端单测 409 条通过。
- 下一步：后续新增第三方输入或自定义单行控件时，必须让 placeholder、search input 和真实 input 的 line-height 进入 L1 垂直节奏扫描；不要再靠固定 inner input height 修 caret。
- 阻塞/风险：本轮不改 schema、migration、JSON-RPC、RBAC、菜单真源、WorkflowUsecase、Fact usecase、客户配置、部署脚本或原型状态；当前工作区已有多组非本轮文案 / 原型改动，本轮未回退、未提交。

## 2026-06-26 清空已选按钮显隐与 hover 边框收口

- 完成：来源导入弹窗的“清空已选”只在已选来源行存在时渲染，未选择时只保留“未选择来源”摘要，不再展示禁用清空按钮；清空后按钮从弹窗中移除并恢复导入按钮禁用态。
- 完成：业务选中操作区和来源导入弹窗里的清空类 link 按钮默认透明边框 / 无阴影，hover 或键盘 focus-visible 时通过 inset 边线显示边框，避免默认态出现截图里的常驻描边；同时在空选择态隐藏选中操作区内的 link 清空按钮。
- 完成：`style:l1` 的来源导入回归补充默认无清空按钮、选中后出现、默认无边框、hover 显示边框、清空后按钮移除的断言；采购订单场景验证时顺手补齐 compact input 的既有垂直节奏样式，保证该场景能继续执行到来源导入回归。
- 验证：追加前 `progress.md` 为 398 行、78264 字节，未达到归档阈值；`corepack pnpm exec eslint --no-warn-ignored src/erp/components/business-list/SourceImportPickerModal.jsx scripts/styleL1.mjs`、`corepack pnpm css`、`corepack pnpm test`、`STYLE_L1_PORT=4179 STYLE_L1_SCENARIOS=business-formal-module-shells-desktop corepack pnpm style:l1`、`STYLE_L1_PORT=4181 STYLE_L1_SCENARIOS=purchase-order-date-filter-desktop corepack pnpm style:l1`、`STYLE_L1_PORT=4182 STYLE_L1_SCENARIOS=business-module-dark-customers-desktop corepack pnpm style:l1`、`git diff --check` 均通过；前端单测 409 条通过。
- 下一步：后续新增来源选择器或选中行操作区时，继续复用当前“有选中才显示清空、默认无边框、hover/focus 才显边线”的共享样式，不要在单页重复写禁用清空按钮。
- 阻塞/风险：本轮不改 schema、migration、JSON-RPC、RBAC、菜单真源、WorkflowUsecase、Fact usecase、客户配置、部署脚本或原型状态；当前工作区已有多组非本轮原型、文案和输入控件改动，本轮未回退、未提交。
