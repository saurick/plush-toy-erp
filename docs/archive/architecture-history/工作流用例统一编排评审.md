# 工作流 Usecase 统一编排历史评审 / Historical Workflow Usecase Review

> 历史边界：本文记录七条局部 Workflow rule 迁移期间的评审过程，文中的前端编排、公开 `upsert_business_state`、非终态 `rejected`、应用层下游任务去重和“尚无流程运行时”等口径已经过期。当前边界请读 `docs/architecture/状态工作流事实边界.md`、`docs/workflow/业务与协同流程地图.md` 和 `docs/product/多甲方角色能力与流程编排.md`，当前行为以代码与测试为准。
...
第三条仓库入库专项评审和落地边界见：`/Users/simon/projects/plush-toy-erp/docs/architecture/仓库入库工作流用例评审.md`。

### 1.1 后端规则执行时序图 / Backend Rule Execution Sequence

```mermaid
sequenceDiagram
  autonumber
  participant UI as 岗位端 / 桌面端
  participant API as JSON-RPC update_task_status
  participant RBAC as RBAC / 角色边界
  participant UC as WorkflowUsecase
  participant Repo as WorkflowRepo transaction
  participant Task as workflow_tasks
  participant Event as workflow_task_events
  participant Biz as workflow_business_states
  participant Downstream as 下游 workflow_tasks

  UI->>API: 提交 task_id、target_status、reason、actor
  API->>RBAC: 校验权限码、owner_role_key、assignee_id、任务当前状态
  RBAC-->>API: 允许本次动作
  API->>UC: UpdateTaskStatus
  UC->>UC: 校验 task_status_key 与 blocked / rejected reason
  UC->>UC: 按 source_type、task_group、owner_role_key、payload 识别特殊规则
  UC->>Repo: 在事务内写任务状态、事件、业务状态和可选下游任务
  Repo->>Task: 更新当前任务状态与时间字段
  Repo->>Event: 追加 status_changed 事件
  alt 命中特殊 workflow rule
    Repo->>Biz: upsert 协同业务状态
    opt 规则允许派生下游任务
      Repo->>Downstream: 幂等创建或复用下游任务
      Repo->>Event: 追加下游任务 created 事件
    end
  else 未命中特殊 workflow rule
    Repo-->>UC: 只保存当前任务状态与事件
  end
  Repo-->>UC: 提交事务
  UC-->>API: 返回任务、业务状态和下游任务结果
  API-->>UI: 刷新任务列表 / 业务页面
```

上图是当前稳定主路径：前端只提交动作，后端统一做权限、状态、原因、特殊规则识别和事务写入。Workflow 规则可以推进协同业务状态或派生下游任务，但不能在这条链路里直接写库存、出货、应收、开票、付款等事实。
