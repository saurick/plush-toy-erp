# 出货放行 Workflow Usecase 历史评审 / Historical Shipment Release Workflow Review

> 历史边界：本文记录 `shipment_release` 最小后端规则落地时的评审过程，其中“未来 ShipmentUsecase”“尚未实现出货事实”等结论已经过期。当前边界请读 `docs/architecture/状态工作流事实边界.md`，当前实现请核对 `OperationalFactUsecase`、`operational_fact` JSON-RPC、Ent schema 与测试。
...
| `server/internal/service/jsonrpc_workflow.go` | `update_task_status` 继续走动作级权限、owner / assignee / status 校验。warehouse 可处理未指派的 warehouse `shipment_release`，非 warehouse 不能越权，super_admin 可处理未指派任务。 | 不新增 API，也不伪造旧前端 follow-up。 |
