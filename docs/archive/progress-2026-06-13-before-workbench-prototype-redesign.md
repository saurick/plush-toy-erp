...
## 2026-06-13 19:11 CST

- 完成：处理后台进入时出现“同步管理员权限失败，请稍后重试”的会话兜底问题。前端错误码消费层新增 `ADMIN_SESSION_UNAVAILABLE_ERROR_CODES` 和 `isAdminSessionUnavailableCode`，只把 `AdminDisabled / AdminNotFound` 识别为“当前后台 profile 同步时管理员会话不可继续”。
- 完成：`ERPLayout` 在 `admin.me` 返回当前管理员不存在或已禁用时，清理 admin 登录态、清空当前 profile，并通过统一 `authBus` 重新登录流程跳回 `/admin-login`；`PermissionDenied`、普通业务错误和现有 `AuthRequired / AuthExpired / AuthInvalid` 仍保持原语义，不扩大自动登出范围。
- 验证：本机后端 `/healthz=ok`、`/readyz=ready`；`TRIAL_ACCOUNT_PASSWORD=12345678 TRIAL_ACCOUNT_BACKEND_URL=http://127.0.0.1:8300 node scripts/qa/trial-account-rbac.mjs` 通过 9 个 demo 账号 `admin_login + me`；`node --test web/src/common/consts/errorCodes.test.mjs` 通过；`pnpm --dir web test` 通过，前端 305 项通过；`pnpm --dir web lint` 通过；`pnpm --dir web build:desktop` 通过。
- 下一步：若目标环境仍看到该提示，应优先确认浏览器是否持有旧 admin token、目标账号是否被禁用或已删除；正常情况下现在会清理旧登录态并要求重新登录，而不是停留在同步失败提示。
- 阻塞/风险：本轮不改后端 RBAC 真源、错误码码位、schema / migration、角色矩阵、菜单权限或生产数据；当前工作区仍有非本轮原型文件改动，本轮未回退、整理、提交或推送这些无关现场。

## 2026-06-13 19:18 CST

- 完成：根据用户截图继续收口“同步管理员权限失败，请稍后重试”重复 toast。将 `HTTPUnauthorized / AdminRequired / AuthCurrentUserFailed` 也纳入后台 profile 同步时的 admin 会话不可用判断，避免旧 admin token、非 admin token 或当前用户加载失败时继续按普通业务错误弹 toast。
- 完成：新增 `adminProfileSync` 纯 helper。`ERPLayout` 现在区分四类处理：会话不可用只触发一次重新登录；已有本地 profile 时后台同步失败只保留缓存并写 console warning，不再打扰用户；没有缓存时普通失败最多提示一次；真正 `AuthRequired / AuthExpired / AuthInvalid` 仍走既有全局鉴权处理。
- 验证：`node --test web/src/common/consts/errorCodes.test.mjs web/src/erp/utils/adminProfileSync.test.mjs` 通过；`pnpm --dir web test` 通过，前端 308 项通过；`pnpm --dir web lint` 通过；`pnpm --dir web build:desktop` 通过；in-app Browser 刷新 `http://localhost:5175/erp/dashboard` 后页面非空、无 Vite overlay、无同步权限 toast、console error/warn 为空；`git diff --check` 通过。
- 下一步：若用户浏览器仍显示旧 toast，应强制刷新当前前端页面或确认访问的是否为已重新构建 / 已部署的新 bundle；目标环境需要发布新前端镜像后才会生效。
- 阻塞/风险：本轮不改后端 RBAC 真源、错误码码位、schema / migration、角色矩阵、菜单权限或生产数据；已有缓存 profile 时同步失败会继续显示当前缓存菜单，后续具体接口仍由后端权限守卫兜底。当前工作区仍有非本轮原型文件改动，本轮未回退、整理、提交或推送这些无关现场。

## 2026-06-13 20:02 CST

- 完成：进入 `trade-erp` 真实出运明细页截图参考弹窗，保存新建/编辑大表单、来源导入、回收站、列顺序和删除确认截图到 `output/prototype-modal-reference-20260613/`；据此重做 `action-modal-drawer-standard-v1/index.html` 为可切换的业务弹窗标准样板，并同步 README 资产说明。
- 验证：内联脚本语法检查通过；`git diff --check -- docs/product/prototypes/action-modal-drawer-standard-v1/index.html docs/product/prototypes/README.md` 通过；浏览器验证桌面 5 个弹窗状态均可切换且无页面级横向溢出，移动 390px 下大表单/来源导入/回收站/列顺序/删除确认均无页面级横向溢出，宽表格仅在弹窗内部横向滚动。
- 下一步：等待用户确认这套弹窗样板是否继续吸收到真实页面或共享组件；未获确认前继续保持 To Implement，不改 Current。
- 阻塞/风险：本轮只改原型与原型 README，未改正式运行时页面、字段真源、来源导入门禁或回收站/删除 usecase。

## 2026-06-13 20:15 CST

- 完成：按用户截图补齐 `business-module-page-standard-v1/index.html` 表头级列顺序和排序入口；除选择列外，每个业务列都增加三点列设置按钮和上下排序箭头，并同步 README 说明。
- 验证：内联脚本语法检查通过；`git diff --check -- docs/product/prototypes/business-module-page-standard-v1/index.html docs/product/prototypes/README.md` 通过；浏览器验证桌面 8 个业务列表头均有列菜单和排序入口且表头无内部溢出，移动 390px 下页面无横向溢出，宽表格仅在表格容器内横向滚动。
- 下一步：如进入真实页面实现，应把表头菜单接到当前管理员列顺序偏好，把排序接到真实查询参数或前端当前结果排序，不能只保留静态图标。
- 阻塞/风险：本轮只改 To Implement 原型，不改正式运行时表格组件、API 排序参数、账号列顺序持久化或后端查询。

## 2026-06-13 20:24 CST

- 完成：补齐业务模块标准页里的回收站空态弹窗，工具条“回收站”现在可打开弹窗，包含批量恢复、刷新、已选数、表头和“回收站暂无记录”空态；README 已同步。
- 验证：内联脚本检查和 `git diff --check` 通过；浏览器验证桌面可打开/关闭且无横向溢出，移动 390px 下页面无横向溢出，宽表格仅在弹窗内部滚动。
- 阻塞/风险：仅为 To Implement 原型，不改真实回收站 API、恢复/彻底删除规则或权限。

## 2026-06-13 20:32 CST

- 完成：将弹窗样板明细区从 `trade-erp` 产品条目改为本项目通用“明细行（产品 / 物料）”，来源导入也改为来源单据口径。
- 验证：脚本检查、`git diff --check`、桌面和 390px 移动端浏览器布局验证通过；未改运行时代码。
