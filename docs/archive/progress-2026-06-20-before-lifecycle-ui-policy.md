...
## 2026-06-20 生产页标准业务页骨架收口

- 完成：按 `plush-page-design-governance` 将 `生产排程`、`生产异常` 从独立统计条 / 筛选条收口到标准 `BusinessOperationPanel`，补齐当前操作区、单选主表、完成 / 阻塞 / 催办入口和原因弹窗；页面仍只调用 Workflow usecase。
- 完成：`生产进度` 保持 Operational Fact 事实页语义，移除普通业务列表中的内部主键列，并把对象、仓库、批次、单位、销售订单、往来方等裸 ID 展示改为“内部引用”口径，不伪造缺失的名称快照。
- 完成：生产协同登记弹窗改用业务动作弹窗标题、共享表单网格和 `DateInput`；同步更新业务模块标准页原型 README 的已吸收范围，但不把 To Implement 资产晋级 Current。
- 验证：追加前 `progress.md` 为 307 行、66533 字节，未达到归档阈值；`pnpm --dir web lint` 通过；`pnpm --dir web css` 通过；`pnpm --dir web test` 通过，364 项；`STYLE_L1_PORT=4320 STYLE_L1_SCENARIOS=business-formal-module-shells-desktop pnpm --dir web style:l1` 通过，1 个组合场景；`git diff --check` 通过。
- 下一步：如后续要把生产事实页的内部引用替换为产品 / 仓库 / 单位名称，需要后端 read model 或 snapshot 字段配合，不能在前端伪造。
- 阻塞/风险：本轮未改 schema、migration、RBAC、菜单、WorkflowUsecase、OperationalFact usecase、客户配置、部署或正式文档清单；生产事实页当前 API 仍只返回部分内部引用 ID，真实业务名称展示需后续由后端 read model / snapshot 字段单独评审。

## 2026-06-20 出库页视图 Tabs 位置对齐

- 完成：复查 `出库管理` 与 `库存台账` 的运行时结构，确认位置差异来自 `OperationalFactWorkspace` 把视图 tabs 放在筛选 / 当前操作卡片之前，而库存台账把 tabs 放在表格卡片顶部；已为共享 `BusinessDataTable` 增加可选 `tableHeader` 插槽，并把出库页 `出货出库 / 库存预留` tabs 移入表格卡片顶部。
- 完成：补充 `business-formal-module-shells-desktop` L1 断言，校验出库页 tabs 归属 `.erp-business-module-table-card`，且位于筛选 / 当前操作卡之后、表格之前；原型资产只读取和遵循，未晋级 Current。
- 验证：追加前 `progress.md` 为 316 行、67982 字节，未达到归档阈值；`pnpm --dir web exec eslint --ext .js --ext .jsx src/erp/components/business-list/BusinessListLayout.jsx src/erp/pages/OperationalFactsPage.jsx scripts/style-l1/businessFormalScenarios.mjs` 通过；`pnpm --dir web css` 通过；`STYLE_L1_PORT=4321 STYLE_L1_SCENARIOS=business-formal-module-shells-desktop pnpm --dir web style:l1` 通过，1 个场景；`pnpm --dir web test` 通过，364 项；`git diff --check` 通过。
- 下一步：若后续希望所有多视图业务页都强制同一位置，可把 `tableHeader` 使用规则沉淀到更明确的共享页面 API 或文档，但不在本轮扩大。
- 阻塞/风险：本轮未改 schema、migration、RBAC、菜单、WorkflowUsecase、OperationalFact usecase、客户配置、部署、正式文档清单或原型状态；当前工作树仍有并行未提交改动，后续提交需按路径精确区分。

## 2026-06-20 全页面真实读取错误修复

- 完成：全面检查桌面后台 32 个正式路由，定位“服务器内部错误”主因不是前端 mock 层，而是本地开发库 Atlas migration 停在 `20260617124401`，代码已查询 `finance_facts / customers / sales_orders` 新增字段；已按项目主路径执行 `cd server && make migrate_apply`，dev DB 升至 `20260619155648` 且 pending 0。
- 完成：修复审计日志页空筛选参数问题，新增 `buildAuditLogParams` 统一构造 `audit_logs` JSON-RPC payload，空 `source / event_key / keyword / created_from / created_to` 不再发送，避免后端把空时间字段判为参数不合法；新增单测并接入 `pnpm test`。
- 验证：追加前 `progress.md` 为 307 行、66533 字节，未达到归档阈值；`cd server && make migrate_status` 显示 pending 0；直接调用 `business.dashboard_stats`、`masterdata.list_customers`、`sales_order.list_sales_orders`、`operational_fact.list_finance_facts` 均返回 `code=0`；真实登录全页面只读回归 32/32 通过；`pnpm --dir web lint`、`pnpm --dir web css`、`pnpm --dir web test` 通过，364 项；`pnpm --dir web style:l1` 通过，67 个场景。
- 下一步：目标环境或其他开发库若仍出现同类客户 / 销售订单 / 财务页面内部错误，先执行 `make migrate_status` 确认是否同样 pending，再决定是否 apply migration。
- 阻塞/风险：本轮未改 schema、migration 文件、RBAC、菜单、WorkflowUsecase、OperationalFact usecase、客户配置或部署脚本；只修审计日志前端参数构造。当前工作区另有生产页标准业务页骨架相关未提交改动，非本轮页面错误修复成果，后续提交需按路径精确区分。

## 2026-06-20 表头列顺序展示列移动修复

- 完成：全局排查正式业务主表列顺序逻辑，确认移动顺序属于管理员偏好和扫读效率，不影响后端业务事实；操作列、选择列、展开列、行内编辑器和非主业务列表仍可作为例外，不机械强制所有表格可移动。
- 完成：修复共享列顺序 key 解析：无 `dataIndex/key`、只通过 `sortValue/render/exportValue` 展示的业务列，表头快捷菜单和列顺序面板按原始列对象位置解析 fallback key，不再出现“左移 / 右移 / 移到最前 / 移到最后”全部灰掉的误锁定状态。
- 完成：扩展 `moduleTableColumns` 单测覆盖无 key 展示列移动；扩展 `business-formal-module-shells-desktop` L1 回归，在出货单“客户”展示列上验证表头菜单可移动并保存，同时补齐出货明细弹窗关闭等待，避免弹窗遮挡表头按钮。
- 验证：追加前 `progress.md` 为 332 行、71577 字节，未达到归档阈值；`node --test src/erp/utils/moduleTableColumns.test.mjs` 通过，10 项；`pnpm --dir web exec eslint --ext .js --ext .jsx src/erp/components/business-list/ColumnOrderModal.jsx src/erp/utils/moduleTableColumns.mjs src/erp/utils/moduleTableColumns.test.mjs` 通过；`pnpm --dir web test` 通过，365 项；`STYLE_L1_PORT=4174 STYLE_L1_SCENARIOS=business-formal-module-shells-desktop pnpm --dir web style:l1` 通过，1 个场景。
- 下一步：若后续发现某个主业务列表仍有展示列无法移动，优先检查该列是否被克隆后丢失原始列对象引用；能补稳定 `key` 时补业务语义 key，不在页面局部绕开共享列顺序逻辑。
- 阻塞/风险：本轮未改 schema、migration、RBAC、菜单、Workflow / Fact usecase、客户配置、部署、原型状态或正式文档清单；当前工作树仍有并行未提交改动，后续提交需按路径精确区分。

## 2026-06-20 debug 业务数据清空范围补齐

- 完成：补齐 `debug.clear_business_data` 的当前业务表 allowlist，新增清理 `outsourcing_order_items`、`outsourcing_orders` 和 `processes`，清理顺序按委外明细、委外源单、工序档案的外键依赖放置。
- 完成：扩展数据层测试，先真实创建工序、委外加工厂、委外订单和委外明细，再执行业务数据清空；断言清理结果包含三类表，且最终当前项目业务表全部为空。
- 完成：同步 `README.md`、`server/README.md` 和 `docs/当前真源与交接顺序.md` 的 debug 清表范围；`server/internal/data/model/migrate/.DS_Store` 已通过 Finder 移到废纸篓。
- 验证：追加前 `progress.md` 为 341 行、73507 字节，未达到归档阈值；`cd server && go test ./internal/data -run TestDebugSeedRepo_ClearBusinessDataDeletesCurrentProjectBusinessTables -count=1` 通过；`cd server && go test ./internal/data -count=1` 通过；`cd server && make data` 通过，Atlas 显示 migration directory synced 且未生成新迁移；`find server/internal/data/model/migrate -maxdepth 1 -name '.DS_Store' -print` 无输出。
- 下一步：若后续新增长期维护的业务主表或 source document 表，必须同步评审是否纳入 debug 业务数据清空 allowlist 和 `assertProjectBusinessTablesEmpty`。
- 阻塞/风险：本轮未改 schema、migration、RBAC、菜单、页面运行时、Workflow / Fact usecase、客户配置、部署或文档清单；页面治理 skill 仅用于确认本轮不触达 UI / 原型状态，因此未执行浏览器级样式回归。

## 2026-06-20 业务列表删除 / 回收站边界收口

- 完成：按 `plush-page-design-governance` 和 `plush-docs-governance` 将业务对象列表口径收敛为“不提供通用删除 / 回收站壳能力”；主数据退出使用默认走启用 / 停用，Source Document 走取消 / 关闭 / 归档，Fact / Ledger 走取消 / 冲正 / 调整或只读，Workflow 走任务状态和事件。
- 完成：同步 `AGENTS.md` 和 `docs/当前真源与交接顺序.md`，明确只有无引用、未进入业务链路且完成后端 usecase、RBAC、审计、引用检查和测试的草稿或误建对象，才可单独评审删除 / 恢复；表单内草稿明细删除不等同于列表级删除 / 回收站。
- 完成：继续补清具体交互规则：若某对象未来允许删除，必须由后端 usecase 检查状态和引用关系；有引用或已生效时拒绝并返回可读原因，前端提示替代动作。按用户意图区分停用 / 禁用 / 归档、草稿删除 / 清空明细、取消 / 关闭 / 冲正 / 作废 / 红冲 / 版本归档、筛选隐藏和管理员级数据修复 / 合并 / 迁移专项。
- 完成：补清归档与回收站边界：归档表示不再活跃但仍是有效历史记录，应继续可查、可引用、可审计；支持归档生命周期的对象只提供“已归档”筛选 / 视图和取消归档动作，不放入回收站，也不使用“还原删除”语义。
- 完成：补充 `businessModuleNavigation.test.mjs` 断言共享 `BusinessListToolbarActions` 不暴露 `批量删除`、`回收站`、`DeleteOutlined`、`InboxOutlined`，并继续保留 `导出当前结果` 与 `列顺序`。
- 完成：补充 `businessModuleNavigation.test.mjs` 断言 `AGENTS.md` 与 `docs/当前真源与交接顺序.md` 保留删除引用检查、可读拒绝原因、业务生命周期替代、数据修复专项，以及“归档不进回收站 / 已归档视图”口径。
- 验证：追加前 `progress.md` 为 350 行、75141 字节，未达到归档阈值；`cd web && node --test src/erp/utils/businessModuleNavigation.test.mjs src/erp/utils/moduleTableColumns.test.mjs` 先通过 15 项，补充治理断言后通过 16 项，加入归档 / 回收站断言后仍通过 16 项；`git diff --check` 通过；`rg` 复查共享列表 toolbar 未出现通用删除 / 回收站入口，仍保留的 `DeleteOutlined / InboxOutlined` 命中来自 BOM、入库、出货等页面的草稿明细删除或空态 / 导入类图标，不是列表级删除 / 回收站。
- 下一步：如后续某类主数据确需真实删除误建记录，先在后端 usecase 设计引用检查和审计语义，再决定是否在该对象入口单独开放删除；不要恢复通用 toolbar 删除 / 回收站。
- 阻塞/风险：本轮不新增后端删除 / 恢复 API，不新增 schema，不跑 migration，不改 Workflow / Fact usecase、菜单、RBAC、客户配置、部署或原型状态；当前工作树仍有大量并行未提交改动，提交时需按路径精确区分。
...
## 2026-06-20 加工环节页面冗余与列顺序收口

- 完成：按 `plush-page-design-governance` 恢复 `/erp/engineering/processes` 加工环节页的导出当前结果、列顺序按钮、表头列设置和列顺序弹窗，不再把 `processes` 从共享业务主表列顺序能力中特殊排除。
- 完成：把 `/erp/engineering/` 纳入 ERP 外层页头隐藏范围，让加工环节页只保留内容区语义标题和一段边界说明，避免外层页头与 `PageHeaderCard` 重复显示。
- 完成：补 `business-formal-module-shells-desktop` L1 断言，覆盖加工环节页无外层重复页头、工具区按钮可见、列顺序弹窗和表头菜单可保存，以及无横向溢出。
- 验证：追加前 `progress.md` 为 370 行、79388 字节，未达到归档阈值；`pnpm --dir web exec eslint --ext .jsx --ext .mjs src/erp/pages/V1MasterDataPage.jsx src/erp/components/ERPLayout.jsx scripts/style-l1/businessFormalScenarios.mjs` 通过；`STYLE_L1_PORT=4322 STYLE_L1_SCENARIOS=business-formal-module-shells-desktop pnpm --dir web style:l1` 通过；`pnpm --dir web css` 通过；`pnpm --dir web exec node --test src/erp/utils/businessModuleNavigation.test.mjs src/erp/utils/moduleTableColumns.test.mjs src/erp/utils/masterDataOrderView.test.mjs` 通过，27 项；`pnpm --dir web test` 通过，367 项。
- 下一步：若后续要把其他工程页加入同组，继续复用自包含页头和共享列顺序能力，避免再按页面类型绕开共享列表工具。
- 阻塞/风险：本轮未改 schema、migration、RBAC、菜单、MasterData API、Workflow / Fact usecase、客户配置、部署、原型状态或正式文档清单；当前工作树有并行未提交改动，提交时需按路径精确区分。
