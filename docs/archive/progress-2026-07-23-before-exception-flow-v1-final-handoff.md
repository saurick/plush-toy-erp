...
- 当前 Git 由单一 owner 收口。此前发布任务的提交、推送与 133 授权不外推到本次文档治理；本次未获 stage、commit、push 或部署授权。现有 133 仍只代表上一固定发布版本，客户 UAT / 签收仍是独立关口。
...
验证：文档收敛阶段 affected 曾通过 `12 / 12`、`42 / 42`、Web `1734 / 1734` 与客户边界 `76 / 76`；页面改造后 Web `1726 / 1726`、定向 `45 / 45`、Chromium `4 / 4`、CSS、lint、skill health 和 diff 通过。当前 affected 复跑被任务外财务岗位 guardrail 合同 1 项中止；未改 schema、RBAC、业务 runtime 或部署，未 UAT，也未 stage、commit 或 push。
...
完成：新增 `docs/customers/yoyoosun/甲方角色职责与业务流转确认表.md`，按 R01–R09 岗位、A01–A07 审批、P01–P09 主流程、H01–H20 交接、X01–X12 异常和 C01–C09 决策组织甲方面谈，并补充逐岗卡与逐审批卡；把甲方目标、产品基础能力、永绅配置、目标环境、用户验收和交付范围分栏，避免把目标流程或旧版本技术试用写成当前交付。`角色能力与流程矩阵.md` 收窄为内部精确权限与证据手册，`流程编排闭环矩阵.md` 收窄为 Workflow / ProcessRuntime / UI 节点合同；决策、待办、差异和交付状态分别回到各自真源。

完成：按当前代码和配置修正两类漂移口径。首次 IQC 拒绝已有退回供应商 / 供应商补换正式处置，但确认会取消整张未入库收货且不写库存，补换新来源、部分数量语义和跨岗位普通页面仍未闭环；真实收付款、多来源核销、部分核销、冲正和红冲已是 Product Core 本地能力，但永绅财务岗位尚无对应页面 / 权限，当前异常与财务切片未整体发布到 133。重复的 `差异登记.md` 已删除；相关客户差异现统一进入 `客户交付矩阵.md` 的差异分区。客户目录 README、仓库导航、文档清单、产品 / 财务 / 流程当前真源、培训 / 导入说明和自动化守卫已同步。

验证：`bash scripts/qa/affected.sh --run` 按 T0 / T1 / T5 / T6 影响面执行完成：文档与角色合同 `11 / 11`、WIP / 角色 / 原型直接合同 `34 / 34`、客户配置与私有部署边界 `76 / 76`，全部 `0 fail / 0 skip`；Web ESLint 通过。另行组合运行文档库存、角色手册和生产 WIP 合同为 `17 / 17`，17 个 Mermaid 块均由测试中的真实解析器通过；`node scripts/qa/phase-label-boundaries.mjs` 与 `git diff --check` 通过。首次 affected 执行仅在 diff-check 发现 3 个文件末尾多余空行并停止，修正后已从头重跑完成。

下一步：甲方面谈直接使用新确认表逐段签认；会后把已决策项写入 `决策日志.md`、未决项写入 `问题待办.md`、批准差异写入 `客户交付矩阵.md` 的差异分区，再决定永绅财务 entitlement、首次拒收跨岗位页面和补换新来源是否进入实现。

未做 / 风险：本轮未改 RBAC、菜单、运行时代码、schema 或 migration，未连接或写入 133，未部署、未执行目标账号 smoke，也未获得甲方 UAT / 签收；未运行最终 `full.sh` / `strict.sh`。贷项单正式页面只提供应收 / 应付来源，但后端尚缺同等 `fact_type` 强校验；首次拒收处置的部分数量与整单取消合同仍待产品和甲方确认。
