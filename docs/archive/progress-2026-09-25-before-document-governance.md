...
### 仓库 IQC 通用来料验收（2026-09-19）

- 用户已授权实现职责入口、真实分批清点、可扩展验收项、异常更正与入库、快速录入、研发工作台同步及定向验证七项。最新甲方反馈为 IQC 归属仓库并负责清点、检验；不再实施采购清点分工。
- 复用采购收货、质检、拒收处置和库存真源；新增送货标示数量与通用检查项证据，库存仍只由正式入库动作更新。精确路径、hunk 与验证见 `output/qa/iqc-acceptance-20260919/handoff.json`。
- 本地七项已实现：职责与取消草稿权限、实点 / 标示及拆卷、可扩展检查项、质检纠错及来源退货 / 调整、失败保留录入、研发工作台与岗位文档、定向验证。Ent / Atlas 生成零漂移、数据库字典和 db-guard 通过；隔离 PostgreSQL 263 项零跳过、历史数据升级、Go biz / data / service、前端 45 项、工作台相关 102 项及 production build 通过；正式组件宽屏 / 窄屏与失败重试经过浏览器验证。
- 9/19 共享开发库已升级至 `20260919004642`，pending=0、备份恢复与 health/ready 通过；修复预检误拦撤回状态和备份参数。证据：`output/qa/migration-repair-20260919/`。未发布交付目标；业务表单仅做隔离验证。任务外 trial-role-entry-docs 仍因 HEAD 已存在的 fast QA 文案漂移失败，未混入修复。保留采购打印、委外、共享组件等其他改动；无 commit、push 或部署授权。
...
- 修正目标宿主机登记及四个远端执行器的身份校验，统一为 `r740xd`；保留固定用户、目录、数据库和公网入口校验。ShellCheck 同时检查 source 引用，目标相关回归及提交钩子通过；修正已提交推送为 `fa8f3820b44c89472f4cc528b908b49531e6964a`。
- 同 SHA 的 GitLab CI `172` 最终 28 项通过。首次浏览器检查遇到 `ERR_NETWORK_CHANGED`，最终汇总按零重试合同阻断；单独重跑浏览器作业及汇总后，五项检查均一次通过，旧失败记录保留。GitHub main 镜像已读回同 SHA。
- 首次 GitLab release `173` 在发布 `2026.09.17-1` 时失败：CI 回执复用、版本目录、源码检查及五类真实业务 PDF 的 A4、嵌入字体和中文校验已通过，但 Trivy 默认 Google 镜像站连接超时，未登记正式制品。扫描入口改为固定官方 `ghcr.io/aquasecurity/trivy-db:2`，修正提交为 `0063cce57d75f31bb3b87751561da157e844d9c9`；Runner 真实下载完整数据库通过，用时约 220 秒，继续要求完整漏洞扫描。
- 随后的 CI `174` 再现浏览器首场景 `ERR_NETWORK_CHANGED`，汇总继续按零重试合同阻断。只读 Docker events 与作业日志显示，浏览器开始加载时同机附件集成测试正在销毁网络；DAG 改为等待 PostgreSQL 升级及关键数据库检查结束后再启动浏览器，不忽略网络错误、不放宽重试要求。修正提交 `b0d863adc14ee9682ef8be23f557f7949afefe69` 的 CI `175` 已 28 项通过，浏览器场景全部一次通过。
- Release `176` 已通过五类真实 PDF 和漏洞库下载，但扫描阻断 `libpcre2-8-0` 的两项有修复版本的 HIGH 漏洞。原固定快照已含 `10.42-1+deb12u1`，而基础镜像仍带 `10.42-1`；隔离容器升级计划读回仅该包需要升级。Dockerfile 改为按同一固定快照先升级继承的系统包，再安装 Chromium，保留基础 digest、快照和浏览器固定版本。最终发布仍需修正提交的完整 CI、镜像扫描与 release 终态。
- 系统包修正 `4f8d931f00a041598f66a3fa72dd4a355f77ae91` 的 CI `177` 已 28 项通过。Release `178` 首次下载 Trivy 遇到 TLS EOF，显式重跑后 PDF、漏洞扫描和候选制品校验均通过；隔离演练的迁移、登录、配置、PDF、备份恢复与重启通过，但最后清理附件目录发生 EACCES，未登记正式版本。清理入口补齐当前临时工作区内的附件目录，保留路径身份、无网络低权限 helper、失败阻断和零残留要求；CI 同时保留失败演练的脱敏回执。36 项定向回归通过，Runner 一次性验证副本已用同一修正模块清理该失败现场，读回工作区不存在、容器残留 0；副本也已删除。后续仍以新提交的完整 CI 与发布终态收口。
- demo / customer-test 只完成当前运行态的只读预检，尚未部署、迁移或重建；目标选择仍待明确，专用 Package 取件凭据尚未加载。新版本部署前还需重新核对已准备的独立附件 bucket / 待启用配置，以及 fresh backup、迁移和对应公网读回。手工 CD 证据保存在 ignored 的 `output/qa/cd-20260917/`，不伪造为工作台 operation，不代表甲方验收。

### 页面字段、履约 MVP 与 V7 模拟数据（2026-09-16，本地）

- 按暂定分工接通 17 类来源待办：仓库到货及入库、质检、生产提加工要求、财务兼采购确认合同，再经发料、回货、逐关检验、包装及成品入库。异常、取消、重放沿用领域事务；流程地图有登记一致性测试。
- 对照私有原表整理采购、加工字段顺序与双方信息，统一收货及待办用语；扫描 68 个字段文件、核对 51 个页面入口。399 项造数测试、13 类表单浏览器场景及定向 Go / 前端回归通过；原件未进入产品仓。
- 共享开发库已按正式 prepare / execute 应用 `20260916090000`，140/140、pending=0，health / ready 通过。V7 九阶段造数及数据核对完成，包含工程样品与用料审批、45 张已出货单、274 笔财务记录和 69 条入库交接核对；旧数据保留。回执与精确交接见 `output/qa/page-fields/`。客户环境未发布；浏览器检查不等于 PDF 像素一致、全新隔离库验收或甲方 UAT。

### CI 失败与共享开发库启动恢复（2026-09-16）

- 修正财务采购单只读权限、共享任务处理链告警和数据库字典的旧测试断言；继续禁止采购写权限扩散，字典按已移除的四个字段及一个约束核对。gRPC 升至 `1.83.1`，严格漏洞扫描通过；本地启动保留具体 db-guard 诊断，不再把所有工作区数据库规则失败都报告成 schema / migration 不一致。
- 按本轮授权完成同一次 `migrate_prepare → migrate_execute`：operation `d0f18d56-330f-4e1c-bdd0-7e9efd0f06fd`，备份 `br-yoyoosun-20260916T102200+0800` 已真实恢复并验证升级；两条 pending migration 只执行一次，共享开发库从 `20260911164509` 升至 `20260915160215`，读回 `139/139`、`pending=0`、迁移验证及后端 health / ready 通过。回执与备份保留在 ignored 的 `output/dev-workbench/` 对应目录。
- 实测 `pnpm start --local --restart`：升级前业务入口转到可用的迁移恢复页；升级后完整启动预检通过，登录入口恢复，现有客户入口可读取工作台任务。启动、客户角色、告警、字典与文档的定向回归通过；完整 CI 以本轮推送后同一 SHA 的 GitLab 终态为准，既有失败流水线不能当成通过。
- 本次仅处理已授权的代码提交推送、共享开发库与本地运行恢复；没有发布 demo / customer-test、改客户配置 revision 或执行客户 UAT。原文归档两段已完成记录后保留活跃事项与风险，防止过程文件越过 80 KiB。

### 材料汇总固定入口与移动端审批（2026-09-14，本地完成）

- 电脑端看板中心、移动端待办页增加材料汇总查询入口；用料任务直接打开对应提交记录。清单读取用料源单并保留退回历史，手机沿用原表列顺序、部位展开与岗位审批，订单结束后只读。
- 本地开发配置已按正式校验、发布、切换和有效会话读回流程更新为 `yoyoosun-customer-package-v7.local-11df2d5dc696907a.runtime-v1`，仅开放老板、财务、工程固定入口；无 schema / migration 变更。原两张待审体验单保持不变，另用模拟单完成手机老板审核、财务核价及两张采购单生成。
- 定向 Go 验证 441 个测试及子测试通过，8 个需要隔离 PostgreSQL 的客户配置专项跳过；前端 295 项、配置合同 59 项通过，静态检查与构建通过。实际浏览器覆盖电脑、手机窄屏、暗色、历史只读、无权限、空结果和网络失败恢复；回执位于 ignored 的 `output/local-bom-review-20260914/` 与 `output/playwright/material-summary-20260914/`。未提交、推送或部署，未执行远端 CI、隔离 PostgreSQL 并发专项或真机触屏验收。

### 销售订单 Excel 批量导入与原表核对（2026-09-12，本地完成）

- 汇总表按订单编号分组，可多选或全选、逐单编辑后批量新建草稿；每单保留全部明细，保存前检查所有订单，失败只重试未完成项。款号、类别、船头版、工艺、交期和备注进入现有字段，图片作为订单附件；原表设计师及出货、付款信息仅供核对，不生成相关业务事实。
- 订单行增加只读原表证据 `import_source`，覆盖文件摘要、工作表、行号、单元格与图片文件名；保存后可回看，敏感内容沿用字段权限。`make data` 生成 Ent 与 `20260911164509_migrate.sql`，按正式 prepare / execute 完成共享开发库备份、隔离恢复、一次 apply 与零漂移读回；137/137、pending=0，health / ready 通过，`pnpm start` 的恢复页与迁移后正常登录入口均实测可达。
- 验证：Web 单测 2615 项、相关文档 14 项、Go core / biz / data / service / server 包、lint、build 与 db-guard 通过；3 个脱敏浏览器场景及原始 Excel 的 16 单 / 78 行整批场景通过，78 行各 13 个业务字段独立比对一致，全部匹配到原表图片。浏览器保存使用隔离模拟接口，未把客户订单写入共享业务库；以上不代表目标发布或客户验收。
- Git handoff：一组“完善销售订单 Excel 批量导入与原表资料核对”，包含解析 / 批量编辑、来源字段与生成物、附件映射、测试和正式文档；保留已有表单布局、BOM、原型与其他页面改动。未获 commit / push / 发布授权；混合文件按来源核对 hunk，不整文件收口。

### 本地附件迁入统一 SeaweedFS（2026-09-12，demo / test 待发布）
...
- demo / test 接入：2026-09-12 已在 133 新增独立 `plush-demo-133-files` bucket、仅限该桶的 S3 身份和待启用配置；合成文件上传 / 下载 SHA-256、匿名与跨桶请求拒绝通过，验证文件已清理。两套运行配置和数据库未改动；demo 的 27 个附件、9,001,107 字节仍在 PG，test 为 0 个附件。主机运行文档、本地副本和脱敏迁移回执已同步；当前健康、ready、公网入口与原运行 SHA 均通过读回。
- 发布阻断与 Git 交接：`a64743115999bffc5e6acce8f53af6bb44c3c3ee` 的 main CI 161 通过，正式 release 162 / job 2126 因镜像未复制 Vite 依赖 `web/scripts/localPort.mjs` 失败。本轮仅在 `server/Dockerfile` 补一条 COPY；同 SHA 源码加此修复在 Runner VM 的独立目录完成真实 Docker `web-builder` 构建与客户配置叠加，未发布验证镜像。待授权提交 / 推送这一行及本节记录，再走新 SHA 的正式 CI / Release、备份恢复和两目标迁移；目标直接取件所需专用只读 Package token 仍待配置。其他业务、表单及测试改动属于其他任务，保留原样；当前未提交、推送或切换业务服务。
...
| 事项                         | 当前结论                                                                                                                                         | 详细证据               |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------- |
| 老板与 PMC 进度看板 | 原业务看板改为订单交付 / 生产执行查询，后端按正式事实与责任范围汇总，提供风险筛选、明细侧栏和原单追溯；本地验证完成，未提交或发布 | [进度查询合同](server/docs/api.md#进度看板-business) |
| 共享开发库迁移终态回执       | 本地共享开发库六个入口在成功、no-op、ready、需人工动作、阻断、失败和结果未知时统一输出完整脱敏摘要；真实只读 status 为 `107 / 107`、pending `0`  | [归档原文](docs/archive/progress-2026-09-17-before-cd-closeout.md)           |
| 共享开发库迁移主路径可操作性 | 裸旧命令已安全兼容到高层编排；登记共享库已由 `105 / 107` 一次升级到 `107 / 107`、pending `0`，备份恢复、schema / 数据 / 权限和后端运行读回均通过 | [归档原文](docs/archive/progress-2026-09-17-before-cd-closeout.md)           |
...
- 列顺序和全筛选导出的实现及历史验证见[归档原文](docs/archive/progress-2026-09-19-before-iqc-acceptance-closeout.md)；以下保留当时未决事项。
- 构建历史阻塞已不再复现：2026-09-24 本地 Web 构建通过。该批目标环境发布、岗位 smoke 与客户 UAT 仍需各自证据；本轮未提交或推送。
...
- `docs/archive/progress-2026-09-19-before-iqc-acceptance-closeout.md`：业务列表列顺序与全筛选导出的完整原文；未决风险留在当前页。

- `docs/archive/progress-2026-09-17-before-cd-closeout.md`：共享开发库迁移主路径和终态回执的两段已完成记录原文；当前活跃事项及风险保留。

- `docs/archive/progress-2026-09-16-before-ci-migration-closeout.md`：2026-09-04 报价运费与 2026-09-05 双目标发布原文；待办、风险及其他活跃事项保留。
