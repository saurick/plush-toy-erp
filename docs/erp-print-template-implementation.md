...
- 两套模板都通过同一条“打印中心 -> 独立弹窗 -> 右侧纸面 DOM 输出 PDF / 打印”的主链打开。
- 这条主链已继续向 `trade-erp` 靠拢：先开壳页、再进入模板工作台、窗口级 `state` 隔离草稿、工作台稳定后持续回写整窗 HTML 快照，刷新时优先由壳页直接恢复当前窗口。
- 当前实现仍比 `trade-erp` 更轻：正文继续由 React 工作台渲染，不是 builder + 独立静态 runtime，但分页运行时已经统一收口到共享 `printPageMargin` 主链。

## 2. 当前代码真源

| 层 | 当前文件 | 职责 |
| --- | --- | --- |
| 打印中心入口 | `web/src/erp/pages/PrintCenterPage.jsx` | 选模板、打开独立弹窗 |
...
| PDF 输出 | `web/src/erp/utils/printPdf.mjs` | 冻结当前 DOM，走后端 `/templates/render-pdf` 生成 PDF |
...
- 保留和 `trade-erp` 一致的“先有壳页，再进正文”的统一打开方式
...
- 在线预览 PDF / 下载 PDF：`printPdf.mjs` 会先 blur 当前焦点元素，再 clone 当前整页 DOM，移除运行时高亮类后交给后端 `/templates/render-pdf`
...
- 当前只覆盖两套合同模板，还没有收口成 `trade-erp` 那种多模板统一 runtime

## 6. 后续改造守则

- 若继续扩模板，优先复用 `printWorkspace.js + print-window-shell.html + PrintWorkspaceShell.jsx` 这条主链，不要再为新模板单独发明新的弹窗入口和新的草稿 key。
- 若后续继续对齐 `trade-erp`，优先补更强的共享编辑 runtime，不要重新退回“每个模板页自己兜刷新 / 自己猜分页”的分叉写法。
- 若修改打印输出逻辑，必须同时检查：
  - 左侧字段面板是否仍和右侧纸面同步
  - 当前窗口级草稿是否仍独立
