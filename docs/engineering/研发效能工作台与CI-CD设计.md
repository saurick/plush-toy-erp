...
| GitHub mirror | GPT Review、外部只读浏览、历史 Release 读取 | 不接受直接 main 写入，不运行仓库 CI，不创建新 publication |
...
GitHub 不保留仓库 workflow 或发布写入 helper；历史 Actions 运行只作审计，不能证明当前候选。恢复任何 GitHub publication 能力都必须重新进入专项设计、代码与远端权限评审，不能把旧 workflow 直接补回。
...
迁移页在停止后端之前检查完整工具能力：兼容 `docker` CLI/socket 的可用容器运行环境、Atlas v1.3.0、PostgreSQL 18 `pg_dump` / `psql` 和备份恢复基础命令。实现不绑定 macOS 或某一桌面产品；Docker Engine、Docker Desktop、Colima、Rancher Desktop、OrbStack，以及提供兼容入口的 Podman 都按同一能力合同判断。工具不完整只返回脱敏阻断和下一步，不自动启动本机应用、不自动 apply，也不把前端恢复页可达当成数据库已迁移。
...
- GitHub 没有发布写路径；任何恢复 proposal 必须先证明不会与 GitLab canonical release 并行。
...
| `/__dev/delivery` | 交付运行任务入口与四类环境 / 验收事实对比 | 数据准备摘要、Delivery Bridge 与固定目标 operation 回执 |
...
- 四个一级路由和十四个二级任务路由统一提供开发工作台全局菜单、当前页高亮、复制当前深链和按需打开来源文档。一级菜单按稳定责任域命名为“总览、产品工程、质量验证、交付运行”；二级菜单按开发者要完成的任务或查看的对象命名，页面标题可在短名基础上补充完整职责，路由和内部 key 不随文案调整。
...
“质量验证”一级页只负责选择下一项任务；“交付运行”一级页先对比本地、demo、test 和隔离验收四类事实，再选择客户配置、数据库迁移、版本发布或恢复任务。两者都不复制子页的详细 operation 历史；full / strict 运行结果、阶段和历史统一在“质量门禁”核对，当前 SHA 的 strict 发布资格统一在“版本发布”核对。
...
该页把内核、权限、治理、业务链、易用性、文档和原型入口组织为“按问题找入口 / 项目图视角”两个同级、互斥且 URL-backed 的 Tab。默认 `view=questions` 按“查看当前产品能力、核对权限结果、判断规则、查看业务链、检查员工能否看懂并独立完成、搜索文档、评审原型”七类用户问题组织并列问题清单，不用序号暗示固定执行顺序；`view=relationships` 把业务对象、状态、岗位、依赖和证据看作节点，按业务与事实、状态与流程、权限与责任、产品结构与治理、质量与 CI、交付与证据六类图模型提供分类说明、证据边界和现有入口。前一种视角聚焦七个产品工程工具，后一种还会关联质量与交付证据入口；两者都复用当前页面、菜单和正式真源。未知、重复或无关参数统一恢复到规范化查看方式。
...
- `q / status / role` 写入 URL；空白默认值省略，未知、重复或无关参数恢复到规范状态，避免刷新、分享或前进后退后出现不可解释的空结果。
...
- `?path=<markdown-path>#<section-anchor>` 可直达文档和章节；标题锚点与 GitHub 的空格、标点及重复标题规则保持一致。确需稳定别名时，只支持在目标标题前单独一行声明 `<a id="..."></a>` 或 `<a name="..."></a>`，查看器将合法别名安全映射到该标题，不开启任意 Markdown HTML。页面选择文档或一至六级标题会同步 URL，浏览器前进后退和刷新可恢复；相对 Markdown 链接继续留在开发文档查看器，站外链接保持普通外链行为。
...
该页浏览 `docs/product/prototypes` 下的 HTML、PNG 和截图证据，支持分类、分组折叠、当前资产和本地置顶恢复；“交互控件规范”额外提供下述共享运行组件的实时样例。新用户默认只展开当前资产所在目录，状态说明、资产统计和技术来源按需展开；已有目录偏好继续恢复。筛选无结果时预览同步为空；每个资产可打开对应 README、复制仓库路径，并通过隔离 sandbox 预览。全屏预览进入弹窗焦点、圈定 Tab、Escape 关闭并恢复触发按钮。卡片参照范围不是正式菜单、路由、权限或 `seedData` 映射表。

<a id="interaction-controls"></a>

#### Tab、筛选与按钮 / Interaction Controls

开发工作台 → 产品原型 → **交互控件规范**（`/__dev/prototypes?controls=1`）直接渲染项目共享组件，可切换浅深主题、页签、筛选和演示动作。该入口反映当前组件外观，不是单独维护的静态 HTML 原型，也不证明具体业务流程或目标部署已验收。历史原型中的控件皮肤不作为新实现依据。

| 用户意图 | 复用入口 | 可见特征与边界 |
| --- | --- | --- |
| 切换同级内容区 | `SlidingTabs` | 每项有页签边界，整组始终使用同一中性边框色；选中只增加底色和字重，不添加深色框或粗底边。下方内容与页签保持邻近。 |
| 切换少量互斥视角，如订单交付 / 生产执行 | `SlidingSegmented` | 明确的分组外框和连续滑块，外框、选项与滑块使用同一中性边框色，选中使用底色和字重；桌面默认按内容宽度排列，只有窄区域才用 `block`，不把两个短选项铺成整页大卡片。 |
| 移动底部导航、步骤等已有自定义布局 | `SlidingTabList` | 复用指示器和方向键，保持布局的岗位含义；自定义仅处理布局，不能隐藏选中位置。 |
| 缩小当前结果，如逾期 / 到期 / 受阻 | `FilterChip` | 使用真实按钮与 `aria-pressed`，整组始终使用同一中性边框色；选中只增加底色和字重，不添加勾号、深色框或粗底边。数量是标签的一部分。 |
| 移动端搜索、筛选与连续结果 | `MobileSearchInput`、`Popover`、`erp-mobile-controls`、`FilterChip` | 搜索框与筛选入口同高 46px，共用边框、文字和底色；同一列表只保留一个自由文本搜索，单号、客户、产品、负责人等可搜索身份统一进入主搜索，筛选层不得再放第二个文本搜索。高频分类保留一行，2–4 组少量次级条件使用锚定在筛选按钮下方的轻量下拉层，覆盖列表而不把页面内容向下推。离散条件点击即生效；按钮只统计下拉层内不可见的已应用条件。有限互斥选项使用 `SlidingSegmented`，筛选标题、选项文字和重置动作共用居中基线，不使用占高的大型下拉表单。同级 Tab 分别保存搜索与筛选状态；切换 Tab 恢复该页自己的条件，筛选层的重置只恢复当前 Tab 的排序和状态，主搜索由输入框的清除动作单独处理，二者都不能改掉相邻页。单一连续结果流保留后端分批读取，但移动端不得显示上一页、下一页或页码；用户接近列表底部时自动追加下一批，追加失败只在列表尾部重试，不能清空已读内容。清除搜索可触区域至少 44px，清除后保留输入焦点。输入字号至少 16px，中文组词期间不发起搜索。移动列表默认不提供精确起止日期输入；确有高频时间需求时先使用“今天 / 近 7 天 / 近 30 天 / 本月”等一键时间段，只有业务明确要求自定义日期时才进入单独的移动日历流程。 |
| 执行保存、刷新等操作 | Ant Design `Button`；原生移动控件复用 `erp-control-button` | 一组只突出一个主动作；主动作实心、次动作描边、辅助动作文字、危险动作采用危险色。图标按钮有可访问名称，提交中阻止重复执行。 |
| 仅展示状态或数字 | 文本、Tag、统计展示 | 不绘制可点击反馈，不假装成按钮；需要操作时提供独立且明确的入口。 |

外观真源为 `web/src/erp/styles/app/control-affordance.css`，位移动画真源为 `tabs-motion.css`；页面仅安排间距、宽度、排列与所需尺寸。常规桌面控件保持约 32–38px，移动触摸区域至少 44px。移动页面列表上方默认不超过三行高频控件且总高不超过 180px；少量浅层条件使用锚定下拉层，下拉层宽度不得超出视口、高度默认不超过视口的 52%，内容超出时内部滚动。只有条件很多、存在多级选择或需要比较大量选项时才使用全屏层或底部抽屉。压缩 Y 轴优先减少空白、重复标题和低频控件，不得将触摸区压到 44px 以下，也不得隐藏高频操作。不可用项明确弱化并阻止交互；可聚焦控件必须有清晰焦点，不能用颜色变化作为唯一选择信号。长标签、大数量和窄屏允许在局部容器内滚动，不能裁掉关键字或让整个页面横向溢出；优先保留主要筛选可见，避免不必要地挤占列表与表格。
...
- `本地长期基础数据 / core-demo` 只允许登记的 `192.168.0.133:5432/plush_erp` 或 `plush_erp_*_dev`，先确认 migration 已到 head，再顺序准备十个演示账号、当前 V7 的 11 个单位与 4 个仓库。它不生成材料、产品、工艺、BOM、客户、订单、Workflow 或 Fact；这些版本化业务数据由 `scenario-demo` 接续准备，避免两套基础资料和生产工序语义并存。稳定 upsert 不等于跨入口事务，也不提供按 operation 删除。
- `业务场景演示数据 / scenario-demo` 固定使用 `yoyoosun-manual-acceptance / 2026.09.16-v7 / 20260916-V7`，只允许 `127.0.0.1:8300` 对应的登记 133 长期开发库。用户确认后先稳定准备本地岗位账号与至少 30 条由真实控制面操作产生的审计样例，再通过正式 `validate / publish / transition check / activate or rollback / effective-session readback` 对齐当前跟踪的 yoyoosun 本地测试配置，之后才准备 Source Document、已登记的 ProcessRuntime、模拟岗位任务和来源驱动 Fact。同批只允许精确创建或读回；半批、字段或身份漂移直接阻断，不提供清理或重置。收付款覆盖已批准、两笔已过账和已冲销，红冲覆盖一条有效红冲与一组原红冲 / 反向红冲。岗位到期时间是固定 V7 快照，不保证长期维持“今天 / 本周”相对语义；数据前置不替代浏览器验证和岗位人工验收。
...
- `scenario-demo` 的页面操作固定为“读取预检 → 点击生成 → 自动准备并冻结 `planHash`、`runId`、仓库和目标摘要 → 核对固定目标 / V7 基线 / 数据范围 / 长期保留边界 → 确认生成 → 异步执行 → 读取回执”，不要求手输长确认串。其他 profile 继续使用完整确认串。执行前身份变化会使原计划失效；页面刷新可恢复最近 operation。进程中断或结果不明确时显示 `not_proven`，不会自动重试；用户可重新准备更晚的同目标 scenario plan 并再次确认，以同一固定批次显式补齐，其他 profile、不同目标或仍在运行的 operation 继续阻断。
- 摘要把 operation 明确分成 `currentOperations / historicalOperations / unresolvedOperations`。只有数据版本、run id、semantic digest 与当前 V7 完全一致，且完整回归仍绑定当前业务链摘要的回执，才参与执行恢复和环境结论；旧 V5 / V6 或旧验证合同回执继续保留用于追溯，未知、字段不完整或当前身份却无法通过读回校验的合同失败关闭，不会被“最近一次成功”误选为当前证据。
...
- `dataVersion` 是一轮可重复、可验收的冻结业务数据基线，不是 Git commit、operation 或每次造数的版本。业务链数据摘要和验证摘要都相同时，长期同批数据仍可用；只有验证摘要变化时，以新 operation / batch 绑定 exact commit 重新核验但继续当前 V7。数据摘要变化时先在隔离批次修正和验证，只有单位含义、记录结构、生命周期 / 状态、业务链映射、稳定编码或数量合同不兼容，或者准备冻结下一轮甲方测试基线时，才集中升级 `dataVersion`。已持久冻结的旧基线不得静默改写；纯样式、重构、性能或不改变数据结果的修复不升级当前冻结版本。完整回归仍默认每次使用新隔离批次，长期保留规则只服务 `core-demo / scenario-demo` 的日常联调边界。
...
- operation 使用 `0600` 原子状态、幂等键和跨 Vite 进程排他锁。活动 operation 只保留一个串行状态轮询，慢响应不会叠加第二个请求或让旧响应覆盖新状态。migration / schema / guard / 备份编排真源、目标状态或备份文件身份变化会使旧计划失效；未变化且文件大小与 SHA-256 均读回一致的备份恢复报告可以复用，避免同一计划因非写入阻断反复 dump / restore。命令行 `make migrate` 与该页面复用同一 service；非交互调用必须显式使用 prepare / execute 两阶段，prepare 成功不能冒充已迁移。
...
- release readiness 必须显式选择 `deployments/<customer-key>/evidence/releases/<release-batch>` 的已登记批次，不猜 `latest`、不接受父目录或路径穿越。受控 Bridge 先以 `compile` 模式从独立正式登记入口生成并原子写入 release manifest，再对所选证据批次运行只读 readiness gate；它不写目标环境、不发布、不激活“正式版”。备用命令只复制带精确批次的输入模板，不拼未替换的 `<release-batch>` 或旧 manifest 路径；正式执行器继续要求目标端点、令牌、确认短语、release report 和 authenticated readback。
...
- demo 只有运行、当前候选、migration / 客户配置和当前 V7 场景回执同时对齐才可显示数据已读回。test 即使 health / ready / 公网入口和当前候选都对齐，甲方验收仍保持“未证明”；运行正常不等于数据干净，也不等于 UAT。
