...
采购订单打开采购合同时，单位显示由采购合同打印草稿映射层收口：优先取单位选项或快照里的英文 code；已知中文单位会规范为打印短码：`个 / 件 / 只 / 片 / 条 / 支 / 枚 / 颗 / 粒 / 块 / 张 / 本 / 根 / 束 / 组 / 台 / 把 / 板 / 筒` -> `PCS`，`对 / 双` -> `PAIR`，`套` -> `SET`，`米 / 厘米 / 毫米 / 码 / 尺 / 英寸` -> `M / CM / MM / YD / FT / IN`，`千克 / 公斤 / 克 / 吨 / 磅 / 盎司` -> `KG / G / T / LB / OZ`，`箱 / 盒 / 包 / 袋 / 卷 / 瓶 / 桶` -> `CTN / BOX / PKG / BAG / ROLL / BTL / DRUM`，`打 / 罗 / 令 / 扎` -> `DOZ / GRS / REAM / BDL`。无法确认的来源单位保留原文，避免把未知单位伪造成错误英文。
...
- 两套正式模板必须在 `printTemplates.mjs` 声明 `fieldRequirements`、`moduleKeys` 和 `factBoundary: read_snapshot_only`。字段合同只描述打印草稿需要哪些业务快照，不代表模板可以生成采购、委外、库存、出货或财务事实。
