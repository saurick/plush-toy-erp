...
- 第三方基础镜像必须固定版本 tag；PostgreSQL、Jaeger 和业务镜像都不能使用 `latest`，生产 `.env` 由 `production-preflight.sh` 阻断浮动 tag、placeholder secret、SMS mock、debug 写入开关、PostgreSQL 非 loopback 暴露和 Jaeger 非 loopback 暴露
