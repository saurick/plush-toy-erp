...
| 出货单提交待放行 | `notification_type=task_created`、`alert_type=shipment_pending`；`shipment_release done` 只写 `shipping_released`，不推进 `shipped` | warehouse；sales、PMC 按实际任务关注 |
| 出货准备或出货确认超时 | `task_due_soon` / `task_overdue`，或 `shipment_due` / `shipment_risk` | warehouse、sales、PMC，必要时 boss |

成品抽检、成品入库和 `shipment_release` 的协同状态已由后端收口；真实出货已由 `shipments / shipment_items`、`OperationalFactUsecase` 和 Inventory 的 `OUT / REVERSAL` 主路径承接。通知层只读取任务和事实结果，不新增重复的生产、出货、库存或财务真源；完整预留冻结和财务核销仍按各自领域 usecase 评审。
