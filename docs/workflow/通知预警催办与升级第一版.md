...
通知和预警用于减少状态沟通，不替代业务事实。预警来自服务端任务状态、业务投影、截止时间、优先级和受控风险标记。当前没有独立持久化的通知链：任务事件只能证明任务发生过什么，payload 只是当前快照，都不能证明消息已经向某人送达。
...
| `shipment_due` | warning / critical | Shipment 财务审批或仓库显式出货待办即将到期 / 已超时；小于等于 1 天按 critical |
| `material_shortage` | critical | payload 或阻塞原因明确缺料 / 欠料 |
| `vendor_delay` | warning / critical | 委外延期；已超期按 critical |
| `production_scheduling_pending` | warning / critical | 生产订单已发布，等待 PMC 完成排程；已超期按 critical |
...
| `rework_pending` | warning / critical | 返工要求未闭环；已超期按 critical |
...
| `approval_pending` | warning | 当前 approval 责任池的审批或退回待处理；销售、采购和 Shipment 财务审批不固定为老板 |
...
| warehouse | `inbound`、`outbound`、`inventory`；`shipping-release` 仅为历史协同读取 |
...
催办与状态动作共用任务 `version` 和事件 receipt。客户端提交当前 `expected_version` 与顶层 `idempotency_key`，repo 以 ID + 非终态状态 + version 做 CAS，并在成功时原子更新催办快照、`urge_count`、task version 以及包含服务端 intent hash 和原始任务快照的同版本事件。两个同版本催办只能一个提交；同 task、key、actor、动作、原因和业务 payload 的网络重放直接返回首次快照，不再次增加 `urge_count`，同 key 改 intent 返回 `40920`，新 key 不能绕过终态。未知结果时前端保留冻结 attempt 原样重试，不重新生成 key。
...
当前 v1 不建立独立 notifications 表，不推送外部渠道；页面提醒由 `workflow_tasks`、`workflow_task_events`、`workflow_business_states` 和服务端 / 前端受控投影共同形成。这只是风险提示，不是具有接收人、渠道、送达、失败、重试和确认记录的完整通知链。
...
当前不新增 `outsource` 岗位任务端入口，委外回货跟踪和返工 / 补做先由 production 岗位任务端承接。岗位任务端按服务端 owner / assignee / pool、active revision entitlement 和分页查询可见任务；前端只筛选已加载页面，不能用固定 `limit` 推断岗位全量或扩大数据范围。
...
| 生产订单发布待排程 | `notification_type=task_created`、`alert_type=production_scheduling_pending` | PMC；critical_path 纳入生产主链关注 |
| 返工记录过账待处理 | `notification_type=task_created`、`alert_type=rework_pending` | production；PMC 通过 critical_path 关注 |
| 出货单待财务放行 | `notification_type=approval_required`、`alert_type=shipment_pending`；`shipment_finance_approval` 通过后只写版本化财务门禁，不推进 `SHIPPED` | 当前 Shipment 财务审批责任池；sales、PMC 只按有效权限 / 责任关注 |
| 财务已放行待仓库出货 | `notification_type=task_created` 或页面风险投影、`alert_type=shipment_pending` | warehouse；是否生成独立待办以当前后端合同为准，不能由前端补造 |
| 出货准备或出货确认超时 | `task_due_soon` / `task_overdue`，或 `shipment_due` / `shipment_risk` | warehouse、sales、PMC，必要时 boss |

成品抽检和成品入库协同由后端收口；新的 Shipment 流程只负责财务审批与放行门禁，历史 `shipment_release` 不再有公开 producer。真实出货由 `shipments / shipment_items`、`OperationalFactUsecase` 和 Inventory 的 `OUT / REVERSAL` 主路径承接。通知层只读取任务、门禁和事实结果，不新增重复的生产、出货、库存或财务真源，也不冒充任务链、审批链或业务审计链。
