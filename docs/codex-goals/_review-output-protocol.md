...
`.codex-review/latest.md` 只报告当前 Goal 的内容，包括目标、允许范围、实际纳入文件、验证命令、提交状态和本轮风险。

如果 Goal 运行期间其他会话修改了非本 Goal 路径，Codex 必须把这些改动视为“非本轮现场”：

- 不得写成本轮完成内容。
- 不得纳入本轮新增 / 修改文件清单。
- 不得为了收口擅自回退、删除、格式化或提交。
- 需要提交时只能按当前 Goal 允许路径精确 stage。
- 如果外部改动影响验收或提交，只在“非本轮现场 / 阻塞风险”中点名路径、说明影响和建议处置。
