...
const USAGE = `Current customer import dry-run tooling
...
function renderMarkdownReport({ sourcePath, existingPath, outDir, command, data }) {
  ...
  return `# Current Customer Import Dry-run Report

## Command

\`\`\`bash
${command ?? 'node scripts/import/currentCustomerDryRun.mjs ...'}
\`\`\`
...
export async function runCli(argv = process.argv.slice(2)) {
  ...
  const command = `node scripts/import/currentCustomerDryRun.mjs ${argv.map(quoteArg).join(' ')}`
