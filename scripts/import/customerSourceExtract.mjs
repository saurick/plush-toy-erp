...
const HEADER_ALIASES = {
  ...
  contactName: ["联系人"],
  phone: ["联系电话"],
  companyLiaisonName: ["公司对接人"],
  companyLiaisonPhone: ["对接人电话"],
  address: ["地址", "加工商地址", "供应商地址"],
  supplierCategory: ["类别"],
  supplierProcessCapability: ["加工工序"],
  invoiceType: ["开票类型"],
  invoiceRate: ["开票点数"],
  bankAccount: ["银行卡号"],
  requester: ["下单人", "委托人"],
...
export async function runExtraction(options = {}) {
  ...
  await writeJson(
    path.join(outDir, "source-snapshot.extracted.json"),
    sourceSnapshot,
  );
  await writeJson(
  ...
  await writeJson(
    path.join(outDir, "customer-import-config.candidate.json"),
    importConfig,
  );
  await writeJson(path.join(outDir, "extraction-summary.json"), summary);
  await writePrivateEvidenceFile(
    path.join(outDir, "extraction-report.md"),
    report,
  );
...
async function readManifestWorkbooks(manifestResult) {
  const sourceEntries = selectStructuredExtractSources(
    manifestResult.sources,
  ).filter((source) =>
    XLSX_EXTENSIONS.has(path.extname(source.relativePath).toLowerCase()),
  );

  if (sourceEntries.length === 0) {
    throw new CliError(
      "No structured .xlsx sources found in the source manifest",
      2,
    );
  }
  ...
  for (const sourceEntry of sourceEntries) {
    ...
    if (
      workbook.sha256 !== sourceEntry.sha256 ||
      workbook.sizeBytes !== sourceEntry.actualSizeBytes
    ) {
...
function createCollector(customerKey, options = {}) {
  ...
  const workbookSourceIndex = new Map(
    (options.workbooks ?? [])
      .map((workbook) => [workbook.fileName, workbook.sourceManifest])
  ...
  return {
    ...
    addSource(input) {
      ...
      const source = {
        ...
        sourceManifestId:
          workbookSourceIndex.get(input.fileName)?.sourceId ?? null,
        sourceManifestPath:
          workbookSourceIndex.get(input.fileName)?.relativePath ?? null,
        sourceFileSha256:
          workbookSourceIndex.get(input.fileName)?.sha256 ?? null,
        fileName: input.fileName,
        sheetName: input.sheetName,
...
function extractMaterialSummarySheet(workbook, sheet, collector) {
  ...
  collector.addMapping(
    buildMapping(
      ...
      sheet,
      header.rowNumber,
      "materials / products / units / purchase_orders",
      [
        "产品编号",
        "产品名称",
        "材料品名",
  ...
  for (const row of rowsAfter(sheet, header.rowNumber)) {
    ...
    const productNo =
      valueByAlias(row, header.map, "productNo") || context.productNo;
    const productName =
      valueByAlias(row, header.map, "productName") || context.productName;
    ...
    addMaterialCandidate(collector, workbook, sheet, row, {
      ...
      unit,
      sourceOrderNo:
        valueByAlias(row, header.map, "orderNo") || context.orderNo,
    });
    collector.addSource({
      ...
      fields: {
        ...
        purchase_quantity:
          ...
          valueByAlias(row, header.map, "totalQuantity"),
...
function extractMaterialDetailSheet(workbook, sheet, collector) {
  ...
  for (const row of rowsAfter(sheet, header.rowNumber)) {
    ...
    carryField(
      carried,
      "materialCategory",
      valueByAlias(row, header.map, "materialCategory"),
    );
    carryField(
      carried,
      "materialName",
      valueByAlias(row, header.map, "materialName"),
    );
    carryField(
      carried,
      "vendorItemNo",
      valueByAlias(row, header.map, "vendorItemNo"),
    );
    carryField(
      carried,
      "materialSpec",
      valueByAlias(row, header.map, "materialSpec"),
    );
...
function extractPurchaseSummarySheet(workbook, sheet, collector) {
  ...
  collector.addMapping(
    buildMapping(
      ...
      sheet,
      header.rowNumber,
      "purchase_orders / materials / suppliers / units",
      [
        "产品订单编号",
        "产品编号",
        "产品名称",
  ...
  for (const row of rowsAfter(sheet, header.rowNumber)) {
    ...
    const supplierName =
      valueByAlias(row, header.map, "supplierShortName") ||
      valueByAlias(row, header.map, "vendorItemNo");
    ...
    collector.addSource({
      ...
      fields: {
        ...
        ...buildExpectedReturnDateCandidate(
          valueByAlias(row, header.map, "returnDate"),
        ),
        note: valueByAlias(row, header.map, "note"),
      },
    });
...
function extractOutsourcingSummarySheet(workbook, sheet, collector) {
  ...
  for (const row of rowsAfter(sheet, header.rowNumber)) {
    ...
    const productNo = valueByAlias(row, header.map, "productNo");
    const processingItem = valueByAlias(row, header.map, "processingItem");
    const processName = valueByAlias(row, header.map, "processType");
    const supplierName = valueByAlias(row, header.map, "supplierShortName");
    const unit = valueByAlias(row, header.map, "unit");
    if (!productNo && !processingItem && !processName && !supplierName) {
      continue;
    }
    ...
    addUnitCandidate(collector, workbook, sheet, row, unit);
    addProcessCandidate(collector, workbook, sheet, row, processName);
    addSupplierCandidate(collector, workbook, sheet, row, {
      supplierName,
    ...
    collector.addSource({
      ...
      domain: "outsourcing",
      fields: {
        outsourcing_order_no: valueByAlias(
          row,
          header.map,
          "outsourcingOrderNo",
        ),
...
function extractSupplierDirectorySheet(workbook, sheet, collector) {
  ...
  for (const row of rowsAfter(sheet, header.rowNumber)) {
    ...
    const processCapability = valueByAlias(
      row,
      header.map,
      "supplierProcessCapability",
    );
    const supplier = addSupplierCandidate(collector, workbook, sheet, row, {
      supplierCode: valueByAlias(row, header.map, "supplierCode"),
      ...
      note: valueByAlias(row, header.map, "note"),
      preserveSourceRow: true,
    });
    addProcessCandidate(collector, workbook, sheet, row, processCapability);
    addSupplierProcessCapabilityCandidate(
      collector,
      workbook,
      sheet,
      row,
    ...
    if (contactName || phone) {
      collector.addUnique({
        ...
        fields: {
          ...
          ...buildContactPhoneCandidate(phone),
          note: "owner_id 需在 supplier 唯一匹配后人工回填；不得按名称猜 owner_id。",
        },
      });
    }
    const companyLiaisonName = valueByAlias(
      row,
      header.map,
      "companyLiaisonName",
    );
    const companyLiaisonPhone = valueByAlias(
      row,
      header.map,
      "companyLiaisonPhone",
    );
    if (companyLiaisonName || companyLiaisonPhone) {
      collector.addSource({
        sourceType: "Customer Material",
        sourceKind: "xlsx_sheet",
        moduleKey: "customer_material",
        fileName: workbook.fileName,
        sheetName: sheet.name,
        rowNumber: row.rowNumber,
        domain: "customer_material",
        fields: {
          supplier_name: supplierName || shortName,
          company_liaison_name: companyLiaisonName,
          company_liaison_phone: companyLiaisonPhone,
          review_required: true,
          note: "公司侧对接人不能自动写为供应商联系人；需确认是合同委托方代表还是内部跟单责任人。",
        },
      });
    }
  }
...
function extractContractTemplateSheet(workbook, sheet, collector) {
  ...
  const domain = hasHeader(header.map, "采购订单号")
    ? "purchase_orders"
    : "outsourcing";
  collector.addMapping(
    buildMapping(
  ...
  for (const row of rowsAfter(sheet, header.rowNumber)) {
    ...
    const lineName =
      valueByAlias(row, header.map, "materialName") ||
      valueByAlias(row, header.map, "processName");
...
function addProcessCandidate(collector, workbook, sheet, row, processName) {
  const normalizedName = normalizeText(processName);
  if (!normalizedName || isBadIdentityValue(normalizedName)) {
    return null;
  }
  return collector.addUnique({
    uniqueKey: normalizedName,
    sourceType: "Data Import Source",
    sourceKind: "xlsx_sheet",
    moduleKey: "processes",
    fileName: workbook.fileName,
...
function addSupplierProcessCapabilityCandidate(
  collector,
  workbook,
  sheet,
  row,
  ...
  if (
    ...
    isBadIdentityValue(normalizedSupplierName) ||
    isBadIdentityValue(normalizedProcessName)
  ) {
    return null;
  }
  return collector.addUnique({
    uniqueKey: `${normalizedSupplierName}|${normalizedProcessName}`,
    sourceType: "Data Import Source",
    sourceKind: "xlsx_sheet",
    moduleKey: "supplier_process_capabilities",
    fileName: workbook.fileName,
    sheetName: sheet.name,
    rowNumber: row.rowNumber,
    domain: "supplier_process_capabilities",
    fields: {
      supplier_name: normalizedSupplierName,
      process_name: normalizedProcessName,
      review_required: true,
      note: "supplier_id 与 process_id 必须分别唯一匹配后再建立能力关系。",
    },
  });
}
...
function addSupplierCandidate(collector, workbook, sheet, row, input) {
  ...
  if (
    (!supplierName && !shortName) ||
    isBadIdentityValue(supplierName || shortName)
  ) {
    return null;
  }
  return collector.addUnique({
    uniqueKey: [
      normalizeText(input.supplierCode),
      supplierName,
      shortName,
      input.preserveSourceRow
        ? `${workbook.fileName}|${sheet.name}|${row.rowNumber}`
        : "",
    ].join("|"),
    sourceType: "Data Import Source",
    sourceKind: "xlsx_sheet",
...
function isPurchaseSummarySheet(sheetName, fileName) {
  return (
    /原辅料采购汇总|^Sheet1$/u.test(sheetName) &&
    /辅材|包材|模板/u.test(fileName)
  );
}
...
function mappedHeadersByAlias(header, aliasKeys) {
  const labels = [];
  const seenIndexes = new Set();
  for (const aliasKey of aliasKeys) {
    const aliases = HEADER_ALIASES[aliasKey] ?? [aliasKey];
    const index = aliases
      .map((alias) => headerIndex(header.map, alias))
      .find((candidate) => candidate !== null);
    if (index === undefined || seenIndexes.has(index)) {
      continue;
    }
    const label = normalizeText(header.row.values[index]);
    if (!label) {
      continue;
    }
...
function extractProductContext(sheet) {
  ...
  return {
    ...
    documentDate: normalizeDateLike(
      findLabeledValue(topRows, /日期|定单日期/u),
    ),
  };
}
...
function buildImportConfigCandidate({
  customerKey,
  generatedAt,
  manifestResult,
  workbooks,
  ...
  return {
    ...
    recommendedImportSequence: [
      ...
      {
        step: 2,
        domains: ["products", "materials", "processes", "suppliers"],
        action: "review_then_import_master_data",
        reason:
      ...
      {
        step: 3,
        domains: ["supplier_process_capabilities"],
        action: "review_after_supplier_process_match",
        reason:
          "供应商和工序两端都必须先唯一匹配，能力关系不能按名称直接写入。",
      },
      {
        step: 4,
        domains: ["contacts", "customer_material"],
      ...
      },
      {
        ...
        reason:
          "采购订单和委外源单据当前仍是 deferred domain；本提取只保留来源快照和字段映射。",
      },
    ],
...
function buildExtractionReport({
  ...
  const lines = [
    ...
    ...importConfig.recommendedImportSequence.map(
      (item) =>
        `${item.step}. ${item.domains.join(", ")}: ${item.action}。${item.reason}`,
    ),
    "",
    "## 边界 / Boundary",
    "",
...
function parseZip(buffer) {
  ...
  assertBufferRange(
    ...
    centralOffset,
    centralSize,
    "Invalid xlsx zip central directory bounds",
  );
  ...
  for (let index = 0; index < entryCount; index += 1) {
    assertBufferRange(
      ...
      offset,
      46,
      "Invalid xlsx zip central directory entry",
    );
    ...
    assertBufferRange(
      ...
      localHeaderOffset,
      30,
      "Invalid xlsx zip local header bounds",
    );
    entries.set(name, {
      name,
      flags,
...
function extractZipEntry(zip, name) {
  ...
  assertBufferRange(
    ...
    "Invalid xlsx zip local entry bounds",
  );
  ...
  if (entry.method === 0) {
    ...
    throw new CliError(
      `Unsupported xlsx zip compression method ${entry.method}`,
      2,
    );
  }
  if (output.length !== entry.uncompressedSize) {
    throw new CliError(
      "XLSX zip entry size does not match its directory record",
      2,
    );
  }
...
function buildExpectedReturnDateCandidate(value) {
  const normalized = normalizeDateLike(value);
  if (!normalized) {
    return {};
  }
  if (/^\d{4}-\d{2}-\d{2}$/u.test(normalized)) {
    return { expected_return_date: normalized };
  }
  ...
  if (calendarDate) {
    ...
    if (
      ...
      return {
        expected_return_date: `${String(year).padStart(4, "0")}-${String(month).padStart(2, "0")}-${String(day).padStart(2, "0")}`,
      };
    }
  }

  return {
    return_date_source_text: normalized,
    return_date_review_required: true,
  };
}

function buildContactPhoneCandidate(value) {
  const phone = normalizeText(value);
  if (!phone) {
    return {};
  }
  const compact = phone.replace(/[\s()\-]/gu, "");
  const mainlandMobile = compact.replace(/^\+?86/u, "");
  if (/^1[3-9]\d{9}$/u.test(mainlandMobile)) {
...
function buildSourceId(input) {
  const hash = createHash("sha256")
    .update(
      `${input.customerKey}|${input.domain}|${input.fileName}|${input.sheetName}|${input.rowNumber}|${JSON.stringify(input.fields)}`,
    )
    .digest("hex")
    .slice(0, 12);
...
async function main() {
  try {
    ...
    console.log(
      `Extracted ${result.summary.sourceCount} source row(s) from ${result.summary.workbookCount} workbook(s).`,
    );
