...
import { OUTPUT_FILES } from './customerSourceSnapshotFreezeCheck.mjs'

const testDir = path.dirname(fileURLToPath(import.meta.url))
const repoRoot = path.resolve(testDir, '../..')
const cliPath = path.join(testDir, 'customerSourceSnapshotFreezeCheck.mjs')
const sourceFixture = path.join(testDir, 'fixtures/customers/yoyoosun/source-snapshot.freeze.sample.json')
const existingFixture = path.join(testDir, 'fixtures/customers/yoyoosun/existing-v1.freeze.sample.json')

test('help 输出可运行', () => {
  const result = runCli(['--help'])
  assert.equal(result.status, 0)
  assert.match(result.stdout, /Customer source snapshot freeze checker/)
...
test('duplicate sourceId 记录 blocker 并锁定 valid=false', async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'customer-freeze-duplicate-'))
...
test('invalid domain 记录 blocker 并锁定 valid=false', async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'customer-freeze-domain-'))
  const sourcePath = path.join(tempDir, 'source.json')
  const outDir = path.join(tempDir, 'out')
...
test('fields 非 object 记录 blocker 并锁定 valid=false', async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'customer-freeze-fields-'))
  const sourcePath = path.join(tempDir, 'source.json')
  const outDir = path.join(tempDir, 'out')
...
test('missing source reference 记录 blocker 并锁定 valid=false', async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'customer-freeze-reference-'))
  const sourcePath = path.join(tempDir, 'source.json')
  const outDir = path.join(tempDir, 'out')
  const row = sourceRow()
  delete row.fileName
...
test('forbidden field 被记录', async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'customer-freeze-forbidden-'))
  const sourcePath = path.join(tempDir, 'source.json')
  const outDir = path.join(tempDir, 'out')
  await writeSource(sourcePath, [
    sourceRow({
...
test('sensitive field 被记录但不输出原始敏感值', async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'customer-freeze-sensitive-'))
  const sourcePath = path.join(tempDir, 'source.json')
  const outDir = path.join(tempDir, 'out')
  await writeSource(sourcePath, [
    sourceRow({
...
test('shipping_released / shipped 混淆被记录', async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'customer-freeze-shipping-'))
  const sourcePath = path.join(tempDir, 'source.json')
  const outDir = path.join(tempDir, 'out')
  await writeSource(sourcePath, [
    sourceRow({
...
test('workflow done / fact posted 风险被记录', async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'customer-freeze-workflow-'))
  const sourcePath = path.join(tempDir, 'source.json')
  const outDir = path.join(tempDir, 'out')
  await writeSource(sourcePath, [
    sourceRow({
...
async function tempOutDir() {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'customer-source-freeze-'))
