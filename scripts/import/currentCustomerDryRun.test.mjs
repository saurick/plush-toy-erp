...
import { OUTPUT_FILES } from './currentCustomerDryRun.mjs'

const testDir = path.dirname(fileURLToPath(import.meta.url))
const repoRoot = path.resolve(testDir, '../..')
const cliPath = path.join(testDir, 'currentCustomerDryRun.mjs')
const sourceFixture = path.join(testDir, 'fixtures/current/source-snapshot.sample.json')
const existingFixture = path.join(testDir, 'fixtures/current/existing-v1.sample.json')

test('help 输出可运行', () => {
  const result = runCli(['--help'])
  assert.equal(result.status, 0)
  assert.match(result.stdout, /Current customer import dry-run tooling/)
...
test('invalid value path 产生 date、quantity 和 money 证据', async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'current-import-invalid-'))
...
test('Print Template Input 与 Industry Template Candidate 不会自动 create/update', async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'current-import-source-type-'))
  const sourcePath = path.join(tempDir, 'source.json')
  const outDir = path.join(tempDir, 'out')
  await writeFile(
    sourcePath,
...
test('--strict-source 缺 source metadata 时返回非 0', async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'current-import-strict-'))
...
async function tempOutDir() {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'current-import-dry-run-'))
