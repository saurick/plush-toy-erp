...
const USAGE = `Current source snapshot freeze checker
...
export async function runFreezeCheck(options) {
  ...
  const command = options.command ?? 'node scripts/import/currentSourceSnapshotFreezeCheck.mjs ...'

  const metadata = {
    freezeId: `current-freeze-${sourceSha256.slice(0, 12)}-${existingSha256.slice(0, 12)}`,
    freezeDate,
    ...
    cli: {
      name: 'currentSourceSnapshotFreezeCheck.mjs',
      command,
      outputFiles: OUTPUT_FILES,
    },
    noRealImport: true,
    canExecuteRealImport: false,
    generatedBy: 'scripts/import/currentSourceSnapshotFreezeCheck.mjs',
...
function renderMarkdownReport({ sourcePath, existingPath, outDir, command, metadata, summary, sourceRows }) {
  ...
  return `# Current Source Snapshot Freeze Check Report
...
3. Keep product_skus and purchase_orders deferred unless a later Goal explicitly changes the boundary.
...
export async function runCli(argv = process.argv.slice(2)) {
  ...
  const command = `node scripts/import/currentSourceSnapshotFreezeCheck.mjs ${argv.map(quoteArg).join(' ')}`
