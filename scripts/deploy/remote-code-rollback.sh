...
write_receipt() {
  ...
  jq -n \
    ...
    '{
      ...
      serviceSwitchStarted: $serviceSwitchStarted,
...
[[ "$(docker image inspect --format '{{.Id}}' "$server_ref")" == "$server_content_id" &&
"$(docker image inspect --format '{{.Id}}' "$web_ref")" == "$web_content_id" ]] ||
