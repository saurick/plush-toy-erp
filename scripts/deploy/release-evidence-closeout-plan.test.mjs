...
const scriptPath = path.join(repoRoot, "scripts/deploy/release-evidence-closeout-plan.mjs");
...
function writeToolScripts(root) {
  const backupScript = path.join(root, "deployments/yoyoosun/scripts/run-backup-restore-rehearsal.sh");
  const smokeScript = path.join(root, "deployments/yoyoosun/scripts/run-smoke.sh");
...
test("closeout plan reports missing local prerequisites for draft evidence", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "release-closeout-plan-missing-"));
  ...
  const immutable = plan.actions.find((action) => action.id === "immutable-version");
  assert.equal(immutable.resolvedInputs.RELEASE_VERSION.value, "20260629T1200-draft");
  assert.equal(immutable.resolvedInputs.RELEASE_VERSION.source, "release-evidence.md");
  assert.equal(immutable.prerequisiteChecks.find((item) => item.id === "RELEASE_VERSION").kind, "evidence");
  assert.equal(
    immutable.missingPrerequisites.some((item) => item.id === "RELEASE_VERSION"),
    false,
  );
  assert.match(immutable.missingPrerequisites.map((item) => item.id).join("\n"), /SERVER_IMAGE/);
  assert.match(immutable.missingPrerequisites.map((item) => item.id).join("\n"), /MIGRATION_BEFORE/);
  assert.equal(immutable.gateSummary.errorCount > 0, true);
  assert.match(immutable.gateSummary.sampleErrors.join("\n"), /release-evidence\.md|image-digests\.txt/);
  assert.match(
  ...
  const preflight = plan.actions.find((action) => action.id === "production-preflight");
  ...
  assert.match(preflight.missingPrerequisites.map((item) => item.message).join("\n"), /env file is missing/);
  const backup = plan.actions.find((action) => action.id === "backup-restore-rehearsal");
  assert.match(backup.missingPrerequisites.map((item) => item.id).join("\n"), /SOURCE_POSTGRES_DSN/);
  assert.equal(
    backup.missingPrerequisites.some((item) => item.id === "RELEASE_VERSION"),
    false,
  );
  assert.equal(backup.resolvedInputs.RELEASE_VERSION.value, "20260629T1200-draft");
});

test("closeout plan marks machine actions runnable when prerequisites are present", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "release-closeout-plan-ready-to-run-"));
  const { evidenceDir } = writeDraftEvidence(root);
  writeToolScripts(root);
  const envFile = "server/deploy/compose/prod/.env";
  fs.mkdirSync(path.join(root, "server/deploy/compose/prod"), { recursive: true });
  fs.writeFileSync(path.join(root, envFile), "APP_IMAGE=registry.example.invalid/plush/server@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\n");
  ...
  assert.equal(plan.actions.find((action) => action.id === "production-preflight").canRun, true);
  assert.equal(plan.actions.find((action) => action.id === "release-signoff").manualOnly, true);
});

test("closeout plan requires target backend and token for customer config effective session", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "release-closeout-plan-config-"));
  ...
  const action = plan.actions.find((item) => item.id === "customer-config-effective-session");
  assert(action);
  assert.match(action.missingPrerequisites.map((item) => item.id).join("\n"), /SMOKE_BACKEND_URL/);
  assert.match(action.missingPrerequisites.map((item) => item.id).join("\n"), /CUSTOMER_CONFIG_ADMIN_TOKEN/);
});

test("closeout plan deduplicates missing prerequisites for composite actions", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "release-closeout-plan-dedupe-"));
  const { evidenceDir, absoluteDir } = writeDraftEvidence(root);
  fs.writeFileSync(
    path.join(absoluteDir, "customer-config-manifest-evidence.json"),
    JSON.stringify(
  ...
  assert.equal(missingIds.filter((id) => id === "RELEASE_ENVIRONMENT").length, 1);
  assert.equal(action.resolvedInputs.RELEASE_VERSION.value, "20260629T1200-draft");
});

test("closeout plan reuses release batch fields from evidence across machine actions", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "release-closeout-plan-evidence-inputs-"));
  const { evidenceDir, absoluteDir } = writeDraftEvidence(root);
  ...
  assert.equal(backup.resolvedInputs.RELEASE_VERSION.source, "release-evidence.md");

  const smoke = plan.actions.find((action) => action.id === "target-smoke");
  assert.equal(smoke.canRun, true);
  assert.equal(smoke.resolvedInputs.RELEASE_VERSION.source, "release-evidence.md");
  assert.equal(smoke.resolvedInputs.RELEASE_ENVIRONMENT.value, "customer-trial");

  const rollback = plan.actions.find((action) => action.id === "rollback-forward-fix");
  assert.equal(rollback.canRun, true);
  assert.equal(rollback.resolvedInputs.RELEASE_ENVIRONMENT.source, "release-evidence.md");
});

test("closeout plan blocks smoke URLs that contain credentials", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "release-closeout-plan-url-"));
  const { evidenceDir } = writeDraftEvidence(root);
  writeToolScripts(root);
...
test("closeout plan CLI JSON can fail on blocked prerequisites", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "release-closeout-plan-cli-"));
  const { evidenceDir } = writeDraftEvidence(root);
  ...
  assert.equal(Array.isArray(payload.actions[0].gateSummary.sampleErrors), true);
  assert.equal(payload.actions[0].gateSummary.errorCount > 0, true);
  assert.match(
    payload.actions.find((action) => action.id === "immutable-version").inputTemplateCommand,
    /immutable-version-evidence\.mjs .*--print-input-template/,
  );
});

test("closeout plan text output includes gate summary for present unverified actions", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "release-closeout-plan-text-"));
  const { evidenceDir } = writeDraftEvidence(root);
