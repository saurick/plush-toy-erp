...
Options:
  ...
  --deployment-target <demo-133|customer-test-133>
                         Required with --evidence-dir; binds release evidence to one target.
...
async function verifyTargetRuntimeIdentity({
  backendURL,
  identity,
  ...
  try {
    response = await fetchImpl(
      ...
      ).toString(),
      {
        ...
        headers: {
          Accept: "text/plain",
          "X-ERP-Runtime-Identity-Scope": identity.scope,
          "X-ERP-Expected-Runtime-Identity-SHA256":
            identity.expectedDigestSha256,
        },
        redirect: "error",
      },
    );
  } catch {
    throw new CliError("target runtime identity preflight failed");
  }

  const proof = response?.headers
    ?.get?.("X-ERP-Runtime-Identity-Proof")
    ?.trim()
    ?.toLowerCase();
  if (!response?.ok || proof !== "matched-v1") {
    throw new CliError("target runtime identity preflight did not match");
  }
...
async function executeOperations({
  ...
  const runtimeIdentityVerification =
    activate || rollback
      ? await verifyTargetRuntimeIdentity({
          backendURL,
          identity: targetRuntimeIdentity,
          fetchImpl,
        })
      : null;
...
async function writeReport({
  ...
  await writeFile(
    ...
    [
      ...
      `| runtimeIdentityVerified | ${runtimeIdentityVerification?.status === "verified"} |`,
...
export async function runCustomerConfigRelease(options, runtime = {}) {
  ...
  return writeReport({
    ...
    runtimeIdentityVerification: results.runtimeIdentityVerification,
