...
DEV-only `/__dev/version-center` 只暴露五个动作：`dispatch-release`、`prepare-promotion`、`execute-promotion`、`prepare-rollback`、`execute-rollback`。浏览器不能提供 repo、workflow、target、路径、SSH、环境变量、shell、SQL 或 Docker 命令；同一时刻只允许一个 test-133 执行器。页面的 CI/CD 效能区只读展示同 workflow 中位数、观测关键路径、失败原因、BuildKit 命中、制品大小和最近传输速率，并可展开 job / step；operation Drawer 展示远端阶段、Server / Web 制品、digest、备份与传输耗时，不建立第二套流水线状态。
