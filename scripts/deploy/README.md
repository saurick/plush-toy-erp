...
固定 `test-133` 的 promotion、rollback 和数据库重建包统一使用 `rsync 3.x over SSH` 传输到 operation 专属 `incoming` 目录。传输仍固定目标、端口、远端 `/usr/bin/rsync`、严格 host key、精确文件白名单、私有文件权限与十分钟超时，不启用 `--delete` 或无收益的压缩；远端执行前继续校验完整 SHA-256。promotion operation 记录白名单文件总字节、实际 rsync 调用耗时和有效吞吐，失败也保留已知传输时间；目标 preflight 会在缺少兼容 rsync 时阻断，不退回 SCP 或其他隐式路径。

DEV-only `/__dev/version-center` 只暴露五个动作：`dispatch-release`、`prepare-promotion`、`execute-promotion`、`prepare-rollback`、`execute-rollback`。浏览器不能提供 repo、workflow、target、路径、SSH、环境变量、shell、SQL 或 Docker 命令；同一时刻只允许一个 test-133 执行器。页面的 CI/CD 效能区按完整发布、相同 SHA 复用和 CI 分别展示中位数，并展示观测关键路径、失败原因、BuildKit 命中、制品大小和最近传输速率；公网入口与 133 SHA 一致性位于首屏。job / step 和 operation 阶段按需展开，中文为主、原始英文仅用于追溯，不建立第二套流水线状态。
