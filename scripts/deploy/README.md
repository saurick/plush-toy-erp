...
3. 先校验并加载制品，再运行本地隔离发布演练。演练只使用 `server/deploy/compose/prod/compose.yml`，本地测试配置许可同时绑定 exact run ID、`plush_erp_release_<run-id>` 与现场 PostgreSQL system identifier；不修改长期 `plush_erp`，也不保留一次性数据库、门禁 identity 或 bootstrap secret。
