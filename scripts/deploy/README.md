...
| `bash deployments/yoyoosun/scripts/rotate-credentials-133.sh` | 在已有备份且目标 release / migration / operation id 精确绑定后，按版本化凭据合同从发布工作站 Keychain 读取彼此独立的管理员 / UAT 岗位外部 secret，经 SSH stdin 原子轮换稳定 admin 与十个 `uat_*`；SMS 手机号仅在已人工录入时绑定，持久 marker 使未知结果可按同 operation id 安全重放                                                                                                                                                                                                                                                                                                                   | 是；仅允许精确 133 目标显式确认后执行                              |
