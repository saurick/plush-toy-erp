...
test("finance field contract reports unexplained blanks, mismatched canonical terms, and invalid audit", () => {
  ...
  assert.ok(
    result.violations.some(
      (item) => item.factNo === "AR-MISMATCH" && item.field === "payment_term",
    ),
  );
  assert.ok(
    result.violations.some(
