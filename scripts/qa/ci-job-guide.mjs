...
const RAW_CI_JOB_GUIDES = [
  ...
  },
  {
    ...
    label: "发布前置合同",
    summary: "验证生产发布前置检查的输入、失败关闭和证据边界。",
    checks: ["生产 preflight 合同", "发布身份与输入约束"],
    outcome: "生成 Node 发布前置分片回执。",
  },
  {
  ...
  {
    name: "quality_resource_contract",
    label: "资源合同检查",
    summary: "验证生产管理员初始化等资源敏感流程的静态合同与场景清单。",
    checks: ["初始化脚本合同", "资源敏感场景清单"],
    outcome: "生成资源合同分片回执。",
  },
  {
    name: "quality_resource_runtime",
    label: "资源运行检查",
    summary: "实际运行资源敏感场景，并核对进程、端口和临时资源清理。",
