...
function buildSalesOrders(prefix, count, customers, products, anchorDate) {
  ...
  return Array.from({ length: count }, (_, offset) => {
    ...
    return {
      ...
      tax_mode: "INCLUSIVE",
      tax_rate: "13",
      freight_terms: index % 4 === 0 ? "INCLUDED" : "EXCLUDED",
      quoted_freight_amount: index % 4 === 0 ? null : "0",
...
async function applyDocumentGroup({
  ...
  for (const record of records) {
    ...
    assertPersistedSourceRecord({
      ...
      decimalFields: headerDecimalFields,
    });
...
async function createSourceDocuments({
  ...
  await applyDocumentGroup({
    ...
    headerDecimalFields: ["tax_rate", "quoted_freight_amount"],
