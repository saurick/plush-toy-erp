...
if [ -f "$ROOT_DIR/scripts/qa/private-deployment-package-closure.test.mjs" ]; then
  echo "[qa:full] 运行多客户私有化复制模拟闭环工具测试"
