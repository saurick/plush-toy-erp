...
const DESKTOP_PLANS = Object.freeze({
  ...
  "production-exceptions": {
    isList: true,
    minimumRecords: 1,
    minimumRecordUnit: "正式生产异常申请及其审批任务",
    keyStates: ["已批准", "超领", "待执行"],
    whatToDo: [
      "你要以老板岗位按正式审批任务编号查找，并打开同一条超领生产异常申请。",
      "你要核对申请数量、批准数量、来源生产订单、审批意见和流程轨迹，再到生产订单查看剩余可领额度。",
    ],
    whatToSee: [
      "应看到异常申请、正式审批任务和 ProcessRuntime 指向同一来源，状态为已批准且批准数量清楚可核对。",
      "应看到超领审批只形成可消费额度，不会自行写领料事实；生产岗位仍须从生产订单显式领料。",
    ],
  },
  "shipping-release": {
    isList: true,
    minimumRecords: 45,
    minimumRecordUnit: "已出货记录对应的正式财务审批任务",
    keyStates: ["已完成"],
    whatToDo: [
      "你要以财务岗位按任务编号、状态和来源出货单查找，打开正式的出货财务审批任务。",
      "你要核对审批意见、办理轨迹和财务放行记录，再到出货单页面核对仓库实际出货结果。",
    ],
    whatToSee: [
      "应看到任务来源、财务主办岗位、审批结果和流程轨迹清楚，并能关联回同一张出货单。",
      "应看到财务审批只记录出货门禁；仓库仍须独立执行实际出货，审批任务不会冒充库存扣减、应收或收付款。",
    ],
  },
  outbound: {
    isList: true,
  ...
  payables: {
    ...
    whatToSee: [
      ...
      "应看到应付过账不等于已经付款；已结清样本能够回到正式付款核销，页面不提供直接结清旁路，取消保留明确时间与结果。",
    ],
  },
  receivables: {
    isList: true,
