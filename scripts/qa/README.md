...
| `bash scripts/qa/fast.sh`                                                                                                           | 高频快速检查，只运行显式 `fast` Node 测试组，并覆盖文档清单、命名边界、客户配置、菜单、Web 静态检查和 server quick                                                                                                                                                                          | 日常开发后                                      |
...
| `bash scripts/qa/prepare-push.sh`                                                                                                   | 对最终 clean HEAD 和真实远端 ref 聚合范围完整执行一次 full；前后状态一致才在 Git common dir 签发 30 分钟回执，避免在已打开的 SSH push 连接里等待 full                                                                                                                                       | commit 后、立即 push 前                         |
...
| `node scripts/qa/test-coverage-collect.mjs --profile baseline --write`                                                              | 在同一仓库身份下运行非数据库 baseline，采集 Go / Web 代码覆盖、显式业务场景、字段联动、导入合同和受影响 T0-T8 状态，再原子写入证据并聚合 latest；运行期身份变化即失败                                                                                                                       | 刷新开发工作台真实覆盖证据前                    |
...
| `bash scripts/qa/strict.sh`                                                                                                         | full 的真实覆盖超集：先运行独有 shell / YAML 检查，再以 strict profile 单次运行 full；扩展视口、零 warning 和严格 govulncheck 各执行一次                                                                                                                                                    | 发版前 / 大改后                                 |
| `bash scripts/qa/full.sh`                                                                                                           | 完整本地检查；一次运行四个显式 Node 测试组，不复跑会由 Web / server 全集覆盖的 fast 子集；另含 secrets、Chromium、存量升级、当前 schema PostgreSQL、前后端测试 / 构建和 govulncheck                                                                                                         | 独立完整诊断、prepare-push 或 strict 内部       |
...
| `.github/workflows/ci.yml`                                                                                                          | 单一 job：pull request 运行 affected，main push 运行 full，手工只允许二者；验证运行可取消同 ref 旧任务，不复制第二套业务规则                                                                                                                                                                | pull request、main push、手工触发               |
| `.github/workflows/release.yml`                                                                                                     | 固定 exact SHA 的独立发布 job；复用或运行一次 strict，构建一次不可变制品并发布 GHCR / GitHub Release；同 SHA 并发不自动取消                                                                                                                                                                 | 明确发布操作                                    |
...
| `node scripts/qa/manual-acceptance-catalog.mjs`                                                                                     | 生成 52 项只读基线验收目录；客户退货与收付款页已进入同一正式目录和长期数据前置合同，默认只输出、不连接后端                                                                                                                                                                                 | 准备全页面试用验收范围时                        |
| `node scripts/qa/local-acceptance-lifecycle.mjs --commit <sha> --run-id <run>`                                                      | 默认输出本地统一生命周期 plan；显式 `--execute` 后在两个按批隔离库串行完成 migration、九岗位数据、52 项只读浏览器与四条真实写异常流，并在成功或失败后停服、删库和读回残留                                                                                                                   | 对 clean exact SHA 做本地完整技术验收时         |
| `node scripts/qa/scenario-demo-data.mjs`                                                                                            | 默认只读输出固定 V5 长期共享库计划；精确 plan digest 和确认串同时匹配后，才在登记 106 开发库稳定准备岗位账号与至少 30 条真实控制面审计样例，通过正式客户配置 API 对齐当前跟踪 revision，再串行准备 Source、ProcessRuntime、九岗位长期工作台模拟任务、旧 `PLAIN5` 任务正式终态化与 Fact。每岗位仍为 20 条，其中 12 条无到期日、低风险并长期留在“待我处理”；旧批次不物理删除，临期相对时间场景留给隔离完整验收。读回区分 42 项查询可证明数据前置、10 项仅浏览器可证明项，以及仍未执行的全部 52 项页面操作 / 人工验收；长期事实矩阵固定覆盖 4 条客户退货、4 条收付款和 3 条红冲。固定本机目标证明后可使用本地开发账号约定，显式服务端环境覆盖优先，不清理、不重置 | 需要为日常联调长期保留固定业务场景数据时        |
...
| `node scripts/qa/manual-acceptance-task-data.mjs --source-report <report> --data-version 2026.07.16-v5 --run-id 20260716-V5`        | 生成九岗位各 20 条、共 180 条仅供列表 / 办理交互的模拟任务；apply 时另从同批 source report 选择 5 张模拟销售订单，走正式 ProcessRuntime 路径形成已启动、待办、阻塞、退回和完成证据。整批仍为 `simulatedOnly`，不代表真实客户数据或 UAT                                                      | 准备岗位任务端数据与流程位置证据前              |
...
| `node scripts/qa/manual-acceptance-readiness.mjs`                                                                                   | 生成 52 项只读就绪核验计划，并校验每页只引用共享 role / source / task / facts / catalog 阶段；显式 `--verify --backend-url` 才查询运行数据                                                                                                                                                  | 写入后核对页面数据是否达到手工验收门槛时        |
| `node scripts/qa/manual-acceptance-browser.mjs --plan --base-url <local-url> --backend-url <local-url>`                             | 生成 52 项本机浏览器验收计划；真实模式只登录、逐页读取和切换只读任务页签，列表及两个数据看板都必须取得当前页面数据证据，不点击业务写动作                                                                                                                                                    | 核对真实账号、页面、岗位端和打印入口时          |
| `node scripts/qa/exception-flow-real-write-browser.mjs ...`                                                                         | 仅在显式确认的全新本地 `browser_actions` 隔离库中，用真实 Chromium 和真实后端办理 Sales Return、Finance Payment、Inventory Adjustment、Production OVER_ISSUE 四条业务写链；它补充 52 项只读页面检查中的真实写动作证据，不是长期场景数据来源或客户 UAT                                         | 异常流主路径完成 API / 单元验证后的本地写验收   |
...
| 文档、命名与真源守卫 | `docs-inventory.test.mjs`、`schema-docs.test.mjs`、`yoyoosun-role-flow-handbook.test.mjs`、`phase-label-boundaries.mjs`、`experimental/canonical-runtime-audit.mjs`                                                                                                                                                                                                                                                                                                                                                                                                              | 前四者阻断路径、数据字典、永绅角色 / 权限 / 流程手册和命名漂移；canonical broad scan 仅为显式非阻断实验审计，不进入 fast / affected，不能替代逐域合同、migration、目标结构读回或 runtime 验证                         |
...
| 本地开发库迁移       | `dev-database-migration-operation-store.mjs`、`../../web/dev-server/devDatabaseMigrationPlugin.mjs`、`../../web/dev-server/devDatabaseMigrationRuntime.mjs`                                                                                                                                                                                                                                                                                                                                                                                                                          | 只包装登记共享开发库的 status / plan / backup-restore / apply / readback / restart；固定意图、幂等、单执行锁、无自动重试，不接受任意目标、命令、SQL 或凭据                                                     |
...
CI action 固定到审核过的 commit，工具链读取 `.n-node-version`、`web/package.json#packageManager` 和 `server/go.mod`。普通 `ci.yml` 对 PR 运行 affected、对 main push 运行 full，只有 full 才执行 Ent / Atlas 零漂移和 committed source archive；普通反馈不默认 strict。独立 `release.yml` 只接受默认分支可达的 exact SHA，先恢复或运行同 fingerprint 的唯一 strict 终态，再构建一次不可变制品。`db-guard` 只是提前阻断的静态启发式，不能替代 Ent / Atlas 零漂移、冻结树 fresh / upgrade 验证或目标环境 evidence。
...
- 数据库迁移操作状态固定留在私有 ignored 目录 `output/dev-workbench/database-migration-operations/**`，备份恢复制品固定留在 `output/dev-workbench/database-migration-backups/**`。公开 operation 不返回内部确认值、PID、命令、DSN 或真实路径；执行器只允许登记共享开发库，使用幂等索引和跨进程排他锁，同一计划只 apply 一次，进程中断或提交结果不明确时先读回而不自动重试。
...
52 个正式验收目标统一登记在 `manual-acceptance-page-data-contract.mjs`。每页只能引用共享的 `role / source / task / facts / catalog` 阶段，不允许页面自带 builder、脚本或另一套 fixture；业务看板可以同时消费多个共享阶段，但不能另造页面数据。`manual-acceptance-dataset-runner.mjs` 直接消费相同的阶段入口。新增页面、probe 或入口发生漏登、重复或分叉时，readiness 合同测试会 fail closed。
...
`readiness` 独立命令保持严格非绿：42 项可查询数据全部通过、5 项模板预览与 5 项打印工作台仍待浏览器时，报告为 `queryChecksPassed=true / queryEvidenceComplete=false` 并退出 1。顶层 dataset runner 只在“0 项查询失败、恰好这 10 项打印目标 `not_proven`、其余 42 项全过”时把数据底座记为已证明，同时明确写入 `browserEvidencePending=true`；任意其他缺口仍立即阻断。浏览器还必须为 5 个预览各证明至少 1 份可见数据，并为每个打印工作台证明本批来源至少 5 条，再打开精确 25 行单据和真实 PDF。最终只有同批浏览器报告的 `acceptancePassed=true` 才能宣称 52 项自动化验收完成。
...
退出批次前必须先 dry-run。该入口只处理没有 active / blocked ProcessRuntime 阻断的源单；当前 V5 特意保留已启动、待办、阻塞和退回流程证据，不能用业务取消伪造流程撤销。需要连同这些流程证据完整清理时，本地应重建专用验收库，133 必须走受控数据库重建 / 恢复流程；不得让本脚本部分执行后冒充整批已退出。其余可处理源单进入取消 / 归档状态，主数据转为停用；已过账库存、出货和财务历史继续保留，不做物理删除。本地和 133 都走同一 target policy，133 仍要求精确确认和带外证明：
...
选择器优先复用同名 `*.test.mjs`；页面、共享布局和样式会提示补定向 `STYLE_L1_SCENARIOS`；业务事实 repo/usecase 会升级到本地隔离 PostgreSQL 关键事务门禁；schema/migration 会运行只读守卫、数据层测试和数据字典零漂移检查，但不会自动执行可能改写生成文件的 `make data`；`server/docs/database/**` 与业务语义 catalog 也会直接运行数据字典检查；部署、全局入口、无独立测试的 QA 脚本和未知路径会保守升级到 `full.sh`。

`affected` 是开发期快速反馈入口，不修改 Git hooks。最终 clean HEAD 先运行 `prepare-push.sh`，再立即按相同 remote/ref 执行 push；pre-push hook 只复核固定位置的短期回执、真实 stdin/range、clean HEAD、gate/environment/TTL，并实时运行 `git log --check` 与逐 range 严格 secrets，不在已打开的远端连接上重跑 full。发版前仍需 `strict.sh` 和目标环境 migration、health/smoke、备份恢复及回滚 evidence。
