...
| `.github/workflows/ci.yml`                                                                                                          | `plan → quality → CI Gate`：先用可信范围扫描候选历史，再按 affected/full 计划条件准备工具、Chromium 和 PostgreSQL；PR schema 路径额外执行 `make data` 零漂移，main push 执行 full；同 ref 旧验证可取消                                                                                       | pull request、main push、手工触发               |
...
CI action 固定到审核过的 commit，工具链读取 `.n-node-version`、`web/package.json#packageManager` 和 `server/go.mod`。普通 `ci.yml` 先在可信 plan job 扫描候选提交历史，再对 PR 运行 affected、对 main push 运行 full；Go、Web、Atlas、Chromium 和 PostgreSQL 均按计划准备，文档类 PR 不启动数据库。full 始终执行 Ent / Atlas 零漂移和 committed source archive；PR 触及 schema、generated migration descriptor 或 Atlas 配置时也先执行 `make data` 并要求零漂移。普通反馈不默认 strict。CI / strict 复用锁文件绑定的 pnpm store、Playwright Chromium、固定版本 Go 工具与经固定 SHA-256 复核的 gitleaks archive；缓存损坏会被丢弃并重新下载，缓存命中不能跳过 checksum、依赖安装合同或任何门禁。独立 `release.yml` 只接受等于最新 `origin/main` 的 exact SHA，先恢复或运行同 fingerprint 且绑定具体 run attempt 的唯一 strict 终态，再以一次 Buildx Bake 共享图并行调度不可变 Server / Web 制品；相同 SHA 已有完整六项 Release 时复用已验证终态，不为不同 SHA 猜测 digest。`db-guard` 只是提前阻断的静态启发式，不能替代 Ent / Atlas 零漂移、冻结树 fresh / upgrade 验证或目标环境 evidence。
