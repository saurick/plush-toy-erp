...
| `bash scripts/qa/prepare-push.sh --full`                                                                                            | 经逐次确认后处理 high-risk 计划或发布候选；完整执行 full，并在前后身份和容器清理读回一致后签发同类短期回执                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | full 已明确确认、发布候选准备时                  |
...
`affected` 既是开发期快速反馈入口，也是普通低风险 push 的最终执行器，但 `prepare-push` 会在 clean HEAD 和真实 aggregate range 上独立重算，不能复用脏树上的旧结果。计划含 full local gate 或 required follow-up 时默认停止，只有逐次显式 `--full` 才运行完整门禁。随后按相同 remote/ref push；hook 只复核固定位置的短期回执、真实 stdin/range、clean HEAD、gate/environment/TTL，并实时运行 `git log --check` 与逐 range 严格 secrets。发版前仍需 `strict.sh` 和目标环境 migration、health/smoke、备份恢复及回滚 evidence。
