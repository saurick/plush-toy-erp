...
| `bash scripts/qa/full.sh`                                                                                                           | 完整本地检查；一次运行四个显式 Node 测试组，不复跑会由 Web / server 全集覆盖的 fast 子集；另含 secrets、Chromium、存量升级、当前 schema PostgreSQL、前后端测试 / 构建和 govulncheck；各高成本阶段只计时一次                                                                                   | 独立完整诊断、prepare-push 或 strict 内部       |
...
| `node scripts/qa/candidate-sha-freeze.mjs --sha <40sha> --terminal <strict-terminal.json>`                                          | 在已通过 strict 的 clean exact HEAD 上固定执行复用/失败/公网读回/DEV 隔离合同，以及版本中心浅色桌面和深色移动端真实浏览器场景；生成单一候选冻结回执，不替代远端 CI、Release 或 133 发布                                                                                                      | 最终候选第一次正式 push 前                      |
...
| `.github/workflows/ci.yml`                                                                                                          | `plan → quality → CI Gate`：先用可信范围扫描候选历史，PR 运行 affected/full；默认分支 exact SHA 完成 strict 后上传来源绑定回执；同 ref 尚未完成且已被新 SHA 取代的旧 CI 可取消                                                                                                                   | pull request、main push、手工触发               |
| `.github/workflows/release.yml`                                                                                                     | 固定 exact SHA 的独立发布 job；优先可信复用同仓库受保护默认分支的 CI strict 回执，仅刷新过期的时效性漏洞检查，再构建一次不可变制品并发布 GHCR / GitHub Release；制品发布开始后不自动取消                                                                                                    | 明确发布操作                                    |
...
| `node scripts/qa/manual-acceptance-catalog.mjs`                                                                                     | 生成 52 项只读基线验收目录；返工回厂与补发、收付款页已进入同一正式目录和长期数据前置合同，默认只输出、不连接后端                                                                                                                                                                           | 准备全页面试用验收范围时                        |
...
| `node scripts/qa/exception-flow-real-write-browser.mjs ...`                                                                         | 仅在显式确认的全新本地 `browser_actions` 隔离库中，用真实 Chromium 和真实后端办理 Rework Intake、Finance Payment、Inventory Adjustment、Production OVER_ISSUE 四条业务写链；它补充 52 项只读页面检查中的真实写动作证据，不是长期场景数据来源或客户 UAT                                        | 异常流主路径完成 API / 单元验证后的本地写验收   |
...
| 质量门禁运行与治理   | `run-gate-with-receipt.mjs`、`dev-quality-gate-operation-store.mjs`、`quality-gate-catalog.mjs`、`../../web/dev-server/devQualityGatePlugin.mjs`                                                                                                                                                                                                                                                                                                                                                                                                    | DEV 页面只接受 `full / strict` 固定动作；正式 runner 与回执仍是唯一执行和结果真源，本地 operation 只保存有界脱敏状态，目录只引用正式来源而不复制命令或测试列表                                               |
...
`/__dev/quality-gates` 通过 development-only Bridge 异步调用 `run-gate-with-receipt.mjs --gate full|strict`。浏览器不能提交命令、参数、路径、数据库、环境变量、Git ref、DSN、凭据或 SSH 目标；服务端固定校验 loopback、same-origin、CSRF、JSON 合同和请求大小，并与 coverage / testing 共用唯一 QA 执行锁。运行、取消和超时均按精确进程组收口，进入 Server 阶段后只有取得一次性数据库 cleanup 读回、进程组消失和正式 receipt 才能标为完全通过；浏览器关闭或开发服务中断不会伪造绿色结果。
