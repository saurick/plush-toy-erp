...
| `node scripts/qa/scenario-demo-data.mjs`                                                                                            | 默认只读输出固定 V5 长期共享库计划；精确 plan digest 和确认串同时匹配后，才在登记 106 开发库通过正式 API 串行准备 Source、ProcessRuntime、模拟岗位任务与 Fact，并按 40 项数据前置 + 10 项待浏览器检查读回；固定本机目标证明后可使用本地开发账号约定，显式服务端环境覆盖优先，不清理、不重置 | 需要为日常联调长期保留固定业务场景数据时        |
