...
Trivy 工具固定版本压缩包经按需代理下载，保留重试与 SHA-256 校验并允许最多 15 分钟；漏洞库固定从官方 `ghcr.io/aquasecurity/trivy-db:2` 获取，避免默认优先连接 Runner 不可达的 Google 镜像站。漏洞库下载与完整扫描共享 30 分钟超时预算，以容纳代理链路的带宽波动；超时、下载不完整、校验不符或扫描失败仍阻断发布。
