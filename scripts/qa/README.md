...
| 覆盖证据             | `erp-field-linkage.mjs`、`test-coverage-collect.mjs`、`test-coverage-report.mjs`、`dev-coverage-operation-store.mjs`                                                                                                                                                                                                                                                                                                                                                                                                                                                              | baseline 只执行明确列出的非数据库本地测试；DEV 一键入口只接受固定 collect intent，以幂等索引和跨进程锁串行执行；业务域只按显式场景 ID 计数，未采集、过期、跳过、阻塞和零执行必须单列，不把历史绿色换算成覆盖率 |
...
- 一键采集操作状态固定留在私有 ignored 目录 `output/dev-workbench/coverage-operations/**`，只公开脱敏阶段投影，不保存或返回 stdout、stderr、PID、命令、幂等 key 和环境变量。跨 Vite 进程共用排他锁；同 key 重试复用同一操作，活动操作期间其他 key 也只能读回当前操作。
...
- baseline 采集器先冻结仓库 identity 与 `affected` 层级，使用仓库锁定的 Node 与 pnpm，实际执行 Go / Web（含 `pretest`）/ import / field-linkage 后再次核对同一 identity 并自动聚合；字段联动 TAP、字段联动报告、baseline evidence 和 candidate latest 全部先写 `output/qa/coverage/.staging/<uuid>/**`，候选报告中的 evidence path 只映射到固定 canonical 路径。候选 schema / repository / staging 泄漏检查和读回通过后，才按 baseline evidence、字段联动 evidence、latest report 的顺序原子提升；latest 提升后的 identity 复核失败会恢复旧 latest。它不写 PostgreSQL、不运行真实业务浏览器、不探测 readiness、不部署、不做 UAT。受影响但未执行的层级必须保留 `missing`，只有未受影响层可写 `not_applicable`。
