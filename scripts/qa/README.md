...
| `bash scripts/qa/full.sh`                                                                                                           | 完整本地检查；在本项目 AUX 段选择空闲端口、串行自启当前 worktree Vite 并运行 Chromium，另含 fast、secrets、历史存量升级与当前 schema PostgreSQL、前后端测试 / 构建和 govulncheck        | 独立完整诊断、prepare-push 或 strict 内部        |
