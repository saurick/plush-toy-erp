...
CI action 固定到审核过的 commit，工具链读取 `.n-node-version`、`web/package.json#packageManager` 和 `server/go.mod`。普通 `ci.yml` 先在可信 plan job 扫描候选提交历史，再对 PR 运行 affected、对 main push 运行 full；Go、Web、Atlas、Chromium 和 PostgreSQL 均按计划准备，文档类 PR 不启动数据库。full 始终执行 Ent / Atlas 零漂移和 committed source archive；PR 触及 schema、generated migration descriptor 或 Atlas 配置时也先执行 `make data` 并要求零漂移。普通反馈不默认 strict。独立 `release.yml` 只接受等于最新 `origin/main` 的 exact SHA，先恢复或运行同 fingerprint 且绑定具体 run attempt 的唯一 strict 终态，再从一个共享 Dockerfile 缓存图各构建一次不可变 Server / Web 制品。`db-guard` 只是提前阻断的静态启发式，不能替代 Ent / Atlas 零漂移、冻结树 fresh / upgrade 验证或目标环境 evidence。
