...
| `node scripts/qa/pnpm-audit-retry.mjs`                                                                                              | 执行固定的 production/high pnpm 审计；只对网络、429 和 5xx 瞬态错误重试一次，单次 100 秒、总计 205 秒封顶；合法漏洞报告不重试，高危或严重漏洞立即阻断，错误只输出有界脱敏诊断                                                                                                                                                                                                  | GitLab security 分片内部；可作本地只读诊断          |
