...
test('mobile workflow runtime browser smoke creates simulated workflow task only', () => {
  ...
  assert.equal(qualityTask.customer_key, 'yoyoosun')
  ...
  assert.equal(warehouseInboundTask.customer_key, 'yoyoosun')
  assert.match(
  ...
  assert.equal(warehouseInboundTask.payload.notification_type, 'inbound_pending')
...
test('mobile workflow runtime browser smoke preflight report is no-write and redacted', async () => {
  ...
  try {
    ...
    assert(report.realSmokeRequires.includes('local backend health is reachable'))
...
test('mobile workflow runtime browser smoke preflight blocks external non-yoyoosun base URL', async () => {
  ...
  process.env.MOBILE_WORKFLOW_BROWSER_SMOKE_BACKEND_HEALTH_URL =
    'http://127.0.0.1:1/healthz'
  process.env.MOBILE_WORKFLOW_BROWSER_SMOKE_PASSWORD = 'present-for-preflight-only'
  process.env.TRIAL_ACCOUNT_PASSWORD = ''
  process.env.ERP_ROLE_DEMO_PASSWORD = ''
...
test('mobile workflow runtime browser smoke report is redacted local evidence only', () => {
  ...
  assert.doesNotMatch(serialized, /"taskGroup"/u)
  assert.doesNotMatch(serialized, /"status"\s*:\s*"(ready|blocked|done|rejected)"/u)
  assert.doesNotMatch(serialized, /"action"\s*:\s*"(block|blocked|complete|done|reject|rejected|urge|urge-only)"/u)
...
test('mobile workflow runtime browser smoke docs keep no-write and report boundary', async () => {
  ...
  const webReadme = await readFile(path.join(repoRoot, 'web/README.md'), 'utf8')
...
test('mobile workflow runtime browser smoke does not contain real import or fact writes', async () => {
  ...
  assert.match(source, /quality done task should retain mobile action evidence ref/u)
  assert.match(source, /warehouse inbound done task should retain mobile action evidence ref/u)
