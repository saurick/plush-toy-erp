...
const DEFAULT_OUT_DIR =
  "output/customers/yoyoosun/phase7-simulated-trial";
const SIMULATION_PREFIX = "SIM-YOYOOSUN-PHASE7";
const CONFIRM_PHRASE = "APPLY_SIMULATED_PHASE7_DATA";
...
const USAGE = `Phase 7 simulated trial data
...
Apply simulated data through V1 JSON-RPC:
  ...
  PHASE7_SIM_PASSWORD='replace-with-password' \\
    node scripts/qa/phase7-simulated-trial-data.mjs \\
      --apply \\
...
Apply credentials:
  ...
  demo_purchase for supplier data. Set PHASE7_SIM_PASSWORD, TRIAL_ACCOUNT_PASSWORD,
  or ERP_ROLE_DEMO_PASSWORD for the shared role-account password.

  Optional overrides:
  PHASE7_SIM_CUSTOMER_USERNAME / PHASE7_SIM_CUSTOMER_PASSWORD
  PHASE7_SIM_SUPPLIER_USERNAME / PHASE7_SIM_SUPPLIER_PASSWORD
  PHASE7_SIM_SALES_USERNAME / PHASE7_SIM_SALES_PASSWORD

  PHASE7_SIM_ADMIN_TOKEN or PHASE7_SIM_ADMIN_USERNAME/PASSWORD may be used only
  when that account really has all required V1 customer, supplier, contact, sales
  order, and sales order item permissions.

This script is Phase 7 simulated/demo tooling only. It never executes real customer import,
...
function parseCliArgs(argv) {
  const options = {
    ...
    help: false,
    out: DEFAULT_OUT_DIR,
    backendURL: process.env.PHASE7_SIM_BACKEND_URL || DEFAULT_BACKEND_URL,
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (FORBIDDEN_ARG_PATTERN.test(token)) {
      throw new CliError(
        `Phase 7 simulated data script refuses real import style flag: ${token}`,
        2,
      );
    }
  ...
  return {
    ...
    records: {
      customer: {
        ...
        name: "Phase 7 模拟客户 001",
        short_name: "模拟客户001",
        tax_no: "SIMULATED-NO-TAX",
        note: "Phase 7 simulated/demo data only; not real customer data.",
      },
      supplier: {
        code: `${SIMULATION_PREFIX}-S001`,
        name: "Phase 7 模拟供应商 001",
        short_name: "模拟供应商001",
        supplier_type: "material",
        tax_no: "SIMULATED-NO-TAX",
        note: "Phase 7 simulated/demo data only; not real supplier data.",
      },
      customerContact: {
        owner_type: "CUSTOMER",
        name: "Phase 7 模拟客户联系人",
        mobile: "13000000001",
        email: "phase7-customer@example.invalid",
        title: "模拟联系人",
        is_primary: true,
        note: "Phase 7 simulated/demo contact only.",
      },
      supplierContact: {
        owner_type: "SUPPLIER",
        name: "Phase 7 模拟供应商联系人",
        mobile: "13000000002",
        email: "phase7-supplier@example.invalid",
        title: "模拟联系人",
        is_primary: true,
        note: "Phase 7 simulated/demo contact only.",
      },
      salesOrder: {
        ...
        note: "Phase 7 simulated/demo sales order only; not shipped and not a fact posting.",
      },
      salesOrderItem: {
        line_no: 1,
        ...
        product_name_snapshot:
          optionalText(options.productName) || "Phase 7 模拟产品",
        color_snapshot: "模拟配色",
        ...
        note: "Phase 7 simulated/demo order item only; requires an existing active product and unit.",
      },
    },
  };
}
...
function assertDatasetBoundary(dataset) {
  if (!dataset?.simulatedOnly || dataset?.realCustomerImport !== false) {
    throw new CliError(
      "Phase 7 dataset must be simulatedOnly and realCustomerImport=false",
    );
  }
  const text = JSON.stringify(dataset);
  if (!text.includes(SIMULATION_PREFIX)) {
    throw new CliError(`Phase 7 dataset must include ${SIMULATION_PREFIX}`);
  }
  for (const forbidden of [
    ...
    if (Object.keys(dataset.records || {}).includes(forbidden)) {
      throw new CliError(
        `Phase 7 dataset contains forbidden record ${forbidden}`,
      );
    }
  }
}
...
function requireApplyInputs(options, dataset) {
  ...
  if (process.env.PHASE7_SIM_CONFIRM !== CONFIRM_PHRASE) {
    throw new CliError(`apply requires PHASE7_SIM_CONFIRM=${CONFIRM_PHRASE}`);
  }
...
async function rpcCall({
  ...
  if (FORBIDDEN_METHOD_PATTERN.test(method)) {
    throw new CliError(`forbidden Phase 7 method ${method}`);
  }
...
function sharedPassword() {
  return (
    process.env.PHASE7_SIM_PASSWORD ||
    process.env.PHASE7_SIM_ADMIN_PASSWORD ||
...
async function resolveAdminToken({ backendURL, fetchImpl }) {
  if (process.env.PHASE7_SIM_ADMIN_TOKEN) {
    return process.env.PHASE7_SIM_ADMIN_TOKEN;
  }
  const username = process.env.PHASE7_SIM_ADMIN_USERNAME;
  const password = sharedPassword();
  if (!username || !password) {
    throw new CliError(
      "apply requires PHASE7_SIM_ADMIN_TOKEN or PHASE7_SIM_ADMIN_USERNAME with PHASE7_SIM_ADMIN_PASSWORD/TRIAL_ACCOUNT_PASSWORD/ERP_ROLE_DEMO_PASSWORD",
    );
  }
...
async function resolveApplyTokens({ backendURL, fetchImpl }) {
  if (process.env.PHASE7_SIM_ADMIN_TOKEN || process.env.PHASE7_SIM_ADMIN_USERNAME) {
  ...
  if (!defaultPassword) {
    throw new CliError(
      "apply requires PHASE7_SIM_PASSWORD/TRIAL_ACCOUNT_PASSWORD/ERP_ROLE_DEMO_PASSWORD for role-account mode, or PHASE7_SIM_ADMIN_TOKEN/PHASE7_SIM_ADMIN_USERNAME for all-permission mode",
    );
  }
  const credentials = {
    customer: {
      username: process.env.PHASE7_SIM_CUSTOMER_USERNAME || "demo_sales",
      password: process.env.PHASE7_SIM_CUSTOMER_PASSWORD || defaultPassword,
    },
    supplier: {
      username: process.env.PHASE7_SIM_SUPPLIER_USERNAME || "demo_purchase",
      password: process.env.PHASE7_SIM_SUPPLIER_PASSWORD || defaultPassword,
    },
    sales: {
      username: process.env.PHASE7_SIM_SALES_USERNAME || "demo_sales",
      password: process.env.PHASE7_SIM_SALES_PASSWORD || defaultPassword,
    },
  };
  const tokens = {};
  const cache = new Map();
...
async function writeReport({ options, dataset, results }) {
  ...
  await writeFile(
    path.join(options.out, "phase7-simulated-trial-report.json"),
    `${JSON.stringify(report, null, 2)}\n`,
    "utf8",
  );
  await writeFile(
    path.join(options.out, "phase7-simulated-trial-report.md"),
    [
      "# Phase 7 Simulated Trial Data Report",
      "",
      `| mode | ${report.mode} |`,
      ...
      "",
      "This is simulated/demo Phase 7 trial data only. It is not customer real data, not real import evidence, and not a shipment, inventory, finance, invoice, payment, receivable, or payable fact posting.",
      "",
    ].join("\n"),
    "utf8",
  );
...
async function runPhase7SimulatedTrialData(options, deps = {}) {
...
async function runCli() {
  ...
  const report = await runPhase7SimulatedTrialData(options);
  console.log(
    `phase7 simulated trial report: ${path.join(
      options.out,
      "phase7-simulated-trial-report.json",
    )}`,
  );
