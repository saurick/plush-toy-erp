...
test("manual acceptance data has single, medium and 25-line documents", () => {
  ...
  for (const key of ["salesOrders", "purchaseOrders", "outsourcingOrders", "bomVersions"]) {
...
test("contacts and production workflow pages cover empty, multiple and state-rich data", () => {
  ...
  assert.equal(report.evidence.taskGroups.production_scheduling, 20);
  assert.equal(report.evidence.taskGroups.production_exception, 20);
});
...
test("attachment matrix requires multiple realistic files and workflow CAS", () => {
  ...
  assert.equal(
    ATTACHMENT_OWNER_SCENARIOS.find((item) => item.ownerType === "workflow_task")
...
test("capacity and stress profiles are isolated from manual acceptance claims", () => {
  ...
  assert.equal(report.pressureTestClaim.capacityProfileRequiresIsolatedDatabase, true);
  assert.equal(report.pressureTestClaim.stressProfileRequiresIsolatedDatabase, true);
  assert(CAPACITY_PROFILES.capacity.workflowTasks > CAPACITY_PROFILES.manual.workflowTasks);
  assert(CAPACITY_PROFILES.stress.concurrentUsers > CAPACITY_PROFILES.capacity.concurrentUsers);
  assert(report.pressureTestClaim.requiredMetrics.includes("p50、p95、p99 响应时间"));
});
