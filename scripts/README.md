...
准备记录按工具检查、迁移计划、真实备份、隔离恢复和升级验证更新当前步骤与事件时间。
备份脚本使用 `printf` 生成说明、配置和报告，避免本机 Bash heredoc 管道写入阻塞，
无需修改全局 Bash 配置。迁移命令超时会先终止所属进程组，宽限期后强制结束残留子进程，
清理完成后才返回失败并释放操作占用；准备超时可重新准备，apply 结果未知仍须先读回。
