...
| `scripts/qa/scenario-demo-data.mjs`                                                                                                                                                                                | 固定 V5 长期场景数据控制器：只允许 106 登记长期开发库和 `127.0.0.1:8300`，默认零认证、零写入地输出绑定仓库、migration、runtime identity、客户配置和组件 digest 的不可变计划；精确确认后按 core reference、账号、Source、ProcessRuntime / 模拟岗位任务、来源驱动 Fact、readiness 串行执行。同批只允许精确创建或读回，不提供清理；40 / 50 只证明数据前置，10 项浏览器检查与人工验收仍待执行                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 需要在共享开发库长期保留可复用业务场景数据时                                                              |
...
- `scenario-demo` 固定为 `yoyoosun-manual-acceptance / 2026.07.16-v5 / 20260716-V5`，只允许 `127.0.0.1:8300` 对应的登记 106 长期开发库。它通过正式 API 准备 Source Document、5 条可证明 ProcessRuntime、模拟岗位任务和来源驱动 Fact；同批只允许精确创建或读回，半批、字段漂移、目标变化或代码变化都会阻断。它不清空已有数据；岗位到期时间是固定 V5 快照，不保证长期维持“今天 / 本周”相对语义。读回只证明 40 / 50 项数据前置，另 10 项浏览器检查和人工验收仍未完成。
...
`scenario-demo` 的日常页面路径不要求先在终端配置凭据：只有在 `127.0.0.1:8300`、登记 106 长期开发库、当前 migration 和 runtime identity 都已证明后，后台才使用项目登记的本机开发账号约定。显式 `MANUAL_ACCEPTANCE_PASSWORD` / `ERP_ROLE_DEMO_PASSWORD` 与 `MANUAL_ACCEPTANCE_ADMIN_PASSWORD` / `REAL_LOGIN_ADMIN_PASSWORD` 覆盖值仍优先，且都不会进入浏览器、命令参数或回执。直接选择“业务场景演示数据”，点击“生成业务场景测试数据”，核对固定目标、V5 批次、数据范围和长期保留边界后点“确认生成”即可；无需手输长确认串，也不需要 `make dev_restart`。只有调整 Vite 进程环境覆盖值时才需重启一次 `pnpm start`。
