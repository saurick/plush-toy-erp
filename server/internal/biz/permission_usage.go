...
func buildBuiltinPermissionUsages() map[string]PermissionUsage {
	...
	add(PermissionSystemRolePermissionManage,
		menuPermissionSurface("permission-center", "role-templates", "角色模板", "save-role-settings", "保存岗位设置", permissionControlButton, "一次保存可委派业务权限、数据范围和菜单布局", permissionMethods("admin", "set_role_settings"), systemUsageConditions),
	)
	...
	addCRUD(PermissionSupplierRead, PermissionSupplierCreate, PermissionSupplierUpdate, PermissionSupplierDisable, "suppliers", "suppliers", "供应商与加工厂", "供应商", "masterdata", []string{"get_supplier", "list_suppliers"}, []string{"create_supplier", "save_supplier_with_contacts"}, []string{"update_supplier", "save_supplier_with_contacts"}, []string{"set_supplier_active"})
	addCRUD(PermissionMaterialRead, PermissionMaterialCreate, PermissionMaterialUpdate, PermissionMaterialDisable, "materials", "materials", "材料档案", "材料", "masterdata", []string{"get_material", "list_materials", "list_material_warehouses"}, []string{"create_material"}, []string{"update_material"}, []string{"set_material_active"})
	...
	addMenu(PermissionEngineeringMaterialFinanceApprove, "sales-orders", "sales-order-actions", "销售订单", "engineering-material-finance_approve", "审核批准工程采购", permissionControlForm, "审核批准工程采购", permissionMethods("sales_order", "finance_review_engineering_material_request"), businessUsageConditions)
	addMenu(PermissionSalesOrderEngineeringUpdate, "sales-orders", "sales-order-actions", "销售订单", "sales-order-engineering", "工程资料与打样", permissionControlForm, "显示并允许维护工程资料和打样进度", permissionMethods("sales_order", "save_sales_order_engineering"), businessUsageConditions)
	...
	addMenu(PermissionSalesOrderUpdate, "sales-orders", "sales-orders", "销售订单", "edit-sales-order", "编辑销售订单和表单", permissionControlForm, "显示并允许编辑", permissionMethods("sales_order", "save_sales_order_with_items", "reorder_sales_order_items"), businessUsageConditions)
	...
	for index := range workflowTaskReadSurfaces {
		if workflowTaskReadSurfaces[index].PageKey == "global-dashboard" {
			workflowTaskReadSurfaces[index].BackendMethods = append(
				workflowTaskReadSurfaces[index].BackendMethods,
				permissionMethods("workflow", "get_workbench", "list_workbench_role_tasks")...,
			)
		}
	}
	...
	purchaseOrderUpdateMethods := permissionMethods("purchase_order", "save_purchase_order_with_items", "reorder_purchase_order_items")
	...
	addMenu(PermissionOutsourcingOrderUpdate, "processing-contracts", "outsourcing-orders", "委外订单", "edit-outsourcing-order", "编辑委外订单和表单", permissionControlForm, "显示并允许编辑", permissionMethods("outsourcing_order", "save_outsourcing_order_with_items", "reorder_outsourcing_order_items"), businessUsageConditions)
	...
	purchaseReceiptCreateMethods := append(permissionMethods("purchase", "create_purchase_receipt_from_purchase_order", "add_purchase_receipt_item"), permissionMethods("customer_config", "execute_material_supply_purchase_receipt_create")...)
	addMenu(PermissionPurchaseReceiptCreate, "accessories-purchase", "purchase-order-actions", "采购到货", "create-purchase-receipt", "登记来料实点数量", permissionControlForm, "显示并允许登记", purchaseReceiptCreateMethods, businessUsageConditions)
	addMenu(PermissionPurchaseReceiptCancelDraft, "inbound", "purchase-receipt-actions", "采购到货", "cancel-purchase-receipt-draft", "取消未入库到货登记", permissionControlButton, "显示并允许取消未入库记录", permissionMethods("purchase", "cancel_purchase_receipt_draft"), businessUsageConditions)
	...
	addBackend(PermissionPurchaseReturnRead, permissionMethods("purchase", "get_purchase_return", "list_purchase_returns", "get_purchase_rejection_disposition", "list_purchase_rejection_dispositions"), businessUsageConditions)
	add(PermissionPurchaseReturnCreate,
		...
		menuPermissionSurface("quality-inspections", "quality-actions", "质检动作", "create-purchase-return-from-quality-inspection", "退供应商", permissionControlButton, "显示并允许生成采购退货", permissionMethods("purchase", "create_purchase_return_from_quality_inspection", "create_purchase_rejection_disposition"), businessUsageConditions),
	)
	addMenu(PermissionPurchaseReturnPost, "inbound", "purchase-returns", "采购退货", "post-purchase-return", "确认采购退货", permissionControlButton, "显示并允许确认", permissionMethods("purchase", "post_purchase_return", "post_purchase_rejection_disposition"), businessUsageConditions)
	addMenu(PermissionPurchaseReturnCancel, "inbound", "purchase-returns", "采购退货", "cancel-purchase-return", "取消采购退货", permissionControlButton, "显示并允许取消", permissionMethods("purchase", "cancel_purchase_return", "cancel_purchase_rejection_disposition"), businessUsageConditions)

	// Warehouse and shipment.
	addMenu(PermissionWarehouseManage, "inventory", "warehouse-settings", "仓库设置", "warehouse-settings", "仓库名称、类型和启停", permissionControlButton, "显示并允许维护", permissionMethods("masterdata", "create_warehouse", "update_warehouse"), businessUsageConditions)
	...
	add(PermissionWarehouseInboundConfirm,
		menuPermissionSurface("inbound", "purchase-receipt-actions", "采购入库", "confirm-inbound", "确认或取消采购入库", permissionControlButton, "显示并允许确认", inboundConfirmMethods, businessUsageConditions),
		menuPermissionSurface("production-progress", "production-fact-actions", "生产记录与成品入库", "confirm-finished-goods-inbound", "核对并确认成品入库", permissionControlButton, "显示并允许核对、确认或撤销", permissionMethods("operational_fact", "save_production_completion_draft", "post_production_fact", "cancel_production_fact"), businessUsageConditions),
	)
	...
	addMenu(PermissionWarehouseAdjustmentCreate, "inventory", "inventory-operations", "库存作业", "manage-inventory-operation", "盘点、调拨与受控人工调整", permissionControlButton, "显示并允许办理", warehouseAdjustmentCreateMethods, businessUsageConditions)
	addMenu(PermissionWarehouseAdjustmentApprove, "inventory", "inventory-adjustment-approval", "库存作业", "approve-inventory-adjustment", "审批人工库存调整", permissionControlButton, "显示并允许审批", append(permissionMethods("inventory", "get_inventory_operation", "list_inventory_operations"), permissionMethods("workflow", "complete_task_action")...), businessUsageConditions)
	...
	addMenu(PermissionShipmentUpdate, "shipments", "shipments", "出货单", "update-shipment", "编辑尚未进入质检或审批的出货草稿", permissionControlButton, "显示并允许编辑", permissionMethods("operational_fact", "list_shipment_source_candidates", "save_shipment_draft"), businessUsageConditions)
	...
	qualityInspectionReadMethods := append(
		permissionMethods("quality", "get_quality_inspection", "list_quality_inspections", "list_finished_goods_quality_inspections"),
		permissionMethods("customer_config", "start_production_exception_approval_process", "get_production_exception_approval_process")...,
	)
	...
	add(PermissionQualityExceptionHandle,
	...
	add(PermissionFinancePayableConfirm,
		menuPermissionSurface("payables", "payable-actions", "应付动作", "confirm-payable", "确认或取消应付", permissionControlButton, "显示并允许处理", permissionMethods("operational_fact", "post_finance_fact", "cancel_finance_fact"), businessUsageConditions),
	...
	add(PermissionFinanceReceivableRead,
		menuPermissionSurface("receivables", "receivables", "应收管理", "receivable-list", "应收列表和详情", permissionControlPage, "允许进入并查看", append(permissionMethods("operational_fact", "list_finance_facts"), permissionMethods("customer_config", "execute_finance_payment_post")...), businessUsageConditions),
	...
	addMenu(PermissionFinanceReceivableConfirm, "receivables", "receivable-actions", "应收动作", "confirm-receivable", "生成、确认或取消应收", permissionControlButton, "显示并允许处理", permissionMethods("operational_fact", "create_receivable_from_shipment", "post_finance_fact", "cancel_finance_fact"), businessUsageConditions)
	...
	addBackend(PermissionFinanceReportRead, nil, append(append([]string{}, businessUsageConditions...), "报表权限不直接开放应收、应付、发票或对账明细"))
	financePaymentReadMethods := append(
		permissionMethods("operational_fact", "list_finance_payments", "get_finance_payment", "list_finance_credit_notes", "get_finance_credit_note"),
		permissionMethods("customer_config", "start_finance_payment_approval_process", "get_finance_payment_approval_process", "execute_finance_payment_post")...,
	)
	addMenu(PermissionFinancePaymentRead, "finance-payments", "finance-payments", "收付款核销", "finance-payment-list", "收付款单与红冲记录列表、详情", permissionControlPage, "允许进入并查看", financePaymentReadMethods, businessUsageConditions)
	addMenu(PermissionFinancePaymentCreate, "finance-payments", "finance-payment-actions", "收付款动作", "create-finance-payment", "创建真实收付款单、启动审批或按生命周期约束取消", permissionControlButton, "显示并允许创建或取消", append(permissionMethods("operational_fact", "create_finance_payment", "cancel_finance_payment"), permissionMethods("customer_config", "start_finance_payment_approval_process")...), businessUsageConditions)
	addMenu(PermissionFinancePaymentApprove, "finance-payments", "finance-payment-actions", "收付款动作", "approve-finance-payment", "审批或驳回收付款", permissionControlButton, "显示并允许审批", permissionMethods("workflow", "complete_task_action"), businessUsageConditions)
	addMenu(PermissionFinancePaymentPost, "finance-payments", "finance-payment-actions", "收付款动作", "post-finance-payment", "审批后过账并进行多单核销", permissionControlButton, "显示并允许处理", permissionMethods("customer_config", "execute_finance_payment_post"), businessUsageConditions)
	addMenu(PermissionFinancePaymentReverse, "finance-payments", "finance-payment-actions", "收付款动作", "reverse-finance-payment", "冲正已过账收付款", permissionControlButton, "显示并允许处理", permissionMethods("operational_fact", "reverse_finance_payment"), businessUsageConditions)
	addMenu(PermissionFinanceCreditNoteCreate, "finance-payments", "finance-credit-note-actions", "财务红冲", "create-finance-credit-note", "创建应收应付红冲单", permissionControlButton, "显示并允许创建", permissionMethods("operational_fact", "create_finance_credit_note"), businessUsageConditions)
	addMenu(PermissionFinanceCreditNoteReverse, "finance-payments", "finance-credit-note-actions", "财务红冲", "reverse-finance-credit-note", "反向红冲错误红冲单", permissionControlButton, "显示并允许处理", permissionMethods("operational_fact", "reverse_finance_credit_note"), businessUsageConditions)
	...
	productionWIPReadMethods = append(productionWIPReadMethods, permissionMethods("customer_config", "start_production_exception_approval_process", "get_production_exception_approval_process", "execute_production_exception_process")...)
	...
	addMenu(PermissionProductionWIPAssign, "production-orders", "production-wip", "工序办理", "assign-production-wip", "拆分、取消批次并安排本厂或外发", permissionControlButton, "显示并允许安排", permissionMethods("production_wip", "execute_production_wip_action", "prepare_production_outsourcing_order"), businessUsageConditions)
	...
	addMenu(PermissionProductionCompletionCreate, "production-orders", "production-order-actions", "生产动作", "create-production-completion", "登记生产完工", permissionControlButton, "显示并允许登记", permissionMethods("operational_fact", "create_production_completion_from_order", "save_production_completion_draft"), businessUsageConditions)
	addMenu(PermissionProductionMaterialIssueCreate, "production-orders", "production-order-actions", "生产动作", "create-production-material-issue", "登记生产领料", permissionControlButton, "显示并允许登记", permissionMethods("operational_fact", "create_production_material_issue_from_order", "save_production_material_issue_draft"), businessUsageConditions)
