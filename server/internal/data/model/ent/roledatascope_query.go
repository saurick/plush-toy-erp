...
func (_q *RoleDataScopeQuery) QueryRole() *RoleQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(roledatascope.Table, roledatascope.FieldID, selector),
...
func (_q *RoleDataScopeQuery) WithRole(opts ...func(*RoleQuery)) *RoleDataScopeQuery {
	query := (&RoleClient{config: _q.config}).Query()
...
//		GroupBy(roledatascope.FieldRoleID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.RoleDataScope.Query().
//		Select(roledatascope.FieldRoleID).
//		Scan(ctx, &v)
...
func (_q *RoleDataScopeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*RoleDataScope, error) {
	...
	if query := _q.withRole; query != nil {
		if err := _q.loadRole(ctx, query, nodes, nil,
			func(n *RoleDataScope, e *Role) { n.Edges.Role = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RoleDataScopeQuery) loadRole(ctx context.Context, query *RoleQuery, nodes []*RoleDataScope, init func(*RoleDataScope), assign func(*RoleDataScope, *Role)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*RoleDataScope)
	for i := range nodes {
		fk := nodes[i].RoleID
...
func (_q *RoleDataScopeQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withRole != nil {
			_spec.Node.AddColumnOnce(roledatascope.FieldRoleID)
		}
	}
