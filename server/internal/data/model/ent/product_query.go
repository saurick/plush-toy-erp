...
func (_q *ProductQuery) QueryDefaultUnit() *UnitQuery {
...
// QueryBomHeaders chains the current query on the "bom_headers" edge.
func (_q *ProductQuery) QueryBomHeaders() *BOMHeaderQuery {
...
func (_q *ProductQuery) WithDefaultUnit(opts ...func(*UnitQuery)) *ProductQuery {
	query := (&UnitClient{config: _q.config}).Query()
...
// WithBomHeaders tells the query-builder to eager-load the nodes that are connected to
// the "bom_headers" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ProductQuery) WithBomHeaders(opts ...func(*BOMHeaderQuery)) *ProductQuery {
	query := (&BOMHeaderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if query := _q.withBomHeaders; query != nil {
...
func (_q *ProductQuery) loadDefaultUnit(ctx context.Context, query *UnitQuery, nodes []*Product, init func(*Product), assign func(*Product, *Unit)) error {
...
func (_q *ProductQuery) loadBomHeaders(ctx context.Context, query *BOMHeaderQuery, nodes []*Product, init func(*Product), assign func(*Product, *BOMHeader)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(bomheader.FieldProductID)
	}
...
func (_q *ProductQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withDefaultUnit != nil {
			_spec.Node.AddColumnOnce(product.FieldDefaultUnitID)
		}
	}
