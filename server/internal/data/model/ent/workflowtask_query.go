...
func (_q *WorkflowTaskQuery) QueryEvents() *WorkflowTaskEventQuery {
	query := (&WorkflowTaskEventClient{config: _q.config}).Query()
...
func (_q *WorkflowTaskQuery) WithEvents(opts ...func(*WorkflowTaskEventQuery)) *WorkflowTaskQuery {
	query := (&WorkflowTaskEventClient{config: _q.config}).Query()
...
//		GroupBy(workflowtask.FieldTaskCode).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.WorkflowTask.Query().
//		Select(workflowtask.FieldTaskCode).
//		Scan(ctx, &v)
...
func (_q *WorkflowTaskQuery) loadEvents(ctx context.Context, query *WorkflowTaskEventQuery, nodes []*WorkflowTask, init func(*WorkflowTask), assign func(*WorkflowTask, *WorkflowTaskEvent)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(workflowtaskevent.FieldTaskID)
	}
	query.Where(predicate.WorkflowTaskEvent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(workflowtask.EventsColumn), fks...))
	}))
