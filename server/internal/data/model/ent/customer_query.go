...
// QuerySalesOrders chains the current query on the "sales_orders" edge.
func (_q *CustomerQuery) QuerySalesOrders() *SalesOrderQuery {
...
// WithSalesOrders tells the query-builder to eager-load the nodes that are connected to
// the "sales_orders" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *CustomerQuery) WithSalesOrders(opts ...func(*SalesOrderQuery)) *CustomerQuery {
	query := (&SalesOrderClient{config: _q.config}).Query()
...
func (_q *CustomerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Customer, error) {
	...
	if query := _q.withSalesOrders; query != nil {
...
func (_q *CustomerQuery) loadSalesOrders(ctx context.Context, query *SalesOrderQuery, nodes []*Customer, init func(*Customer), assign func(*Customer, *SalesOrder)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(salesorder.FieldCustomerID)
	}
	query.Where(predicate.SalesOrder(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(customer.SalesOrdersColumn), fks...))
	}))
