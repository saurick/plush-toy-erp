...
func postOutsourcingDispositionRework(ctx context.Context, tx *inventoryDBTx, row *ent.OutsourcingReturnDisposition, actorID int) error {
	...
	_, err = createProductionWIPChildBatch(ctx, tx.client, batchRow, operationRow.ID, batchNo, biz.ProductionWIPFlowRework, row.Quantity, actorID, &row.Reason)
	return err
}
...
func entOutsourcingDispositionToBiz(row *ent.OutsourcingReturnDisposition) *biz.OutsourcingReturnDisposition {
	...
	return &biz.OutsourcingReturnDisposition{ID: row.ID, DispositionNo: row.DispositionNo, QualityInspectionID: row.QualityInspectionID, OutsourcingReturnFactID: row.OutsourcingReturnFactID, DispositionType: row.DispositionType, Status: row.Status, Quantity: row.Quantity, ProductionWIPBatchID: row.ProductionWipBatchID, Reason: row.Reason, PostedAt: row.PostedAt, PostedBy: row.PostedBy, CancelledAt: row.CancelledAt, CancelledBy: row.CancelledBy, CancelReason: row.CancelReason, CreatedBy: row.CreatedBy, Version: row.Version}
}
