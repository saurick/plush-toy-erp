...
func TestLoadAdminRBACUsesRolePermissionSource(t *testing.T) {
	...
	for _, stmt := range []string{
		`CREATE TABLE roles (
			...
			name text not null,
				...
				secondary_menu_paths text not null default '[]'
			)`,
		...
		`INSERT INTO roles (id, role_key, name, role_type, disabled, sort_order, version, navigation_mode, primary_menu_paths, secondary_menu_paths) VALUES
				(1, 'purchase', '采购', 'business_default', false, 10, 4, 'custom', '["/erp/purchase/accessories"]', '["/erp/master/partners/suppliers"]'),
	...
	if admin.Roles[0].NavigationMode != biz.RoleNavigationModeCustom ||
		...
		len(admin.Roles[0].SecondaryMenuPaths) != 1 ||
		admin.Roles[0].SecondaryMenuPaths[0] != "/erp/master/partners/suppliers" {
...
func TestLoadAdminRBACSuperAdminGetsAllPermissions(t *testing.T) {
	...
	if _, err := db.ExecContext(context.Background(), `CREATE TABLE roles (id integer primary key, role_key text not null, name text not null, description text not null default '', builtin boolean not null default false, role_type text not null default 'custom', disabled boolean not null default false, sort_order integer not null default 0, version integer not null default 1, navigation_mode text not null default 'recommended', primary_menu_paths text not null default '[]', secondary_menu_paths text not null default '[]')`); err != nil {
