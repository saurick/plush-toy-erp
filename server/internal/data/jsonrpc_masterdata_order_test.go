...
func newMasterDataJSONRPCTestData(repo *stubMasterDataJSONRPCRepo, admin *biz.AdminUser) *JsonrpcData {
	return &JsonrpcData{
		log:          log.NewHelper(log.With(log.NewStdLogger(io.Discard), "module", "data.jsonrpc.masterdata.test")),
		adminReader:  stubAdminAccountReader{admin: admin},
		masterDataUC: biz.NewMasterDataUsecase(repo),
	}
}

func newSalesOrderJSONRPCTestData(repo *stubSalesOrderJSONRPCRepo, admin *biz.AdminUser) *JsonrpcData {
	return &JsonrpcData{
		log:          log.NewHelper(log.With(log.NewStdLogger(io.Discard), "module", "data.jsonrpc.sales_order.test")),
		adminReader:  stubAdminAccountReader{admin: admin},
		salesOrderUC: biz.NewSalesOrderUsecase(repo),
	}
}

func TestJsonrpcData_MasterDataCreateCustomerRequiresAdminAndPermission(t *testing.T) {
...
func TestJsonrpcData_ContactAPIUsesUsecaseOwnerGuard(t *testing.T) {
...
func TestJsonrpcData_SalesOrderItemAPIUsesUsecaseProductUnitGuard(t *testing.T) {
...
func TestJsonrpcData_RBACIncludesV1MasterDataAndOrderPermissions(t *testing.T) {
