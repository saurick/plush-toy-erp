...
func salesReturnToMap(item *biz.SalesReturn) map[string]any {
	...
	for _, line := range item.Items {
		items = append(items, map[string]any{"id": line.ID, "line_no": line.LineNo, "shipment_item_id": line.ShipmentItemID, "product_id": line.ProductID, "product_code": line.ProductCode, "product_name": line.ProductName, "product_sku_id": optionalIntValue(line.ProductSkuID), "product_sku_code": optionalStringValue(line.ProductSkuCode), "product_sku_name": optionalStringValue(line.ProductSkuName), "warehouse_id": line.WarehouseID, "warehouse_code": line.WarehouseCode, "warehouse_name": line.WarehouseName, "unit_id": line.UnitID, "unit_code": line.UnitCode, "unit_name": line.UnitName, "lot_id": optionalIntValue(line.LotID), "lot_no": optionalStringValue(line.LotNo), "quality_inspection_id": line.QualityInspectionID, "current_quality_inspection_id": line.CurrentQualityInspectionID, "current_quality_inspection_no": line.CurrentQualityInspectionNo, "current_quality_inspection_status": line.CurrentQualityInspectionStatus, "current_quality_inspection_result": optionalStringValue(line.CurrentQualityInspectionResult), "quantity": line.Quantity.String(), "source_shipped_quantity": line.SourceShippedQuantity.String(), "active_returned_quantity": line.ActiveReturnedQuantity.String(), "remaining_returnable_quantity": line.RemainingReturnableQuantity.String(), "condition": line.Condition, "note": optionalStringValue(line.Note)})
	}
	return map[string]any{"id": item.ID, "return_no": item.ReturnNo, "shipment_id": item.ShipmentID, "shipment_no": item.ShipmentNo, "customer_id": item.CustomerID, "customer_name": item.CustomerNameSnapshot, "status": item.Status, "reason": item.Reason, "version": item.Version, "approved_at": optionalTimeUnix(item.ApprovedAt), "approved_by": optionalIntValue(item.ApprovedBy), "rejected_at": optionalTimeUnix(item.RejectedAt), "rejected_by": optionalIntValue(item.RejectedBy), "reject_reason": optionalStringValue(item.RejectReason), "received_at": optionalTimeUnix(item.ReceivedAt), "received_by": optionalIntValue(item.ReceivedBy), "cancelled_at": optionalTimeUnix(item.CancelledAt), "cancelled_by": optionalIntValue(item.CancelledBy), "cancel_reason": optionalStringValue(item.CancelReason), "reversed_at": optionalTimeUnix(item.ReversedAt), "reversed_by": optionalIntValue(item.ReversedBy), "reverse_reason": optionalStringValue(item.ReverseReason), "created_by": item.CreatedBy, "created_at": item.CreatedAt.Unix(), "items": items}
}
