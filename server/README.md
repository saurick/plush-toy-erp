...
任务动作终态不等于来源事实终态。排程完成只把业务投影推进到生产执行中，生产异常完成只记录异常已安排处理，历史 `shipment_release` 协同任务完成只推进到 `shipping_released`；都不代写生产、质检、库存、出货或财务 Fact。新的出货财务 approval 完成后由绑定领域命令写 Shipment `APPROVED` 放行门禁，但仍不等于 `SHIPPED`、库存 `OUT`、应收或开票。来源后续关闭、取消或真实出货时，领域事务会继续把同一来源投影更新为 `closed / cancelled / shipped`，并追加来源状态事件，但不会修改既有 task status 或伪造历史 `done / rejected`。
...
`get_task_board` 是 `workflow.task.read` 下的任务看板只读投影。它在同一查询快照中先应用 owner role / assignee 可见性与关键词、状态、到期、来源筛选，再返回全量 `total`、互斥的 `actionable / exception / due / finished` 计数和每栏有界分页。`source_types` 是稳定的来源候选 facet，只受任务可见性与显式 owner role 范围约束，不受当前关键词、状态、到期或来源值影响，避免选中来源后下拉候选塌缩。`exception` 只包含 `blocked`；`rejected` 在生命周期中仍是 settled，不进入 due 且不可再操作，与 `done` 一起只读归入 `finished`。默认每栏最多返回 5 条，单栏聚焦页最多 50 条；`lane_key` 只改变返回的分页栏，不改变完整 `total / counts / source_types`。未知状态会 fail closed，不会被静默漏计。
...
linked ProcessRuntime 对账在任务已提交后失败时返回可重试未知结果，客户端必须保留同一 attempt 再取 receipt。前端会为一次用户 intent 冻结业务参数、`expected_version` 和安全 UUID；HTTP 408、网络中断、5xx 或结构不合法的 success response 都保留同一 key，任务抽屉、原因和证据也保持可重试。同一 task 的正式动作使用同步 in-flight lease 在请求发出前互斥，避免完成 / 阻塞 / 退回 / 催办跨动作双发；`version / idempotency_key / intent_hash` 均为内部技术字段，不面向业务用户展示。服务启动后还会立即执行并每 30 秒重复一次收窄对账：每批最多 100 条，只选择 active ProcessInstance 下已 `done / rejected`、原处理人已持久化而关联 `human_task / approval` 节点仍 `active` 的任务，使用原处理人调用同一幂等结算路径。扫描按 Workflow task ID 游标前进，扫到末尾后从头复核；单条失败仍推进批次游标，避免固定首批长期占满，同时只记录 task / process / node ID 并继续同批其它记录。它不扫描 `domain_command` durable result、不重放领域副作用、不处理逾期升级。Go 与 JS 已共同消费共享 intent vectors，mixed evidence、raw whitespace key 和 relations 由同一 golden 锁定；定向结果不能外推为 final full/strict 或页面级浏览器回归（Style L1）已完成。
...
`system.version` 是公开只读的运行身份回读，返回构建时注入的人工发布版本、完整与短 Git SHA，以及是否具备正式身份；不返回运行配置、凭据或客户数据。正式 Web 静态包和 Server / Web 镜像必须绑定同一 `RELEASE_VERSION + GIT_SHA`，后台和手机端据此提示一致、缺失或不一致；未注入身份的本地进程只报告 `local`，不能作为发布或目标部署证据。
...
以上方法是服务端公开能力清单，不等于正式 Web 页面已全部可达。销售订单页和采购订单页都先调用对应 start，再显式执行各自首个 Source Document submit command；出货页只启动 finished-goods-delivery 财务审批。财务放行不再提供公开 execute，由 approval 对账触发内部自动命令；旧长图 execute 只服务冻结在途 revision，不是当前正式页面入口。
...
管理员补偿恢复的上下文读取与写入共用同一失败关闭范围：只接受收付款、库存人工调整和生产异常三类 process key，且冻结快照必须全部为 `attempt=1`，不能包含 branch、fan-out、join 或 returnTo 等非线性 policy。当前分支图不会交给线性恢复算法猜测处理，需人工核对；这与上述 Workflow 终态到 active 人工 / 审批节点的周期对账是两条独立边界。
...
这组接口使用 `shipment.read / shipment.create / shipment.ship / shipment.cancel` 动作权限；来源候选以及绑定销售订单或订单行的新建还同时要求 `sales_order.read / sales_order_item.read`。`list_shipment_source_candidates` 是服务端唯一的可出货来源候选合同。公开 `create_shipment / add_shipment_item / submit_shipment_release` 已退出；页面通过 `customer_config.start_finished_goods_delivery_process` 启动 active revision。启动事务先重验已有非取消成品质检，随后直接生成财务审批；审批通过自动执行 `shipment.finance_release`，在同一领域事务内 CAS 写入 `finance_release_status / version / actor / process instance / node / note` 和 durable command result。流程不创建或判定质检，也不执行出货或创建应收。

`ship_shipment` 才把出货单推进到 `SHIPPED` 并写库存 `OUT`。服务层和数据层都会要求 Shipment 当前 `finance_release_status=APPROVED`，并继续校验质检、销售来源数量、库存预留和可用量；当前正式页面与旧在途流程恢复共用这一不可绕过门禁。任何一项失败都不写出货或库存。
...
当前新建成品交付图由 manifest 编译为 `finished_goods_delivery / shipment_finance_approval`。财务同意走 `shipment_finance_release → end`；财务拒绝走命名领域命令 `shipment_finance_reject → shipment_finance_rejected_end`，原子把版本化放行门禁写为 `REJECTED` 并保留原因、actor 与流程锚点。`start_finished_goods_delivery_process` 要求 `shipment.create + shipment.read`，只从 active customer config 显式创建并启动 shipment 业务引用的 ProcessInstance；来源事务会在创建前重验 Shipment 状态和已有成品质检，但不创建、不提交、不判定 Quality Fact。首节点直接创建财务 `workflow.task.approve`，没有后端隐藏的质检、出货或应收节点。

`execute_finished_goods_delivery_quality_decide / execute_finished_goods_delivery_shipment_ship / execute_finished_goods_delivery_receivable_lead` 只为已冻结的旧在途 `quality_finance_ship_receivable` revision 保留恢复能力；当前 manifest schema 不再接受该 variant 新建流程，正式 Web 页面也不调用这些 execute 方法。成品质检走 quality 页面，`ship_shipment` 走出货页面，应收 / 发票 / 收付款走 finance 页面；旧 handler 不能成为绕过这些页面和事务边界的旁路。

财务审批同意后，`shipment.finance_release` 自动命令锁定 Shipment，以 `finance_release_status=PENDING` 和 `finance_release_version` 做 CAS，在同一事务写 `APPROVED`、版本、时间、actor、流程实例 / 节点锚点、审批意见和 durable result。财务审批拒绝后，`shipment.finance_reject` 使用相同门禁与锚点写 `REJECTED` 和拒绝原因；两条分支分别进入自己的 end 并完成 ProcessInstance，不把流程 `completed` 解释成已批准。随后只有正式 `ship_shipment` 能把已 `APPROVED` 的出货单推进到 `SHIPPED` 并写库存 `OUT`；应收 / 发票 / 收付款继续从已出货来源独立办理。任何 Workflow task done、财务门禁或旧 `shipment_release` 状态都不得解释为自动出货、自动扣库存、自动生成应收或自动开票。
