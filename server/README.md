...
`get_task_process_context` 是 `workflow.task.read` 下的流程位置只读接口，只接受任务 ID。服务端先按当前账号可见性读取任务，再使用任务已持久化的 ProcessRuntime 锚点核对来源、流程实例和关联节点；无锚点、来源不一致、节点不属于该实例或关联节点不存在都会拒绝，不从 task group、名称或 payload 猜测。响应只返回业务来源、流程实例摘要、全部节点、当前节点和已完成节点，不返回流程定义 hash、edge、分支选择或策略快照。桌面任务抽屉和手机任务详情据此显示业务流程、来源单据、发起时间、流程状态，以及已执行 / 当前 / 受阻执行轨迹、本任务锚点和重试次数；`waiting` 可能属于未选分支，前端不得把它排成确定的未来步骤。明确带 `simulated_only` 或验收批次来源的无流程任务显示“模拟展示数据”边界，不冒充流程闭环。
...
`purchase_order` JSON-RPC 域承载采购订单 Source Document / Purchase Commitment 主路径。采购订单表单只通过 `save_purchase_order_with_items` 聚合写入并使用单头 CAS。正式页面从 `DRAFT` 启动 `material_supply`，显式执行首个 `purchase_order.submit` 命令后才进入 `workflow.task.approve`；批准后的自动命令只调用 `PurchaseOrderUsecase.ApprovePurchaseOrderForProcessCommand`，在同一事务写 `SUBMITTED → APPROVED` 与 durable result。公开 `submit_purchase_order / approve_purchase_order` 路由都已移除，close / cancel 与历史查询保留。start、submit command 和审批对账都是可重试的持久步骤；任一步失败都保留可恢复证据，不把任务状态伪装成采购单状态。采购订单不写库存、批次、应付、发票或付款事实。
...
ProcessRuntime 的通用启动仍只在 usecase 层暴露；公开 `customer_config` 只保留三条来源绑定入口：`DRAFT` 销售订单启动 `sales_order_acceptance`、`DRAFT` 采购订单启动 `material_supply`、`DRAFT` 出货单启动 `finished_goods_delivery`。三者均要求目标动作权限与精确来源读权限；数据层在同一事务锁定来源，从真实单据派生 canonical `business_ref_no` 并复核状态，只允许同来源、同 idempotency key 与同定义的已创建流程重放；销售订单 / 采购订单已由首节点提交或出货流程已推进时，只允许既有流程精确重放，不允许新建。旧的入库单起点 `start_material_supply_process` 已退役。`StartProcessInstance` 首次启动会激活首个 `waiting` 节点，重试只执行有界恢复；它不提供后台 scheduler，不扫描或跳过中间节点，也不因“启动”本身写库存、出货、质检、财务或其他 Fact。
...
ProcessRuntime 当前 handler registry 同时保留新建图所需命令和旧在途 revision 的冻结命令；是否可执行必须再由当前流程定义、冻结 revision、节点、权限与业务引用共同决定，不能凭 handler 存在推断页面可达。`sales_order.submit` 和 `purchase_order.submit` 分别调用唯一 Source Document usecase 并校验流程业务引用；approval 完成后自动调用 `sales_order.activate_after_approval` 或 `purchase_order.approve_after_workflow`。当前客户 manifest 只允许 `sales_order_acceptance`、`material_supply / purchase_order_approval` 和 `finished_goods_delivery / shipment_finance_approval` 三个注册图；后两图分别止于采购批准和 Shipment 财务放行。正式销售、采购页面都执行 `start → submit command`，不再直连旧 submit API；出货页只启动财务审批，质检、确认出货和应收继续走独立领域页面。上述本地代码边界不代表目标 active revision、部署 evidence、九岗位 smoke 或 UAT 已闭环。
...
财务审批完成后，`shipment.finance_release` 自动命令锁定 Shipment，以 `finance_release_status=PENDING` 和 `finance_release_version` 做 CAS，在同一事务写 `APPROVED`、版本、时间、actor、流程实例 / 节点锚点、审批意见和 durable result，并结束 ProcessInstance。随后只有正式 `ship_shipment` 能把出货单推进到 `SHIPPED` 并写库存 `OUT`；应收 / 发票 / 收付款继续从已出货来源独立办理。任何 Workflow task done、财务门禁或旧 `shipment_release` 状态都不得解释为自动出货、自动扣库存、自动生成应收或自动开票。
