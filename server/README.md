...
`get_task_process_context` 是 `workflow.task.read` 下的流程位置只读接口，只接受任务 ID。服务端先按当前账号可见性读取任务，再使用任务已持久化的 ProcessRuntime 锚点核对来源、流程实例和关联节点；无锚点、来源不一致、节点不属于该实例或关联节点不存在都会拒绝，不从 task group、名称或 payload 猜测。响应只返回业务来源、流程实例摘要、全部节点、当前节点和已完成节点，不返回流程定义 hash 或策略快照。桌面任务抽屉和手机任务详情据此显示业务流程、来源单据、发起时间、当前节点、已完成节点和最终状态；明确带 `simulated_only` 或验收批次来源的无流程任务显示“模拟展示数据”边界，不冒充流程闭环。
