...
`/templates/render-pdf` 模板 PDF 生成入口已接入 active module states 本地门禁：已登记 `template_key` 中，`material-purchase-contract` 要求 `purchase_orders=enabled`，`processing-contract` 和 `engineering-work-instruction` 要求 `outsourcing_orders=enabled`，`engineering-material-detail` 和 `engineering-color-card` 要求 `material_bom=enabled`，`read_only / disabled / 缺失` 会拒绝生成 PDF；未知模板 key 会返回参数错误，不再绕过模块归属判断。该切片只证明正式 PDF 模板生成入口的本地门禁，不代表打印留档回写、Excel 母版回写、其它普通后端业务 API 或目标环境 release evidence 已闭环。
