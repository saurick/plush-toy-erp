...
三类看板任务均由领域事务生成，不接受页面或流程配置手填：生产订单从 `DRAFT` 下达到 `RELEASED` 时生成 `production_scheduling`，生产返工事实从 `DRAFT` 过账到 `POSTED` 时生成 `production_exception`，`DRAFT` 出货单通过 `submit_shipment_release` 显式提交时生成 `shipment_release`。任务、`created` 事件和初始 `workflow_business_states` 使用 `workflow.source-task/v1` 合同与同一来源意图摘要原子落库；任务编号分别固定为 `source-production-scheduling-<生产订单ID>`、`source-production-exception-<返工事实ID>`、`source-shipment-release-<出货单ID>`。三个 task group 和三类编号前缀是 Product Core 保留命名空间，公开 `create_task`、ProcessRuntime 显式 / 默认节点任务和客户流程合同均会拒绝占用。
...
以上方法是服务端公开能力清单，不等于正式 Web 页面已全部可达。当前只有 `start_sales_order_acceptance_process / execute_sales_order_acceptance_submit` 已由销售订单页调用；`material_supply` 4 个、`finished_goods_delivery` 5 个 start / execute 动作仍是 `partial / backend-only`。`purchase.add_purchase_receipt_item` 也只是公开后端能力，当前正式 Web UI 无调用入口。
...
这组接口使用 `shipment.read / shipment.create / shipment.ship / shipment.cancel` 动作权限；来源候选以及绑定销售订单或订单行的新建还同时要求 `sales_order.read / sales_order_item.read`，避免用出货创建权越权读取销售来源。`list_shipment_source_candidates` 是服务端唯一的可出货来源候选合同，支持关键词、销售订单、`limit / offset` 分页，返回订单行身份、状态、规范十进制字符串数量、仅由 `SHIPPED` 累计的已出货量、剩余量、可选性和不可选原因；普通 `DRAFT` 不预占。`get_shipment` 只接受正整数 `id`，与 `list_shipments` 共用 `shipment.read` 和同一出货聚合投影，用于相关单据按真实 ID 精确定位，不把 ID 当模糊关键词。新建表单只使用 `create_shipment_with_items`，在一个后端事务中创建出货单头和全部明细；来源绑定时必须是 ACTIVE 销售订单且每行完整绑定同单 OPEN 来源行，后端锁定后重验客户、产品 / SKU、单位和累计余量，并从销售订单不可变客户快照派生出货快照，客户端伪造或陈旧快照不能覆盖。无销售来源的手工草稿继续保持独立边界，只要求 `shipment.create` 与 `shipments=enabled`，且不允许残留来源行 ID。同一幂等键只有规范派生后的同一头行 payload 才返回原聚合，不同 payload 或存量快照漂移明确冲突。公开 `create_shipment / add_shipment_item` 拆分写入口已退出，避免网络重试重复明细和多行中途失败留下半保存草稿；既有出货明细只读查看。`submit_shipment_release` 只接受 `DRAFT` 出货单 ID，锁定出货单并校验当时的出货前成品检验：没有检验可按当前可选策略提交，一旦存在则必须全部达到 `PASSED + PASS / CONCESSION`；任务创建成功后，同一出货单不能再补建检验。接口返回 `{workflow_task, created}`，相同来源精确重放返回既有任务。放行任务 `done` 只写 `shipping_released` 协同投影，不调用出货或库存 usecase。

`ship_shipment` 才把出货单推进到 `SHIPPED` 并写库存 `OUT`。服务层先给出可读的放行错误，数据层仍会在同一出货事务内按确定性编号、任务组、来源、责任岗位、`workflow.source-task/v1` 合同和来源意图摘要重新核对可信任务，并要求 task status 为 `done`；随后再校验质检、销售来源数量、库存预留和可用量。任何一项失败都不写出货或库存。成功后把来源业务投影更新为 `shipped`。
...
`operational_fact` 的 shipment 出货写入口已接入 active module states 本地门禁：`create_shipment_with_items` 要求 `shipments=enabled`，其正式依赖闭包同时要求 `sales_orders / inventory` 等依赖保持 `enabled`；因此把 `sales_orders` 手工漂移为 `read_only / disabled / 缺失` 仍会阻断并且不调用出货仓储。来源绑定请求在模块门禁之外还要求销售订单头行读取权限；无来源手工草稿不追加该读取权限，但不绕过 `shipments` 的依赖闭包。`submit_shipment_release` 要求 `shipments / workflow_tasks=enabled`，`ship_shipment / cancel_shipment` 要求 `shipments / inventory / workflow_tasks=enabled`。写入模块或依赖的不可写状态会返回参数错误且不调用 `OperationalFactUsecase` 写任务、出货或库存事实。`get_shipment / list_shipments` 保留历史读取边界，不把模块关闭理解成历史出货不可查。该切片只证明 shipment 出货普通业务 API 的本地门禁，不代表其它 `operational_fact` 写入口整体、打印、其它导入入口或目标环境 release evidence 已闭环。
...
`operational_fact` 的 finance 创建入口已收口为 `create_receivable_from_shipment / create_invoice_from_shipment / create_payable_from_purchase_receipt / create_payable_from_outsourcing_return / create_reconciliation_from_finance_fact`；往来方、币种、金额和业务来源由后端从已过账源事实派生，公开 `create_finance_fact` 已退役。`post_finance_fact / settle_finance_fact / cancel_finance_fact` 继续承担允许类型的状态动作，`PAYMENT` 不开放正式创建。正式来源的 `DRAFT / POSTED` 财务事实可通过 `cancel_finance_fact` 进入 `CANCELLED` 并保存 `cancelled_at / cancelled_by / cancel_reason`；草稿取消不写库存，无正式来源草稿 fail closed，非取消对账子事实继续阻断上游取消。相同 actor + reason 可精确重放，改 actor 或 reason 返回幂等冲突；`SETTLED` 不能直接取消。写入口要求 `finance=enabled`，`read_only / disabled / 缺失` 会返回参数错误且不调用领域用例。`list_finance_facts` 保留历史读取边界。
