...
| `ERP_PDF_ALLOW_LOCAL_NO_SANDBOX` | `0` | 仅由 Linux root 本地 `make run / make dev_restart` 计算为 `1`，解决 Playwright Chromium 无法以 root+sandbox 启动；服务端同时核对 Linux 与 effective UID 0，生产 Compose 不设置 |
