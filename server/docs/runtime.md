...
- `/templates/render-pdf`
  - 在线 PDF 渲染入口，使用共享 Headless Chromium 进程生成 PDF；生产镜像默认内置 `/usr/bin/chromium`，并通过 `ERP_PDF_RENDER_CONCURRENCY` 限制并发；服务启动后默认异步执行一次中文合同 PDF 预热，`/readyz` 在预热完成前保持未就绪。通过 `ERP_PDF_WARMUP=async/off` 控制预热
