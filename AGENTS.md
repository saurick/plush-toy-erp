...
- 会话中收到符合上述来源规则的 yoyoosun 意见时，Codex 自动更新 [甲方反馈记录](docs/customers/yoyoosun/甲方反馈记录.md)，只记收到日期、来源、事项、脱敏摘要及可用附件引用；同一意见去重补充，实质变化保留前后记录，无新意见不更新。原件沿用客户 Private 存储。该文件仅作输入备忘，不是需求、业务、实现、发布或验收真源，也不产生实施、Git 或发布授权；不增加处理状态、需求编号或每会话文件。
