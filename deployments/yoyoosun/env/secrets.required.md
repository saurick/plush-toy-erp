...
- `customer-trial-133` 是登记的隔离测试目标，公开测试凭据固定为 `admin/adminadmin` 和十个 demo 共用 `12345678`，以 `credential.contract.json` 为唯一真源，不写服务器 steady `.env`。其他 staging / UAT / 生产目标不得复用。
