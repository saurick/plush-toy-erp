...
function printHelp() {
  process.stdout.write(
    [
      ...
      `    --customer-config-revision ${CUSTOMER_TRIAL_CONFIG_REVISION} \\`,
      ...
      "",
      "说明:",
      "  base-release 只验证运行身份、health/ready 和基础页面；不读取验收账号、SMS、PDF 或客户配置。",
      "  customer-trial-acceptance 才验证固定 admin、demo-133 的 11 个 UAT 账号、SMS、PDF 和配置读回。",
...
function printInputTemplate() {
  const template = {
    ...
    checks: [
      ...
      "login-page",
      "mobile-role-route",
      "credential-login-matrix (demo: admin + 11 UAT identities; customer-test: admin only)",
    ...
    commands: [
      "bash deployments/yoyoosun/scripts/run-smoke.sh --profile base-release --endpoint https://demo.example.invalid --backend-url https://api.demo.example.invalid --product-commit <40-character-lowercase-git-sha> --migration-version <14-digit-atlas-version> --deployment-target demo-133 --environment demo-133 --report deployments/yoyoosun/evidence/releases/<YYYY-MM-DD>/smoke-test-report.json",
      `CUSTOMER_CONFIG_ADMIN_TOKEN='<admin-token>' bash deployments/yoyoosun/scripts/run-smoke.sh --profile customer-trial-acceptance --endpoint https://demo.example.invalid --backend-url https://api.demo.example.invalid --product-commit <40-character-lowercase-git-sha> --migration-version <14-digit-atlas-version> --credential-operation-id <lowercase-uuid-v4> --deployment-target demo-133 --environment demo-133 --report deployments/yoyoosun/evidence/releases/<YYYY-MM-DD>/smoke-test-report.json --admin-username admin --admin-password-env MANUAL_ACCEPTANCE_ADMIN_PASSWORD --uat-password-env MANUAL_ACCEPTANCE_UAT_PASSWORD --sms-phone-env MANUAL_ACCEPTANCE_SMS_PHONE --customer-config-revision ${CUSTOMER_TRIAL_CONFIG_REVISION} --admin-token-env CUSTOMER_CONFIG_ADMIN_TOKEN`,
    ],
    requiredReadbackEvidence: [
      ...
      "check name=credential-login-matrix binds deploymentTarget, commandTarget, targetIdentity and database; demo totalAuthenticated=12, customer-test totalAuthenticated=1",
...
function targetDatabase(deploymentTarget) {
  process.stdout.write(getDeploymentTarget(deploymentTarget).database.name);
}
...
function runtimeIdentityProof(file, httpCode) {
  const proofs = fs
    ...
    .map((line) =>
      line
        .match(/^x-erp-runtime-identity-proof\s*:\s*(\S+)\s*$/iu)?.[1]
        ?.trim()
        .toLowerCase(),
    )
    .filter(Boolean);
