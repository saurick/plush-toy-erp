...
说明:
  只在发布工作站运行。脚本从 credential.contract.json 指定的 macOS Keychain
  alias 读取 admin、demo 密码和短信登录手机号，经 SSH stdin 临时注入 133 的
  一次性 Compose 容器。密码、token、手机号不会进入 argv、服务器 steady env
...
while [[ $# -gt 0 ]]; do
  ...
  --backup-sha256) backup_sha256="${2:-}"; shift 2 ;;
  --report) report="${2:-}"; shift 2 ;;
  --confirm) confirm="${2:-}"; shift 2 ;;
  -h | --help) print_help; exit 0 ;;
  *) echo "[rotate-credentials-133] 不支持的参数: $1" >&2; print_help >&2; exit 2 ;;
  esac
done

[[ "$ssh_target" =~ ^[A-Za-z0-9._-]+@[A-Za-z0-9._:-]+$ ]] || { echo "[rotate-credentials-133] --ssh-target 格式非法" >&2; exit 2; }
[[ "$expected_release" =~ ^[a-f0-9]{40}$ ]] || { echo "[rotate-credentials-133] --expected-release 必须是 40 位小写 SHA" >&2; exit 2; }
[[ "$expected_migration" =~ ^[0-9]{14}$ ]] || { echo "[rotate-credentials-133] --expected-migration 必须是 14 位 Atlas version" >&2; exit 2; }
[[ "$operation_id" =~ ^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$ ]] || { echo "[rotate-credentials-133] --operation-id 必须是小写 UUID v4" >&2; exit 2; }
[[ "$backup_file" =~ ^/[A-Za-z0-9._/-]+$ && "$backup_file" != *"/../"* && "$backup_file" != *"/./"* ]] || { echo "[rotate-credentials-133] --backup-file 必须是无 dot segment 的绝对路径" >&2; exit 2; }
[[ "$backup_sha256" =~ ^[a-f0-9]{64}$ ]] || { echo "[rotate-credentials-133] --backup-sha256 必须是 64 位小写 hex" >&2; exit 2; }
[[ -n "$report" ]] || { echo "[rotate-credentials-133] --report 必填" >&2; exit 2; }
expected_confirm="ROTATE_YOYOOSUN_CREDENTIALS_133:${expected_release}:${expected_migration}:${operation_id}"
[[ "$confirm" == "$expected_confirm" ]] || { echo "[rotate-credentials-133] --confirm 与目标不匹配" >&2; exit 2; }

command -v node >/dev/null 2>&1 || { echo "[rotate-credentials-133] 缺少 node" >&2; exit 1; }
command -v security >/dev/null 2>&1 || { echo "[rotate-credentials-133] 缺少 macOS security" >&2; exit 1; }
command -v ssh >/dev/null 2>&1 || { echo "[rotate-credentials-133] 缺少 ssh" >&2; exit 1; }

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)"
contract_file="$script_dir/../env/credential.contract.json"
[[ -f "$contract_file" ]] || { echo "[rotate-credentials-133] credential contract 不存在" >&2; exit 1; }

IFS=$'\t' read -r admin_service admin_account demo_service demo_account phone_service phone_account public_passwords_csv < <(
  node - "$contract_file" <<'NODE'
const fs = require("node:fs");
const contract = JSON.parse(fs.readFileSync(process.argv[2], "utf8"));
const admin = contract?.credentials?.admin?.keychain;
const demo = contract?.credentials?.demo?.keychain;
const phone = contract?.smsLoginIdentity?.keychain;
const publicPasswords = contract?.policy?.localOnlyPublicPasswords;
...
if (
  ...
  !text(admin?.service) || !text(admin?.account) ||
  !text(demo?.service) || !text(demo?.account) ||
  !text(phone?.service) || !text(phone?.account) ||
  !Array.isArray(publicPasswords) || publicPasswords.length !== 1 ||
  publicPasswords[0] !== "12345678" ||
...
process.stdout.write([
  ...
  phone.service,
  phone.account,
  publicPasswords.join(","),
].join("\t") + "\n");
NODE
)

admin_password="$(security find-generic-password -w -s "$admin_service" -a "$admin_account")"
demo_password="$(security find-generic-password -w -s "$demo_service" -a "$demo_account")"
sms_phone="$(security find-generic-password -w -s "$phone_service" -a "$phone_account")"

[[ ${#admin_password} -ge 8 && ${#admin_password} -le 20 ]] || { echo "[rotate-credentials-133] admin Keychain 密码长度非法" >&2; exit 1; }
[[ ${#demo_password} -ge 8 && ${#demo_password} -le 20 ]] || { echo "[rotate-credentials-133] demo Keychain 密码长度非法" >&2; exit 1; }
[[ "$admin_password" != "$demo_password" ]] || { echo "[rotate-credentials-133] admin 与 demo 密码必须不同" >&2; exit 1; }
IFS=',' read -r -a public_passwords <<< "$public_passwords_csv"
for public_password in "${public_passwords[@]}"; do
  [[ "$admin_password" != "$public_password" && "$demo_password" != "$public_password" ]] || { echo "[rotate-credentials-133] Keychain 命中本地公开密码" >&2; exit 1; }
done
[[ "$sms_phone" =~ ^1[3-9][0-9]{9}$ ]] || { echo "[rotate-credentials-133] SMS Keychain 手机号必须是规范化的中国大陆手机号" >&2; exit 1; }
...
node - "$report_tmp" "$expected_release" "$expected_migration" "$operation_id" <<'NODE'
const fs = require("node:fs");
const [file, release, migration, operationId] = process.argv.slice(2);
