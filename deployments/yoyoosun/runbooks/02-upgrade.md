...
9. 若本轮创建、恢复或重建过数据库，必须在入口关闭状态下按合同 Keychain alias 读取彼此独立的管理员 / UAT 岗位外部 secret，重新轮换稳定 `admin` 与十个 `uat_*`，并撤销旧会话；不得使用本地 Demo 的 `adminadmin` / `12345678`。普通应用升级未触碰数据库账号时也必须运行真实登录矩阵，不能复用旧 token 代替密码验证。SMS 手机号未人工录入时不阻断。
10. 执行 smoke 和日志检查；正式 smoke 必须包含 `credential-login-matrix`，稳定 `admin` 和十个 `uat_*` 全部以当前外部 secret 取得新 token。全部门禁通过后才执行公网 / 客户入口切流。
