...
| `devDataPreparationPlugin.mjs`      | 提供单一数据准备 operation 真源；同一 Scenario profile 显式绑定本地或 133，冻结当前 V7、release、数据库、migration、客户配置与回滚点，长期数据与隔离验收不互相替代 |
...
- 本地 Core profile 固定执行当前 V7 合同的角色账号、11 个审定模拟单位与 4 个仓库引用，只允许登记的长期开发库和服务端生成的精确确认；它不再从工作台调用旧 `SIM-PLUSH-CORE` 全量 seed，材料、产品、工序、BOM、Source、Task 与 Fact 由同一 V7 Scenario 合同补齐并独立读回。旧 V5 / V6 回执只作历史追溯，不参与当前执行、恢复选择或环境结论。
