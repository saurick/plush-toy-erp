...
- 管理员登录页同时保留密码登录和短信登录；当前短信通道仍是后端 mock delivery，真实登录烟测仍使用密码链路
...
`pnpm smoke:mobile-auth-login-route` 当前覆盖全部 8 个角色移动端入口的未登录拦截、登录失效回登录页、重新登录后回跳任务页、任务 / 预警 / 通知 / 进度展示、移动端不显示说明 / 角色文案，以及退出登录清空登录态。
...
- 移动端复用管理员登录态；未登录或登录失效时进入 `/admin-login`，登录后回到任务页，并提供退出登录按钮
