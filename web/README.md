...
- 任务识别组件展示来源投影绑定的产品主图：桌面 64px、手机 52px，保持轮廓，缺图、物料或无有效 `product.read` 时显示同尺寸图标占位，加载失败也保留图片区域。列表延迟读取 160px 缩略图并按当前账号权限范围去重；详情按需放大原图。主图清空、来源切换和解除关联不回填旧引用；物料项不借用产品图片。
