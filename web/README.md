...
| `local dev` super admin                                                    | 第一层前端 RBAC 菜单路径不依赖 `allowedMenuPaths`，菜单定义使用 Product Core 默认导航，不被客户静态菜单子集或 active pages 收窄；隐藏 URL 仍按 `resolveCurrentNavigationEntry` 解析，未登记路径只得到显示 fallback，不得到授权 page key。                                                                                                                                                                                    | `super_admin_product_core`                                                  |
| 正式 / 非前端 DEV 构建 super admin，正常 active revision                   | 前端菜单路径不依赖 `allowedMenuPaths`，菜单定义使用 Product Core 默认导航，也不再被 active pages / active actions / field policy 收窄；用于查看产品核心和当前客户系统所有已登记业务能力。若 effective session 带有客户 key，则 `dataRuntimeScope=customer_runtime`、`canMountCustomerBusinessPages=true`，业务页仍按该客户运行环境读取当前部署数据库；后端写入口仍按模块状态、业务状态、Workflow / Fact 边界和审计门禁执行。 | `super_admin_product_core`                                                  |
| 正式 / 非前端 DEV 构建 super admin，`effective_session_sync_failed` 空投影 | 前端菜单路径不依赖 `allowedMenuPaths`，也不因 sync failure 空投影隐藏导航定义里的业务页；此时 `dataRuntimeScope=sync_failed_diagnostic`、`canMountCustomerBusinessPages=false`，客户业务数据页显示隔离空态，不挂载真实业务 `Outlet`。这只是诊断和审阅可见性，不证明原始 URL 已登记、已授权或可渲染客户业务内容，也不绕过任何后端写入门禁。                                                                                   | `super_admin_product_core`                                                  |
