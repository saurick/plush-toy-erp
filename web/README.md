...
文字按钮的水平内边距由 `src/App.jsx` 的 Ant Design `Button` token 统一提供：普通和小号按钮均为 12px，大号按钮为 16px。工具栏、当前操作区、弹窗和浮层沿用这套基线；同一规格的主按钮、次按钮、禁用和 loading 状态不另设 padding。普通业务动作按内容定宽，状态主动作只保留防止切换抖动的最小宽度，长文案仍可伸展。窄屏当前操作区允许长文案换行，保留内边距和至少 44px 触控高度。纯图标、行内文字链接和手机整行按钮按各自用途保留紧凑外形或触控布局，不用固定宽度修补普通按钮的留白。
...
- 菜单主列表的新增 / 编辑统一使用 `BusinessFormPage`，在当前模块路由内切换主内容区，保留列表筛选、选择与滚动位置。正文独立滚动，底部固定“返回列表 / 保存”；已支持的只读查看复用此骨架。内容、待上传附件或产品图片有修改时，返回、切换菜单、浏览器后退和刷新前提示；保存中阻止离开。适用客户、供应商、材料、产品、产品规格、加工环节、BOM、销售 / 采购 / 生产订单、加工合同、出货单、质检补建、收付款 / 红冲登记、生产事实草稿及员工账号创建 / 资料修改。编辑过程中的快捷新建、来源选择、预览，以及审批、核销、冲销、岗位分配等专项动作继续使用弹窗。详情抽屉只用于显式只读核对。生产排程和出货放行页只读取并处理来源生成的 Workflow 协同任务；生产异常处置页以独立页签分开处置申请和待审批任务。三者都不提供通用任务创建弹窗，也不能把协同结果写成生产订单、生产异常执行、出货单、库存、财务或发票事实；来源、打印、删除等未接入真实 usecase 的动作不能写成真实业务动作。
