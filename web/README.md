...
- 开发环境额外提供 `http://localhost:5175/__dev/docs` 作为本地开发态文档查看器；该入口左侧专用于按真实目录树浏览仓库 tracked Markdown，并在搜索框下方提供浏览器本地持久化的置顶文档区，当前打开文档和目录树展开状态也会写入浏览器本地偏好以便刷新后恢复；搜索态显示匹配结果，右侧标题栏图钉可置顶 / 取消置顶当前文档，置顶区行内图钉可直接取消置顶，目录树和搜索结果行内图钉可快速置顶 / 取消置顶，章节标签可滚动到对应标题并提供回到顶部，Markdown fenced `mermaid` 代码块会只读渲染为图表，不进入侧栏、seedData、RBAC 或产品内文档 registry，生产构建不可访问。
