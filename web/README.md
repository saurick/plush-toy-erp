...
| `local dev` super admin                                                    | 第一层前端 RBAC 菜单路径不依赖 `allowedMenuPaths`，也不被 active pages 收窄；菜单项过滤只会放开导航定义里的页面，隐藏 URL 仍按 `resolveCurrentNavigationEntry` 解析，未登记路径只得到显示 fallback，不得到授权 page key。                                                                                                                                                              | `super_admin_product_core`                                                  |
