...
| 本地服务              | `pnpm start`、`pnpm start:yoyoosun`、`pnpm audit:yoyoosun-entry`、`pnpm serve:prod`、`pnpm preview:yoyoosun`                                                                                                                                                                                                                                                                                       | 默认前端入口、永绅 yoyoosun 热更新开发、端口审计和静态包预览                                                                           | `pnpm start` 启动桌面后台与 `/m/<role>/tasks` 岗位任务端共用的单端口 Vite；`start:yoyoosun` 用 Vite dev server 和 HMR 注入永绅静态客户配置；`preview:yoyoosun` 构建并预览生产包形态；`audit:yoyoosun-entry` 只读检查本机端口归属、customer-config、asset 和后端 health；这些入口都不 publish / activate 后端 customer config；岗位任务端不再保留按角色拆端口入口                                                                                         |
...
- `start:yoyoosun` 默认从 `5176` 起探测可用端口，启动 Vite dev server，并通过 dev-only middleware 从 `config/customers/yoyoosun/customer-config.example.js` 和 `public-assets/` 提供 `/customer-config.js`、`/customer-assets/yoyoosun/*`；客户工程图和来源资料不会公开提供。它保留 HMR，不构建生产包，不调用 `customer_config.validate / publish / activate / rollback`，不写数据库。
