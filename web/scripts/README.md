...
- `start:yoyoosun` 默认从 `5176` 起探测可用端口，启动 Vite dev server，并通过 dev-only middleware 从 `config/customers/yoyoosun/customer-config.example.js` 和 `assets/` 提供 `/customer-config.js`、`/customer-assets/yoyoosun/*`；它保留 HMR，适合本地开发调试，不构建生产包，不调用 `customer_config.validate / publish / activate / rollback`，不写数据库。
