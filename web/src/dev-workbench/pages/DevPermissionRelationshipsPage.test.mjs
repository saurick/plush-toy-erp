...
const navigationSource = readFileSync(
  fileURLToPath(
    new URL('../config/devPermissionNavigation.mjs', import.meta.url)
  ),
  'utf8'
)
const navigationComponentSource = readFileSync(
  fileURLToPath(
    new URL(
      '../components/DevPermissionNavigationOverview.jsx',
      import.meta.url
    )
  ),
  'utf8'
...
test('dev permission relationship page remains valid JSX', async () => {
  ...
  await transformWithEsbuild(
    ...
    'DevPermissionNavigationOverview.jsx',
    {
      loader: 'jsx',
      jsx: 'automatic',
    }
...
test('permission relationship implementation stays under the dev-only tree', () => {
  ...
  assert.match(
    source,
    /\.\.\/components\/DevPermissionNavigationOverview\.jsx/u
  )
  assert.doesNotMatch(
    source + navigationComponentSource,
    /\.\.\/\.\.\/erp\/components/u
  )
})
