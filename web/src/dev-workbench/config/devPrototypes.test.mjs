...
import { printTemplateCatalog } from '../../erp/config/printTemplates.mjs'

const repoRoot = path.resolve(import.meta.dirname, '../../../..')
const prototypesPageSource = readFileSync(
  path.join(repoRoot, 'web/src/dev-workbench/pages/DevPrototypesPage.jsx'),
  'utf8'
)
...
test('devPrototypes: 登记当前原型与样板资产并区分类型和状态', () => {
  ...
  assert.deepEqual(
    ...
    ['mobile-role-tasks-v2', 'mobile-role-tasks-implemented']
  )
  assert.deepEqual(
...
test('devPrototypes: 岗位任务端 v2 保持三类查询视图、统一任务附件且无空回执入口', () => {
  ...
  assert.match(html, /filter: 'todo'/u)
  assert.match(html, /views: \['todo'\]/u)
  assert.match(html, /views: \['todo', 'risk'\]/u)
  assert.match(html, /views: \['history'\]/u)
  ...
  assert.match(html, /receipt\.status === 'unknown'/u)
  assert.match(html, /receipt\.status === 'failed'/u)
  assert.match(html, /task\.receipt\.status = 'confirmed'/u)
  assert.match(html, /任务附件/u)
  assert.match(html, /历史处理线索/u)
  assert.doesNotMatch(html, /现场证据/u)
  assert.match(html, /const hasMultipleActions = task\.actions\.length > 1/u)
  assert.match(html, /role="radiogroup"/u)
  assert.match(
    html,
    /input class="action-choice__radio" type="radio" name="task-action"/u
  )
  assert.match(html, /data-single-action-summary/u)
  assert.match(html, />本次操作<\/h1>/u)
  assert.match(html, /'确认' \+ escapeHTML\(action\.label\)/u)
  assert.doesNotMatch(html, /class="action-choice" type="button"/u)
  assert.doesNotMatch(html, /\.action-choice\[aria-pressed="true"\]/u)
  assert.match(
    html,
    /\.topbar \.icon-button \{[\s\S]*?width: 44px;[\s\S]*?height: 44px;/u
  )
  assert.doesNotMatch(html, /暂时没有处理回执/u)
  assert.doesNotMatch(
    html,
  ...
  assert.match(
    readme,
    /按 \*\*Current\*\* 登记到中央原型清单和开发态原型查看器/u
  )
  assert.match(readme, /当前 \*\*Current\*\* 选中任务流程/u)
})
...
test('devPrototypes: 业务列表更多操作默认关闭并在窄屏收敛动作', () => {
  ...
  assert.match(html, /id="statusActionMenu"[\s\S]*?role="menu"[\s\S]*?hidden/u)
