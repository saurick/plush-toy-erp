...
test('FL_processing_contract_table_headers__paper_uses_shared_subject_aware_columns processingContractTemplate: 纸面表头复用字段真源', () => {
  ...
  const paperSource = readFileSync(
    fileURLToPath(
      new URL(
        '../components/print/ProcessingContractPaper.jsx',
        import.meta.url
      )
    ),
    'utf8'
  ...
  assert.equal(columnLabels.processingItem, '加工项目')
  assert.equal(columnLabels.processCategory, '工序类别')
  assert.equal(columnLabels.productNo, '产品 / 材料编号')
  assert.match(paperSource, /PROCESSING_CONTRACT_TABLE_COLUMNS\.map/u)
  assert.doesNotMatch(paperSource, /<th>产品编号<\/th>/u)
  assert.doesNotMatch(paperSource, /<th>工序名称<\/th>/u)
})
...
test('FL_processing_contract_business_draft__does_not_fill_missing_business_fields_from_template_sample processingContractTemplate: 业务带值草稿不从加工合同样例兜底真实缺值', () => {
  ...
  assert.deepEqual(draft.appendixImages, [])
  assert.deepEqual(draft.merges, [])
})

test('processingContractTemplate: 旧版固定附件按左右顺序迁入共享末尾附图', () => {
  const draft = normalizeProcessingContractDraft({
    attachments: {
      'attachment-2': {
        name: '右图.png',
      ...
      'attachment-1': {
        name: '左图.png',
        dataURL: 'data:image/png;base64,bGVmdA==',
        mimeType: 'image/png',
      },
    },
  })
  ...
  assert.deepEqual(
    draft.appendixImages.map((image) => image.name),
    ['左图.png', '右图.png']
  )
  assert(!Object.hasOwn(draft, 'attachments'))
})

test('processingContractTemplate: 显式清空末尾附图后不会从旧版附件复活', () => {
  ...
  const normalizedDraft = normalizeProcessingContractDraft({
    draftVersion: 2,
    attachments: legacyAttachments,
    appendixImages: [],
  })
  const businessDraft = createProcessingContractBusinessDraft({
    draftVersion: 2,
    attachments: legacyAttachments,
    appendixImages: [],
  })

  assert.equal(normalizedDraft.draftVersion, PROCESSING_CONTRACT_DRAFT_VERSION)
  assert.equal(businessDraft.draftVersion, PROCESSING_CONTRACT_DRAFT_VERSION)
  assert.deepEqual(normalizedDraft.appendixImages, [])
  assert.deepEqual(businessDraft.appendixImages, [])
  assert(!Object.hasOwn(normalizedDraft, 'attachments'))
  assert(!Object.hasOwn(businessDraft, 'attachments'))
})
...
test('processingContractTemplate: 委外事实只带入合同打印可确认快照字段', () => {
  ...
  assert.equal(draft.lines[0].productName, '')
  assert.equal(draft.lines[0].processingItem, '')
  ...
  assert.match(draft.lines[0].remark, /产品 \/ 材料：已关联/u)
...
test('processingContractTemplate: 委外事实追溯优先显示业务单号且不暴露未知 key', () => {
  ...
  assert.match(remark, /产品 \/ 材料：MAT-26001/u)
...
test('FL_processing_contract_fact_trace__uses_business_numbers_without_internal_ids processingContractTemplate: 委外事实追溯不因内部 ID 缺失丢失业务来源号', () => {
  ...
  assert.match(remark, /产品 \/ 材料：半成品兔头/u)
...
test('FL_processing_contract_material_subject__maps_material_snapshots processingContractTemplate: 布料加工合同显示材料编码和名称', () => {
  ...
  assert.equal(columnLabels.productNo, '产品 / 材料编号')
  assert.equal(columnLabels.productName, '产品 / 材料名称')
  ...
  assert.deepEqual(draft.lines[0], {
    ...
    productOrderNo: 'SO-MATERIAL-001',
    productNo: 'MAT-FABRIC-018',
    productName: '短毛绒布',
    processingItem: '复合面料',
    supplierAlias: '布料复合加工厂',
    processCategory: '布料复合',
    unit: '米',
    unitPrice: '2.5',
    quantity: '20',
    amount: '50',
  ...
  assert.equal(draft.lines[0].productOrderNo, 'SO-MATERIAL-001')
})
...
test('FL_processing_contract_print_party_defaults__uses_customer_config_party_defaults_only processingContractTemplate: 加工合同打印草稿只从客户配置带入委托方默认值', () => {
  ...
  assert.equal(draft.supplierContact, '加工厂联系人')
  assert.equal(draft.supplierSigner, '')
  assert.equal(draft.lines[0].processingItem, '脸*1')
  assert.equal(draft.lines[0].processCategory, '车缝')
