...
> 目的：把毛绒 ERP 当前怎样安全排查一条业务链路说清楚。`/erp/qa/business-chain-debug` 是本地 / 开发调试中心，不是生产运维工具，也不是任意造数或清库工具。

## 1. 当前结论

- 当前 6 条链路只代表 ERP v1 主干闭环，不代表所有业务链路已经全量覆盖。
- 当前业务保存层仍复用 `business_records` 和 `business_record_items`。
- 当前 workflow 状态真源是 `workflow_business_states`。
- 当前协同任务真源是 `workflow_tasks` 和 `workflow_task_events`。
- 当前没有行业专表、真实库存流水 / 库存余额专表、财务专表、总账、凭证、纳税申报，也没有 notification 独立表和外部推送。
...
- 调试页已经接入受控后端能力：seed 表示生成调试数据，cleanup 表示清理调试数据，dryRun 表示只预览不执行，debugRunId 表示本次调试编号。
...
| ----------------------------- | ---------------------------------------- | --------- | ------------------------------------- | ------------------------------------ | ------------------------------------------------------------------ |
| `order_approval_engineering`  | 订单提交 -> 老板审批 -> 工程资料任务     | 已接入 v1 | `business_records` + `workflow_tasks` | 单元测试 + `style:l1` + 移动端 smoke | 没有后端 workflow usecase 统一编排，没有真实 E2E 造数 runner       |
| `purchase_iqc_inbound`        | 采购到货 -> IQC -> 入库                  | 已接入 v1 | `business_records` + `workflow_tasks` | 单元测试 + `style:l1` + 移动端 smoke | 没有真实库存流水 / 库存余额专表                                    |
...
| `finished_goods_shipment`     | 成品完工 -> 成品抽检 -> 成品入库 -> 出货 | 已接入 v1 | `business_records` + `workflow_tasks` | 单元测试 + `style:l1` + 移动端 smoke | 没有 `production_order` / `shipment_order` / `inventory_txn` 专表  |
...
| 欠料预警 -> 采购补料                    | deferred | 当前有预警和采购链路，但没有库存余额 / 材料需求 / 缺料计算真源。                                                    | `inventory_balance` / `material_requirement` 稳定后再做。             |
| 仓库发料 / 委外发料 / 生产领料          | deferred | 当前覆盖入库和出货，不覆盖领料 / 发料流水。                                                                         | `inventory_txn` 专表评审后补。                                        |
| 库存盘点 / 库存调整 / 异常件            | deferred | 当前没有真实库存流水和库存余额。                                                                                    | `inventory_txn` / `inventory_balance` / `adjustment_order` 评审后补。 |
...
2. 只允许 local / dev 环境，并且必须开启 `ERP_DEBUG_SEED_ENABLED=true`。
...
3. 必须开启 `ERP_DEBUG_CLEANUP_ENABLED=true`，且 `ERP_DEBUG_CLEANUP_SCOPE=debug_run`。
4. remote / prod / shared 环境默认拒绝 destructive cleanup，即使前端按钮被误打开，后端也会拒绝。
...
- 不提供清空真实业务数据。
- 不提供无 where 删除。
- 不绕开 `business_records`、`workflow_tasks`、`workflow_task_events` 和 `workflow_business_states`。
