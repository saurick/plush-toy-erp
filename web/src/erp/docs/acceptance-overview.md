...
- 当前闭环由“前端 v1 编排 + 后端 workflow usecase 渐进迁入”共同承接；老板审批、IQC 和采购仓库入库三条最小规则已进入后端，委外、成品、出货、应收、应付主干仍以 v1 编排和通用业务记录承接。
...
| 评审项                    | 当前结论                                                                      | 推荐下一步                                                        | 本轮边界                                                         |
| ------------------------- | ----------------------------------------------------------------------------- | ----------------------------------------------------------------- | ---------------------------------------------------------------- |
| workflow usecase 统一编排 | 已渐进迁入老板审批、IQC 和采购仓库入库三条最小规则，不支持一次性重写 6 条闭环 | 继续按真实链路逐条迁入后端，补强幂等、事务、事件和业务状态一致性  | 不把尚未评审的委外、成品、出货、应收、应付一次性搬进后端 usecase |
