...
| 后端迁移边界 | 当前只迁第一条老板审批派生规则，不是完整 workflow engine。幂等按 `source_type + source_id + task_group + owner_role_key + 非终态状态` 在应用层事务内查询后创建，暂未加 DB unique constraint；极端并发下后续仍需 DB unique constraint 或 advisory lock 加固。其余 IQC、委外、成品、出货、应收、应付闭环仍待后续迁入。                                 |
