...
test('mobileTaskView: 品质和仓库能看到采购到货闭环任务字段', () => {
  ...
  assert.equal(qualityViews[0].business_status_label, '来料检验（IQC）待处理')
...
test('mobileTaskView: explainMobileTaskVisibility 解释负责岗位和已结束任务', () => {
...
test('mobileTaskView: explainMobileTaskVisibility 解释 PMC 关注原因', () => {
  const explanation = explainMobileTaskVisibility(
    task({
      owner_role_key: 'purchase',
      task_status_key: 'blocked',
  ...
  assert.equal(explanation.visible, true)
  assert(explanation.reasons.some((item) => item.includes('已阻塞或被退回')))
  assert(explanation.reasons.some((item) => item.includes('关键环节')))
  assertNoVisibleTechnicalTaskFields(explanation)
})

test('mobileTaskView: explainMobileTaskVisibility 解释老板关注原因', () => {
  ...
  assert(highPriority.reasons.some((item) => item.includes('优先级较高')))
  assert.equal(financeCritical.visible, true)
  assert(financeCritical.reasons.some((item) => item.includes('财务风险')))
...
test('mobileTaskView: explainMobileTaskVisibility 解释生产关注和不可见原因', () => {
  ...
  assert(
    notVisible.blockers.some((item) =>
      item.includes('这项任务由仓库负责，不在老板的任务列表中')
    )
  )
  assert(
    notVisible.blockers.some((item) =>
      item.includes('没有需要当前岗位关注的风险或提醒')
    )
  )
