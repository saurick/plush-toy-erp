...
test('businessRecordPrintDraft: 辅材采购记录带值生成采购合同草稿', () => {
  const draft = buildBusinessRecordPrintDraft('accessories-purchase', {
    ...
    due_date: '2026-05-02',
    payload: {
      supplier_contact: '陈小姐',
      supplier_phone: '13800000000',
      supplier_address: '东莞市测试路 1 号',
    },
    items: [
      {
  ...
  assert.equal(draft.supplierContact, '陈小姐')
  assert.equal(draft.supplierPhone, '13800000000')
  assert.equal(draft.supplierAddress, '东莞市测试路 1 号')
  ...
  assert.equal(draft.lines[0].vendorCode, 'HN-12')
  assert.equal(draft.lines[0].remark, '黑色')
...
test('businessRecordPrintDraft: 辅材采购缺少真源字段时不沿用样例残值', () => {
  ...
  assert.equal(draft.supplierAddress, '')
  assert.equal(draft.buyerCompany, '本公司')
  assert.equal(draft.buyerContact, '')
  assert.equal(draft.buyerPhone, '')
  assert.equal(draft.buyerAddress, '')
...
test('businessRecordPrintDraft: 加工记录带值生成加工合同草稿', () => {
  const draft = buildBusinessRecordPrintDraft('processing-contracts', {
    ...
    payload: {
      product_order_no: 'SLO260101-L1',
      orderer_phone: '13900000000',
    },
    due_date: '2026-05-06',
    items: [
      {
  ...
  assert.equal(draft.lines[0].remark, '先做头部')
