...
test('engineeringPrintTemplates: BOM 版本带值生成作业指导书工程资料草稿', () => {
  ...
  assert.equal(draft.rows[0].type, WORK_INSTRUCTION_ROW_TYPES.remark)
  assert.equal(draft.rows[0].text, '按签样执行')
})
...
test('engineeringPrintEditor: 作业指导书默认样例只保留首个备注前模板', () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      WORK_INSTRUCTION_ROW_TYPES.step,
      WORK_INSTRUCTION_ROW_TYPES.title,
      WORK_INSTRUCTION_ROW_TYPES.note,
      WORK_INSTRUCTION_ROW_TYPES.step,
      WORK_INSTRUCTION_ROW_TYPES.step,
      WORK_INSTRUCTION_ROW_TYPES.remark,
    ]
  )
  assert.deepEqual(
...
test('engineeringPrintEditor: 客户样例覆盖 Sheet1 文本但不预置作业行图片', () => {
  ...
  assert.match(
    withSample.rows.find((row) => row.type === WORK_INSTRUCTION_ROW_TYPES.note)
      .text,
    /头车5mm止口/u
  )
...
test('engineeringPrintEditor: 作业指导书续页说明行后重新从 1 编号', () => {
  const draft = createEngineeringPrintDraft(WORK_INSTRUCTION_TEMPLATE_KEY, {
    continuationPages: [
      {
        rows: [
