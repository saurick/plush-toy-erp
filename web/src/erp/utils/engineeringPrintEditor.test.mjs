...
test('engineeringPrintTemplates: BOM 版本带值生成物料明细并保留产品核心字段', () => {
  const draft = buildMaterialDetailDraftFromBOMVersion(
    ...
    },
    {
      ...
      productImages: {
        ...
        secondary: {
          name: '产品侧面.png',
          dataURL: 'data:image/png;base64,c2Vjb25kYXJ5',
          mimeType: 'image/png',
        },
      },
    }
  ...
  assert.equal(draft.images.header_left.name, '产品正面.jpg')
  assert.equal(
    draft.images.header_right.dataURL,
...
test('engineeringPrintTemplates: BOM 版本带值生成作业指导书工程资料草稿', () => {
  const draft = buildWorkInstructionDraftFromBOMVersion(
    ...
    },
    {
      ...
      products: [
        {
          id: 11,
          code: 'SIM-YOYOOSUN-UAT-20260715-V2-PROD-015',
          name: '抱抱猴子',
          style_no: '26204#',
          customer_style_no: 'YS26204',
        },
      ],
  ...
  assert.equal(draft.orderNo, 'WL260102')
  assert.equal(draft.versionText, '')
  assert.equal(draft.processName, '')
  assert.equal(draft.processDateText, '')
  assert.equal(draft.department, '生产部/品质部')
  assert.equal(draft.maker, '制表人')
  assert.equal(draft.designer, '罗伟')
  assert.equal(draft.auditor, '审核人')
  assert.equal(draft.remark, '按签样执行')
  assert.equal(draft.images.header.name, '产品正面.jpg')
  assert.equal(draft.images.header_right.name, '产品侧面.png')
...
test('engineeringPrintTemplates: 委外订单带值生成外发作业指导书草稿', () => {
  const draft = buildWorkInstructionDraftFromOutsourcingOrder(
    ...
    ],
    {
      ...
      productImages: {
        primary: {
          name: '委外产品图.jpg',
          dataURL: 'data:image/jpeg;base64,b3V0c291cmNpbmc=',
          mimeType: 'image/jpeg',
        },
      },
    }
  ...
  assert.equal(draft.images.header.name, '委外产品图.jpg')
  assert.equal(draft.images.header_right.dataURL, '')
  ...
  assert.equal(draft.rows[1].type, WORK_INSTRUCTION_ROW_TYPES.text)
  assert.equal(
    draft.rows[1].text,
    '加工厂：成慧怡；来源订单：SO260601；回货日期：2026-07-20'
  )
  assert.equal(draft.rows[2].type, WORK_INSTRUCTION_ROW_TYPES.step)
  assert.equal(
    draft.rows[2].text,
    '加工项目：脸*1；工序：车缝；产品：26204# / SKU-RED-M / 抱抱猴子；数量：300只；要求：按签样执行'
  )
  assert.equal(
...
test('engineeringPrintTemplates: 作业指导书本页工序和日期槽独立保存', () => {
  const draft = createEngineeringPrintDraft(WORK_INSTRUCTION_TEMPLATE_KEY, {
    processName: '手工',
    processDateText: '2026年7月18日',
    continuationPages: [
      {
        processName: '车缝',
        processDateText: '2026年7月19日',
        rows: [],
      },
      { rows: [] },
    ],
  ...
  assert.equal(draft.processName, '手工')
  assert.equal(draft.processDateText, '2026年7月18日')
  assert.equal(draft.continuationPages[0].processName, '车缝')
  assert.equal(draft.continuationPages[0].processDateText, '2026年7月19日')
  assert.equal(draft.continuationPages[1].processName, '')
  assert.equal(draft.continuationPages[1].processDateText, '')
})

test('engineeringPrintTemplates: business drafts keep product image slots empty without configured images', () => {
  const materialDetail = buildMaterialDetailDraftFromBOMVersion({
    product_id: 11,
    items: [],
  })
  const workInstruction = buildWorkInstructionDraftFromBOMVersion({
    product_id: 11,
    items: [],
  })

  assert.equal(materialDetail.images.header_left.dataURL, '')
  assert.equal(materialDetail.images.header_right.dataURL, '')
  assert.equal(workInstruction.images.header.dataURL, '')
  assert.equal(workInstruction.images.header_right.dataURL, '')
})

test('engineeringPrintTemplates: 作业指导书产品选项回退不把款号片段拼进名称', () => {
  const draft = buildWorkInstructionDraftFromBOMVersion(
    { product_id: 11, items: [] },
    {
      productOptions: [{ value: 11, label: '26204# / 抱抱猴子 / YS26204' }],
    }
  )
...
test('engineeringPrintTemplates: a secondary-only product image occupies the first print slot', () => {
  const productImages = {
    secondary: {
      name: '唯一产品图.png',
      dataURL: 'data:image/png;base64,c2Vjb25kYXJ5',
  ...
  assert.equal(materialDetail.images.header_left.name, '唯一产品图.png')
  assert.equal(materialDetail.images.header_right.dataURL, '')
  assert.equal(workInstruction.images.header.name, '唯一产品图.png')
  assert.equal(workInstruction.images.header_right.dataURL, '')
})
...
test('engineeringPrintEditor: 空白模板显式清空业务字段但保留版式骨架', () => {
  ...
  assert.deepEqual(material.appendixImages, [])
  assert.equal(colorCard.productName, '')
  assert.equal(colorCard.blocks.length, 1)
  assert.deepEqual(colorCard.appendixImages, [])
  assert.equal(workInstruction.productNo, '')
  assert.equal(workInstruction.processName, '')
  assert.equal(workInstruction.processDateText, '')
  ...
  assert.deepEqual(workInstruction.appendixImages, [])
})

test('engineeringPrintTemplates: 物料明细旧页底图片仅在新字段缺失时迁入末尾附图', () => {
  const legacyDraft = createEngineeringPrintDraft(
    MATERIAL_DETAIL_TEMPLATE_KEY,
    {
      images: {
        footer_right: {
          name: '旧右图.svg',
          dataURL: 'data:image/svg+xml;base64,cmlnaHQ=',
        },
        footer_left: {
          name: '旧左图.svg',
          dataURL: 'data:image/svg+xml;base64,bGVmdA==',
        },
      },
    }
  )
  const clearedDraft = createEngineeringPrintDraft(
    MATERIAL_DETAIL_TEMPLATE_KEY,
    {
      appendixImages: [],
      images: {
  ...
  assert.deepEqual(
    legacyDraft.appendixImages.map((image) => image.name),
    ['旧左图.svg', '旧右图.svg']
  )
  assert.deepEqual(clearedDraft.appendixImages, [])
})
...
test('engineeringPrintTemplates: 作业行图片标注归一化后可随草稿恢复', () => {
    ...
    rows: [
      {
        ...
        images: [
          ...
          {
            name: '尺子图.jpg',
            dataURL: 'data:image/jpeg;base64,ruler-fixture',
            mimeType: 'image/jpeg',
            annotations: [
              {
                id: 'second-image-distance',
                type: 'measurement',
                text: '15 mm',
                start: { x: 32, y: 64 },
                end: { x: 68, y: 64 },
  ...
  assert.equal(restored.rows[0].images.length, 2)
  ...
  assert.equal(restored.rows[0].images[0].annotations[1].text, '30 mm')
  assert.equal(restored.rows[0].images[1].name, '尺子图.jpg')
  assert.equal(restored.rows[0].images[1].annotations.length, 1)
  assert.equal(restored.rows[0].images[1].annotations[0].text, '15 mm')
  assert.deepEqual(restored.rows[0].images[1].annotations[0].start, {
    x: 32,
    y: 64,
  })
  assert.equal(restored.rows[0].images[1].annotations[0].labelOffset, -9)
})
