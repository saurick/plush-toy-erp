...
test('payableReconciliationFlow: 应付协同相关单据不透出质检 raw key', () => {
  const purchaseTask = buildPurchasePayableRegistrationTask(
    purchaseRecord({
      payload: {
        ...purchaseRecord().payload,
        iqc_result: 'custom_iqc_result_key',
      },
    }),
    { id: 501 },
    { nowMs: NOW_MS }
  )
  const outsourceTask = buildOutsourcePayableRegistrationTask(
    outsourceRecord(),
    { id: 502 },
    { nowMs: NOW_MS }
  )
  ...
  assert.equal(
    purchaseTask.payload.related_documents.includes('IQC 结果：质检已记录'),
    true
  )
  assert.equal(
    purchaseTask.payload.related_documents.some((item) =>
      item.includes('custom_iqc_result_key')
    ),
    false
  )
  assert.equal(
    outsourceTask.payload.related_documents.includes('检验结果：合格'),
    true
  )
  assert.equal(
    outsourceTask.payload.related_documents.some((item) =>
      item.includes('pass')
    ),
    false
  )
...
test('payableReconciliationFlow: 缺 source_no / document_no 时不崩溃', () => {
  ...
  const taskWithInternalSourceNo = buildPurchasePayableRegistrationTask(
    purchaseRecord({
      document_no: '',
      source_no: '71',
      title: '',
  ...
  assert.equal(task.payload.related_documents.includes('采购单：71'), false)
  assert.equal(
    taskWithInternalSourceNo.payload.related_documents.includes('采购单：71'),
    false
  )
  assert.equal(
    taskWithInternalSourceNo.payload.related_documents.includes('采购单已关联'),
    true
  )
  assert.equal(
