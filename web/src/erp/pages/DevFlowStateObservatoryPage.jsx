...
const LAYER_PRESENTATION = Object.freeze({
  ...
  exception: { label: '异常', description: '阻塞、退回和 guard' },
...
function graphTransitionLabel(transition, targetState) {
  if (
    /resume|restore|reopen|reactivate|unblock|恢复|解除/iu.test(
      `${transition.key} ${transition.action}`
    )
  ) {
    return '恢复'
  }
...
function transitionLayerValues(transition, activeLayers, statesByKey) {
  ...
  if (
    activeLayers.has('exception') &&
    /block|reject|cancel|exception|error|异常|阻塞|退回|取消/iu.test(
      transitionText
    )
  ) {
    addUnique(semantic, '异常')
  }
...
function buildFlowMermaid(flow, layerKeys) {
...
function MachineView({ flow, layerKeys }) {
  const mermaid = useMemo(
    () => buildFlowMermaid(flow, layerKeys),
    [flow, layerKeys]
  )
  if (!flow) {
    return (
  ...
  return (
    <div className="erp-dev-flow-state-view-stack">
      ...
      <section className="erp-dev-flow-state-definition">
        ...
        <TransitionCards transitions={flow.transitions} />
        <EvidenceDisclosure
          context="machine"
          label="查看本状态机的实现依据"
...
export default function DevFlowStateObservatoryPage() {
  ...
  const scopeKey =
    scopeOptions.find((scope) => scope.key === requestedScopeKey)?.key || 'all'
  const view = VIEW_KEYS.has(requestedView) ? requestedView : DEFAULT_VIEW
  ...
  const requestedLayerKeys = cleanText(searchParams.get(QUERY_KEYS.layers))
    .split(',')
    .map((key) => key.trim())
    .filter((key) => availableLayerKeys.has(key))
    ...
    () =>
      (catalog?.flows || []).filter(
        (flow) =>
          ...
          (!normalizedKeyword || flow.searchText.includes(normalizedKeyword))
      ),
    [catalog?.flows, normalizedKeyword, scopeKey]
  )
  const selectedFlow =
  ...
  const renderView = () => {
    ...
    if (view === 'machine') {
      return <MachineView flow={selectedFlow} layerKeys={layerKeys} />
    }
    if (view === 'dictionary') {
      return (
        <DictionaryView
  ...
  return (
    ...
    >
      ...
      <main className="erp-dev-flow-state-main" data-flow-state-view={view}>
        ...
        {catalogState.status === 'ready' && !unknownFlow ? renderView() : null}
      </main>
    </div>
  )
}
