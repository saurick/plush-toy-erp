...
test('devPrototypes: 中央 README 与静态查看器同步登记本轮重设计资产', () => {
  ...
  assert.match(prototypeRegistryReadmeSource, /截至 2026-07-18/u)
  assert.match(prototypeRegistryReadmeSource, /十五个产品内核相关 HTML/u)
  assert.match(
    prototypeRegistryReadmeSource,
    /mobile-role-tasks-v2\/index\.html.*十五个产品内核相关 HTML 继续保留 To Implement/u
  )
})
...
test('devPrototypes: 岗位任务端 v2 保持三类查询视图且无空回执入口', () => {
  const html = readFileSync(
    path.join(
      repoRoot,
  ...
  assert.match(
    readme,
    /继续按 \*\*To Implement\*\* 登记到中央原型清单和开发态原型查看器/u
  )
  assert.match(readme, /当前 \*\*To Implement\*\* 工作流升级原型/u)
})
