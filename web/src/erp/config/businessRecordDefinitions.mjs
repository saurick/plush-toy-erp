...
const MODULE_OVERRIDES = Object.freeze({
    ...
    itemFields: [
      {
        key: 'item_name',
        label: '资料项',
        placeholder: '色卡、图片、作业指导等',
      },
      { key: 'spec', label: '引用 / 说明' },
    ],
    formFields: [
      {
        key: 'document_no',
      ...
      { key: 'payload.sop_ref', label: '作业指导书引用' },
    ],
    tableColumns: [
  ...
  'project-orders': {
    ...
    itemFields: [
      ...
      { key: 'payload.production_qty', label: '生产数量', type: 'number' },
      { key: 'payload.unshipped_qty', label: '未出货数', type: 'number' },
      { key: 'unit', label: '单位' },
      { key: 'unit_price', label: '单价', type: 'number' },
      { key: 'amount', label: '金额', type: 'number' },
      { key: 'payload.line_remark', label: '备注' },
    ],
    formFields: [
      { key: 'document_no', label: '订单编号', placeholder: '留空自动生成' },
      { key: 'document_date', label: '下单日期', type: 'date' },
      ...
      { key: 'payload.color', label: '颜色' },
      { key: 'quantity', label: '订单数量', type: 'number' },
      { key: 'payload.head_loss_qty', label: '损头版', type: 'number' },
      { key: 'payload.production_qty', label: '生产数量', type: 'number' },
      { key: 'payload.unshipped_qty', label: '未出货数', type: 'number' },
      { key: 'due_date', label: '出货日期', type: 'date' },
      { key: 'payload.business_owner', label: '跟单业务人员' },
      { key: 'payload.image_ref', label: '图片引用' },
      { key: 'payload.category', label: '类别' },
      { key: 'payload.order_unit_price', label: '订单单价', type: 'number' },
    ],
    tableColumns: [
      { key: 'document_date', label: '下单日期', width: 130 },
      ...
      { key: 'payload.color', label: '颜色', width: 100 },
      { key: 'quantity', label: '订单数量', width: 100 },
      { key: 'payload.production_qty', label: '生产数量', width: 100 },
      { key: 'payload.unshipped_qty', label: '未出货数', width: 100 },
      { key: 'due_date', label: '出货日期', width: 130 },
      { key: 'payload.business_owner', label: '跟单业务人员', width: 140 },
      { key: 'payload.category', label: '类别', width: 100 },
      { key: 'payload.order_unit_price', label: '单价', width: 90 },
    ],
  },
  'material-bom': {
    itemTitle: 'BOM 明细',
    itemFields: [
      ...
      { key: 'payload.color', label: '颜色' },
      { key: 'unit', label: '单位' },
      { key: 'payload.assembly_part', label: '组装部位' },
      { key: 'payload.piece_count', label: '部件数量', type: 'number' },
      { key: 'payload.unit_usage', label: '单位用量', type: 'number' },
      { key: 'payload.loss_rate', label: '损耗率', type: 'number' },
      { key: 'quantity', label: '总用量', type: 'number' },
      { key: 'payload.process_prepare_note', label: '工序说明' },
      { key: 'payload.process_type', label: '工序类别' },
      { key: 'supplier_name', label: '建议供应商' },
      { key: 'item_name', label: '备注' },
    ],
    formFields: [
      ...
      { key: 'unit', label: '单位' },
      { key: 'supplier_name', label: '建议供应商' },
      { key: 'payload.spare_qty', label: '备品数量', type: 'number' },
      { key: 'payload.designer_name', label: '设计师' },
      { key: 'payload.material_direction', label: '毛向 / 材料方向' },
      { key: 'payload.color_card_ref', label: '色卡引用' },
      { key: 'payload.sop_ref', label: '作业指导书引用' },
      { key: 'payload.source_date', label: '来源日期', type: 'date' },
    ],
    tableColumns: [
      ...
      { key: 'payload.source_date', label: '来源日期', width: 130 },
    ],
  },
  'accessories-purchase': {
    ...
    itemFields: [
      ...
      { key: 'payload.supplier_short_name', label: '厂商简称' },
      { key: 'payload.return_date', label: '回货日期', type: 'date' },
      {
        key: 'payload.contract_printed',
        label: '已打印合同',
    ...
    formFields: [
      ...
      { key: 'payload.actual_unit_price', label: '实际单价', type: 'number' },
      { key: 'payload.inbound_qty', label: '回货数量', type: 'number' },
      { key: 'payload.quantity_diff', label: '数量差异', type: 'number' },
      { key: 'payload.orderer_name', label: '下单人' },
      { key: 'payload.orderer_phone', label: '联系电话' },
      { key: 'payload.order_month', label: '下单月份' },
      {
        key: 'payload.contract_printed',
        label: '已打印合同',
        options: YES_NO_OPTIONS,
      },
      { key: 'due_date', label: '回货日期', type: 'date' },
    ],
    tableColumns: [
      { key: 'document_no', label: '采购单号', width: 150 },
      { key: 'source_no', label: '产品订单编号', width: 150 },
      { key: 'product_no', label: '产品编号', width: 130 },
      { key: 'product_name', label: '产品名称', width: 180 },
      { key: 'supplier_name', label: '供应商', width: 160 },
      { key: 'payload.supplier_short_name', label: '厂商简称', width: 120 },
      { key: 'material_name', label: '辅材 / 包材', width: 180 },
      { key: 'payload.supplier_item_no', label: '厂商料号', width: 140 },
      { key: 'quantity', label: '采购数量', width: 100 },
      { key: 'amount', label: '采购金额', width: 110 },
      { key: 'due_date', label: '回货日期', width: 130 },
      { key: 'payload.orderer_name', label: '下单人', width: 110 },
      { key: 'payload.contract_printed', label: '已打印合同', width: 120 },
    ],
  },
  'processing-contracts': {
    ...
    itemFields: [
      ...
      { key: 'payload.material_qty', label: '材料数量', type: 'number' },
      { key: 'payload.material_unit', label: '材料单位' },
      {
        key: 'payload.processing_inbound_qty',
        label: '回货数量',
        type: 'number',
      },
      { key: 'payload.quantity_diff', label: '数量差异', type: 'number' },
    ...
    tableColumns: [
      { key: 'document_no', label: '委外加工订单号', width: 170 },
      { key: 'source_no', label: '产品订单编号', width: 150 },
  ...
  'production-progress': {
    ...
    formFields: [
      ...
      { key: 'source_no', label: '订单编号 / 来源排单' },
      { key: 'payload.customer_order_no', label: '客户订单号' },
      { key: 'customer_name', label: '客户' },
      { key: 'title', label: '进度事项', required: true },
      { key: 'product_no', label: '产品编号' },
      { key: 'product_name', label: '产品名称' },
      { key: 'payload.color', label: '颜色' },
      { key: 'payload.order_qty', label: '订单数量', type: 'number' },
      { key: 'quantity', label: '完成数量', type: 'number' },
      { key: 'payload.production_qty', label: '生产数量', type: 'number' },
      { key: 'payload.unshipped_qty', label: '未出货数', type: 'number' },
      { key: 'supplier_name', label: '车间 / 加工厂' },
      { key: 'document_date', label: '回填日期', type: 'date' },
      { key: 'due_date', label: '出货 / 下一节点日期', type: 'date' },
      { key: 'payload.business_owner', label: '跟单业务人员' },
      { key: 'payload.image_ref', label: '图片引用' },
      { key: 'payload.category', label: '类别' },
      { key: 'payload.order_unit_price', label: '订单单价', type: 'number' },
    ...
    tableColumns: [
      ...
      { key: 'source_no', label: '订单编号', width: 150 },
      { key: 'customer_name', label: '客户', width: 130 },
      { key: 'payload.customer_order_no', label: '客户订单号', width: 150 },
      { key: 'product_no', label: '产品编号', width: 130 },
      { key: 'product_name', label: '产品名称', width: 180 },
      { key: 'payload.color', label: '颜色', width: 100 },
      { key: 'payload.order_qty', label: '订单数量', width: 100 },
      { key: 'quantity', label: '完成数量', width: 100 },
      { key: 'payload.production_qty', label: '生产数量', width: 100 },
      { key: 'payload.unshipped_qty', label: '未出货数', width: 100 },
      { key: 'supplier_name', label: '车间 / 加工厂', width: 160 },
      { key: 'document_date', label: '回填日期', width: 130 },
      { key: 'due_date', label: '出货 / 下一节点日期', width: 160 },
      { key: 'payload.business_owner', label: '跟单业务人员', width: 140 },
    ],
  },
