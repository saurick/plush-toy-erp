...
本目录是 Phase 11 的私有化客户包模板入口，用于准备新增客户时的 `docs/customers/<customer-key>/`、`config/customers/<customer-key>/` 和 `deployments/<customer-key>/` 边界。
...
- `simulatedCustomerKey` 只用于本地 Phase 11 evidence，不应创建为正式客户目录。
