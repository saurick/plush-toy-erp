...
`config/customers/<customer-key>/customer-config.example.js` 的 `legalNotice` 是登录前公开规则中的客户事实来源，包含版本、生效日期、个人信息处理者、联系渠道、主要存储位置、跨境口径和实际外部处理方。Product Core 提供中性正文与最小默认值，不硬编码真实客户名称；正式交付必须确认该配置与目标部署、短信服务和客户内部 retention 制度一致。任何实质变化都会改变前端内容指纹，并要求登录账号重新“已阅读并知悉”。

该配置不是同意记录、签署合同或数据库权限真源。正式委托处理边界见 `docs/security/`；真实签署协议和客户联系人保留在客户受控资料中。
...
- “权限管理 → 审批责任”只承接 Product Core 登记的销售订单、采购订单和 Shipment 财务放行三类可配置责任。runtime manifest 为三类事项生成固定 `approval.*` 责任池，业务管理角色为 `primary/100`，老板为 `escalation/300`；管理员以新 immutable revision 覆盖角色或具名成员。启用事项至少需要一个成员；停用事项可以无成员，并在创建 ProcessInstance 前阻断相应的新流程，不终止旧 revision 的在途实例。收付款、库存人工调整和生产异常同样由 manifest 选择正式 ProcessRuntime，但审批 / 执行责任池由 Product Core 合同固定，不进入这三项可配置模板。没有正式门禁的 PMC / 工程自由审批继续失败关闭。
