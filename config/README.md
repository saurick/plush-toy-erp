...
- 审批责任只承接 Product Core 登记的销售订单、采购订单和 Shipment 财务放行。runtime manifest 为三类事项生成固定 `approval.*` 责任池，业务管理角色为 `primary/100`，老板为 `escalation/300`；管理员从“权限管理 → 审批责任”以新 immutable revision 覆盖角色或具名成员。启用事项至少需要一个成员；停用事项可以无成员，并在创建 ProcessInstance 前阻断相应的新流程，不终止旧 revision 的在途实例。付款、库存调整和没有正式门禁的 PMC / 工程事项继续失败关闭。
